...
- Normal Developer ID releases leave Electron's Touch ID WebAuthn authenticator disabled because ADE does not ship the restricted keychain access-group entitlement without a provisioning profile. `ADE_ENABLE_TOUCH_ID_WEBAUTHN=1` is only for an explicitly provisioned build that also adds the matching entitlement; `ADE_ENABLE_TOUCH_ID_WEBAUTHN=0` keeps it disabled. When enabled, these credentials are device-bound to the Mac's Secure Enclave and the global Electron browser session; they are not iCloud-synced passkeys. ADE does not claim Apple Passwords extension support.
