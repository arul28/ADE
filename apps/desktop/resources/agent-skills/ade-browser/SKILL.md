...
ade --socket browser close --tab <id> --text
```

Close the tab you opened when the task is done — `close` (alias `close-tab`)
names the tab with `--tab <id>` or a bare positional id, and closing it ends any
recording or browser session bound to that tab.
...
- Use `ade --socket browser proof --tab <id> --caption "..."` or `ade --socket browser proof --browser-session <id> --caption "..."` to promote a fresh browser observation into the durable proof drawer. Add `--har` to export the tab's HAR in the same call and file it as a `browser_trace` artifact under the same owners as the screenshot — a picture of the broken page plus the requests behind it. `--har` requires network logging to be on for that tab (`browser network on --tab <id>`); with it off the command fails and says so rather than filing half the proof. To file a file you produced yourself (a recording, a HAR you exported by hand), use `ade proof attach <path> --caption "…" --text`: it imports only from the project root, the lane worktree, `.ade/{artifacts,cache,tmp}`, the OS temp dir, `/tmp`, and `~/.agent-browser` (`.ade/secrets` is denied), and it ends with a confirmation line naming the lane and chat it filed under — read that line before reporting proof. The **ade-proof-artifacts** skill has the details.
- **A new tab is a launchpad, not a home page.** `browser new-tab` with no URL parks on `about:blank` with `isLaunchpad: true` and the title "New tab"; ADE issues no request you did not ask for. Closing the last tab leaves zero tabs — the pane shows its launchpad, and revealing the panel does not conjure a replacement. `open-request` and the agent `open` command are unchanged: they still navigate. The launchpad offers the dev servers ADE noticed plus a "Recently used" list of the last 10 pages that window loaded, stored per tab collection in the renderer and clearable by the human. It keeps only `origin + pathname`, refuses any URL whose query or fragment names a credential (`?code=`, `#access_token=`, `?token=`), and refuses ephemeral loopback ports (≥ 32768) so a dead tunnel forward is never offered again. There is no `ade browser recents`: it is a human affordance with no agent semantics, so do not treat it as history you can read.
