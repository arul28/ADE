...
- Because CLI bridge calls route to a project tab collection, confirm the target project has an ADE window or project tab open before taking a screenshot or selecting context.
...
- A global authenticated profile is a security boundary. GitHub, AWS/cloud account surfaces, and any origin with cookies in the global profile require a native human grant per chat/lane before an agent can inspect or control them. Use `browser authorize` when a synchronous status/session command asks for approval; navigation commands request the same approval directly. Grants are memory-only and disappear on ADE restart. Unknown agent-triggered cross-origin navigation is paused for a cookie check, and high-risk popups are blocked until the agent navigates there explicitly and the human approves.
...
- Signed packaged macOS builds enable Electron's Touch ID WebAuthn authenticator with ADE's matching keychain access-group entitlement; source builds require `ADE_ENABLE_TOUCH_ID_WEBAUTHN=1` for explicit testing, and `ADE_ENABLE_TOUCH_ID_WEBAUTHN=0` disables it. These credentials are device-bound to the Mac's Secure Enclave and the global Electron browser session; they are not iCloud-synced passkeys. ADE does not claim Apple Passwords extension support.
