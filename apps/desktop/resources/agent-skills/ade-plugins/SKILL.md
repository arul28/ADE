...
Build the row this way rather than hand-assembling one out of `stack`, `badge`, `text` and `button` nodes. A hand-built row costs about seven nodes, and `maxNodes` is 200 — so the panel caps out near 27 rows. **A list is one node however rich its rows are**, which makes `maxListItems` (1000, bound) the real ceiling. The cap on `actions` counts what survived parsing, so a malformed entry does not spend a slot.

**How many rows a reader sees, and when.** A list draws **100** and then offers the rest: under the rows sits a line reading `Showing 100 of 143` with a **Show more** beside it, and desktop / web / iOS also load the next page when the reader scrolls to the end, up to the 1000-row ceiling. A terminal reader selects a numbered Show more row — it never auto-loads. The page count is the CLIENT's — it never reaches your handler, never enters a `where`, and never rides on an action payload, exactly like a folded `group`. Two consequences worth designing around:
...
- **A truncated list says so.** At the ceiling the line reads `Showing the first 1000` with no button. Before, a list simply stopped and looked complete — so if your collection can exceed 1000 rows, give the reader a `segmented` filter to narrow it rather than relying on the page.

**The 1000 applies to bound rows.** Rows in a collection never enter `schema_json`, so 1000 of them cost one node and no bytes. Rows written **inline** in `items` do spend bytes, and there `maxSchemaBytes` (65,536) is the real ceiling: a fully dressed row measures around 400–600 bytes, so an inline rich list runs out of budget somewhere under 200 rows and the writer refuses the whole panel. A thousand plain inline rows would be ~82 KiB. If your list is long, bind it.

A row may declare `preview: { title?, text? }`. Desktop and web show it as a hover card; iOS as a context-menu preview; the TUI omits it.
