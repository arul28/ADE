...
/**
 * Canonical Droid model ids surfaced when the CLI can't be probed (provider
 * unauthenticated). Kept in renderer-safe shared code so the picker can show
 * the full list with sign-in CTAs.
 *
 * Order matches the main-process default in `droidModelsDiscovery.ts` so the
 * two sources don't drift. Update both when the curated list changes.
 */
export const DROID_CANONICAL_MODEL_IDS: readonly string[] = [
  "claude-opus-4-6",
...
/**
 * Canonical Cursor SDK model ids — surfaced even when no Cursor API key is
 * configured so the user can browse the catalog before signing in.
 */
export const CURSOR_CANONICAL_MODEL_IDS: readonly string[] = [
  "auto",
  "composer-2",
  "claude-sonnet-4.5",
  "claude-opus-4.5",
...
/**
 * Canonical OpenCode provider/model pairs surfaced when the OpenCode binary
 * isn't yet installed/probed. The picker materializes these into dynamic
 * OpenCode descriptors via `createDynamicOpenCodeModelDescriptor`.
 */
export const OPENCODE_CANONICAL_PROVIDER_MODELS: ReadonlyArray<{ providerId: string; modelId: string }> = [
  ...
  { providerId: "together", modelId: "qwen-2.5-coder-32b" },
];
