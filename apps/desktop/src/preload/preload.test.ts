...
describe("preload OAuth bridge", () => {
  ...
  /**
   * The window-capture bridge is GONE, not inert.
   *
   * `getSimulatorWindowState`, `listSimulatorWindowSources`,
   * `retainWindowParking`, `releaseWindowParking`, `revealSimulator` and
   * `openSystemSettings` described a picture of Simulator.app — which window,
   * whether it was minimized, which macOS privacy grant was missing. The
   * helper reads the framebuffer, so there is no window and no grant. The
...
describe("per-chat runtime routing", () => {
  ...
  async function mountBridge(
    activeBinding: typeof machineA | typeof machineB = machineA,
    ...
    const invoke = vi.fn(async (channel: string, arg?: unknown): Promise<unknown> => {
      ...
      if (channel === IPC.localRuntimeCallAction || channel === IPC.remoteRuntimeCallAction) {
        const request = (arg as {
          request?: { domain?: string; action?: string; args?: unknown; arg?: unknown };
        } | undefined)?.request;
        if (runtimeAction) return { result: await runtimeAction(request ?? {}) };
  ...
  it("releases the previous binding's subscription when the active pump switches machines", async () => {
    ...
    try {
      ...
      const unsubscribe = bridge.automations.onEvent(vi.fn());
      await vi.advanceTimersByTimeAsync(0);
      const releaseCalls = () =>
        invoke.mock.calls
          .filter(([channel]) => channel === IPC.runtimeEventsRelease)
