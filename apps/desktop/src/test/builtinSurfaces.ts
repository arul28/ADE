 ...
 *
 * The product hides Graph, History, the iOS Simulator pane and Electron Control
 * unless the owning plugin is installed and enabled on this machine, and hidden
 * is the default rather than a fallback. A suite that renders one of those
 ...
 *
 * Cursor Cloud, Linear and Review run the other way: their plugins REPLACE
 * ADE's compiled surfaces, so seeding one here is how a suite describes the
 * machine where the built-in is GONE, and a suite that says nothing gets the
 * built-in — which is what a machine without the plugin has always had.
 */
