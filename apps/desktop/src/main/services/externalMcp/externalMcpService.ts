...
export function createExternalMcpService(args: ExternalMcpServiceArgs) {
  ...
  const emit = (event: ExternalMcpEventPayload): void => {
    args.onEvent?.(event);
  ...
  const readPersistedUsageEvents = (limit: number): ExternalMcpUsageEvent[] => {
    const db = args.db ?? null;
    const projectId = args.projectId?.trim() ?? "";
    if (!db || !projectId.length) {
      return usageEvents.slice(-Math.max(1, Math.min(limit, usageEvents.length))).reverse();
    }
    const rows = db.all<PersistedUsageRow>(
      `
        select
          id,
          server_name,
    ...
    return rows.map((row) => ({
      ...
      namespacedToolName: row.namespaced_tool_name,
      safety: row.safety === "read" || row.safety === "write" ? row.safety : "unknown",
      callerRole:
        row.caller_role === "cto"
        || row.caller_role === "orchestrator"
        || row.caller_role === "agent"
        || row.caller_role === "external"
        || row.caller_role === "evaluator"
          ? row.caller_role
          : "external",
  ...
  const emitServerState = (runtime: RuntimeServerState): void => {
    emit({
  ...
  const emitToolsRefreshed = (runtime: RuntimeServerState): void => {
    emit({
      type: "tools-refreshed",
      at: nowIso(),
      serverName: runtime.resolvedConfig.name,
      state: runtime.state,
      toolCount: runtime.toolMap.size,
  ...
  const recordBudgetUsage = async (
    ...
    if (args.db && args.projectId) {
      args.db.run(
        `
          insert into external_mcp_usage_events (
            id,
            project_id,
            server_name,
            tool_name,
  ...
  return {
    ...
    getUsageEvents(limit = 100): ExternalMcpUsageEvent[] {
      return readPersistedUsageEvents(limit);
    },

    onEvent(listener: (event: ExternalMcpEventPayload) => void): () => void {
