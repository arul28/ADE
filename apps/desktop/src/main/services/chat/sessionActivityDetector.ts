...
const SHELL_TOOLS = new Set([
  ...
  "run_command", "command", "local_shell",
]);
const PLAN_TOOLS = new Set(["exitplanmode", "enterplanmode", "update_plan", "create_plan"]);
...
const TEST_RUNNER =
  /\b(vitest|jest|pytest|mocha|ava|rspec|phpunit|node\s+(?:\S+\s+)*--test\b|playwright\s+test|go\s+test|cargo\s+(?:test|nextest)|swift\s+test|deno\s+test|bun\s+test|dotnet\s+test|mvn\s+(?:\S+\s+)*test|gradlew?\s+(?:\S+\s+)*test|xcodebuild\b[^\n]*\btest\b|(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?test\b)/;
const WATCH_LOOP =
  /\b(gh\s+(?:pr\s+checks\b[^\n]*--watch|run\s+watch)|ade\s+chat\s+wait|watch\s+-n)\b|\b(?:until|while)\b[^\n]*;\s*do\b[^\n]*\bsleep\b/;
const CHECK_STATUS = /\bgh\s+(?:pr\s+checks|run\s+(?:view|list)|pr\s+view\b[^\n]*(?:checks|statusCheckRollup|mergeStateStatus))/;
const SHIP = /\b(git\s+(?:commit|push|rebase|merge|cherry-pick)|gh\s+pr\s+(?:create|merge|ready)|prs\s+(?:create|land|merge)|(?:lanes|git)\s+(?:commit|push))\b/;
const STATIC_CHECK =
  /\b(tsc\b|typecheck|type-check|eslint|biome\s+(?:check|lint)|ruff\b|mypy\b|clippy\b|cargo\s+(?:check|build)|swift\s+build|xcodebuild\b|(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:lint|build|check)\b)/;
const DRIVE_APP = /\bade\s+(?:app-control|browser|mac-desktop|apple|ios-sim|proof)\b|\bagent-browser\b/;
const REVIEW_READ = /\bgh\s+(?:pr\s+diff|api\s+\S*(?:comments|reviews))/;
/** The agent reporting its own status is not evidence of anything. */
const ADE_STATUS_COMMAND = /\bchat\s+(?:activity|note|ask)\b/;

/** Classify one shell command line. Unknown commands read as reading. */
export function classifyShellCommand(command: string): SessionActivitySignal {
  const text = command.trim();
  if (!text || ADE_STATUS_COMMAND.test(text)) return READ;
  if (TEST_RUNNER.test(text)) return strong("testing");
  if (WATCH_LOOP.test(text)) return strong("monitoring");
  if (SHIP.test(text)) return weak("shipping");
  if (CHECK_STATUS.test(text)) return weak("monitoring");
  if (STATIC_CHECK.test(text) || DRIVE_APP.test(text)) return weak("testing");
  if (REVIEW_READ.test(text)) return weak("reviewing");
  return READ;
}
 ...
 * the dual-review").
 */
function classifyDelegationParts(description: string, prompt: string): SessionActivitySignal {
  const fromDescription = classifyDelegation(description);
  if (fromDescription.kind === "activity" || !prompt) return fromDescription;
  return classifyDelegation(prompt.slice(0, 240));
}
...
function hasArgs(args: unknown): boolean {
  return Boolean(args) && typeof args === "object" && Object.keys(args as object).length > 0;
}
...
/** ACP's `ToolKind`, for tools whose name says nothing. */
function classifyAcpToolKind(kind: string | undefined, args: unknown): SessionActivitySignal | null {
  switch (kind) {
    case "read":
    case "search":
    case "fetch":
    ...
    case "execute":
      return classifyShellCommand(stringField(args, ["command", "cmd", "script", "input"]));
    default:
      return null;
  }
}
 ...
 */
export function classifySessionActivityEvent(event: AgentChatEvent): SessionActivitySignal | null {
  switch (event.type) {
    case "command":
    ...
    case "subagent_started":
      ...
      return classifyDelegationParts(`${event.description ?? ""} ${event.label ?? ""}`, "");
    case "tool_call": {
      ...
      if (SUBAGENT_TOOLS.has(name)) {
        ...
        return classifyDelegationParts(
          stringField(args, ["description", "subagent_type"]),
      ...
      return classifyAcpToolKind(event.toolKind, args) ?? READ;
    }
    default:
      return null;
  }
