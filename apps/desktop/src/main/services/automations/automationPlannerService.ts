      ...
      actions: {
        ...
        items: {
          oneOf: [
            ...
            // Note: sync-to-mirror intentionally excluded by policy; legacy rules remain viewable/migratable.
          ]
        }
      }
    },
...
function buildPlannerPrompt(args: {
  ...
  return [
    ...
    "",
    "Policy:",
    "- Do not use hosted-only actions. Do NOT output action type 'sync-to-mirror'.",
...
function normalizeDraft(args: {
  ...
  for (let idx = 0; idx < (args.draft.actions ?? []).length; idx += 1) {
    ...
    const base = {
      type,
      ...(safeTrim(action?.condition) ? { condition: safeTrim(action.condition) } : {}),
    ...
    if (type === "sync-to-mirror") {
      issues.push({
        level: enabled ? "error" : "warning",
        path: `actions[${idx}].type`,
        message: "sync-to-mirror is no longer supported for automations. Remove or disable this rule."
      });
    ...
    simulate(req: AutomationSimulateRequest): AutomationSimulateResult {
      ...
      const actions: AutomationSimulationAction[] = normalized.actions.map((action, index): AutomationSimulationAction => {
        ...
        if (action.type === "sync-to-mirror") {
          warnings.push("Unsupported: sync-to-mirror is blocked for automations.");
        }
