...
export function buildOrchestratorRoleDirective(args: {
  ...
  if (args.role === "lead") {
    ...
    lines.push("4a. **Finishing choice:** call `chooseFinishingMode` — ask whether, when the run finishes, to stop at validated code in the worktree, or push the branch + open a PR + update Linear. It records `manifest.finishing`. If the user picks `pr`, after validation passes you spawn a finishing worker to push, open the PR, update Linear, and register the pr_link / linear_issue / deeplink evidence (skill §5.5).");
