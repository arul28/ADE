...
export function createAiIntegrationService(args: {
  ...
  const getMode = (): AiProviderMode => {
    ...
    return availability.claude || availability.codex ? "subscription" : "guest";
  };
  ...
  const resolveProviderForTask = (taskType: AiTaskType, modelOverride?: string | null): AgentProvider => {
    const snapshot = projectConfigService.get();
    const defaults = TASK_DEFAULTS[taskType];
    const override = extractTaskOverride(snapshot, taskType);

    const preferred = toStringOrNull(override.provider) ?? toStringOrNull(modelOverride);
  ...
  const buildExecutorOpts = (args: {
    ...
    return {
      ...
      maxBudgetUsd: toNumberOrNull(claudePermissions.maxBudgetUsd) ?? toNumberOrNull(claudePermissions.max_budget_usd) ?? undefined,
      permissions: {
        mode: permissionMode,
      ...
      providerConfig: {
        claude: {
          ...
          settingSources: Array.isArray(claudePermissions.settingSources)
            ? claudePermissions.settingSources.filter((entry): entry is "user" | "project" | "local" =>
                entry === "user" || entry === "project" || entry === "local"
              )
            : Array.isArray(claudePermissions.settings_sources)
              ? claudePermissions.settings_sources.filter((entry): entry is "user" | "project" | "local" =>
                  entry === "user" || entry === "project" || entry === "local"
                )
              : [],
  ...
  const executeTask = async (args: ExecuteAiTaskArgs): Promise<ExecuteAiTaskResult> => {
    ...
    const provider = resolveProviderForTask(args.taskType, null);
