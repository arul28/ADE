...
describe("autoRebaseService", () => {
  ...
  describe("dismissStatus", () => {
    it("hides an attention status until the parent head changes", async () => {
      const service = createService();
      laneList = [
      ...
      await expect(service.listStatuses()).resolves.toEqual([]);

      db.setJson("auto_rebase:status:lane-a", {
        laneId: "lane-a",
        parentLaneId: "root",
        parentHeadSha: "parent-2",
        state: "rebasePending",
