...
function chooseSuggestedCommand(args: { commands: string[]; warnings: string[] }): { suggestedCommandLine: string | null; suggestedCommand: string[] | null } {
...
export function createCiService(args: {
  ...
  const scan = async (): Promise<CiScanResult> => {
    ...
    const diff: CiScanDiff | null = lastImport
      ? (() => {
          const prev = lastImport.jobDigests ?? {};
          const prevIds = new Set(Object.keys(prev));
          const currIds = new Set(Object.keys(jobDigests));
          ...
          for (const id of currIds) {
            ...
            if (prev[id] !== jobDigests[id]) changed += 1;
            else unchanged += 1;
          }
