...
describe("linearIngressService (file group)", () => {
  ...
  describe("linearIngressService", () => {
    ...
    it("accepts local webhook deliveries signed with the relay secret", async () => {
      ...
      await service.ensureRelayWebhook(true);
      const url = new URL(service.getStatus().localWebhook.url!);
      const body = JSON.stringify({
        type: "Issue",
        action: "update",
        data: { id: "issue-1", identifier: "ADE-68", title: "Webhook test" },
      });
      const signature = createHmac("sha256", "relay-secret").update(Buffer.from(body)).digest("hex");
