 ...
 * waiting for the socket to open. The two ways to empty one mean OPPOSITE
 * things — which is the whole reason this is a module. `cancelQueued()` is a
 * barge-in, or audio a call no longer wants: those chunks were CANCELLED, so
 * the drop tally is untouched. `forgetCall()` is a new call or a teardown:
 * nothing about the last call's audio, lost or cancelled, belongs to the next
 * one.
 *
 ...
 * bounded amount of memory, and what the bound threw away is a number that is
 * shown or logged rather than one nobody sees.
 */
