...
function writeDevShim(args: {
  ...
  const script = [
    ...
    "    exec \"$ADE_CLI_NODE\" --import tsx \"$CLI_JS\" \"$@\"",
    "  fi",
    "  if [ -x \"$APP_EXE\" ]; then",
    "    ELECTRON_RUN_AS_NODE=1 exec \"$APP_EXE\" --import tsx \"$CLI_JS\" \"$@\"",
    "  fi",
    "  if command -v node >/dev/null 2>&1; then",
    "    NODE_MAJOR=$(node -p \"Number(process.versions.node.split('.')[0])\" 2>/dev/null || echo 0)",
    "    if [ \"$NODE_MAJOR\" -ge 22 ]; then",
    "      exec node --import tsx \"$CLI_JS\" \"$@\"",
    "    fi",
    "  fi",
    "  if command -v asdf >/dev/null 2>&1; then",
    "    exec asdf exec node --import tsx \"$CLI_JS\" \"$@\"",
    "  fi",
    "  echo \"ade: Local source CLI fallback requires tsx. Run npm --prefix apps/ade-cli install or npm --prefix apps/ade-cli run build.\" >&2",
    "  exit 127",
    "fi",
    "if [ -n \"${ADE_CLI_NODE:-}\" ]; then",
