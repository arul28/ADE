...
export type CreateAppControlServiceArgs = {
  projectRoot: string;
  logger: Logger;
...
export function normalizeProjectRoot(projectRoot: string | null | undefined, fallback: string): string {
  const raw = projectRoot?.trim();
  return path.resolve(raw?.length ? raw : fallback);
}

export function normalizeCwd(cwd: string | null | undefined, projectRoot: string): string {
  const raw = cwd?.trim();
 ...
 * refuses `SystemInfo`.
 */
export async function readAppProcessId(port: number): Promise<number | null> {
  try {
...
function readPsField(pid: number, field: "ppid" | "pgid"): Promise<number | null> {
  return new Promise((resolve) => {
    execFile(
      "ps",
...
/**
 * POSIX: true when `pid` descends from `ancestorPid`, walking `ps -o ppid=`.
 * The fallback kill target comes from CDP or from whoever listens on the debug
 * port, so on a port collision it can be an unrelated app; only a process in
 * the launch terminal's tree is ADE's to kill.
 */
async function isPosixDescendantOf(pid: number, ancestorPid: number): Promise<boolean> {
  let current = pid;
  for (let depth = 0; depth < 64; depth += 1) {
    if (current === ancestorPid) return true;
    if (current <= 1) return false;
    const parent = await readPsField(current, "ppid");
 ...
 * missed it (a launcher that detached it, or one that exited first). The
 * caller has already checked the pid belongs to the launch terminal's tree.
 * Windows goes through the canonical `taskkill /T /F`. POSIX signals the
 * app's process group when the app leads its own group (so its helpers go
 * with it), else the pid alone — never a group it merely belongs to, which
 * would be the launcher's — SIGTERM and then SIGKILL after the grace.
 */
async function killProcessTreeIfAlive(
  ...
  const target = (await readPsField(pid, "pgid")) === pid ? -pid : pid;
  try {
    process.kill(target, options.force ? "SIGKILL" : "SIGTERM");
  } catch {
    return;
  }
...
export const listDriversFor = (activeSession: AppControlSession | null): AppControlDriversResult => ({
...
export const requireSupportedDriver = (value: AppControlDriver | null | undefined): AppControlDriver => {
...
export const providersFor = (activeSession: AppControlSession | null): AppControlStatus["providers"] => {
...
/** What a lane controller needs from its service. */
export type AppControlLaneControllerContext = {
  args: CreateAppControlServiceArgs;
...
export function createAppControlLaneController(context: AppControlLaneControllerContext, laneId: string) {
  ...
  const startScreencast = async (sessionId: string, targetId: string | null, cdpEndpoint: string): Promise<void> => {
    ...
    client.on("Page.screencastFrame", (params) => {
      ...
      const frame: AppControlScreencastFrame = {
        ...
        data,
        mimeType: "image/jpeg",
        // Whole pixels on the wire: CDP reports fractional device sizes at
        // some zoom levels, and the iOS client decodes these as integers.
        // The scales above keep the exact ratio.
  ...
  const stop = async (stopArgs: AppControlStopArgs = {}): Promise<{ ok: true; previousSession: AppControlSession | null }> => {
    ...
    // Read while the app still answers CDP and its launcher still runs; it is
    // the fallback kill target. On POSIX it must sit in the launch terminal's
    // tree: the pid comes from CDP or the debug port's owner, and a port
    // collision would otherwise name an unrelated app. Windows' target goes
    // through the taskkill tree kill instead.
    const resolvedAppPid = previousSession?.terminalSessionId
      ? await resolveAppProcessId().catch(() => null)
      : null;
    const launcherPid = previousSession?.pid ?? null;
    const launchedAppPid = resolvedAppPid && (
      process.platform === "win32"
      || (launcherPid != null && await isPosixDescendantOf(resolvedAppPid, launcherPid).catch(() => false))
    )
      ? resolvedAppPid
      : null;
    if (resolvedAppPid && !launchedAppPid) {
      args.logger.info("app_control.stop_kill_target_not_in_tree", { laneId, pid: resolvedAppPid, launcherPid });
    }
...
export type AppControlLaneController = ReturnType<typeof createAppControlLaneController>;
