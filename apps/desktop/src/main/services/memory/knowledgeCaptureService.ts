...
const PR_BOT_PATTERNS = [
  ...
  /walkthrough/i,
  /sequence diagram/i,
  /automated review suggestions/i,
  /^#{1,6}\s+.*\bCodex Review\b/i,
  /^#{1,6}\s+(?:summary|findings?|review|nit|suggestion|issue|bug|security|performance|tests?)\b/i,
  /^\s*\*\*(?:summary|finding|review|nit|suggestion|issue|bug|security|performance|tests?)\b/i,
  /!\[[^\]]*\b(?:P[0-3]|priority|severity|badge)\b[^\]]*\]\(/i,
  /^\*\*\[[^\]]*\b(?:critical|high|medium|low|investigate|p[0-3])\b[^\]]*\]\*\*/i,
  /\bP[0-3]\s+Badge\b/i,
  /\bcodecov\b/i,
  /\bcoderabbit\b/i,
  /\bgreptile\b/i,
...
const GENERIC_PR_FEEDBACK_PATTERNS = [
  /^\s*>?\s*@(?:copilot|coderabbit(?:ai)?|greptile)\s+review\b/i,
...
function stripQuotedReviewCommands(value: string): string {
  return value
    .split(/\r?\n/g)
    .map((line) => line.replace(/^\s*>\s?/, "").trim())
    .filter((line) => !/^\s*@(?:copilot|coderabbit(?:ai)?|greptile)\s+review\b/i.test(line))
    .filter((line) => !/^here(?:'|’)s the review\b/i.test(line))
    .join("\n")
    .replace(/^\s*@(?:copilot|coderabbit(?:ai)?|greptile)\s+review\b[^\n.?!]*(?:[.?!]|\s|$)/i, "")
    .replace(/^here(?:'|’)s the review\b[^\n.?!]*(?:[.?!]|\s|$)/i, "")
    .trim();
}
...
function isLowSignalPrFeedbackContent(value: string): boolean {
  ...
  if (/^\s*>?\s*@(?:copilot|coderabbit(?:ai)?|greptile)\s+review\b/i.test(trimmed) || isPrBotContent(trimmed)) {
    return true;
  }
...
export function createKnowledgeCaptureService(args: {
  ...
  const extractPrFeedbackDraft = (text: string, fileScopePattern?: string | null): CaptureDraft | null => {
    const cleaned = stripQuotedReviewCommands(text);
    if (isLowSignalPrFeedbackContent(cleaned)) return null;
    const actionable = extractActionableDrafts(splitGuidanceLines(cleaned));
  ...
  const capturePrFeedback = async (argsInput: {
    ...
    for (const comment of comments) {
      ...
      const cleanedBody = stripQuotedReviewCommands(stripHtmlTags(body));
      if (!cleanedBody || !hasMinimumMeaningfulContent(cleanedBody)) continue;
      const draft = extractPrFeedbackDraft(cleanedBody, cleanText(comment.path) || null);
      if (!draft) continue;
    ...
    for (const review of reviews) {
      ...
      const cleanedBody = stripQuotedReviewCommands(strippedBody);
      if (!cleanedBody || !hasMinimumMeaningfulContent(cleanedBody)) continue;
      ...
      const draft = extractActionableDrafts(splitGuidanceLines(cleanedBody), fallbackCategory)[0];
      if (!draft) continue;
      if (isLowSignalPrFeedbackContent(draft.content)) continue;
      const memory = recordCandidate({
        sourceType: "pr_feedback",
