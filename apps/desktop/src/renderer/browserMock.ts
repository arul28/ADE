...
if (typeof window !== "undefined" && shouldInstallBrowserMock(window)) {
  ...
  (window as any).ade = {
    app: {
      ...
      getInfo: resolved({
        ...
        localRuntime: {
          ...
          serviceInstall: {
            ...
            attempted: false,
            path: null,
            message: "Background service installation is not available in the browser mock.",
            exitCode: null,
            updatedAt: null,
          },
          serviceHealth: {
            ...
            running: null,
            path: null,
            message: "Background service status is not available in the browser mock.",
            checkedAt: null,
          },
        },
      }),
      ...
      openProjectInNewWindow: resolvedArg({ windowId: 2, project: MOCK_PROJECT }),
    ...
    remoteRuntime: {
      ...
      addProject: async (_id: string, rootPath: string) => ({
        projectId: `mock-${rootPath.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "project"}`,
        rootPath,
        displayName: rootPath.split(/[\\/]/).filter(Boolean).at(-1) || "Mock project",
        addedAt: Date.now(),
      ...
      callAction: async (_id: string, _projectId: string, request: { domain: string; action: string }) => ({
        domain: request.domain,
        action: request.action,
        result: request.domain === "lane" && request.action === "list"
          ? [{ id: "lane-main", name: "Main", branchName: "main", laneType: "primary" }]
          : null,
        statusHints: {},
      }),
      streamEvents: resolvedArg({ events: [], nextCursor: 0, hasMore: false }),
      checkLocalWork: async (project: { projectId: string; displayName: string; gitOriginUrl: string | null }) => ({
    ...
    macosVm: {
      getStatus: resolvedArg({
        ...
        activeProvider: {
          ...
          available: false,
          version: null,
          detail: "Browser preview does not run local VM providers.",
          docsUrl: "https://cua.ai/docs/lume/guide/getting-started/installation",
        },
        tools: [],
        laneVm: null,
        vms: [],
      ...
      provision: resolvedArg({} as any),
      start: resolvedArg({} as any),
      stop: resolvedArg(null),
      delete: resolvedArg({ deleted: false, previous: null }),
      getAgentGuide: resolvedArg({
        laneId: "lane-main",
        vmName: "browser-preview-vm",
        text: "Browser preview VM handoff.",
        target: {
          kind: "macos_vm_target",
          id: "macos-vm-target:lane-main:browser-preview-vm",
          laneId: "lane-main",
          laneName: "main",
          vmName: "browser-preview-vm",
          provider: "lume",
          state: "not_created",
          hostLanePath: "/mock/lane",
          guestLanePath: "/Volumes/My Shared Files",
          runCommand: "lume run browser-preview-vm --shared-dir /mock/lane",
          sshCommand: null,
          vncUrl: null,
          windowTitleQuery: "browser-preview-vm",
          selectedAt: now,
          metadata: {},
        },
      }),
      focusWindow: resolvedArg({
        laneId: "lane-main",
        vmName: "browser-preview-vm",
        windowTitleQuery: "browser-preview-vm",
        processName: "Lume",
        windowTitle: "browser-preview-vm",
      ...
      captureScreenshot: resolvedArg({
        ...
        laneId: "lane-main",
        vmName: "browser-preview-vm",
        path: "/mock/artifacts/browser-preview-vm.png",
      ...
      selectPoint: resolvedArg({
        source: "coordinate-fallback",
        screenshot: null,
        item: {
          kind: "macos_vm_target",
          id: "macos-vm-target:lane-main:browser-preview-vm:point",
          laneId: "lane-main",
          laneName: "main",
          vmName: "browser-preview-vm",
          provider: "lume",
          ...
          selectedAt: now,
          metadata: {
            selectedPoint: { x: 120, y: 420, coordinateSpace: "window" },
          },
        },
      }),
      click: resolvedArg({
    ...
    cto: {
      ...
      getLinearQuickView: resolvedArg({
        ...
        sdk: {
          ...
          surfaces: ["viewer", "organization", "projects", "teams", "assignedIssues", "issues"],
        },
      }),
