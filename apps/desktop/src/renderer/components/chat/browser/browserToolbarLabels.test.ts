...
describe("browserToolbarLayout", () => {
  it("keeps every control in a pane with room", () => {
  ...
  it("sheds pop-out, then inspect, then the camera, then the device before it touches forward", () => {
    expect(browserToolbarLayout(470)).toMatchObject({ showPopOut: true, showInspect: true, showCamera: true, showDevice: true });
    expect(browserToolbarLayout(420)).toMatchObject({ showPopOut: false, showInspect: true, showCamera: true, showDevice: true });
    expect(browserToolbarLayout(380)).toMatchObject({ showPopOut: false, showInspect: false, showCamera: true, showDevice: true });
    expect(browserToolbarLayout(360)).toMatchObject({ showForward: true, showDevice: true, showCamera: false });
  ...
  it("only offers Attach when there is a selection and room for it", () => {
    ...
    expect(browserToolbarLayout(440, { hasSelection: true }).showAttach).toBe(false);
  });

  it("pins the camera to the row while a recording is running", () => {
    // The camera carries the recording state now, so however narrow the pane
    // gets there is always something on the row that stops the recording.
    expect(browserToolbarLayout(380).showCamera).toBe(true);
    expect(browserToolbarLayout(300).showCamera).toBe(false);
    const recording = browserToolbarLayout(300, { recording: true });
    expect(recording.showCamera).toBe(true);
    expect(recording.urlWidth).toBeGreaterThanOrEqual(BROWSER_TOOLBAR_URL_MIN_WIDTH - 28);
  });

  it("drops Inspect and Attach entirely in a host with nowhere to insert them", () => {
    const shell = browserToolbarLayout(900, { hasSelection: true, canAttachContext: false });
    expect(shell.showInspect).toBe(false);
    expect(shell.showAttach).toBe(false);
  });
  ...
  it("assumes room when the row has not been measured yet", () => {
    ...
    expect(browserToolbarLayout(null).showPopOut).toBe(true);
  });
});
