  ...
  for (const envelope of events) {
    ...
    if (event.type === "user_message") {
      ...
      for (const context of event.contextAttachments ?? []) {
        ...
        // Deliberately NOT gated on the Linear surface (no code change here):
        // this is a past turn, and the row opens linear.app rather than a
        // compiled ADE page, so there is no way in to close. Polarity rule:
        // `../plugins/builtinTabs.ts`.
        addExternalResource(
          external,
          context.issue.url,
