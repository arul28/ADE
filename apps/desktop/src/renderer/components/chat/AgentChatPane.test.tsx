...
function buildCodexSteeringTranscript(sessionId: string): string {
  return `${buildStatusStartedTranscript(sessionId)}${JSON.stringify({
    sessionId,
    timestamp: "2026-03-24T05:57:45.800Z",
    event: {
      type: "approval_request",
      itemId: "steer-1",
      kind: "tool_call",
...
describe("AgentChatPane submit recovery", () => {
  ...
  it("keeps in-pane session tabs working while Codex has a non-blocking steering question", async () => {
    const session = buildSession("session-1");
    installAdeMocks({
      transcript: buildCodexSteeringTranscript(session.sessionId),
    });

    renderTabbedPane(session);

    expect(await screen.findByLabelText("Agent working")).toBeTruthy();
    expect(screen.queryByLabelText("Waiting for your input")).toBeNull();
  ...
  it("drops the scheduled-wake marker when the user retries an auto-resumed turn", async () => {
    ...
    // as the schedule's own turn and skip the cancel it promises.
    const session = buildSession("session-1", { status: "idle", awaitingInput: false });
    const { send } = installAdeMocks({
      sessions: [session],
      eventHistory: {
            ...
            event: {
              ...
              metadata: {
                scheduledWake: {
                  ...
                  firedAt: "2026-07-10T20:30:00.000Z",
                },
  ...
  it("says a turn is running inside the handoff modal instead of hiding the form", async () => {
    const session = buildSession("session-1");
    installAdeMocks({
      transcript: buildStatusStartedTranscript(session.sessionId),
    });
    ...
    expect((await screen.findByRole("button", { name: "Fork chat" }) as HTMLButtonElement).disabled).toBe(true);
  });
