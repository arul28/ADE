        ...
        <div className="border-b border-border/15 bg-amber-500/5 px-2 py-2">
          ...
          {showAutoRebaseSettingsHint ? (
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded border border-sky-500/25 bg-sky-500/8 px-2 py-1.5">
              ...
              <Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={onOpenAutoRebaseSettings}>
                Open settings
              </Button>
            </div>
          ) : null}
              ...
              return (
                <div key={`restack:${s.laneId}`} className="flex flex-wrap items-start justify-between gap-2 rounded shadow-card bg-card/40 p-2">
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    <Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" disabled={Boolean(restackBusyLaneId)} onClick={() => onDeferRestack(s.laneId, 60)}>
                      Defer 1h
                    </Button>
                    <Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" disabled={Boolean(restackBusyLaneId)} onClick={() => onDismissRestack(s.laneId)}>
                      Dismiss
                    </Button>
                    <Button size="sm" variant="primary" className="h-7 px-2 text-[11px]" disabled={Boolean(restackBusyLaneId)} onClick={() => onRestackNow(s.laneId)}>
                ...
                <div key={`auto-rebase:${status.laneId}`} className="flex flex-wrap items-start justify-between gap-2 rounded bg-card/40 p-2 shadow-card">
                  ...
                  <div className="shrink-0">
                    {status.state === "rebaseConflict" ? (
                      ...
                      <Button size="sm" variant="primary" className="h-7 px-2 text-[11px]" onClick={() => onRestackNow(status.laneId)}>
                        <Layers3 className="mr-1 h-3 w-3" />
                        Restack now
                      </Button>
                    )}
