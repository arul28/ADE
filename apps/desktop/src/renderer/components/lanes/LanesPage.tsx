...
export function LanesPage() {
  ...
  const filteredLanes = useMemo(() => {
    const needle = laneFilter.trim().toLowerCase();
    if (!needle) return sortedLanes;
    return sortedLanes.filter((lane) => {
      const haystack = `${lane.name} ${lane.branchRef} ${lane.laneType}`.toLowerCase();
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const nextIdx = (currentIdx + dir + filteredLaneIds.length) % filteredLaneIds.length;
  ...
  }, [filteredLaneIds, filteredSet, selectedLaneId, selectLane, pinnedLaneIds, lanesById]);
      ...
      <div className="border-b border-border px-2 py-1.5">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="h-7 min-w-[220px] rounded border border-border bg-card/70 px-2 text-xs outline-none placeholder:text-muted-fg"
          />
          <Button
            size="sm"
            variant="outline"
          ...
          <div className="ml-auto text-[11px] text-muted-fg">Shift/Cmd/Ctrl-click to split · [ ] navigate · / filter</div>
        </div>
      </div>
