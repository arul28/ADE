...
const DEFAULT_FILTERS: LinearQuickFilters = {
  projectId: "",
  statePreset: "all",
  assigneeId: "",
  priority: "",
...
function safeLoadFilters(projectRoot: string | null | undefined): LinearQuickFilters {
  const key = storageKey(projectRoot);
  if (!key || typeof window === "undefined") return DEFAULT_FILTERS;
  try {
    const parsed = JSON.parse(window.localStorage.getItem(key) ?? "null") as Partial<LinearQuickFilters> | null;
...
function safeSaveFilters(projectRoot: string | null | undefined, filters: LinearQuickFilters): void {
  const key = storageKey(projectRoot);
  ...
  try {
    ...
    // Best effort only; losing this preference should never break the picker.
  }
}

function issueListKey(issue: NormalizedLinearIssue): string {
...
function toLaneIssue(issue: NormalizedLinearIssue | LaneLinearIssue): LaneLinearIssue {
...
export function LinearQuickViewButton() {
  ...
  const [filters, setFilters] = useState<LinearQuickFilters>(() => safeLoadFilters(project?.rootPath));
  ...
  const [createdLaneName, setCreatedLaneName] = useState<string | null>(null);
  ...
  useEffect(() => {
    setFilters(safeLoadFilters(project?.rootPath));
    setIssues([]);
  ...
  const loadQuickView = useCallback((force = false) => {
    ...
    void window.ade.cto.getLinearQuickView()
      .then((data) => {
        ...
        setQuickView(data);
        setVisible(data.connection.connected === true);
      })
      .catch((err) => {
        if (requestIdRef.current !== requestId) return;
  ...
  const loadCatalog = useCallback(() => {
    ...
    if (!cto?.getLinearIssuePickerData) return;
  ...
  const searchIssues = useCallback((append: boolean) => {
    const cto = window.ade.cto;
    if (!cto?.searchLinearIssues) return;
  ...
  }, [filters, open, searchIssues]);

  const updateFilters = useCallback((patch: Partial<LinearQuickFilters>) => {
    const next = { ...filters, ...patch };
    setFilters(next);
    safeSaveFilters(project?.rootPath, next);
  }, [filters, project?.rootPath]);

  const resetFilters = useCallback(() => {
    setFilters(DEFAULT_FILTERS);
    safeSaveFilters(project?.rootPath, DEFAULT_FILTERS);
    setIssues([]);
    setPageInfo({ hasNextPage: false, endCursor: null });
  }, [project?.rootPath]);
  ...
  const displayIssues = useMemo(
    () => sortedIssues(issues, filters.sort),
    [filters.sort, issues],
  );
  ...
  const createLaneForIssue = useCallback(async (issue: NormalizedLinearIssue | LaneLinearIssue) => {
    ...
    try {
      ...
      selectLane(lane.id);
      setCreatedLaneName(lane.name);
      close();
      window.location.hash = `#/lanes?laneId=${encodeURIComponent(lane.id)}&focus=single`;
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to create lane for Linear issue.");
    } finally {
    ...
    <>
      ...
      {open && position ? createPortal(
        <div
          ...
          aria-modal="false"
          aria-label="Linear quick view"
          className="fixed z-[9999] w-[min(1040px,calc(100vw-24px))] overflow-y-auto rounded-xl border border-white/10 bg-[color:var(--ade-shell-surface,#121019)] text-fg shadow-2xl shadow-black/50"
          style={{
            top: position.top,
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                disabled={loading || loadingIssues}
                title="Refresh Linear"
              >
                {loading || loadingIssues ? <CircleNotch size={12} className="animate-spin" /> : null}
                Refresh
              </button>
              <button
                type="button"
          ...
          {error ? (
            <div className="mx-4 mt-3 flex items-start gap-2 rounded-lg border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-[12px] text-amber-100">
          ...
          {createdLaneName ? (
            <div className="mx-4 mt-3 rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-[12px] text-emerald-100">
          ...
          <div className="grid min-h-0 md:grid-cols-[232px_minmax(0,1fr)_334px]">
            ...
            <IssueDetails
              issue={selectedIssue}
              creating={selectedIssue ? creatingIssueId === selectedIssue.id : false}
              onCreateLane={createLaneForIssue}
            />
          </div>
        </div>,
        document.body,
...
function IssueDetails({
  ...
  creating: boolean;
  onCreateLane: (issue: NormalizedLinearIssue) => void | Promise<void>;
}) {
  if (!issue) {
    return (
    ...
    <aside className="flex min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        ...
        <div className="mt-3 grid gap-1.5 text-[11px] text-muted-fg/65">
          ...
          <InfoRow label="Started" value={formatDate(issue.startedAt)} />
          <InfoRow label="Completed" value={formatDate(issue.completedAt)} />
          <InfoRow label="Canceled" value={formatDate(issue.canceledAt)} />
          <InfoRow label="Open blockers" value={issue.hasOpenBlockers ? `${issue.blockerIssueIds.length}` : "None"} />
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-white/10 px-4 py-3">
        <Button
          variant="primary"
          disabled={creating}
          onClick={() => void onCreateLane(issue)}
        >
          {creating ? <CircleNotch size={14} className="animate-spin" /> : <Plus size={14} />}
          Create lane
        </Button>
        {issue.url ? (
