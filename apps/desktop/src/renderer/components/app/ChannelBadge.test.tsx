...
import { ADE_OPEN_CHANNEL_NOTICE_EVENT } from "../../lib/packageChannel";

const originalAde = globalThis.window.ade;

function installAde(packageChannel: string, appVersion = "1.2.51") {
  const getInfo = vi.fn(async () => ({ appVersion, packageChannel }));
  (globalThis.window as unknown as { ade: unknown }).ade = {
    app: { packageChannel, getInfo },
  };
  return { getInfo };
}
...
describe("ChannelBadge", () => {
  ...
  it("asks for the early-build notice when clicked", () => {
    installAde("beta");
    const listener = vi.fn();
    window.addEventListener(ADE_OPEN_CHANNEL_NOTICE_EVENT, listener);
    render(<ChannelBadge version="1.2.51" />);
    fireEvent.click(screen.getByRole("button"));
    window.removeEventListener(ADE_OPEN_CHANNEL_NOTICE_EVENT, listener);
