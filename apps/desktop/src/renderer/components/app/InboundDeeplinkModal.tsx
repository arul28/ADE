...
function originalTargetLabel(target: AppNavigationTarget, entity: "chat" | "lane"): string {
  if ((target.kind === "work" || target.kind === "chat") && target.sessionId) {
    return `Chat ${target.sessionId}`;
  }
  if (target.kind === "lane") return "Lane";
  return entity === "chat" ? "Chat" : "Lane";
}

function getTargetEnvelope(target: AppNavigationTarget): DeeplinkEnvelope | null {
  if ((target.kind === "work" || target.kind === "chat" || target.kind === "lane") && target.envelope) {
...
export function InboundDeeplinkModal({
  ...
  const [currentTarget, setCurrentTarget] = React.useState<InboundDeeplinkTarget>(() =>
    normalizeInboundTarget(target)
  );
  ...
  if (!branchTarget) {
    if (currentTarget.kind === "switch-project") {
      const envelope = getTargetEnvelope(currentTarget.original);
      const repoLabel = envelope?.repoOwner && envelope.repoName
        ? `${envelope.repoOwner}/${envelope.repoName}`
        : null;
      ...
      return renderFrame(
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {rows.map(([label, value]) => (
            ...
            >
              ...
              <div
                style={{
                  fontFamily: label === "Project" || label === "Opens" ? SANS_FONT : MONO_FONT,
                  fontSize: 12,
        ...
        <>
          ...
          <button
            ...
            disabled={busy}
            style={primaryButton({ height: 34, opacity: busy ? 0.6 : 1 })}
          >
            {busy ? "Switching..." : "Switch project and open"}
          </button>
        </>,
      );
    }
        ...
        <button
          ...
          onClick={() => {
            requestLinearIssueQuickView({
              issueIdentifier: envelope.linearIssue!,
              source: "deeplink",
            });
            onClose();
          }}
    ...
    return renderFrame(
      ...
      <>
        <div style={{ fontFamily: SANS_FONT, fontSize: 13, color: COLORS.textPrimary, lineHeight: 1.5 }}>
          {headline}
        </div>
        {!repoLabel ? (
          <div style={{ fontFamily: SANS_FONT, fontSize: 12, color: COLORS.textSecondary, lineHeight: 1.5 }}>
            The link didn't carry anything this machine can open.
          </div>
        ) : null}
        {actions.length > 0 ? (
  ...
  if (preflight?.preflight) {
    ...
    if (branchTarget.prNumber) {
      rows.push(["PR", `#${branchTarget.prNumber}`]);
    } else {
