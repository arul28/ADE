...
// runMaintenanceNow / getRuntimeHealth are being added on the main side by a
// sibling workstream. Feature-detect them via a narrow cast so this renderer
// compiles and degrades gracefully whether or not the daemon exposes them yet.
function getRunMaintenanceNow(): RunMaintenanceNow | undefined {
  const fn = (window.ade?.storage as { runMaintenanceNow?: unknown } | undefined)?.runMaintenanceNow;
  return typeof fn === "function" ? (fn as RunMaintenanceNow) : undefined;
}

function getRuntimeHealthFn(): GetRuntimeHealth | undefined {
  const fn = (window.ade?.app as { getRuntimeHealth?: unknown } | undefined)?.getRuntimeHealth;
  return typeof fn === "function" ? (fn as GetRuntimeHealth) : undefined;
}
...
function MaintenanceJournal({ extras }: { extras: StorageSnapshotExtras | undefined }) {
