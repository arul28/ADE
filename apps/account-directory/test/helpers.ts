...
/**
 * Shared test harness for the account directory.
 *
 * The fake D1 below is the whole reason the suites can be split: it mirrors the
 * SQL the Worker actually issues — predicates included — so a statement that
 * narrows or widens shows up as a failing assertion rather than being absorbed.
 * Every suite that touches machines, revocations, or pairing grants needs it,
 * so it lives here and the suites stay about one thing each.
 */
