...
struct WorkAdeCardView: View, Equatable {
  static func == (lhs: WorkAdeCardView, rhs: WorkAdeCardView) -> Bool {
    // `declaredPanel` and not the whole contribution index: the index carries
    // every contribution for the chat, and comparing it would make each row
    // diff walk a table that has nothing to do with this card. The resolved
    // panel is the only part of it this view draws — and it must be compared,
    // because declarations arrive one round trip after the card does. Without
    // it the equality gate holds the pre-answer render forever and the panel
    // never appears.
    lhs.card == rhs.card
