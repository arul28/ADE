...
struct HubCreatedChat: Equatable {
  ...
  let isCli: Bool
  /// Provider the session was created with (e.g. "claude", "cursor"), so the
  /// toast's Open shortcut can seed a stub with the real chat toolType instead
  /// of guessing from nil.
  let provider: String?
}
