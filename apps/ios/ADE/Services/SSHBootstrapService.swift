...
actor SSHBootstrapService {
  ...
  sh -lc 'IFS= read -r payload || exit 64; attempted=0; output=$(mktemp "${TMPDIR:-/tmp}/ade-pair.XXXXXX") || exit 70; cleanup() { rm -f "$output"; }; trap cleanup EXIT HUP INT TERM; for entry in "stable:.ade" "beta:.ade-beta" "alpha:.ade-alpha"; do channel=${entry%%:*}; home=${entry#*:}; socket="$HOME/$home/sock/ade.sock"; binary="$HOME/$home/bin/ade"; if [ -S "$socket" ] && [ -x "$binary" ]; then attempted=1; if printf "%s\n" "$payload" | env ADE_HOME="$HOME/$home" ADE_PACKAGE_CHANNEL="$channel" "$binary" --socket "$socket" sync pair-device --json-stdin >"$output"; then cat "$output"; exit 0; fi; fi; done; for entry in "stable:.ade" "beta:.ade-beta" "alpha:.ade-alpha"; do channel=${entry%%:*}; home=${entry#*:}; socket="$HOME/$home/sock/ade.sock"; binary="$HOME/$home/bin/ade"; if [ -x "$binary" ]; then attempted=1; if printf "%s\n" "$payload" | env ADE_HOME="$HOME/$home" ADE_PACKAGE_CHANNEL="$channel" ADE_RUNTIME_SOCKET_PATH="$socket" "$binary" sync pair-device --json-stdin >"$output"; then cat "$output"; exit 0; fi; fi; done; if [ "$attempted" -eq 0 ]; then echo ADE_CLI_NOT_FOUND >&2; exit 127; fi; exit 1'
  """
