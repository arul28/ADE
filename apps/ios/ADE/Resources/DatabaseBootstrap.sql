...
create index if not exists idx_queue_rehearsal_state_group on queue_rehearsal_state(group_id);

alter table queue_rehearsal_state add column config_json text not null default '{}';

alter table queue_rehearsal_state add column scratch_lane_id text;

alter table queue_rehearsal_state add column active_pr_id text;

alter table queue_rehearsal_state add column active_resolver_run_id text;

alter table queue_rehearsal_state add column last_error text;

alter table queue_rehearsal_state add column wait_reason text;

alter table queue_rehearsal_state add column updated_at text;
