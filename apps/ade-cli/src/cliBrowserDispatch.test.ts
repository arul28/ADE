 ...
 * shape resolves to and which action args it produces.
 *
 * Split from `cliBrowserGrammar.test.ts`, which is a different job: that file
 * parses cli.ts's own source with the TypeScript compiler API to prove the
 * flag-coverage scans cannot go blind. It shares no machinery with this one.
