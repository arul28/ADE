...
import { resolveMachineAdeLayout, type MachineAdeLayout } from "./machineLayout";
...
function coerceRecord(value: unknown): ProjectRecord | null {
  ...
  const rootPath = typeof record.rootPath === "string" ? normalizeRoot(record.rootPath) : "";
  ...
  return {
    ...
    addedAt: typeof record.addedAt === "number" && Number.isFinite(record.addedAt)
      ? record.addedAt
      : now,
    lastOpenedAt: typeof record.lastOpenedAt === "number" && Number.isFinite(record.lastOpenedAt)
      ? record.lastOpenedAt
      : now,
    gitOriginUrl: typeof record.gitOriginUrl === "string" && record.gitOriginUrl.trim()
...
export class ProjectRegistry {
  ...
  add(rootPath: string): ProjectRecord {
    ...
    const existingIndex = file.projects.findIndex(
      (record) => record.projectId === projectId || record.rootPath === normalized,
    );
  ...
  remove(projectId: ProjectId): boolean {
    const file = this.read();
    const nextProjects = file.projects.filter((record) => record.projectId !== projectId);
  ...
  touch(projectId: ProjectId): ProjectRecord {
    const file = this.read();
    const index = file.projects.findIndex((record) => record.projectId === projectId);
  ...
  private read(): ProjectRegistryFile {
    try {
      ...
      const projects = Array.isArray((parsed as { projects?: unknown }).projects)
        ? (parsed as { projects: unknown[] }).projects.map(coerceRecord).filter((entry): entry is ProjectRecord => entry != null)
        : [];
      return { version: 1, projects };
    } catch (error) {
  ...
  private write(file: ProjectRegistryFile): void {
    ...
    fs.mkdirSync(path.dirname(this.layout.projectsPath), { recursive: true, mode: 0o700 });
