...
function LaneHeader({
  ...
  const ruleLen = Math.max(
    HEADER_RULE_MIN,
    width - GUTTER_CELLS - terminalDisplayWidth(name) - suffix.length - 1,
  );
  return (
  ...
  return (
    <Box flexDirection="column" width={width} marginTop={marginTop} flexShrink={0}>
      ...
      {height >= 3 ? (
        <Text>
          <Text color={railColor}>{restGutter}</Text>
          <MetaLine row={row} width={innerWidth} />
        </Text>
      ) : null}
    </Box>
  );
 ...
 */
function MetaLine({ row, width }: { row: WorkListSessionRow; width: number }) {
  const extras = `${row.marker ? ` ${row.marker}` : ""}${row.hasDraft ? " ✎" : ""}${
    row.machine ? ` ⧉ ${row.machine.name}` : ""
  }`;
  const preview = row.preview?.text ?? "";
  const leftMax = Math.max(1, width - terminalDisplayWidth(extras));
  return (
    <Text>
