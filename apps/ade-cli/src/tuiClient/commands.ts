...
export const BUILTIN_COMMANDS: BuiltinCommand[] = [
  ...
  { name: "/plugin", description: "List Codex plugins (read-only)", placement: "right", providers: ["codex"], category: "Model" },
  ...
  { name: "/cloud", description: "List Cursor Cloud agents for this account", placement: "right", category: "Nav" },
  ...
  // Read-only mirror of the desktop's Work tools pane, the same contract iOS
  // and the hosted web client render. Notably it is how a terminal user learns
  // an agent has handed a browser tab back for them to sign in — without it the
  // lane just looks quiet.
  { name: "/tools", description: "Show the desktop Work tools state for this lane", placement: "right", argumentHint: "[lane-id|name]", category: "Nav" },
  ...
  // An orchestration parent can surface a stalled child turn, so recovery
  // availability is determined from that event's target session.
  { name: "/recover", description: "Recover the latest stalled turn", placement: "right", argumentHint: "<wait|nudge|retry|resume> [turn-id]", category: "Chats" },
  { name: "/run-next", description: "Run an accepted but unprocessed message next", placement: "right", argumentHint: "<steer-id> [session-id]", category: "Chats" },
  { name: "/edit-message", description: "Restore an unprocessed message to the composer", placement: "right", argumentHint: "<steer-id> [session-id]", category: "Chats" },
  { name: "/dismiss-message", description: "Dismiss an accepted but unprocessed message", placement: "right", argumentHint: "<steer-id> [session-id]", category: "Chats" },
...
const LEGACY_LOCAL_COMMAND_ALIASES: Readonly<Record<string, string>> = {
  "/attention": "/activity",
};
...
export function parseCommand(input: string, userCommands: AgentChatSlashCommand[] = []): ParsedCommand | null {
  ...
  const [legacyName = ""] = trimmed.split(/\s+/, 1);
  const replacement = LEGACY_LOCAL_COMMAND_ALIASES[slashCommandKey(legacyName)];
  if (replacement) trimmed = `${replacement}${trimmed.slice(legacyName.length)}`;
