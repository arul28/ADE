...
function paneFrame(args: {
  ...
  return stripAnsi(render(
    <WorkSessionsPane
      ...
      pluginLaneBadges={args.pluginLaneBadges ?? {}}
      pluginChatBadges={args.pluginChatBadges ?? {}}
    />,
  ).lastFrame() ?? "");
}
...
describe("WorkSessionsPane plugin row badges", () => {
  ...
  it("draws a contributed badge on the lane header and the chat card", () => {
    // Two live chats, so the lane keeps a header row of its own to badge.
    const frame = paneFrame({
      lanes: [lane("lane-1", "Feature")],
      sessions: [
      ...
      pluginChatBadges: { "chat-a": strip("$1.20") },
    });

    expect(rowFor(frame, "Feature")).toContain("[risk 8]");
  ...
  it("keeps the lane's own name when a badge cannot fit beside it", () => {
    // A 22-column pane has no room for a 15-character badge, so the badge drops
    // out entirely rather than truncating the lane's identity.
    const frame = paneFrame({
      width: 22,
      lanes: [lane("lane-1", "Feature")],
      sessions: [
