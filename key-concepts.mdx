...
<Info>**📸 Screenshot needed:** A high-level diagram (drawn in ADE's Workspace Graph view) showing a project with a Primary Lane, two Worktree Lanes arranged in a Stack, an open Mission, and a Conflict indicator between two lanes. Labels visible for "Lane", "Stack", "Mission", "Conflict".</Info>
...
<Info>**📸 Screenshot needed:** The Lanes panel showing three lanes: one Primary (with a house icon), one Worktree (green branch icon), and one Attached (chain-link icon). Show the branch name, status badge, and an "active agent" indicator on the Worktree lane.</Info>
...
<Info>**📸 Screenshot needed:** The CTO view showing the CTO agent's chat interface with a recent message like "I've reviewed this week's missions. Auth lane is blocked by a conflict with the billing lane — I've created a conflict pack and assigned a resolver." Show the Linear sync indicator and the "Memory updated" badge.</Info>
...
An **Automation** is a trigger → executor rule that fires an AI agent in the background when a specific event occurs.

### Trigger Types

<CardGroup cols={2}>
  <Card title="Git Events" icon="git-alt">
    Fires on `push`, `pr_opened`, `pr_merged`, `branch_created`, or `status_changed`. Example: auto-run tests when a PR is opened.
  </Card>
  <Card title="Linear Events" icon="list">
    Fires when a Linear issue changes state (e.g., `In Progress` → `In Review`). Example: auto-create a PR when an issue is marked for review.
  </Card>
...
**Night Shift** is a special automation mode that queues up missions to run while you are away. Enable it in the Automations view. ADE drains the Night Shift queue overnight and presents a summary the next morning. Per-run budget limits are enforced so you do not wake up to unexpected bills.
...
<Info>**📸 Screenshot needed:** The Conflicts view showing a risk matrix with three lanes across the top/left, one red high-risk cell, and a details panel on the right showing the overlapping file names and an AI resolution proposal button.</Info>
...
| **Night Shift** | A special automation mode that runs queued work while the developer is away. |
