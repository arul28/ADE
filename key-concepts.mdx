...
A **Worker** (also called an **Employee Agent**) is a sub-agent spawned by the CTO or by a Worker run to handle a specific, scoped task. Workers are the agents that actually write code.
...
| **Worker / Employee Agent** | A sub-agent spawned by the CTO or a Worker run to handle a specific task. |
