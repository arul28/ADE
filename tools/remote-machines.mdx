...
<Note>
Remote machines run macOS or Linux. A Windows desktop can drive them when it is running an installed release build — a locally packaged Windows build ships no macOS or Linux runtime payloads and will refuse. A Windows machine cannot itself be a remote target — ADE cannot bootstrap its runtime onto Windows over SSH. See [ADE on Windows](/getting-started/windows).
</Note>
