...
4. **Convergence rounds** — For PRs with multiple issues, ADE can run multiple resolution rounds automatically. Set the `prs.convergence.maxRounds` key in `.ade/ade.yaml` to cap the number of iterations.

### Pipeline settings

Configure PR convergence behavior per-project. The `prs` section in `.ade/ade.yaml` is optional — projects may omit it entirely and default behavior will apply.
