...
<Frame caption="PRs tab — split-pane with PR list, status badges, CI checks, and reviewer avatars">
  <img src="/images/prs-overview.png" alt="PRs tab showing PR list with status badges on left, and PR detail with CI checks, reviewer avatars, and markdown description on right" />
</Frame>
...
<Frame caption="Stacked PR list with dependency arrows, CI status, and approved/open/draft badges">
  <img
    src="/images/pr-list-stacked-three-prs-merge-order-ci.png"
    alt={`The PR list showing a stack of 3 PRs — "auth-base", "auth-middleware", "auth-endpoints" — connected by vertical dependency arrows indicating merge order (bottom up). Each PR shows its CI status indicator. The bottom PR ("auth-base") has a green "Approved" badge, the middle is "Open", and the top is "Draft".`}
  />
</Frame>
...
<Frame caption="PR Overview CI checks with failed e2e log expanded and Ask CTO button">
  <img
    src="/images/pr-overview-ci-checks-e2e-failure-ask-cto.png"
...
ADE includes a built-in **PR issue resolution** workflow for addressing review comments and failing CI checks directly from the PRs tab.

### How it works

1. **Issue inventory** — ADE scans the PR for failing checks, review comments requesting changes, and merge conflicts. Each item becomes an entry in the issue inventory.
2. **Resolution modal** — Click **Resolve issues** on a PR to open the resolution modal. It lists all open issues with their type and severity.
3. **AI resolution** — Select issues and click **Fix with AI**. ADE launches an agent session scoped to the PR's lane with the issue context pre-loaded. The agent reads the failing check output or review comment, makes fixes, and commits them.
4. **Convergence rounds** — For PRs with multiple issues, ADE can run multiple resolution rounds automatically. Configure `maxRounds` in pipeline settings to cap the number of iterations.

### Pipeline settings

Configure PR convergence behavior per-project:

```yaml
# .ade/ade.yaml
prs:
  convergence:
    autoMerge: false              # auto-merge when all checks pass
    mergeMethod: "squash"         # merge | squash | rebase
    maxRounds: 3                  # max AI resolution rounds
    onRebaseNeeded: "auto"        # auto | ask | skip
```
