...
<CardGroup cols={2}>
  ...
  <Card title="Guardrails" icon="shield-halved">
    Confidence threshold, human review, path/lane allowlists, and optional agent time limits.
  </Card>
...
An automation's agent has **no time limit by default**. Like a chat you start yourself, it runs until the work is done — a release that waits an hour on CI and notarization is fine. If you want a backstop, open **Limits** on the agent step:

- **Stop after** — end the agent's turn after this many minutes, whatever it is doing.
- **Stop when idle for** — end it after this many minutes with no activity. Waiting on a command, a tool, a subagent working in the foreground, or an approval does not count as idle; background tasks and long silent thinking can, so leave some room.

Either one interrupts the agent and keeps its chat open. A **Run command** step has its own **Time limit** (default 5 minutes) that stops the command if it hangs.
