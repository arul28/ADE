...
ADE gives every lane real **PTY sessions** — full xterm.js terminal emulators with VT100/ANSI support, interactive programs, and color output — each scoped to the lane's worktree and environment overlay, isolated from every other lane.
...
<AccordionGroup>
  ...
  <Accordion title="Agent Sessions" icon="robot">
    When an AI agent (running inside a Worker run, Chat, or Automation) executes terminal commands as part of its task, those commands run in an **Agent Session**. Agent sessions are:
    ...
    - **Fully captured**: Every command and every line of output is captured in the session transcript and linked to the worker run or chat turn that spawned it.
    - **Attributable**: The session metadata records which agent model, which worker run/chat ID, and which task step spawned this session.

    <Warning>
    Do not kill an agent session manually while a worker run is running — this will cause the worker run to fail with a terminal error. Use the worker run's **Stop** button in the worker detail panel instead, which handles cleanup gracefully.
    </Warning>
  </Accordion>
</AccordionGroup>
...
| **Agent ID** | For Agent sessions: the agent/worker run that spawned it |
...
<AccordionGroup>
  ...
  <Accordion title="Agent sessions and permission gates" icon="shield-check">
    ...
    A prompt appears in the terminal panel and in the worker detail panel asking you to approve or deny the command. The agent waits for your decision before proceeding.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Workers" icon="bullseye" href="/cto/workers">
    Understand how worker runs spawn and manage Agent Sessions during execution.
  </Card>
</CardGroup>
