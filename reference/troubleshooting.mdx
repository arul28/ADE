...
If a check fails and you'd rather not work it by hand, `ade triage` hands the whole thing to your own coding agent: it writes a redacted machine context plus a repair playbook to a temp directory and starts the first agent CLI it finds (`claude`, `codex`, `cursor-agent`, `opencode`, `droid`). Inside an agent session already, run `ade triage --agent` — it prints the two file paths and a prompt instead of launching anything.
