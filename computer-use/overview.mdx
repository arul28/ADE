...
<Tabs>
  <Tab title="Ghost OS (Recommended)">
    Ghost OS is an external CLI for full macOS desktop automation. It provides complete control over the desktop environment — browsers, native apps, file dialogs, and system UI.
    ...
    **Connection:** detected from the `ghost` CLI on `PATH`.
  ...
  <Tab title="agent-browser">
    ...
    **Connection:** detected from the `agent-browser` CLI on `PATH`.
