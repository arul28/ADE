...
A consolidation release: the Lanes and Runs surfaces shed several legacy panels in favour of fewer, denser screens; the chat transcript adds an in-line file-changes panel; and the renderer picks up batched user-preference persistence, a visibility-aware watchdog, and a unified warmup timer.
...
- The `LaunchPanel` terminal launcher is gone — terminal launches live in Work.

### Runs UI consolidation

The bespoke run micro-pages were absorbed into the main Runs experience:

- `RunControlPage`, `RunPhaseBadge`, `UsageDashboard`, and `WorkerTranscriptPane` are removed — their content now renders inside `RunsPage` and the associated panels there.
...
The internal architecture notes were reorganized: legacy `docs/architecture/*` and `docs/final-plan/*` trees were removed in favor of a new `docs/features/*` hierarchy with per-feature READMEs. The overall docs layout is captured in a top-level `docs/ARCHITECTURE.md` and `docs/README.md`.
