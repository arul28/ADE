...
- **Onboarding improvements** — Refactored setup sections with improved UX, GitHub scope parsing with classic vs fine-grained permission normalization
...
- **Runs & orchestrator** — Improved lifecycle management, convergence pipeline integration in the phase engine
- **Local model discovery** — Cached provider inspection, `loadAttemptId` to ignore stale results, hardened local runtime path resolution
...
- **Stale local model loads** — Fixed race conditions in local model discovery
