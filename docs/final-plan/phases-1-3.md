# Phases 1-3: Foundation (Phase 3 Completion In Progress)
...
## Phase 3 -- AI Orchestrator (Completion Package In Progress — Project Hivemind Shipped)

Goal: Full AI-powered mission orchestration via a Claude session with in-process Vercel AI SDK coordinator tools. The AI orchestrator sits on top of the deterministic runtime (DAG scheduling, claims, context snapshots) while keeping strategic decisions AI-led. The remaining completion package focuses on team runtime foundations, validation loops, policy controls, and lane-safe autonomous rework.
...
- [phase-3-gaps.md](../phase-3-gaps.md) — Phase 3 completion blueprint (team runtime, validation loops, policy model, lane continuity)
...
The completion package now targets autonomous quality and role-driven orchestration. It is informed by Factory Missions best practices: milestone validation, targeted parallelism, planning clarifications, and role-specialized execution.
Reference: https://factory.ai/news/missions

#### W13: Mission Team Runtime Foundations
- Add team template schema for Missions: role roster, role constraints, default model routing, and guardrails.
- Required autonomous mission roles: coordinator, planner capability, validator capability.
- Optional roles (implementer, tester, reviewer, researcher, security specialist) can be added per mission template.
- Team describes capabilities, not fixed worker count; coordinator may spawn multiple workers from the same role.

#### W14: Structured Worker Reporting and Shared Mission Visibility
- Add worker tools: `report_status`, `report_result`, `read_mission_status`, `message_worker`.
- Standardize worker-to-coordinator status payloads (progress, blockers, confidence, next action).
- Show structured worker reports inline in Mission chat/detail (not only raw transcript text).

#### W15: Autonomous Replanning Contract
- Add coordinator tool `revise_plan` for full or partial plan replacement.
- Implement supersede semantics for replaced steps (auditable, not destructive deletion).
- Add staleness/retry triggers that recommend replanning while keeping final decision AI-led.

#### W16: Validation Contracts and Validator Loop
- Introduce `ValidationContract` at step/milestone/mission levels with explicit pass/fail criteria and required evidence.
- Execute validator workers at required gates (step optional, milestone required, mission required).
- Implement rework loop protocol: validator fails -> coordinator routes rework -> validator re-checks.
- Preserve milestone artifacts for each validation pass/fail cycle.

#### W17: Lane Affinity and Rework Continuity
- Enforce step-to-lane ownership across retries and worker replacement.
- Replacement worker inherits same lane and full handoff context by default.
- Explicit lane transfer only through coordinator action with timeline audit entry.

#### W18: Budget-Aware Orchestration Decisions
- Expose runtime budget state to coordinator via `get_budget_status`.
- Feed per-worker token/cost consumption into coordinator context for delegation decisions.
- Under pressure, reduce parallelism and defer optional work while preserving required validation gates.

#### W19: Tool Profile Runtime and Mission-Level MCP Profiles
- Add mission tool-profile schema and role-to-tool binding.
- Allow mission-level MCP profile selection and per-worker profile assignment.
- Support mid-run coordinator updates to tool profile when mission conditions change.

#### W20: Human-in-the-Loop Upgrade (Clarifications + Risk Gates)
- Add explicit plan clarification stage before execution (`always`, `auto_if_uncertain`, `off`).
- Add risk-based approval interventions for high-impact operations.
- Add worker `request_user_input` routed through coordinator.
- Normalize Claude/Codex permission/tool errors into a shared intervention schema.

#### W21: Partial Completion and Recovery Handoff
- Add `partially_completed` mission outcome with structured "done vs remaining" report.
- On coordinator unrecoverable failure, persist a recovery handoff artifact containing last stable plan, completed validations, open work, and lane map.

#### W22: Validation and Soak Testing
- Add end-to-end tests for team templates, policy precedence, validation loops, and lane continuity.
- Add long-horizon orchestrator soak tests (multi-hour) covering replans, retries, and budget pressure.
- Add provider parity tests for normalized permission/tool error behavior.
- Add mission quality tests verifying milestone-gated completion behavior.

### Exit criteria

- Missions launch AI-powered orchestration from plain-English prompts. **(Satisfied)**
- AI orchestrator plans and executes multi-step, multi-lane workflows using MCP tools. **(Partially satisfied — completion package W13-W22 in progress)**
...
- All six coordination patterns function: sequential, parallel fan-out, fan-in, plan-then-implement, review-and-revise, speculative parallel. **(Partially satisfied — fan-out shipped; full pattern hardening in completion package)**
...
- Activity feed, DAG visualization, and worker transcript tailing provide real-time mission observability. **(Activity feed + run narrative satisfied; structured worker reporting remains)**
...
- Team templates and mission policy flags drive orchestration behavior with clear precedence (workspace -> team -> mission).
- Validator gates are enforced for milestone and mission completion.
- Rework keeps lane continuity unless coordinator explicitly re-homes a step.
- Budget-aware delegation, partial completion outcomes, and provider-normalized interventions are production-ready.
