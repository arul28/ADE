...
- **Night Shift configuration**:
  ...
  | │ 3. Migrate legacy API endpoints  Codex          Budget: $3   │  |
  | └──────────────────────────────────────────────────────────────┘  |
  |                                                                    |
  | SUBSCRIPTION STATUS                                                |
...
The original Phase 4 was framed as an "Agents Hub" — a rebranding of Automations into a unified Agents tab with agent identities, agent types (Watcher, Review, Task, Concierge), an agent builder wizard, and chat-to-mission escalation. That entire framing has been eliminated:

- **Agent identities as a product surface** (creating reusable agent personas in a hub) — removed.
- **Watcher agents, Review agents, Task agents** as distinct agent types — removed.
- **The Agents tab entirely** — removed. Automations stays as Automations.
- **Agent Builder wizard** — removed.
- **Agent-First AI Runtime migration** (routing all non-interactive AI through agent runtimes) — removed.
- **Chat-to-mission escalation** — removed (may revisit in a later phase).
- **Concierge Agent** — evolved into the CTO Agent (W1), which is a different concept.
- **Agent schema, data model, identity system** (W2, W3 from old Phase 4) — removed.
- **Rebrand Automations → Agents** (W1 from old Phase 4) — removed. Automations stays.

### What Was Kept but Reframed

- **Memory architecture upgrade** — still applies, now serves both mission workers and the CTO agent.
- **Learning packs** — still relevant, feeds into the memory system.
- **External MCP consumption** — still relevant, both CTO and mission workers can use external MCP tools.
- **Night Shift** — moved from a separate agent type to a mode within the Automations tab.
- **Morning Briefing** — moved from the Agents tab to Automations as a sub-view (also available as a modal overlay).
- **Concierge concept** — evolved into the CTO tab, a persistent project-aware agent rather than an external routing endpoint.

### Missions Workstreams Consolidated into Phase 3

> **Note**: The original Phase 4 included nine missions-related workstreams (Mission Phases Engine, Phase Profiles, Mission Detail View Overhaul, Pre-Mission Launch System, Tiered Validation System, Intervention & Escalation Overhaul, Budget Management, Mission Introspection & Reflection Protocol, and Subscription Usage Tracking). These have all been consolidated into Phase 3 as part of the missions overhaul. See `phase-3.md` for their full specifications. The "What Was Removed" and "What Was Kept but Reframed" sections above refer to the even earlier Agents Hub framing and can be trimmed in a future cleanup pass, since `phase-3.md` now documents the missions overhaul comprehensively.
