...
**W5b (after W5a + stable orchestrator + W6½):** Full ADE tool access, executor modes, and external triggers
...
- **Automation services**:
  ...
  - `automationTriggerService`: local events plus GitHub/webhook adapters
...
- W5b mostly implemented:
  - Automation runtime supports local webhook ingress, GitHub relay polling, executor routing, automation-to-mission dispatch, Night Shift queueing, and rule-level tool-family allowlists.
  - Automation briefing assembly and employee-memory mode reuse the mission/CTO memory briefing path.
  - Remaining work is concentrated in review/publish polish, runtime edge cases, and the unfinished `external-mcp` family that depends on W8.
...
**Tests (W5b — remaining / recommended):**
