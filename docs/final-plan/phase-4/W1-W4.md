...
- **Default heartbeat configurations**:
  ...
  ADE now leans on persistent continuity plus event-driven wakeups. Workers default to wake-on-demand only, and any optional CTO timer should be treated as a cheap poller rather than the source of truth for continuity.
