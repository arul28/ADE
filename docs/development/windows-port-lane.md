...
- **Windows as an SSH-bootstrap target** — the standalone Windows brain is
  installable and updateable, but desktop remote bootstrap still detects and
  uploads only supported macOS/Linux targets. A Windows client can bootstrap
  those targets and reports actionable OpenSSH Client prerequisite diagnostics.
...
7. **Public gates** — enable `ADE_WINDOWS_SIGNED_BUILD_ENABLED=1` to produce
   ...
   flag disabled until the signed installer and mandatory two-version update
   proof pass. Only then may a maintainer enable public release and website flags.
