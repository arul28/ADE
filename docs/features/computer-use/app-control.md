...
Electron Control runs on the machine the chat runs on. The launch terminal, CDP attachment, screencast frames, screenshots, and source-matching all execute on that host; the renderer just streams the resulting frames and chips. Because Electron apps under inspection live on the host's filesystem, Electron Control naturally runs on whichever machine has the source tree — which is the machine of the chat driving it, not necessarily the machine the project tab is bound to. `ChatAppControlPanel` therefore takes a `runtimePin` and carries it on every call.
