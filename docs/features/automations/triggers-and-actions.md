...
- **Relay draining is page-durable.** The WebSocket is only a wake-up hint. Drain repo events with `order=asc&limit=100`, process pages in returned order, persist `nextCursor` only after the page succeeds, and immediately continue while `hasMore` is true; otherwise a burst can be skipped or replayed.
