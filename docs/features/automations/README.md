...
- **Relay wake-ups are not deliveries.** The repo WebSocket only requests a cursor drain. Coalesce concurrent drain requests with one dirty rerun, process ascending pages in returned order, and persist each `nextCursor` only after that page succeeds. Config/repo changes and service shutdown must close the old socket and clear every reconnect/connect/poll timer.
