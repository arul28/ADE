...
| `prCommentMutations.ts` | Shared GitHub comment PATCH and reaction decoding used by the timeline. `createPrCommentMutations` owns `updateGithubCommentByCoords` (GET then PATCH an issue or review comment after verifying PR membership and that `resolveWriteViewerLogin` matches the comment author). `toPrComment` / `toPrReactions` / `reactionToGraphqlEnum` normalize REST counts and GraphQL reaction nodes. `prService.updateComment` (IPC `ade.prs.updateComment`) and in-process `updateCommentByGithub` (review-session edits after a local PR row is gone) go through it; `reactToComment` still issues GraphQL `addReaction` in `prService`. |
