...
| `laneService.updateBranchRef(laneId, branchRef)` | Internal helper used after rename/import paths to keep the active profile and `lanes.branch_ref` in sync. After the transaction commits it emits the refresh-only `lane-updated` lifecycle event so Work hover details, Lanes, and both Git Actions panes replace stale branch identity immediately. |
