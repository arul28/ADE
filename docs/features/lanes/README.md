...
- `src/shared/issueRef.ts` — `IssueRef`, the provider-neutral issue a lane or session link carries. `ISSUE_REF_KEY` (`__issueRef`) is where it is stored inside the existing `issue_json` column; `readLinearIssueRef` / `readGitHubIssueRef` read it or derive one from the legacy fields; `issueRefToStoredLinearIssue` writes it beside a full legacy Linear projection so an older peer still parses the row; `issueRefRowKey` namespaces the `issue_id` column for non-Linear trackers; `canPluginUnlinkIssueRef` is the ownership check. Nothing here adds a column or a table — see [Rule 4 of the CRDT model](../sync-and-multi-device/crdt-model.md).
- `src/shared/issueRefFormat.ts` — provider-neutral formatting of an `IssueRef`: `issueRefLaneName`, `issueRefBranchName` (proven byte-identical to `linearIssueBranchName` for Linear identifiers), `issueRefPrReference` / `issueRefPrMagicWord` (`Fixes` for `linear`, `Closes` for `github`, `Refs` for every other provider regardless of `closeOnMerge`, because nothing else performs the close), and `dedupeIssueRefs`. The branch/lane namers exist but are not yet wired: lane creation still calls `linearIssueBranchName`.
...
- `primaryIssue?: IssueRef | null` and `issueLinks?: IssueLink[]` — the
  same two things again, **on whichever tracker owns them**. A lane's
  issue link is no longer Linear-specific: `IssueRef`
  (`apps/desktop/src/shared/issueRef.ts`) carries `provider`
  (`linear`, `github`, `jira`, …), `issueId`, `key`, `title`, `url`, an
  optional `state` whose `category` is one of
  `triage | backlog | unstarted | started | completed | canceled`, an
  optional `container` (a Linear team, a GitHub repo, a Jira project),
  and a `pluginId` naming who created the link (`core` for ADE
  itself). `IssueLink` wraps it with the same `role`, `source`,
  `includeInPr`, `closeOnMerge` and `evidence` the Linear link has —
  `IssueLinkRole` and `IssueLinkSource` are literal aliases of
  `LaneLinearIssueLinkRole` / `LaneLinearIssueLinkSource`, which never
  carried anything Linear-specific, plus one new source value
  `plugin_link`.

  Both fields are **derived, not stored**: `laneService` projects
  `primaryIssue` out of the lane's `lane_linear_issues` row and
  `issueLinks` out of its `lane_linear_issue_links` rows — the same rows
  that back `linearIssue` / `linearIssueLinks`, which stay populated
  beside them, so nothing that reads the Linear fields loses anything.
  `linkIssueRef` writes only the links table, so a link with
  `role: "primary"` records that role without becoming `primaryIssue`;
  the lane's primary row is still written only by the
  create-a-lane-from-an-issue path.
  A row written before `IssueRef` existed carries no ref and one is
  derived from its legacy fields (`readLinearIssueRef`), so there is no
  migration and no backfill.

  **No schema changed for this.** The ref rides inside the existing
  `issue_json` column under the reserved key `__issueRef`, beside a
  full legacy Linear projection of itself. A peer on an older build
  parses the projection and drops the unknown key — a Jira issue
  renders there under a Linear-labelled badge, which is a mislabel and
  not a break — and an older build that re-links an issue drops the key
  until the next link from a new build restores it. The `issue_id`
  column is namespaced `<provider>:<issueId>` for every tracker except
  `linear`, which keeps the bare id so existing rows are untouched. See
  [Rule 4 of the CRDT model](../sync-and-multi-device/crdt-model.md) for
  why it is a JSON key rather than a column.

  Writers: `laneService.linkIssueRef` / `unlinkIssueRef` /
  `listIssueLinks`. These are the store behind the plugin SDK's
  `ade.lanes.linkIssue` / `unlinkIssue`, which stamp the calling
  plugin's id onto the ref and let a plugin remove only its own links;
  `unlinkIssueRef` without `requirePluginId` is the user's path and can
  remove any link. See
  [features/plugins/README.md](../plugins/README.md#a-plugin-can-link-an-issue-to-a-lane).

  Consumers are partly migrated. `prService` collects a PR's issues
  through `collectLanePrIssueRefs` and formats magic words with
  `shared/issueRefFormat.ts`, but `collectLinearPrIssueReferences`
  still filters to `provider === "linear"` before writing the PR body,
  so a third-party ref is carried that far and dropped.
  `LinearIssueBadge`, `CreatePrModal`, `linearLaneCardService` and the
  Linear merge fan-out in `main.ts` still read the Linear-typed fields
  directly.
...
Note the name collision: `ade.lanes.*` below is the **preload** bridge in the
desktop renderer. The plugin SDK's `ade.lanes` is a different, much smaller
object inside a plugin child process (`list`, `get`, `linkIssue`,
`unlinkIssue`) — see
[features/plugins/README.md](../plugins/README.md#a-plugin-can-link-an-issue-to-a-lane).
`laneService.listIssueLinks` / `linkIssueRef` / `unlinkIssueRef` are service
methods with no IPC channel of their own; the renderer reads the same data off
`LaneSummary.primaryIssue` / `issueLinks`.
...
| `ade.lanes.linkLinearIssues` | `(args: { laneId, issues, role?, source?, includeInPr?, closeOnMerge?, evidence? }) => LaneLinearIssueLink[]` — link one or more Linear issues to an existing lane post-creation. Also surfaced as `lane.linkLinearIssues` through the ADE action registry and the `ade lanes link-linear-issue` CLI command. **Refused for plugins** reaching it through `ade.actions.invoke` (`apps/ade-cli/src/bootstrap.ts:673`): it records no owner, so a plugin's link would be indistinguishable from the user's. Plugins use `ade.lanes.linkIssue`. |
| `ade.lanes.unlinkLinearIssues` | `(args: { laneId, issueId? }) => boolean` — lane-level detach counterpart to `linkLinearIssues`. Omit `issueId` to remove every non-primary link; never touches the lane's primary issue (stored in `lane_linear_issues`). Refused for plugins, same reason and same replacement (`ade.lanes.unlinkIssue`). |
