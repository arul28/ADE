...
1. **Create** — `laneService.create()` resolves the base ref (explicit
   ...
   can show a global success toast without waiting for another list
   poll.
...
| `ade.lanes.lifecycle.event` (push) | `LaneLifecycleEvent` - one-shot `lane-created`, `lane-archived`, or `lane-deleted` event. Local desktop paths emit this IPC channel directly; runtime-backed paths push `lane_lifecycle_event`, and preload merges both sources behind `window.ade.lanes.onLifecycleEvent`. |
