...
| `laneEnvironmentProcesses.ts` | Runtime bookkeeping behind `laneEnvironmentService`: the event fan-out (runtime broadcast plus `onEvent` listeners, each isolated so a throwing listener cannot break init), the spawned setup commands still running keyed by working directory, and the cooperative cancellation flags (`cleanupRequested`, in-flight init counts) that archive/delete/reclaim and an aborted launch raise against an in-flight init. `abortLaneEnvironment` is the only path that kills already-spawned children. |
| `laneEnvironmentSetup.ts` | `runLaneEnvironmentSetup(deps, { laneId, templateId?, includeArchived? })` — the one way a lane's environment is set up after creation: with a lane template merged over the project's lane-environment config, or with the project config alone (a zero-step `completed` progress when neither applies). Shared by the `lane.initEnv` / `lane.applyTemplate` actions, the `ade.lanes.initEnv` / `ade.lanes.applyTemplate` IPC handlers, the sync host's `lanes.initEnv` / `lanes.applyTemplate` commands, and the chat-launch service, so they cannot drift. `includeArchived` defaults to true (the action domain's rule); IPC and sync pass false. Also owns `planNewLaneEnvironment`, which decides from project config alone — before the lane exists — whether a new lane gets any environment setup (a default lane template, a lane env-init config with work in it, or an enabled overlay policy carrying env init). |
| `laneEnvInitMerge.ts` | Dependency-free merge kernel for lane env-init configs and overlay overrides (`mergeLaneEnvInitConfig`, `mergeLaneDockerConfig`, `cloneLaneEnvInitConfig`, `mergeLaneOverrides`). One implementation shared by `projectConfigService`, `laneEnvironmentService`, and `laneEnvironmentSetup` (through which IPC, the ADE action registry, and the ade-cli sync command service reach it); it deliberately imports no services so config parsing can use it without an import cycle. |
| `laneOverlayContext.ts` | `resolveLaneOverlayContext(deps, laneId)` — resolves the lane, its matched overlay overrides (with the active port lease folded in via `applyLeaseToOverrides`), and the effective env-init config. Shared by env init and by the archive / delete / reclaim teardown paths so they cannot disagree about which compose project belongs to a lane. |
| `setupScriptConfig.ts` | `resolveSetupScriptConfig` picks the platform's setup commands / script path (Windows vs Unix variants, falling back to the generic ones) or returns `null` when nothing is configured for this platform; `unsupportedWindowsScriptPathError` rejects a script file Windows cannot launch with an actionable message. |
...
| `renderer/components/lanes/LanePrBadgePopover.tsx`, `LanePrHoverCard.tsx` | Current-branch-only lane badge and its shared fixed-position portal. The primary chip chooses the newest open/draft PR before falling back to recent terminal activity; the complete current-branch list is clamped to the viewport and dismissed on viewport scroll/resize so row/card overflow cannot hide it, while its own list remains scrollable. |
| `renderer/components/lanes/lanePageModel.ts` | Pure lane-page selectors and URL/deletion helpers used by `LanesPage` and unit tests. Owns `selectLanePrs` — strictly lane-owned, because callers pass the project-wide PR list, with only the branch half relaxed so a PR this lane owns and a chat linked stays visible after a branch move — and its chat-side counterpart `selectChatPrs`, which adds the pull requests this chat linked even though another lane owns them (see [Pull requests](../pull-requests/README.md#lane-surfaces-and-chat-surfaces-need-two-different-selectors)). Also base-branch suppression for the primary lane, lane branch/PR role derivation (`active` vs `previous`) for history-aware callers, multi-PR ordering, same-repo GitHub PR guardrails for fork branch-name collisions, ADE-vs-GitHub PR tag precedence, terminal-state GitHub overrides for stale ADE PR rows, deep-link lane selection, action-deeplink query cleanup, create-lane request normalization, delete-start selection fallback, parent-before-child-safe batch delete planning, and `runLaneDeleteBatchWithConcurrency` for limited parallel teardown inside each dependency-safe batch. |
...
| `renderer/components/lanes/LaneGitActionsPane.tsx` | Commit, stash, fetch, sync, push, recent commits. Stashing includes untracked files when the unstaged set contains untracked paths, and stash restore uses the ordinal `stash@{N}` ref returned by `git stash list`. After commit/stash operations it refreshes changes, lane git status, and git metadata while skipping snapshot decorations (`refreshLanes({ includeStatus: true, includeSnapshots: false })`). Seeds its `autoRebaseStatus` from the `autoRebaseStatusSnapshot` prop that `LanesPage` passes from the lane list (`laneSnapshot.autoRebaseStatus`), so opening a lane does not trigger a per-lane probe. A fallback `refreshAutoRebaseStatus` runs only when the snapshot is `undefined`, after a 3.5 s delay, and only while the document is visible. Push is guarded: at click time (never in a memo, so a single-machine project pays nothing) it runs `detectPushDivergence` over the cross-machine lane union via `selectOtherMachineBranchStates`, and shows `PushDivergenceDialog` when another machine holds the same branch with unpushed commits. An explicit `otherMachineBranchStates` prop overrides the union for callers that already have the set. Machine identity comes from `shared/machineIdentity.ts` compared by id, so the guard cannot warn that This computer diverged from itself; display names come from `machineNameForBinding`. The pane (along with `LaneDiffPane` and `CommitTimeline`) also takes an optional `runtimePin`. The default `null` — every pre-existing call site — means the tab's bound machine and is the unchanged path. When set: every git/diff read and write is routed to that machine (`diff.getChanges`, `git.stashList` / `getSyncStatus` / `getConflictState`, commit/stash/fetch/sync/push all take the trailing pin); the panel's cached state gets its own namespace, `pin:<kind>:<key>`, instead of the active project state key, because lane ids are only unique per machine and without it two machines' same-named lanes share one cache entry and a tab switch paints the wrong machine's changes; and the lane (with its parent lane) resolves against `useLanesForPin(pin)`, never against the tab's `lanes` array, for the same per-machine-uniqueness reason — falling back there can match a different machine's lane and drive git on the wrong checkout. Two actions are local-only: `lanes.rebaseStart` and `lanes.createFromUnstaged` have no machine-pin parameter in the lane orchestration IPC, so under a pin they fail loudly with "This action only works on the machine this project tab is connected to." rather than silently acting on the wrong machine.<br><br>The component renders the same git surface twice — as the full-width Lanes **page**, and as a column inside the Work tools **pane** — and every handler is shared. The difference is entirely chrome, so it is one `skin` table read once (commit toolbar strip, Files/History grid, section fill, `SectionCard` chrome, whether `CommitTimeline` draws its own header, label rank) rather than twenty inline `isPane ? … : …` expressions at twenty `style={}` objects; structural differences — which *element* renders — stay as `isPane` branches in the JSX, because a table of JSX would not read better. In the pane the card is dropped entirely (no border, no fill, no rule under the title, just a 12 px muted label over the rows: two nested cards inside a 440 px pane is three frames deep before any content, and the pane already has a header), the actions are ghost glyphs in one 40 px `workToolChrome` row, and labels are sentence case rather than the page's uppercase mono. Stacked sections are no longer an even `1fr 1fr` split: a Files section with nothing to list is worth only its own height — a clean worktree used to spend half the pane on "Clean · nothing to commit" and then a void — so History follows it directly, 12 px below. Stashes count as content, in the same section behind the row's "more". |
| `renderer/components/lanes/overview/LaneDashboard.tsx` | Left column of the Lanes main area; the Git pane (`LaneGitActionsPane` with the Work `pane` skin, its chrome row matched to the sidebar tab rail) is always on the right, split by `detail/LaneSplitBody.tsx`, width saved in `localStorage` as `ade.lanes.gitColumnWidth`. Both columns start right under the app bar. Flat sections in the Work idiom, each hidden when empty: identity (name, branch switcher, base or parent, created time and the chat that started the lane, Linear issue, and ghost buttons for New chat, Open in Files, Reveal and the lane menu), a one-line git status (`laneStatusItems`), the rebase notice, the open PR with its checks (failing/running rows, passing folded), review and merge state, plus earlier PRs behind a disclosure, the lane's chats as Work-style rows with the canonical status label, the stack around the lane, the open PR's changed files or else the commits ahead of the base, and recent activity (8 rows until "Show all activity"). PR status/checks/reviews/files are read only for the lane's open PR, only while the tab is active, and re-read only when the PR list reports that PR changed (`useLanePrDetail`, 60 s module cache). Pure logic lives in `laneOverviewModel.ts`. |
...
- `src/shared/defaultRemoteLaneBase.ts` — remote-first default-base resolution for new lanes: `remoteLaneBaseCandidate` (`main` → `origin/main`; SHAs yield `""`), `selectRemoteLaneBaseRef` (returns the local base branch's configured upstream on its own — branch listings fold a tracked remote ref into the local branch, so `origin/main` is usually absent from the remote rows precisely because `main` tracks it — and otherwise `origin/<base>` only when that remote row exists; the result is unverified, so callers must check it resolves), and `resolveDefaultRemoteLaneBase` (bounded fetch + selection, null on any failure so creation falls back to the local default). Consumed by the renderer's `newLaneBaseSource.ts` (which re-exports `remoteLaneBaseCandidate` under its historical `remoteNewLaneBaseFallback` name) and, through `laneCreateRemoteBase.ts` below, by every host-side base-less lane create.
- `apps/ade-cli/src/services/laneCreateRemoteBase.ts` — the host-side remote-first default base for base-less creates: the sync layer's `lanes.create` (mobile), the ADE RPC server's `create_lane` tool (`ade lanes create`, agent tool calls), and the brain's chat-launch "Fetch base branch" stage. `resolveLaneCreateRemoteBase` returns the ref; `resolveLaneCreateRemoteBaseDetailed` also returns `fetchSucceeded` (null when no fetch ran). It reads `git.newLaneBaseSource`, runs a bounded fetch on the primary lane, picks the ref with `selectRemoteLaneBaseRef`, then verifies it resolves to a commit in the primary worktree (`resolveGitCommit`, i.e. `git rev-parse --verify --quiet <ref>^{commit}`). A configured upstream whose remote ref is gone (`[gone]`) yields null plus a `⚠ Base <ref> no longer exists on the remote; using the local base.` warning, instead of a base every `laneService.create` would reject.
- `src/shared/uuid.ts` — `isUuid` and `requireNormalizedUuid` (trim + lowercase or throw). Every caller-reserved id (a reserved lane id, a chat launch id) goes through it so the same id always maps to the same row and file name; deep-link lane-id validation uses `isUuid` too.
...
- `src/shared/editorTargets.ts` — `EDITOR_TARGETS`, `OpenPathTarget`, `OpenInTarget`, `resolveOpenInTarget` / `canOfferOpenIn` / `isRemoteEditorOpenRequest` / `buildRemoteEditorUrl`. The lane (and Work session) **Open in** submenu is this shape: a worktree path plus optional SSH remote. Paired remotes (`transport: "paired"`) are excluded. See [Opening in an external editor](../files-and-editor/README.md#opening-in-an-external-editor).
...
type LaneStatus = {
  ...
  changedFileCount?: number; // unique changed entries, including untracked files
  staged?: number;         // entries with index changes
  unstaged?: number;       // entries with worktree changes
  untracked?: number;      // untracked entries
  lastCommitAt?: string | null; // current branch tip timestamp
  trackedFileCount?: number | null; // tracked files in the worktree
...
--branch --untracked-files=normal -z`, whose `# branch.head` header carries the
live HEAD branch, so
`headBranchRef` (and the drift detection built on it) costs no extra process
spawn. NUL-delimited records keep newlines in filenames safe, and normal
untracked reporting keeps an untracked directory as one entry. Ignored files
are still not listed (no `--ignored`), so the `dirty` semantics are byte-
...
1. **Create** — `laneService.create()` resolves the base ref (explicit
   ...
   **Default base is remote-first.** The base a new unparented lane
   ...
   auto-create, the iOS create sheet default, headless CLI) and the
   brain's new-lane chat launches get the same remote-first default
   resolved host-side via `resolveLaneCreateRemoteBase` in
   `apps/ade-cli/src/services/laneCreateRemoteBase.ts` — bounded fetch,
   map the primary base branch to its remote-tracking ref (the local
   base branch's configured upstream is trusted as-is, else
   `origin/<base>` when that remote row exists), verify the ref still
   resolves to a commit, and fall back to the legacy local primary tip
   when the source is `"local"`, no remote ref exists, or the chosen
   upstream is gone from the remote. A failed fetch keeps the
   last-known remote ref and warns that it may be stale. This keeps a
   phone-created lane from silently branching off a stale local
   checkout. The iOS
   ...
   Chat-created lanes (the brain's new-lane launches) also get the
   project's default lane template: after the checkout, the launch runs
   `runLaneEnvironmentSetup` with the default template id, or with the
   project's lane env-init config alone when no default template is set.
   See [Chat › New-lane launches](../chat/README.md#new-lane-launches).
...
4. **Import branch** — `importBranch` attaches an existing branch to a
   ...
   (which skips symbolic remote HEADs — `refs/remotes/origin/HEAD`
   shortens to plain `origin`, so it is recognised by its full ref — and
   also returns `lastCommitSha` / `Date` / `Author` /
...
`laneService.create(args, runtimeOptions)` accepts
`LaneCreateRuntimeOptions`, which only in-process callers (the
brain's chat-launch service) pass:

- **`laneId`** reserves the new lane's id. It must be a UUID
  (normalized through `requireNormalizedUuid`) and must not belong to
  an existing lane row. The worktree path is
  `<worktreesDir>/<slug>-<laneId[0:8]>`, so a reserved id also fixes
  the path.
- **`onCheckoutProgress`** receives `{ percent, completed, total }`
  parsed from git's `Updating files:  NN% (n/m)` stderr lines
  (`parseGitCheckoutProgressLine`). The `worktree add` then runs with
  `GIT_PROGRESS_DELAY=0` and `LC_ALL=C` so git prints progress to a
  pipe from the first file, in a parseable locale.
- **`signal`** cancels the checkout. Aborted before git ran (for
  example while waiting on the worktree-mutation guard), the create
  throws and nothing is cleaned up — it must not delete a branch it
  never made. Aborted mid-checkout, `runGit` kills the git process
  tree and the create removes the half-written worktree and its new
  branch through `cleanupCreatedWorktreeLaneAfterCreateFailure`. A
  plain `worktree add -b` failure (e.g. the branch already exists)
  never triggers that cleanup.

Unowned worktree removal (`removeUnownedWorktreeAndBranch`, used by
failed/cancelled creates and reserved-path cleanup) runs
`git worktree remove --force --force`, which also removes a worktree
a killed `git worktree add` left locked "initializing" (a single
`--force` and `worktree prune` refuse it). The fallback deletes the
directory, then `worktree unlock` + `worktree prune` so the path is
reusable even when the directory could not be removed. Directory
removal retries EBUSY / EPERM / EACCES / ENOTEMPTY on a bounded
schedule (about 5 s) for handles a just-killed git or a Windows
indexer/AV scanner still holds.

Two helpers support a launch that retries after a failure or a
restart:

- **`cleanupReservedWorktree({ laneId, name })`** clears whatever an
  earlier failed checkout left at the reserved worktree path, which
  would otherwise make every retry's `worktree add` fail. It is
  deliberately narrow: it does nothing when a lane row owns the id or
  the path, when the path is not directly inside the managed
  worktrees directory, or when a creation is pending there; it only
  deletes the branch checked out there when that branch is an
  auto-lane temporary branch no lane row references. Cleanup errors
  throw `Could not clear the previous checkout at <path>: …`.
- **`findLaneIdentity(laneId)`** returns `{ id, name, branchRef,
  baseRef, worktreePath }` for a live (non-archived) lane row, or
  null. A launch uses it to adopt a lane whose create inserted the row
  but never returned (ADE restarted mid-checkout) instead of failing
  on the reserved id.
...
worktree with `git status --porcelain=v2 --branch --untracked-files=normal -z`;
the `# branch.head` record of that output is the live HEAD branch.
...
`ade.lanes.initEnv` and `ade.lanes.templates.apply` (IPC), the `lane.initEnv`
/ `lane.applyTemplate` actions, the sync host's `lanes.initEnv` /
`lanes.applyTemplate` commands, and the chat-launch environment stage all
call `runLaneEnvironmentSetup` (`laneEnvironmentSetup.ts`). Add environment
setup behavior there, not in one entry point.

## Renderer wiring

The lane list lives in the shared project sidebar: `LanesPage` renders
`LaneSidebarList` through `ProjectSidebarSlot`, so it keeps the same width as
every other tab. The main area shows only the selected lane, with no header bar:
`LaneDashboard` on the left (identity and quick actions, branch, status, rebase
notice, Pull request, Chats, Files changed, Stack, Recent activity) and
`LaneGitActionsPane` on the right, split by a draggable
`LaneSplitBody`. There are no lane tabs or tiled panes. New lanes are created
from the sidebar's "+".

- The lane dashboard's rebase notice reads the lane snapshot's
  `rebaseSuggestion` / `autoRebaseStatus` and offers Rebase and Dismiss.
- The dashboard's Stack section shows the lane stack for the selected lane.
- The dashboard's Chats section lists the lane's sessions in the Work
  status vocabulary; they open in Work. Chat sessions inherit
  `cwd = lane.worktreePath`.
- The Lanes page reads pane overlay data from `appStore` (`lanes`,
  `laneSnapshots`, `refreshLanes`). `refreshLanes` can refresh lane rows,
...
- Multi-lane deep links can pass `laneIds=<id,id,...>` and
  ...
  exist before consuming the link, selects the first lane, and applies
  `inspectorTab` to it. There is no side-by-side lane view any more.
...
- **Open in is one inventory, two hosts.** `buildLaneMenuGroups` owns the
  `open-in` entry; `LaneContextMenu` and `LaneActionsSubmenu` both render
  `OpenInSubmenu` from it. Do not add a second editor picker on the
  session menu for a local singleton — that row already has **Lane ▸**.
  Foreign headerless SSH rows are the exception: the Lane submenu cannot
  resolve a remote editor from the local store, so session-root Open in
  stays. See [terminals-and-sessions](../terminals-and-sessions/README.md).
