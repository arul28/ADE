...
| `laneService.switchBranch(args)` | Performs the checkout: refuses dirty trees, refuses duplicate-owner branches, requires `acknowledgeActiveWork` if active sessions/processes exist, then `git checkout` (or `checkout -b` in `mode: "create"`), updates the lane row, and upserts the branch profile. Existing live PR rows stay mapped to the lane; the renderer derives `active` when `head_branch` matches the current branch and `previous` otherwise. Only deleting the lane detaches its PR history. |
