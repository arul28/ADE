...
--branch --untracked-files=all`, whose `# branch.head` header carries the live
HEAD branch, so
...
`git ls-files -z` supplies the cached `trackedFileCount` used by the Work picker.
