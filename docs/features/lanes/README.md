...
8. **Delete** — `delete({ laneId, deleteBranch?, deleteRemoteBranch?,
   ...
   `git worktree prune` runs before the lane row disappears. Multiple
   delete calls can progress through non-Git teardown independently;
