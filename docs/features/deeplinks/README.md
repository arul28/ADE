...
| `ade://issue/<provider>/<issue-key>[?branch=<branch>&plugin=<plugin-id>]` | `{ kind: "issue", provider, issueKey, branch?, pluginId? }` | The provider-neutral form of the row above, for a tracker a plugin owns. The provider is lowercased so `Linear` and `linear` are one link; the key is not, because it belongs to another system and `owner/Repo#42` is not `owner/repo#42` on GitHub. Validation is asymmetric on purpose: the provider must be a short path-safe name, the key is only checked for being non-empty, whitespace-free and at most 128 characters. `plugin` pins the link to the plugin that minted it and is a hint, not a requirement. `linear` with no plugin named maps to the same navigation target `linear-issue` does; every other provider opens the owning plugin's issue panel. Both routes resolve to `/work` on the web client. |
