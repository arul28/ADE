...
- `apps/desktop/src/renderer/components/settings/AutoUpdatesSection.tsx`
  — the Updates block inside the top Settings > General ADE card. Automatic installation is off by
...
|---|---|---|
| General | `ProjectSection.tsx`, `AdeCliSection.tsx`, `AutoUpdatesSection.tsx`, `KeepAwakeSection.tsx`, `ProductAnalyticsSection.tsx`, `ExperimentsSection.tsx`, `AboutSection.tsx` | The top ADE card shows running/installed/downloaded versions, the runtime service, and update controls; below it are project health, the `ade` command line (`#ade-cli`), **Sleep** (`#keep-awake`, hidden on hosted web — a browser holds no power lock), privacy, and **Experiments** (`#experiments`) — opt-in switches for unfinished surfaces, app-scoped in the renderer preferences blob (today: `experimentsLanesStoryEnabled`, the Lanes story timeline). Legacy `?tab=workspace`, `?tab=project`, `?tab=context`, `?tab=onboarding`, `?tab=help`, and `?tab=tours` land here. |
