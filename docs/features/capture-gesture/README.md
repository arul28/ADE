...
Both key flags are **seeded from `GetAsyncKeyState(VK_LCONTROL/VK_RCONTROL)` at startup** and **cleared on every `settings` command**. The hook only ever learns a key is up from an event, so a Ctrl already held when the helper starts — and ADE starts it while the user is typing — leaves that side stuck `false` until the user releases and presses it again, and a Ctrl held across a disable/enable leaves it stuck `true` so the next press of the *other* Ctrl fires a capture nobody chorded.
