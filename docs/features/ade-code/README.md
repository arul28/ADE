...
The lane-details CHATS rollup uses those same lifecycle facts: a settled
at-rest row counts as closed, `lastTurnFailedAt` counts as killed, and neither
is left in active merely because the underlying chat row still has persisted
status `running`.
...
| `/chat settle [outcome]` | Move the active session into the quiet Settled tier; an outcome becomes the `done:` status line. |
| `/chat unsettle` | Clear the active session's explicit settle declaration. |
