...
- **Header** — project name, active lane, branch, the compact chat-linked PR chip (`#42` or `#42 +1`), the terminal client frame, and the shared machine account state. ADE Code reads account status once while the TUI surface is active; it does not add a poll loop. `ade login` remains the canonical sign-in command.
...
| `/pr` | Open PR details for the **active chat's** linked PRs (not the lane fallback). Empty copy is "This chat has no pull request yet". Create-from-lane records `sessionId`. |
