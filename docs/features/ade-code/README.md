...
| `apps/ade-cli/src/tuiClient/promptSmartLinks.ts` | ADE Code's capability-adapted smart-link helpers. Formats a one-row violet chip strip and makes character Backspace/Delete remove the whole URL when the cursor intersects it; the prompt still contains and sends the canonical raw URL. The strip reads the shared chip model (`apps/desktop/src/shared/chips.ts` — imported directly from the desktop tree, not re-implemented), so the TUI names a link exactly as the desktop and iOS do: `ade://pr/owner/repo/1237` shows as `#1237` rather than the generic `ADE · pr/…` the provider glyph alone produced. Glyphs come from `CHIP_GLYPH_ASCII`, because the desktop table is emoji and emoji occupy one or two cells depending on the terminal. |
...
| `apps/ade-cli/src/tuiClient/externalSessionBrowser.ts` | Pure state/actions for the provider-native session browser. Filters and clamps rows, flattens the shared `planImport` (from `apps/desktop/src/shared/externalSessionPolicy.ts`) into one action per surface and mode, with the target lane, lock reason, note, and `confirmBeforeRun`. Plans with `originLaneId` set to the lane `/import` scanned. Puts `Open existing ADE session` first for imported rows and exposes only Copy actions after it, so Enter never re-imports the original session. `withReloadedExternalSessions` clears the row's lane, action, and confirm picks (`EXTERNAL_SESSION_ROW_RESET`) when a refresh puts a different session at the selected index. |
...
| `apps/ade-cli/src/commands/proxy.ts` | Machine-scoped `ade proxy status|start|stop|login|logout` commands for the Claude/Codex subscription proxy. Status is credential-free; install, configuration, health, and lazy start stay in the shared proxy service. |
...
| `apps/ade-cli/src/tuiClient/modelState.ts` | Pure model/setup state for draft chats and `/model`: GPT-6 Astra default plus Astra/Sol/Terra/Luna ordering, Chat vs CLI interface mode, Cursor chat-vs-CLI availability reconciliation, Codex preset/approval/sandbox mapping, provider-specific permission summaries, host-aware reasoning defaults/visible tiers, Fast Mode support, and the `SetupPaneRow` list rendered in setup panes. GPT-6 Astra and GPT-5.6 label `low` as Light and `xhigh` as Extra High, expose Max on Astra and all three GPT-5.6 models, and add Ultra after Max on Sol/Terra. |
...
| `apps/ade-cli/src/tuiClient/rightPaneFormatters.ts` | Pure formatters for right-pane result panes (PR summary / review / checks / comments, Linear status, Cursor Cloud fleet rows, system details). Keeps `app.tsx` free of ad-hoc rendering helpers. The PR checks formatter does not tally rows itself — it runs them through the shared `rollupPrChecks` so the TUI applies the same CI-producer rule as desktop and iOS, and a payload `checksStatus` of `not_run` still wins, because only the host knows about required contexts. A `not_run` rollup renders as `CI: not run — <reason>` instead of a passing count. See [pull-requests](../pull-requests/README.md#checks-rollup-what-counts-as-a-pass). The PR summary's lane line names the local worktree you can work in and nothing more: a PR without one is fully operable, so a detached row (its lane deleted or moved to another branch, leaving a dangling `laneId` that points at a worktree that is not there) prints no lane line rather than a `was <lane>` history string. Same rule as the `ade prs list` lane column. See [A lane link is not a permission](../pull-requests/README.md#a-lane-link-is-not-a-permission). |
| `apps/ade-cli/src/tuiClient/format.ts` | Transcript rendering helpers for the TUI. `webSearchResultPreviewLines` / `webSearchResultDomain` turn Codex structured web-search `results` into compact `title — domain` lines with a `+N more` tail, shared by `renderChatLines` (subagent pane) and `ChatView` (work log). `renderChatLines` also owns the `[model] Codex → Claude` handoff line, and skips it when a `model_handoff` event names the same provider on both sides (mirroring desktop and iOS) so an older transcript cannot print `[model] Claude → Claude`. It draws the chat's ONE task list (`apps/desktop/src/shared/chatTaskList.ts`, the list the desktop thread card and Tasks section show) once, at the position of the latest list event, via `formatTaskListLines`: a `plan N/M` or `tasks N/M` header (with a short plan explanation before the count) and one glyph line per item. Every earlier `plan` / `todo_update` draws nothing; a Codex plan-mode proposal keeps its own `plan <state>` line. The Chat Info pane shows the same list as one PLAN/TASKS section above the roster, and a task-list `plan` auto-opens that pane like a `todo_update` does (proposal deltas do not, and `plan` is not a flush edge). |
...
| `apps/ade-cli/src/tuiClient/chatInfo.ts` | Builds `ChatInfoSnapshot` for the right-pane Chat Info view (provider/model, lane, the chat's one task list from the shared `deriveChatTaskList` plus the latest plan-mode proposal text, Codex goal, context %, token summary, subagent roster, scheduled work, background work, next wake, the live `usageLimitResume` state, streaming state, and an optional `mcpNote` for chats an SDK embedder created). Consumes the same chat-event stream the TUI is already replaying; Codex goal display treats provider budget-limited states as active because ADE keeps goals unlimited. Schedule and background rows come from the shared `deriveScheduleItems` / `deriveBackgroundItems` split in `apps/desktop/src/shared/chatScheduledWork.ts`, and `isBackgroundShellCommand` filters historical command-shaped snapshots out of the subagent roster. The Chat Info Resume row is not built here: `RightPane` calls the shared `usageLimitResumePill` directly (with an injected `nowMs`), so the TUI row and the desktop pill cannot disagree about what a state is called and there is no second copy table to keep in step. `formatMcpCapabilityNote` renders `mcpCapability` honestly: delivery-only requests say "caller servers injected", not an isolation ADE was never asked for; `delivered: false` wins as unsupported. Chat sessions receive state in `AgentChatSessionSummary`; when a tracked CLI terminal is selected, the Ink root fetches `chat.getScheduledWorkState` whether its process is live or ended, so Chat Info shows its pause state, jobs, and earliest armed fire too. The token summary shows cache reads as `✶` and, when present, Codex cache-write tokens as `✎` (`latestTokenStats` in `adeApi.ts` reads the `cacheWriteTokens` breakdown field 0.145 maps from `cacheWriteInputTokens`). |
...
## CLI chat resume commands

The outer CLI exposes usage-limit recovery for scripts and shells. Run
`ade chat resume-now <session>` (alias: `ade chat resume <session>`) to cancel
the durable arm, reset the two-try streak, and send the ordinary continue
prompt immediately. It prints `Resume sent · turn <turn-id>` on success (the
...
`usageLimitResume` state. The plan runs `chat.getTurnStatus` and, as an
**optional** second step, `chat.getSessionSummary`: exit codes still come from
the turn status, so a brain too old to answer the summary reports the phase
normally and simply prints no `resume` line. `--json` output stays flat — the
turn-status record sits at the top level (`.phase` is where it has always been)
and the summary contributes `usageLimitResume` and `timeZone` as siblings.

The line is intentionally machine-readable and includes the target instant, a
relative countdown, the arm count, and the brain-local reading when
`chat.getSessionSummary` reported the host `timeZone`. It is emitted through
`chatTurnStatusRow`, so it lands inside the key/value header block rather than
below the subagent tree, where it would read as a subagent:

```text
  resume     resumes 2026-07-23T19:31:00.000Z (in 3 min) · usage limit · attempts 1/2 · brain local 7/23/2026, 7:31:00 PM (America/New_York)
```

The other state lines are `resuming now · usage limit · attempts 1/2`;
`paused after 2 tries · turn auto-resume back on to try at <instant>`;
`won't auto-resume (opted out)`; and `usage limit · no reset time`. An `armed`
state with no usable instant reads `resumes when the limit lifts · usage limit`
rather than printing an empty time. The payload is read through the shared
`parseUsageLimitResume`, so an unrecognised blob prints no row at all instead
of a row nobody can act on. The TUI reads the same session summary and renders
...
One deliberate divergence: the TUI keeps rendering the `claude_session_quota`
`ade_card` inline while a resume row is live, where desktop and iOS suppress the
card because their pill sits directly above the transcript. The TUI's Resume row
lives in the side pane, so suppressing the card would leave the transcript with
no account of why the turn stopped.
...
| `/steer send` | Deliver the latest staged steer inline into the active turn (`dispatchSteer mode: "inline"`). Advertised only when the session's provider carries `inline` in `ACTIVE_TURN_DISPATCH_MODES` — the command list is derived from that table, not restated — **and** the session is not a Cursor Cloud one, since a cloud run refuses every steer and the command would always fall back to the queue (`cursorSessionRunsInCloud`, the same helper the desktop pane reads). A refused inline dispatch answers `dispatchedAt: null` without throwing, so the TUI reads the result and says "The message couldn't go into the running turn; it is still queued." instead of claiming a delivery the host's own notice contradicts. Like the host's wording it names no culprit: most refusals are ADE's own and the agent is never asked. |
| `/steer interrupt` | Interrupt the active turn and run the latest staged steer (`dispatchSteer mode: "interrupt"`). Offered on providers whose table entry includes `interrupt`. On Cursor the notice reads "Interrupting Cursor and continuing with the staged message.", because its redirect cancels the run and resends on the same agent thread (`activeTurnInterruptContinues`) instead of folding into a live query; Claude's reads "Interrupting Claude to run the staged message." A mode the session's provider cannot honor is refused with the shared `unsupportedActiveTurnDispatchModeMessage` text rather than TUI-local copy. |
...
| `/cloud` | List active, non-archived Cursor Cloud agents in the right pane (read-only fleet view): one row per agent with a status glyph (`●` running, `✓` finished, `✗` failed/cancelled), name, status, age, and agent id when it fits. Rows sort running-first then newest-first; a footnote states that management actions live on desktop/iOS. Backed by the same account-wide host-side fleet service the desktop and hosted web views use. |
...
| `/pr land [confirm] [merge\|squash\|rebase] [bypass] [delete-remote-branch]` | Merge the active PR after confirmation. The remote head branch is **kept** unless `delete-remote-branch` is passed; the confirmation step says which way it will go, and echoes the token back in the `confirm` line it tells you to run. `--delete-branch` is deliberately not accepted — it already means the *local* branch in `ade lanes delete`. |
| `/pr update-branch [merge\|rebase]` | Update the PR branch with its base. `merge` uses GitHub's update-branch API and needs nothing local; `rebase` rewrites and force-pushes, so it needs a local checkout of the head branch (conflicts are resolved there, then retried). |
...
- The Codex/OpenAI list always begins GPT-6 Astra, then GPT-5.6 Sol, Terra, Luna; Astra is the new-chat default and GPT-5.5 remains below the family. Astra/Luna expose Light, Medium, High, Extra High, Max; Sol/Terra add Ultra after Max. ADE Code uses each descriptor/host row's `defaultReasoningEffort` (`low` for Astra/Sol, `medium` for Terra/Luna) and preserves host-advertised effort ordering.
...
- [Chat feature](../chat/README.md) — in-app Work chat architecture (service + renderer); same agent chat backend, including spawn types and the by-lane nested-subagent list.
- [Terminals and sessions](../terminals-and-sessions/README.md) — shared session filing, including `sessionSpawnNesting.ts` and the [session sidebar](../terminals-and-sessions/ui-surfaces.md#session-sidebar-sessionlistpanetsx).
