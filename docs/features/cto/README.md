...
and listen to one either. That rule was hand-written at five call sites across
the two RPC servers, each with its own copy of the same paragraph; it now lives
beside the category tuple in `shared/types/remoteRuntime.ts`
(`VOICE_RUNTIME_EVENT_CATEGORY`, `refusesVoiceCategory`,
`voiceCategoryRefusalMessage`, `withoutVoiceEvents`, `isVoiceRuntimeEvent`), so
the two servers cannot drift into two answers for one denial.
