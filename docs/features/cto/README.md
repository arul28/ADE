...
- `CtoMemoryPanel.tsx` — "what the CTO remembers": the project brief, ranked facts, and directed threads (read-only; the CTO writes them), an editable notes textarea (save via `window.ade.cto.updateMemory`), a read-only current thread-state, and a collapsible today's daily log. Loads via `window.ade.cto.getMemory`.
...
- `CtoTalkNoticeLine.tsx` — the failure line under the CTO header, and the settings button beside it. Its own module because the button decides *when* there is something to say and the page decides *where* it is drawn; this owns what it looks like and what can be pressed on it, which is why a microphone verdict that lands after the start sheet has closed still has a fix attached. Takes a `CtoTalkNotice` (`{ message, microphone }`) and asks `ctoMicrophoneSettingsAction` for the pane, so Linux — which has no vetted pane URL — simply renders the sentence.
...
- `apps/desktop/src/shared/types/ctoVoice.ts` — the cross-surface voice contract: the call's shape, its state, and the numbers it runs on. It imports nothing, which is what lets the action policy tables read it without dragging `ws`, the API key store and the chat service graph in behind them: `CTO_VOICE_MODEL` (`gpt-realtime-2.1`), `CTO_VOICE_ENDPOINT` (`wss://api.openai.com/v1/realtime`) with `ctoVoiceEndpointUrl()`, `CTO_VOICE_TRANSCRIBE_MODEL` (`gpt-4o-mini-transcribe`) with `CTO_VOICE_TRANSCRIBE_LANGUAGE` (`en`, named rather than guessed per utterance), `CTO_VOICE_USD_PER_MINUTE` (0.05), `CTO_VOICE_SAMPLE_RATE` (24,000), `CTO_VOICE_AUDIO_POLL_INTERVAL_MS` (100 ms), `CTO_VOICE_OWNER_IDLE_TIMEOUT_MS` (15 s), `CTO_VOICE_PREOPEN_AUDIO_LIMIT` (50 frames) and `CTO_VOICE_OUTPUT_AUDIO_QUEUE_LIMIT` (200 chunks), the transcript gate's `CTO_VOICE_MIC_WINDOW_MS` / `CTO_VOICE_MIN_SPEECH_PEAK_LEVEL` / `CTO_VOICE_LOCAL_BARGE_IN_LEVEL` with `CTO_VOICE_LOCAL_BARGE_IN_FRAMES` / `CTO_VOICE_MIN_SPEECH_MS` / `CTO_VOICE_TURN_BURST_LIMIT` / `CTO_VOICE_TURN_BURST_WINDOW_MS` / `CTO_VOICE_TURN_BURST_COOLDOWN_MS` with `CtoVoiceTranscriptRejection` and `ctoVoiceTranscriptHasSpeech()`, `ctoVoiceStatusLine()`, the `CTO_VOICE_ACTIONS` list and its `CtoVoiceAction` type, the `CTO_VOICE_VOICES` list and `CTO_VOICE_DEFAULT` (`marin`), the `CtoVoicePhase` union with `isVoiceCallLive()` / `isVoiceCallVisible()`, `CtoVoiceState` + `CTO_VOICE_INITIAL_STATE`, `CtoVoiceBridge`, `CtoVoiceConfirmation`, `CTO_VOICE_SPOKEN_CONFIRM_WINDOW_MS` (20 s), `ctoVoiceMicrophoneUnavailableMessage()`, `CTO_VOICE_CAPTURE_DEFAULT_NOTE` and the `voiceCostUsd` / `formatVoiceElapsed` / `formatVoiceCost` formatters. Five siblings hold the rest of the contract, so a file that needs one of them does not pull the other four in behind it.
- `apps/desktop/src/shared/types/ctoVoiceTools.ts` — the wire contract for the function tools: `CTO_VOICE_REALTIME_TOOLS` with `CTO_VOICE_TOOL_ASK_CTO` / `CTO_VOICE_TOOL_CANCEL_WORK` / `CTO_VOICE_TOOL_APPROVE` / `CTO_VOICE_TOOL_DENY` / `CTO_VOICE_TOOL_END_CALL`, and the ask modes (`CTO_VOICE_ASK_MODES`, `CTO_VOICE_ASK_MODE_DEFAULT`, `normalizeCtoVoiceAskMode()`, `CTO_VOICE_ASK_QUEUE_LIMIT`). The tool descriptions are the policy: they are the only thing deciding when the model talks to the CTO thread and when it answers for itself.
- `apps/desktop/src/shared/types/ctoVoicePrompt.ts` — `CTO_VOICE_CONTEXT_MAX_CHARS` (6,000), `buildCtoVoiceInstructions()` — the persona brief, the rule for what the model answers itself and what it hands to `ask_cto`, and the fenced context block — and `buildCtoVoiceSpeakInstructions()`, which wraps one ADE-authored line as the fenced read-aloud instruction of a single out-of-band `response.create`.
- `apps/desktop/src/shared/types/ctoVoiceSession.ts` — the two payloads that configure a call, so their shape can be asserted without opening a socket: `buildCtoVoiceSessionUpdate()` (the first `session.update` — turn-detection flags, tool list, audio formats, transcription model, voice) and `buildCtoVoiceInstructionsUpdate()`, the instructions-only half it is built from, which the call re-sends on its own after every completed `ask_cto`. The `event_id` is per-send and belongs to the sender.
- `apps/desktop/src/shared/types/ctoVoiceEvents.ts` — the wire shapes of the events the call service has an opinion about: `CtoVoiceServerEvent` (the typed map its handler table is keyed on), with `CtoVoiceResponseShape`, `CtoVoiceSettledEvent`, `CtoVoiceTranscriptEvent` and `CtoVoiceAudioDeltaEvent`. Every field is optional because this is a wire; what the map buys is a handler whose parameter names the event it was registered under.
- `apps/desktop/src/shared/types/ctoVoiceDestructive.ts` — the gate a spoken "yes" may never pass: `CTO_VOICE_DESTRUCTIVE_TOOLS` + `isDestructiveVoiceTool()`, the destructive command and ADE-action patterns behind `isDestructiveVoiceCommand()`, and `describeVoiceApproval()`, which reads a provider's approval event for the tool name and the verdict.
- `apps/desktop/src/main/services/cto/ctoVoiceCallService.ts` — `createCtoVoiceCallService(deps)`: the WebSocket, the delegation loop, the keep-alive, and `endCall`'s durable `persistCall` write. Five pieces live in siblings, each with a test file of its own: `ctoVoiceFailures.ts` (`describeCtoVoiceSocketFailure`, `describeCtoVoiceServerError`, `isBenignCtoVoiceServerError` / `isCtoVoiceActiveResponseConflict`, `forwardUnexpectedResponse`, `ctoVoiceFrameDurationMs`), `ctoVoiceTurnTiming.ts` (`createTurnTimingRecorder`), `ctoVoiceMicMeter.ts` (`createMicMeter` and the pure `judgeVoiceTranscript`), `ctoVoiceResponseQueue.ts` (the one-at-a-time response lock), `ctoVoiceToolCalls.ts` (`createFunctionCallLedger`, which decides when a function result may go on the wire), `ctoVoiceTurnBurst.ts` (`createTranscriptBurstValve`, the gate that shuts when the transcript source runs away and reopens on quiet) and `ctoVoiceAudioQueue.ts` (`createVoiceAudioQueue`, the bounded queue both the pre-open microphone frames and the model's voice wait in). Events from the session are dispatched through one handler table keyed on the typed `CtoVoiceServerEvent` map rather than a chain of comparisons, so an event carrying two names is two keys on one handler and a field read in the wrong handler is a typecheck failure. The model's function calls are dispatched the same way, keyed on the tool-name constants. Every dependency is injected (`getApiKey`, `runBackendTurn`, `persistCall`, `onExchange`, `now`, `createWebSocket`), so the service never imports the chat service and both the delegation loop and the gate's clock are testable without a model.
- `apps/desktop/src/main/services/cto/ctoVoiceRuntimeService.ts` — `createCtoVoiceRuntimeService(host)`: the runtime-hosted owner of a call. It builds the call service's deps out of an `AdeRuntime` (chat, CTO identity, durable memory, lanes, and `sessionService` for the row's status line), holds the confirm-first hold and the `ownerToken`, publishes state on the `cto_voice` event category, and keeps the output-audio queue that `pullAudio` drains — `ctoVoiceAudioQueue.ts`, whose `cancelQueued()` (a barge-in: cancelled, not lost) and `forgetCall()` are why the drop tally means something. Constructed in `apps/ade-cli/src/bootstrap.ts` and exposed as `AdeRuntime.ctoVoiceCallService`. Everything pure it used to carry — the context block, the scene contract, the spoken/drawn split — lives beside it in `ctoVoiceContext.ts`, so the ORDER of the block's sections and the BOUND on the whole of it can be tested without a project on disk. See [The call brain lives in the runtime](#the-call-brain-lives-in-the-runtime-and-the-desktop-is-a-router).
...
- `apps/desktop/src/shared/runtimeEventPolicy.ts` — the `cto_voice` visibility rule as one module, so the two RPC servers cannot express half of it: `VOICE_RUNTIME_EVENT_CATEGORY`, `hidesVoiceEvent()`, `refusesVoiceCategory()` (a stream asked for BY NAME is refused), `withoutVoiceEvents()` (an uncategorised drain is filtered, so the cursor still advances past what was withheld), and `voiceCategoryRefusalMessage()` so one denial has one wording. Every helper takes the caller's role flag.
- `apps/desktop/src/main/services/cto/ctoVoiceCallHarness.ts` — one live call driven from the wire, shared by every voice suite: `createService()` over the fake socket, `openCall()`, `utter()` / `hearMic()` / `MIC_FRAME` (a transcript with no microphone energy behind it is thrown away, so a test that means "the user said this" has to say it out loud), `askCto()` / `callTool()`, and the readers `spoken()` / `modelResponses()` / `functionOutputs()`. Kept out of any one suite so a suite that moves does not take a copy of the harness with it.
...
Relaying every sentence word for word from a CTO turn would be correct and auditable, and it would take three to five seconds before the first word of "hello" — which is not a conversation. So the split sits one layer out instead: the model owns the talking, the CTO thread owns the work, and `ask_cto` is the seam.
...
They live in `CTO_VOICE_REALTIME_TOOLS` in `shared/types/ctoVoiceTools.ts` rather than beside the socket, because their **descriptions** are the only thing deciding when the model talks to the CTO and when it answers for itself — which makes them worth reading and diffing in one place.
...
**There are two ways a yes can arrive, and only one of them is metered.** Said plainly because the gate above is easy to read as a guarantee it does not give:

- **The transcript parser** — `handleUserTranscript` → `judgeTranscript` → `resolveSpokenConfirmation`. Mic-gated by everything below, and bound to the utterance the question was asked during.
- **The model's own tool** — the realtime model hears the audio itself and calls `approve_pending_action` / `deny_pending_action`. Nothing on ADE's side meters that, and nothing can: the model is listening to the same sound a person is, not to a transcript of it, and a meter here has no vote on what it heard. What it gets instead is the silent `system` note telling it a question is open and to say nothing unless the answer is clear.

Both land on `approvePending` / `denyPending`, which are **idempotent** by confirmation id — whichever path arrives first decides, and the second is a no-op rather than a second decision on a gate that is already open. And both are subject to the one rule neither can talk past: a `CTO_VOICE_DESTRUCTIVE_TOOLS` action is refused with `needs_tap` however clearly the yes was heard.
...
- the burst valve is open. More than `CTO_VOICE_TURN_BURST_LIMIT` (4) accepted transcripts inside `CTO_VOICE_TURN_BURST_WINDOW_MS` (10 s) is one every 2.5 s — faster than a conversation, so it is a transcript source running away. The valve then refuses every **decision** until `CTO_VOICE_TURN_BURST_COOLDOWN_MS` (8 s) passes with no transcript arriving at all; "until the next accepted transcript" cannot be the release condition, because while the gate is shut there are none. Otherwise: `runaway`. It never touches the caption path: a call record with too much in it is readable, and one missing the user's own sentences is not.
  ...
  The valve is `createTranscriptBurstValve` in `ctoVoiceTurnBurst.ts`, and it is **read on every transcript** (`isShut()`, which also restarts the cooldown clock), not only when a question is open. It used to be cleared inside the confirmation branch alone, which meant a talkative call tripped it, never reached the code that clears it, and refused every spoken yes for the rest of the call as `runaway` — with the quiet that should have reopened it having gone by unwatched.
...
and listen to one either. That rule lives in one policy module,
`shared/runtimeEventPolicy.ts` (`VOICE_RUNTIME_EVENT_CATEGORY`,
`hidesVoiceEvent`, `refusesVoiceCategory`, `voiceCategoryRefusalMessage`,
`withoutVoiceEvents`), rather than hand-written at each call site across the two
RPC servers, so they cannot drift into two answers for one denial.
...
to stand alone rather than trust the one before it, and the first two now have
tests of their own. A renderer that names the category — or any category that is
not renderer-visible — is **refused**, not silently downgraded: dropping the
field turned a request for one category into an uncategorised subscription for
*every* category, which is the opposite of what the caller asked for and,
for `cto_voice`, exactly the stream the guard exists to withhold. Nothing on that side consumes
