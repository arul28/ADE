...
- **ADE runtime parity.** `apps/ade-cli/src/headlessLinearServices.ts` instantiates the same flow policy and dispatcher inside `ade serve` — any YAML schema change must pass through the runtime code path as well, otherwise the ADE runtime diverges from the desktop renderer.
