...
- **Daemon parity.** `apps/ade-cli/src/headlessLinearServices.ts` instantiates the same flow policy and dispatcher inside `ade serve` — any YAML schema change must pass through the daemon code path as well, otherwise the runtime daemon diverges from the desktop renderer.
