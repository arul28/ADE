...
Runtime event streaming uses `ade/actions/call` with `name: "stream_events"` for one-shot pulls, and `runtimeEvents.subscribe` (with `runtime/event` notifications) for live streaming. Event reads and subscription acknowledgements have a 30 s transport deadline; a transport timeout is treated like a broken remote stream and gets one reconnect/retry through the existing pool funnel. This deadline is scoped to stream calls, so long-running remote actions retain their own budgets. For remote bindings the desktop reconnects the SSH transport before re-subscribing, matching normal remote action behavior after disconnects. The initial remote subscription starts with `replay: false` when the cursor is still zero, so opening a remote project does not flood the renderer with buffered history before live events arrive; catch-up polls still use a short delay while idle remote polls back off. For local bindings, preload polls the local runtime through `localRuntimeStreamEvents` so runtime-owned chat, terminal, pty, lane, file-watch, process, and test events are delivered through the same renderer fanout used by remote projects.
...
8a. On a macOS remote, if this desktop ships the Mac Desktop driver (`<resources>/native/ade-desktop-driver`, a universal binary) and the remote copy's hash is missing or stale, upload it to `<layout>/bin/resources/native/ade-desktop-driver` with a sidecar `.sha256`. That is where a brain at `<layout>/bin/ade` finds it: `resolveMacDesktopDriverBinary` walks up from its own binary to `resources/native`. The driver is uploaded only when its hash changed and is never re-signed, because macOS ties Screen Recording and Accessibility grants to the binary's code hash — a needless re-upload would take the remote Mac's grants away. A failed upload does not fail the connection; it becomes a `compatibilityWarnings` entry ("Mac Desktop is not available on this machine: ADE could not install its driver (…)").
...
12. If the preferred runtime fails validated initialize, walk the alternate channel homes again with `ade rpc --stdio` against each candidate even when this connection just uploaded a packaged Beta binary. The first home that completes initialize and capability negotiation wins; failed candidates are collected with their launch, initialization, or capability-negotiation phase so the final error remains actionable. The preferred home's failure is listed first and each fallback line starts with "Also tried", so a missing beta install is not read as the real failure. The remote CLI's stderr is reduced by `summarizeRemoteRuntimeStderr` to its error lines — stack frames, Node `ExperimentalWarning`/`DeprecationWarning` lines, and the `ade: Error:` prefix are dropped. The remote handshake check (`machineRuntimeMismatchReason`) runs every check — version, build hash, role — and names each failure plus the checks that passed, instead of stopping at the first. If a fallback wins, the chosen home and environment prefix are retained for every follow-up command and recorded as a compatibility warning.
...
Within a chat pane that pin is resolved once and provided, not re-derived per panel. `components/chat/ChatRuntimeScope.tsx` is the canonical derivation: `AgentChatPane` runs `useChatScopeDerivation` — session → lane (through `useForeignSessionLaneId` when the chat is not in this tab's own session list) → pin — and wraps its panel/drawer subtree in `ChatRuntimeScopeProvider`. Every chat-scoped tool then reads `useChatRuntimeScope()` for `{ pin, binding, laneId, lane, laneWorktreePath, rootPath, isRemote, machineName, online }` instead of asking a global store, which would answer with the *tab's* machine. `pin === null` means, and only means, "this chat lives on the tab's binding". Surfaces reused outside a pane — the CLI session header renders `ChatGitToolbar` with its own pin and no provider above it — take the same derivation from a prop through `useChatRuntimeScopeForPin(pin, laneId, bindingOverride?)`. Lane rows and worktree paths are resolved with `useLanesForPin` from `renderer/state/crossMachineLanes.ts`, never against the tab's `lanes` array: lane ids are unique per machine, not globally, so the tab-bound list can match a different lane sharing the id and hand its worktree path to a tool about to drive another machine.
...
Which local roots a window may address at all is a separate question from which one it is bound to, and `runtimeBridge.collectAuthorizedLocalRuntimeRoots` answers it from the window session. `services/projects/windowTabRootAuthorization.ts` (`resolveWindowTabRoots`) splits a renderer's set-open-tabs call into `tabRoots` — what the window is displaying, renderer-supplied and only ever used to look projects back up, so an unknown path there is harmless — and `authorizedLocalRoots`, the window's local runtime scope, which the bridge reads as "this window opened this checkout". Only roots that resolve to a project this process actually opened join the second list; a path must not earn local-runtime access by appearing in a tab list. `main.ts` also keeps a per-window `windowKnownLocalProjectRoots` set of every local root the window itself opened this session and surfaces it as `knownLocalProjectRoots`, because neither existing signal survives a rebind: `windowProjectTabRoots` is replaced wholesale by the renderer on every tab change, and `windowProjectRoots` is nulled the moment the tab binds to a remote machine. A chat pinned to "This computer" asks precisely that question — its lane lives on this machine whichever machine the tab is bound to — and refusing it left cross-machine Work views unable to act on their own local sessions. Membership is granted only by the window opening the project, never by a renderer-supplied path.
...
- Connection startup failures are backoff-throttled per target so repeated implicit reconnect attempts do not saturate SSH. Explicit Connect bypasses that backoff, clears manual disconnect state, and resets the automatic reconnect failure count. Implicit retries continue with bounded exponential spacing from 30 s up to 5 minutes; there is no permanent ten-failure latch, so a laptop that sleeps or briefly loses a route can recover without requiring a desktop restart.
