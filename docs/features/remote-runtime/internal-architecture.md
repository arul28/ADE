...
- Project registration, file actions, and event polling pass per-call timeout overrides to `RuntimeRpcClient`; ordinary actions keep the client's default timeout. Both pools also keep a small `LONG_RUNNING_*_ACTION_TIMEOUTS` map keyed by `<domain>.<action>`; today it grants `chat.suggestLaneNameFromPrompt` a 120 s budget (a full AI title round-trip) instead of the default 30 s local / 25 s retryable-remote ceiling, so a slow model does not abort auto-lane naming mid-launch.
