...
| `services/editors/editorDetection.ts` | `detectInstalledEditorTargets`: macOS `open -Ra <macAppName>`, else `which` / `where.exe`, PATH augmented by `editorProcessEnv`. Every target is probed in parallel. Duplicate macOS apps that share `macAppName` (`zed` / `zeditor`) collapse to one. On Windows a miss off PATH falls through to `editorWindowsInstall.ts` and records the resolved absolute executable for the open action. |
