...
| `apps/desktop/src/main/services/chat/claudeSdkCompat.ts` | Claude SDK compatibility shims: normalize the interrupt receipt (`still_queued` / `cancelled`) and the rewind `skippedLinks` count, read a session message's model across its historical and current shapes, and answer plugin delivery (`claudePluginDeliveryForSource`) — `"initialize"` over stdin for the bundled / tools-cache binaries, argv for a user-supplied binary whose version ADE cannot vouch for. |
...
| `apps/desktop/src/main/services/chat/chatTextBatching.ts` | Batches streaming assistant text fragments (100 ms) before emission to reduce renderer re-renders. Each flush carries a reason (`timer`, `identityBreak`, `interleave`) that `services/perf/chatTextProbe.ts` records as a `chatTextFlush` event during a perf run. |
| `apps/desktop/src/main/services/perf/chatTextProbe.ts` | Perf-run-only probe over the text coalescer: counts the deltas folded into each flush and emits `chatTextFlush` (chars, deltas coalesced, ms since the previous flush, reason, pid). Zero-cost and allocation-free when `ADE_PERF_RUN_ID` is unset. Lazily calls `initPerfRunFromEnv()` on first flush because the `ade` runtime daemon — which hosts chat sessions in a normal dev session — never loads `main.ts`. |
| `apps/desktop/src/main/services/chat/chatTranscriptEntries.ts` | Flattens an envelope stream into the canonical role-tagged `AgentChatTranscriptEntry[]` behind `chat.getTranscript`, the cursor-paged chat-history reader, and the internal auto-title/handoff transcript reads. Its invariant is that a message's canonical text is the **verbatim** concatenation of its `text` fragments: fragments sharing provider identity (the `messageId`/`itemId` *pair* — Claude sets only `messageId`, Codex sets `messageId` per turn and `itemId` per message, so either alone merges distinct messages) concatenate untouched no matter what interleaves, and only `isTranscriptContentEvent` rows (a deliberate *allowlist* of rendered types) end a run, and only for fragments ADE cannot tie to a provider message. Whitespace-only fragments are real deltas and are dropped only when no run is open to continue. Inventing a `"\n\n"` here spliced separators mid-word and made clients that reconcile canonical text against the live stream (iOS, web) render a message twice. A steered message is written twice as `user_message` — `deliveryState: "queued"` when it is staged, and again when the provider consumes it — so a queued row whose steer id also has a non-queued row in the same batch is skipped and only the graduating row becomes an entry; a still-pending queued steer is the only record of that message and is kept. See [Transcript and turns](transcript-and-turns.md#canonical-assistant-text-fragile--read-before-editing). |
| `apps/desktop/src/main/services/chat/chatSourceAdapters.ts` | Pure provider adapters from native web-tool and citation shapes to `ChatSourceRef`: Codex memory citations, Claude `WebSearch`/`WebFetch` tool results and text-block citations, Cursor `webSearch`/`webFetch`, OpenCode `websearch`/`webfetch`, Droid `WebSearch`/`FetchUrl`, ACP fetch/search tools and `resource_link` blocks. See transcript-and-turns.md → Sources. |
...
| `apps/desktop/src/main/services/chat/cursorSdkPolicy.ts` | Maps ADE permission modes onto Cursor SDK chat mode + approval policy, plus the `fullAuto` marker, and decides which tool calls auto-approve and which require a user prompt. Local workers always pass `sandboxOptions.enabled: false`; ADE never requests Cursor-native sandbox. `fullAuto` names ADE's full-auto permission mode and only affects pool partitioning and log labels — it is not a Cursor SDK option. Hard guards keep tool paths inside the lane, with a read-only exception for this lane's Cursor `terminals` / `agent-transcripts` / `assets` under `~/.cursor/projects/<slug>/` and for `<project>/.ade/attachments` when worker init supplies that project root (a missing root fails closed and is not inferred from the lane; after realpath the directory basename must be `attachments`; writes, shell, other slugs, `.ade/secrets`, and symlink-out-of-tree remain denied). It also owns `CURSOR_SDK_KILL_ESCALATION_MS` (1.5 s), the gap before a kill that both the pool's dispose ladder and the orphan sweep use. |
| `apps/desktop/src/shared/chatErrorPresentation.ts` | Host rewrite for failed turns, shared by every provider path. `presentChatFailure` turns a provider/SDK failure into `{ title, body, nextAction?, technicalDetail? }` for one of `rate_limit` / `auth` / `network` / `busy` / `configuration` / `unknown` (`not_found` deliberately lands on the generic stopped-turn copy, because ADE recycles the agent and resumes). Every sentence is provider-agnostic — `provider` is substituted where the host knows it and the copy stays neutral where it does not — and only a Cursor label may name Cursor's sandbox and ADE's hook fallback. `isSandboxUnsupportedFailureText` is the single sandbox predicate (also imported by `cursorSdkProtocol.ts`), and it **outranks the caller's kind**, so a provider that reports the failure as a plain error still gets the configuration card. `configuration` is the one kind that discards the host message entirely: the raw sentence tells the user to edit `local.sandboxOptions` or `~/.cursor/sandbox.json`, which is never card copy. Raw text survives only in `technicalDetail`, deduplicated across message / detail / error code. `chatErrorKindFromCategory` narrows an event's `errorInfo.category`; `readChatErrorPresentation` is the client-side reader. Clients render `errorInfo.presentation` and never title the row Error or Unknown. |
...
| `apps/desktop/src/main/services/chat/cursorSdkEventMapper.ts` | Translates `@cursor/sdk` stream events into the ADE `AgentChatEventEnvelope` shape consumed by the renderer. SDK `task` messages remain parent-run activity summaries; typed `Task` tool calls/results produce subagent start/result events keyed by tool call id, including the returned child agent id when available. Cursor MCP calls retain provider/tool identity in `event.mcp`; generated-image tools become compact image-generation rows. Cursor's native `updateTodos` tool call maps to a `todo_update` **plus** a `plan` event and the raw tool row is suppressed, because the plan card *is* the rendering of that call and showing both would put the same list on screen twice; only the terminal call is mapped (the tool streams one event per item, each carrying the whole list so far, and emitting the partials would redraw the card per item), item ids are positional so a plan update diffs instead of redrawing, and Cursor's `cancelled` status reads as `pending` on the todo row (which has no failure state) and `failed` on the plan step (which does) — that is a cancelled step, not a failed one. The fenced ` ```ade_update_plan ` control block ADE's Cursor system prompt teaches is still honoured, so a model can publish a plan either way. `user` stream events map to nothing on purpose: ADE emits its own `user_message` row on every send path, so a `user` event — including the echo `Run.steer()` produces — would be a second copy of a row already in the transcript. `todo_update` and `plan` are both in `CURSOR_SDK_VISIBLE_OUTPUT_EVENT_TYPES`, so a turn whose only output was a plan still counts as having produced something. On a terminal `ERROR` status it reads the worker-injected `adeErrorCode` / `adeErrorDetail`, classifies the failure, and hands its Cursor-specific sentence to the shared `presentChatFailure` so the row carries a full `errorInfo.presentation` alongside `errorInfo.category` (`rate_limit`, `network`, `busy`, or `auth` when classification is known). A transport failure reads **Cursor's connection dropped mid-run.** rather than leaking `NGHTTP2_INTERNAL_ERROR` or `[internal] write ECANCELED` into the transcript; the exact Cursor request id and the raw technical lines are preserved in `detail`. Whenever the friendly headline replaces the raw code, that code is kept as the first `detail` line so the underlying failure is still recoverable from the transcript. |
| `apps/desktop/src/main/services/chat/cursorModelsDiscovery.ts` | Probes the live `@cursor/sdk` and `cursor-agent` CLI model lists, merges their descriptors, and records `cursorAvailability` so chat sessions see SDK-capable models while Work CLI launches can include CLI-only models. Both JSON and text probes preserve aliases, descriptions, `parameters[]`, and `variants[]`; `*-fast` CLI rows are folded into their base model as `aliases` + `serviceTiers: ["fast"]` so the picker shows one model with a Fast toggle instead of duplicate "Fast" rows. Parameter and variant metadata is classified into `reasoningTiers` (`none`/`dynamic`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`/`thinking`) and `serviceTiers` (`fast`). Selection lives in `cursorModelSelection.ts` (next row); discovery only owns the catalog. `peekCursorSdkCatalogRows` reads the catalog in memory without a fetch. `readCursorSdkCatalog(apiKey, { failFast })` reads this key's rows from memory, or fetches once; with `failFast`, a fetch for this key that failed in the freshness window answers at once with its cause. The previous minimal `auto` / `composer-2` fallback list has been removed. **Cache resilience:** both the SDK and CLI caches are stale-while-revalidate — last-known-good rows are served well past the 120s freshness window (up to ~6h) and a background warm (at most one attempt per freshness window, so a broken CLI/SDK is not re-spawned on every passive read) refreshes them, so verified-provider models never blink out on passive status reads (`availableModelIds`, mobile, TUI). `markCursorModelCachesStale` ages the caches without dropping rows — generic readiness invalidation (forced status refresh, verifying any provider's key) calls it, while only a cursor key change does a full `clearCursorCliModelsCache`. Auth/SDK-resolution failures drop the SDK cache (a dead key/unusable module must not resurface phantom models); transient failures keep serving last-known-good. When the signed-in CLI reports "No models available" its cache is dropped and a provider runtime failure is surfaced (the stored login lost model access; re-auth via `cursor-agent logout`). |
| `apps/desktop/src/main/services/chat/cursorModelSelection.ts` | Turns a chat's Cursor choices (reasoning effort, Fast or a service tier, model options) into `ModelSelection.params`. All resolvers read the catalog through discovery. A **local** send (`resolveCursorSdkLocalSelection`) reads the catalog in memory. It loads the catalog only when that catalog cannot answer and the chat chose a control (`hasExplicitCursorSelection`). It sends the params that resolved on `ok` and on `partial`. A **cloud create** (`verifyExplicitCursorModelSelection`) fails closed on anything but `ok`. A **cloud follow-up** (`resolveCursorSdkFollowUpSelection`) refuses only a model the catalog does not list. For a catalog that cannot load, or a control the model cannot express, it sends the params that resolved, and the chat shows one notice (`describeUnappliedCursorSelection`). Both cloud paths read this key's warm catalog and do not fetch on every run. `cursorModelFastTierFromCache` answers the Fast question from memory only (`null` = unknown, which keeps Fast). `unsupportedCursorSelection` names the choices an adopted cloud model cannot take. `cursorSdkConfigOptions` reports an option the chat never set as `null` (Cursor's default). |
...
| `apps/desktop/src/main/services/chat/droidSdkWorker.ts` | Node worker that hosts `@factory/droid-sdk/node` (0.9.x moved the Node session APIs off the package root). Streams SDK events back to the main process and forwards permission / ask-user prompts back through the JSON-line protocol. User images are materialized here from attachment paths (`workerAttachmentImages.ts`) rather than as base64 on the JSON IPC pipe. |
| `apps/desktop/src/main/services/chat/droidSdkProtocol.ts` | Worker IPC types: `DroidSdkSessionSettings` (autonomy level, interaction mode, reasoning effort), `DroidSdkReasoningEffort`, `DroidSdkPermissionRequest`/`Decision`, `DroidSdkAskUserRequest`/`Response`, `DroidSdkReady` (handshake with the SDK session id and resolved model; `availableModels` is retained on the wire but stays empty since 0.9.x removed the `initResult` surface), and `DroidSdkSendPrompt`. User images on send are path (or tiny inline `data`) references (`DroidSdkUserImage`), not inlined screenshot bytes; remote `url` images are not part of the union because Droid's stream API has no URL form. |
...
| `apps/desktop/src/renderer/components/chat/ChatWorkflowActiveCard.tsx` | Workflow activity entry point for the Chat Actions → Agents pane. Groups a parent workflow with its emitted agent rows, shows a compact live status card, and opens a focus-trapped modal with provider-reported phases, queued/running/completed/failed counts, per-agent summaries, model, token, tool, duration, and last-tool details. Uses the existing subagent selection and stop callbacks; it never exposes the raw workflow prompt. |
...
| `apps/desktop/src/renderer/components/chat/MosaicCard.tsx` | Interactive mosaic card renderer (text, select, multiselect, number/slider, input, approve/deny, key-value table). Hooked in at `MarkdownBlock`'s code-fence handler (`chatMarkdownBlock.tsx`) behind a Claude-gated `mosaic` context prop from `AgentChatPane`; answered state persists across virtualized unmounts via a session-lifetime latch that rolls back on send failure. Non-Claude sessions render the plain fence. |
| `apps/desktop/src/shared/chatScene.ts` | Scenes — agent-authored generated UI, the counterpart to mosaic. Owns `SCENE_FENCE_LANGUAGE` (`scene`), `SCENE_LIMITS` (96,000 source bytes, 192,000 assembled-document bytes enforced main-side by `scenePrepare`, 120-char title, 8 s ready timeout), `SCENE_CONTENT_SECURITY_POLICY`, `parseSceneFence` / `isSceneParseFailure` (marker line, `unwrapDocument` normalization, byte cap), `hasOpenSceneFence` (true while the last fence in a streaming body is still open), `sceneScopeKeyFor` (a fence's identity: a stable row identity plus a hash of the fence body, so two scenes in one message keep their own still) and `sceneRowIdentity` (that row identity — `messageId`, else `turnId`+`itemId`, else the render key; the render key alone embeds the event's index in the events array, so prepending an older page moved a still's name on disk and every reopen re-ran the scene and filed a second picture) and `extractSceneFence` (the prose and the one scene, split apart) — both built on one line scanner, deliberately, so the streaming guard and the splitter cannot disagree about what a fence is; `SceneTheme` + `SCENE_FALLBACK_THEME`, `buildSceneDocument` (CSP meta first in `<head>`, `escapeForScript` for the injected data and theme, the dependency-free in-frame SDK), and `parseSceneHostMessage` — the shape check every inbound frame message passes before it reaches host state. Shared by both sides, so it uses `TextEncoder` rather than `Buffer`. Documented for agents in the `ade-scene` Agent Skill (`apps/desktop/resources/agent-skills/ade-scene/SKILL.md`). |
| `apps/desktop/src/renderer/components/chat/SceneFrame.tsx` | Host for one scene. Builds the document, resolves a source URL (`window.ade.scene.prepare` → `ade-scene://view/<id>`, blob URL as fallback), renders it in an `<iframe sandbox="allow-scripts">` with `referrerPolicy="no-referrer"`, accepts messages only from its own `contentWindow`, clamps the reported height to 120–760 px, takes the settle-time still, and freezes the frame to a still image when the producing turn or call ends. Whether this mount shows a stored picture instead of running the code is **not** decided here — that is `useSceneStillLatch`'s job; the frame only consumes its `rehydrated` / `undecided` / `storedStillSrc` answer. Both the settle deadline and the one-still latch are armed per DOCUMENT, not per mount, because the voice HUD swaps the source on a single mounted frame. Hooked in at `MarkdownBlock`'s code-fence handler alongside mosaic, but **not** gated on a render context — the sandbox, not the caller, is what makes a scene safe. |
...
| `apps/desktop/src/main/services/scenes/sceneSnapshot.ts` | The freeze and proof plumbing, split out of the store because it shares nothing with it: `clampSceneCaptureRect` (a rect that leaves the page makes `capturePage` resolve empty) and `decodeScenePngDataUrl` (PNG magic checked, 12 MB cap). Also free of Electron. |
| `apps/desktop/src/main/services/scenes/sceneSnapshotIngest.ts` | `ingestSceneSnapshot` — the path jail (lexical, then again on the realpath), the file check, the title clamp and the owner resolution behind the CTO-only `computer_use_artifacts.ingestSceneSnapshot` action. Lives here rather than inline in the action registry's dispatch table, where a security check reads as boilerplate. |
| `apps/desktop/src/main/services/scenes/sceneStills.ts` | Scene stills, both directions. `fileSceneStill` files a settled scene's PNG through the artifact broker tagged `metadata.kind = "scene_still"` (with `sceneScopeKey`, `sceneTitle`, and `voiceCallId` when the scene was drawn on a call) and then holds the two disk bounds: one still per scope key, and `SCENE_STILL_MAX_PER_SESSION` (32) per owning chat, oldest first — both scoped to the owner, so an unowned still prunes nothing. `sceneStillFileLabel` clamps an agent-authored title to 80 chars before it reaches a file name. `findVoiceCallStills` runs the same index backwards for a finished call's **Views drawn** section, querying `metadataKind: "scene_still"` so a busy CTO thread's ordinary proof cannot push the call's own pictures out of the scan window. Also owns the owner resolution the ingest path shares: `resolveSceneStillOwner` (which chat a still belongs to), and `requireSceneStillScopeKey` / `SceneStillScopeKeyError`, the one validation of a scope key a caller must pass before anything is filed under it. One module because the tag is written in one half and matched in the other. |
...
| `apps/desktop/src/renderer/lib/chatHandoffIntent.ts` | Module-local one-shot bridge between a handoff entry point outside the chat pane (the session context menu's **Hand off…** submenu) and the pane that owns the handoff surfaces. `openChatHandoff(sessionId, intent)` records a destination (`"local"` \| `"remote"`) and notifies live listeners; `subscribeChatHandoff` / `takeChatHandoff` let a pane that mounts or re-selects a render later drain the queued intent, because right-clicking a session that is not yet selected lands before the pane has rendered for it. One slot, last write wins — a human picks a destination and the matching pane consumes it within a render. A stale intent (older than 30 s) is discarded rather than consumed, so a command whose pane never mounted cannot fire on a much later selection, and an intent queued for a different session is left in place for its own pane to drain rather than handed to the wrong chat. Deliberately a module rather than a store slice: the payload is a transient command, not state any surface renders. |
...
| `apps/desktop/src/renderer/components/chat/AgentChatMessageList.tsx` | Virtualized transcript renderer. Coalesces resize / measurement updates and, while sticky-to-bottom is active, follows height changes across multiple animation frames so streamed output and late row measurements do not leave the user above the newest message. A shrinking transcript viewport (the composer growing around a long prompt) is treated as a pin, not a user scroll, so a thread stuck to the bottom stays there unless the user scrolls. Programmatic scroll writes are tracked by target scroll position, not a stale counter, so browser-coalesced scroll events do not swallow the next real user gesture. Workspace paths in Markdown links and inline code render with an explicit file glyph/click treatment and open with a relative path + lane id (parsing, resolution, index probing, and the opener live in `chatWorkspacePaths.tsx`, which the list feeds its `runtimePin` so a foreign chat's paths are looked up on the machine that has them). A file in this chat's own lane on this machine opens in the Work tools-pane Files panel next to the conversation; anything else navigates to `FilesTab`, which resolves the target against that machine's workspace roster — so the same click opens the correct file for local, remote-bound, and cross-machine chats without treating a remote path as a local OS file. `work_log_group` rows are filtered out of the rendered timeline entirely. Live turns still expand tools from the working indicator; after `done`, `ChatTurnWorkSummary` is one expandable `N tools · M files` line immediately above the turn's time/usage cutout (left-aligned with Thought), with tools and files between that summary and the footer when expanded (file counts omitted when the turn has a checkpoint-backed `turn_diff_summary`). Older history prefetches roughly two viewport-heights before the top (`resolveOlderHistoryPrefetchTriggerPx`, shared by the scroll handler and the sentinel observer) so the reader normally never sees the head spinner. `stabilizeTranscriptToolActivity` reuses the previous per-turn entry arrays whenever a turn's entries did not change, so a streaming delta does not defeat `React.memo` on every settled turn in the thread. Activity bundles fold todo/scheduled updates and placeholder subagent updates into compact rows, dedupe placeholder subagent parents once the concrete child id is known, and open Chat Info / subagent detail instead of duplicating the drawer roster inline; real subagents instead render as inline `SubagentSpawnCard` / `SubagentResultCard` rows anchored where they started and settled (backgrounded shell commands instead get one compact `BackgroundJobRunRow` per run of consecutive jobs, expandable to one line per job), with jump-to-result / jump-to-start affordances that reuse the stable-row scroll machinery; a run of two or more interrupt-stopped subagents folds into one calm `SubagentStoppedGroupCard` instead of a wall of identical stopped cards. Synthetic scheduled turns render an amber `Woke on schedule` divider with fire time, reason, and late marker; the existing stable-row scroll machinery accepts the Review jump from the while-you-were-away card. Completed-turn dividers bucket chat-owned proof by capture timestamp, expose a collapsed `N proof` chip, and expand the filmstrip directly beneath the producing turn; there is no proof footer pinned to the tail. Codex goal lifecycle events render as compact user-facing rows (`Goal set`, `Goal paused`, `Goal cleared`) instead of raw JSON-RPC/status wording. Codex runtime notices (`codex_safety_buffering`, `codex_moderation_metadata`, `codex_sleep`, `codex_thread_deleted`) render as small transcript chips. `codex_turn_stalled` renders a live recovery card: Wait re-arms the watchdog, Nudge sends a status steer, Retry interrupts and replays work in the same thread, and Resume restarts app-server, resumes the thread, and retries. Handoff brief user messages with `metadata.hideFullPrompt` show only their `displayText` breadcrumb and do not expose or copy the internal prompt body. History seeded into a forked chat (envelopes tagged `providerOrigin: "handoff_fork"`) renders under a single `Forked from the previous chat — full history above` divider pinned to the first live row after the seeded tail, rather than a per-row marker. Every other error event renders as an `InstructionErrorCard` built from the host's `errorInfo.presentation` (or, when a host sends none, from the same derivation applied locally), so a failed turn is a title, a sentence, what to do, and a technical fold rather than a dump of the provider's text. Error events whose `errorInfo.agentCli.category` is `"unauthenticated"` render as the calm `AgentCliAuthCard` (raw 401 behind a `Details` disclosure) rather than the red error block, so a recoverable logout reads as a re-login prompt, not a crash. The live turn's `working for <elapsed>` counter is painted imperatively through a callback ref that survives the status line's mid-turn remount (see [composer-and-ui.md](composer-and-ui.md)). It also owns `ChatInfoHostContext`: `AgentChatPane` provides it because it owns the chat actions pane and listens for `ade:chat:open-info`; `PersonalChatsPage` does not, so transcript affordances that reveal that pane render only where dispatching would actually do something. Scoping is per-subtree by necessity — hidden `ProjectSurface`s stay mounted, so a module-level registry would report a host on the one surface that lacks one. |
| `apps/desktop/src/renderer/components/chat/chatMarkdownBlock.tsx` | `MarkdownBlock` — the transcript's markdown renderer, extracted out of `AgentChatMessageList.tsx` (which imports it), along with the `MosaicRenderContext` type. Accepts an optional `tailMarkdown` so a paced row renders a byte-stable settled body plus a short re-parsing tail inside one prose flow. |
| `apps/desktop/src/renderer/components/chat/AssistantTextBody.tsx`, `textReveal.ts`, `useRevealedText.ts` | Paced text reveal for the trailing streaming assistant text row of a visible main transcript. `textReveal.ts` is the pure state machine (pacing, grapheme-safe cuts, settled/tail block scan, the `ade.textRevealHorizonMs` kill switch), `useRevealedText.ts` the rAF/visibility binding, `AssistantTextBody.tsx` the leaf that re-renders at 60 Hz. Subagent views and ended/idle sessions opt out and keep paint-on-arrival. See [composer-and-ui.md](composer-and-ui.md#paced-text-reveal). |
| `apps/desktop/src/renderer/perf/streamSmoothness.ts` | Perf-run-only rAF sampler that measures how smoothly streaming assistant text actually paints: per ~1s window, how many frames advanced the revealed text and how large the gaps between advancing frames were. Reads the `data-stream-text-len` attribute `AssistantTextBody` writes (revealed length, not store length) and emits `streamSmoothness` events. Inert in production. |
...
| `apps/desktop/src/renderer/components/chat/SubagentActivityCards.tsx` | Inline subagent transcript cards mounted by `AgentChatMessageList` from the render events `chatTranscriptRows.ts` derives. `SubagentSpawnCard` anchors where the agent started (identicon/colour from `chatSubagentIdentity`, task title, spawn-kind/background chips, a single live `running · <activity> · <N> tools · <elapsed>` line that ticks each second); when the agent ends, the same row turns into a `SubagentResultCard` in place (status badge, `ran for` line with counters, report preview, failure `Details`). Both wear one container (`SUBAGENT_CARD_CHROME`), and `SubagentCardGrid` lays consecutive cards of any state side by side, with a short last row stretched to the full width. Both carry a bottom-right provider mark — the same 20px `ToolLogo` a Work session row uses, via `chatToolTypeForProvider` — resolved from the child session's own provider when a spawned ADE chat's provider is known and from the chat's runtime otherwise; the mark is omitted for a blank provider rather than rendering an empty glyph. Backgrounded shell commands render through `BackgroundJobRunRow.tsx` instead (see [transcript-and-turns.md](transcript-and-turns.md)). Status glyphs are Phosphor components, not bare `⚙`/`✓`/`✗` codepoints, which Windows resolves to off-baseline emoji; `SubagentStoppedGroupCard` collapses a run of interrupt-stopped subagents into one amber "N agents stopped when you interrupted" line that expands to a per-agent list with `jump to start` links. |
...
| `apps/desktop/src/renderer/components/chat/RewindFilesConfirmDialog.tsx`, `rewindFilesPreview.ts` | Chat file-rewind confirmation surface. Claude uses the SDK `rewindFiles` control call; Codex uses ADE's git-backed file restore plan plus a version-gated app-server call — `thread/fork` before the target turn on servers >= 0.145.0, and the `thread/rollback` fallback on <= 0.144 or when the target turn has no usable id (pre-0.156 only; 0.156 removed it) (see [agent-routing.md](./agent-routing.md#codex-rewind-and-0145-readiness)). `rewindFilesPreview.ts` maps the selected user message to turn diff summaries and per-file SHA ranges; the dialog lists every restored file, expands rows into `AdeDiffViewer`, and confirms the provider rewind without using browser-native confirm UI. |
...
| `apps/desktop/src/main/services/chat/chatAutoResumeCoordinator.ts` | The whole auto-resume subsystem on the host: per-chat streak state, the durable row, the three system notices, the `usageLimitResume` state transitions, and the races between arming, cancelling and firing. `agentChatService` keeps only thin call sites — it reports a limit error, a dispatch, a dismissal, and a turn finishing, and this decides what each means. Four contracts are load-bearing. **`cancelEpoch`**: arming is asynchronous (it waits for the scheduler, then upserts), so a user message can land while the upsert is in flight and find nothing to cancel; every cancel bumps the epoch and the arm undoes its own row if the epoch moved. **`whenArmed`**: resolves once the in-flight arm has finished writing (or declining to write) its row. The Claude path awaits it before reaping the query — reaping first tore the runtime down mid-upsert, which is how a real limit ended up resuming nothing. **The re-arm cap** (`AUTO_RESUME_MAX_CONSECUTIVE_ARMS = 2`): the reset instant a provider publishes is not always the limit that rejected the turn — Claude's snapshot is session-scoped and can carry a stale `resetsAtMs` forward, so a weekly-limit rejection would arm against a session reset, fail again, and burn a turn every cycle. Two arms against *distinct* reset instants, then the chat emits `Paused after 2 tries` and waits for a human. A resume that runs without hitting the limit again resets the streak, which is why `noteResumeTurnStarted` / `noteTurnFinished` exist: a fired resume always carries `scheduledWake`, exactly what the cancel sweep skips, so without that watch the cap would be a lifetime budget and a healthy headless chat would pause itself after two limits days apart. `resetStreak` is the human override behind Turn on / Try again, and it bumps the cancel epoch so an explicit reset outranks any undo still in flight. Resume now does not use it: the cancel it awaits already zeroes the streak, and only a turn that actually dispatches keeps it zeroed — a failed send restores the streak it borrowed. **Notice ordering**: notices are emitted after one microtask, because this runs inside the event commit before the error event has minted its sequence number and an inline emit would render the notice above the failure it explains. Notices are `system_notice` rows with `noticeKind: "rate_limit"`, attributed to the failed turn, de-duped one per armed fire time. Every transition is also reported through `onResumeStateChanged`, which is the only writer of the `usageLimitResume` contract field. |
| `apps/desktop/src/main/services/chat/attachmentInlineGuard.ts` | The one check every adapter that base64s attachment bytes into a model request runs, plus the stand-in it substitutes. `MAX_PROVIDER_INLINE_IMAGE_BYTES` (10 MB) is deliberately smaller than the 50 MB attachment cap: most providers receive a *path* and do not care how large the file is, but Claude images (`buildClaudeV2Message`), the AI-SDK streaming content OpenCode uses (`buildStreamingUserContent`), and the Droid/Pi prompt blocks (`buildAgentPromptBlocks`) inline the bytes, where base64 inflates by a third and the provider rejects the request. Over the ceiling those three push `inlineAttachmentHintPart` instead — a text part naming the path, the size, and the limit, so the agent can read the file with its own tools and the turn still runs. `workerAttachmentImages.ts` uses the same constant for Cursor/Pi/Droid worker IPC, and it deliberately did not move when the attachment cap did. |
| `apps/desktop/src/shared/chatAttachmentStagingFs.ts` | The disk-side rule for staging a chat attachment, shared by every process that writes into `<projectRoot>/.ade/attachments`: the desktop main process, the ADE action registry the local brain serves, the CLI sync host's HTTP upload route, and `imageAttachment.ts`. `projectAttachmentsDir` is the one spelling of the directory; `stageAttachmentCopy` copies a file that already exists on this machine's disk; `stageAttachmentBytes` writes bytes a caller already holds. `resolveStagedAttachmentExtension` is the one answer to "what extension may a client contribute" (display name first, source path second, `path.extname` being separator-agnostic on both so a Windows source path behaves like a POSIX one). `commitStagedAttachmentPart` is the rename that promotes a `.part` file to its final path, carrying the bounded Windows sharing-violation retry, and `unlinkStagedAttachmentQuietly` is the torn-upload cleanup. Both the CLI host's streamed HTTP upload route and its chunked base64 route go through these, so the two routes cannot disagree about where a file landed or what it is called. Both give the destination a fresh UUID basename with only a regex-validated extension from the caller's display name (`safeAttachmentExtension`), then re-check containment with `path.relative` rather than a string prefix — the prefix form false-rejects a filesystem or Windows drive root and cannot see a cross-drive path at all. `stageAttachmentCopy` is deliberately *not* constrained to the project root (users drag files in from Downloads), stats for size before copying a byte, and must only ever be reachable from the machine that owns the file. There used to be three copies of this rule and only two validated the extension, so the same drag-and-drop produced a differently-named file depending on which process answered. |
...
| `apps/desktop/src/renderer/components/chat/ChatCursorCloudPanel.tsx` | Side panel for Cursor Cloud (background agents): lists all cloud agents across Cursor pages, reveals archived rows on demand, lets the user open an existing cloud chat in ADE, archive/unarchive/cancel, and stream run output. Backed by `ade.ai.cursorCloud.*` IPC. |
| `apps/desktop/src/shared/cursorCloudNaming.ts` | Shared Cursor Cloud rename rule: the blocked sentence plus `cursorOwnsSessionName` / `assertCursorCloudRenameAllowed`. Desktop menus, IPC, ADE `session.updateMeta`, and `work.updateSessionMeta` all use this so a user cannot see two wordings for one refusal. |
| `apps/desktop/src/renderer/lib/cursorCloudUtils.ts` | Cloud launch helpers: origin-push (`pushAutoCreatedLaneOriginForCursorCloud` / `ensureExistingLaneOriginReadyForCursorCloud`), PR create fields, agent URL, and Electron error-wrapper stripping. |
| `apps/desktop/src/renderer/components/chat/useLaneGitRemote.ts`, `useCursorCloudModelEligibility.ts`, `draftModelControls.ts` | Cloud-draft readiness: tri-state git remote probe, catalog eligibility, and reconciling draft reasoning/fast controls when the selected model changes. |
| `apps/desktop/src/renderer/components/app/CursorCloudQuickViewButton.tsx`, `CursorCloudFleetModal.tsx`, `CursorCloudFleetRow.tsx` | Top-bar Cursor Cloud **fleet view** (see [Composer and chat UI › Cursor Cloud fleet view](composer-and-ui.md#cursor-cloud-fleet-view) for the surface contract). The button mounts beside `LinearQuickViewButton` only while a Cursor connection exists — read through a per-project cached `ai.getStatus` reader (60 s connected / 1.5 s disconnected TTLs, checked after a 2 s delay and re-queued on bridge-ready) so it never lands in the Work startup IPC window — and counts finishes from `fleetEvent` pushes into an unread badge while the modal is closed. The modal owns filters (status / lane / archived), the active/lane/unlinked grouping, cached lazy row expansion with usage/artifacts, all row actions, and the honest relay/key-missing/empty/error states; `FleetRow` renders one row plus its overflow menu. Status derivation lives in shared `cursorCloudFleetStatus.ts` so section placement, Stop-button visibility, and filter results cannot drift between main process and renderer. Opening the modal occludes the built-in browser's `WebContentsView` so native content cannot paint over it. |
| `apps/desktop/src/renderer/components/app/DevinCloudQuickViewButton.tsx`, `DevinCloudFleetModal.tsx`, `DevinCloudFleetRow.tsx` | Top-bar Devin **fleet view** (see [Composer and chat UI › Devin Cloud fleet view](composer-and-ui.md#devin-cloud-fleet-view)). The button mounts beside the Cursor quick-view only while `devinCloudGetAuthStatus().configured` is true. The modal owns the provenance chips (Mine / From ADE / All), status and lane filters, the active/unlinked grouping, row actions (open mirrored chat, live `session.url` in the built-in browser, `devin` CLI VM actions — SSH shell, port forward, steer-in-terminal — when the CLI is installed, stop, pull into lane, archive/unarchive, PR, delete), and the pull-into-lane toast's **Continue in lane** action that launches the local `devin` CLI in the merged lane with seeded context. |
| `apps/desktop/src/renderer/components/chat/ChatDevinCloudPanel.tsx` | Chat-drawer panel drafting a lane-bound Devin cloud session: target repo + base branch from the lane, `devin_mode` picker, approval-gate skip, and recent sessions on this repo. |
| `apps/desktop/src/renderer/lib/devinCloudUtils.ts`, `apps/desktop/src/shared/devinCloudFleetStatus.ts` | Renderer launch/format helpers plus the single session-status → fleet-status derivation shared by the modal, rows, and the attention mapping. |
| `apps/desktop/src/renderer/components/chat/ChatSurfaceShell.tsx` | Shell that wraps every chat surface (desktop pane, mobile lane, CTO chat) with a unified header/footer slot and `--chat-accent` CSS variable. Supports a `layoutVariant="mobile"` mode that the iOS companion mirrors. Optional `canvasFill` (Work new-chat draft: `transparent`) lets the host mesh show through instead of `--chat-canvas-bg`. |
...
| `apps/desktop/src/main/services/chat/pendingInputRecovery.ts` | Reads a pending-input card back from the transcript after its process-local waiter is gone. An `accepted` receipt means the card was already answered, so a second response changes nothing. |
| `apps/desktop/src/shared/pendingInputRequest.ts` | The one reader that turns untyped data (a transcript event, a synced envelope) into a `PendingInputRequest`, for the renderer and the chat service. It keeps an explicit `false` on fields such as freeform. |
| `apps/desktop/src/main/services/chat/laneAppleDeviceDirective.ts` | The turn-time hint that tells an agent its lane has an Apple device. See [Apple device](../apple-device/README.md#agent-discovery). |
| `apps/desktop/src/renderer/components/chat/useChatPaneShowRequests.ts` | `ade ui show proof` (and `apple` outside Work) for the chat this pane shows. See [Agents open panes](#agents-open-panes-ade-ui-show). |
| `apps/desktop/src/renderer/components/chat/useArtifactPreview.ts` | Resolves one proof artifact's preview source and failure cause. See [computer use › Renderer](../computer-use/README.md#renderer). |
...
- `activeTool` and `openTools` are **published by the desktop renderer**. The
  ...
  A desktop can publish a tool id this runtime does not know yet (for example
  `pr`, added after older runtimes shipped). `setActiveTool` does not reject
  the publish: it records an unknown active tool as `null` ("no known tool on
  screen") and drops unknown ids from `openTools`, so phones and web clients see only
  tools they can render.
...
| iOS | same two remote commands, gated on `SyncService.supportsWorkToolsState` | Lane tool chips in the chat's floating badge row, after the PR chip (`WorkLaneToolChips.swift`): the lane's running simulator (opens `AppleDeviceViewer`, gated on `apple.status`), browser tabs ("2 tabs") and App Control (app name), which open `WorkToolsSheet`: active-tool card with the open-tab capsules (filled = on screen) and the last frame, then the tab list, then App Control. The Mac's active tool alone is not a chip. Pull to refresh; the sheet polls every 3 s while it is open, and the chat polls both reads every 10 s but **skips its tick while the sheet or the viewer is up**, since both poll the same data. |
| ADE Code (TUI) / `ade` CLI | `work_tools.getLaneState` via `/tools` and `ade work-tools state` | Same read-only mirror in the right pane or as text. `work_tools.actions` lists the full inventory; `setActiveTool` stays user-client-only, so an agent that wants a pane open asks for it with `ade ui show` (below) rather than claiming one is open. It is also how a terminal user learns a browser tab has been handed back for them to sign in. |
...
## Agents open panes: `ade ui show`

An agent can drive a simulator, a browser or a recorder, but only a desktop
renderer can open the panes that show the result. `ade ui show <surface>` asks
the desktop to put a surface of the agent's own chat on screen. The surfaces
are `apple` (the Apple tool in the Work tools pane), `floating-apple` (the
floating device player), `browser` (the Browser tool) and `proof` (the chat's
proof drawer). `ade apple show [--floating]` is the same request.

1. The brain (`apps/ade-cli/src/services/workTools/workToolShowRequests.ts`)
   publishes a `work_tool_show_request` event on the runtime event stream.
   Every desktop on the project reads that stream, local or paired.
2. In the renderer, `renderer/lib/workToolShowRequests.ts` sends the request
   to the pane that has registered for that chat. The Work page registers
   through `useWorkShowRequests.ts`, and only for the session in front. A chat
   pane outside Work registers through `useChatPaneShowRequests.ts` for its
   proof drawer and its Apple drawer.
3. The pane opens the surface and answers `shown` only when the surface's own
   element is mounted, its pane has real width, and the window is visible
   (`renderer/lib/workToolOnScreen.ts`).
4. When no pane has the chat in front, the request is held and the answer is
   `held`. A held request lands when the user opens the chat, and expires
   after 10 minutes.
5. The CLI prints `shown`, `held` or `no_desktop`. It waits up to 8 s, so a
   paired desktop on its idle poll can answer. No answer is `no_desktop`
   (exit 1), never a guess.

An agent's input to the Apple device also sends an automatic request for the
floating player (`auto: true`), at most once per 5 s per chat. The Work page
floats the player only when the Apple tool is not already on screen and the
chat's "Show preview when minimized" is on. The close button on the player
turns that setting off for the chat. An automatic request is never held and
never answered.
...
The SDK's own ask metadata rides along on every Claude approval card.
`defaultToNo` opens the card without a pre-selected Accept, and both
`defaultToNo` and `suppressAlwaysAllowRule` omit "Allow for Session" from the
option list. `suppressAlwaysAllowRule` goes further: the rule the ask would
persist is broader than its own action, so the host refuses to persist one —
answering `accept_for_session` is downgraded to a one-shot allow, and a
session-wide override from an earlier ask does not auto-answer the suppressed
card. Desktop renders its controls from the request's own option list
(`claudeApprovalOptionsOfferSession`) and iOS routes option-bearing asks to its
question card, so the omitted choice disappears there; the ADE Code TUI approval
card is approve/deny only and never answers `accept_for_session`. A path that
still answers it — an older build, a scripted answer, a hardcoded native control
— is downgraded by the host to a one-shot allow rather than persisting a rule.
An option-chip client answers with `accept` plus the chosen value in `answers`
— or, for a typed note, only in `responseText` — so the host reads the request's
own option value as the decision (`claudeApprovalDecisionFromOptionAnswers`): a
Deny chip or a typed "deny" declines and an Allow for Session chip persists,
instead of both allowing. The mapping is guarded to tool approvals; an
`AskUserQuestion` option a model happened to name `deny` is not a verdict.
For an MCP tool
the ask's `mcpServer: { name, source }` is logged
(`agent_chat.claude_tool_approval_mcp_server`) rather than rendered: `source:
"sdk"` means a server this host registered through `createSdkMcpServer`, while
every other source (`user`, `project`, `plugin`, …) is configuration-authored and
its name is untrusted text. ADE opts the CLI into writing a zeroed error result
on a known startup failure (`CLAUDE_CODE_STARTUP_FAILURE_RESULTS`); its
`startup_failure_reason` logs `agent_chat.claude_startup_failure` so a session
that produced nothing has a cause.
...
ADE creates exactly one kind of schedule on the user's behalf. When a turn dies
at a provider usage/rate limit **and** the provider published a reset instant,
`chatAutoResumeCoordinator` arms a durable one-shot at reset + 90 s (providers
report the boundary with minute granularity at best, and a request issued
exactly at it is still rejected often enough to matter) whose prompt asks the
agent to continue the interrupted task without redoing finished work. Only two
providers publish a reset today: Claude's plan-quota snapshot and Codex's
`account/rateLimits`. Every other provider — and Claude or Codex when the
snapshot is empty — arms nothing. A live limit without a reset is still exposed
as `no_reset` in the resume pill and session summary, with **Resume now** and
**Fork in this lane** available; there is simply no scheduled wake.

Everything else follows from that row being an ordinary scheduled-work record.
It survives brain restarts, appears in Chat Info, and cancels through the normal
`chat.cancelScheduledWork`. Delivery obeys the same boundary rule as every other
wake: due during a live turn means retry, never a second prompt into that turn.

**Claude uses the same durable row as everything else.** The SDK option
`autoContinueAtUsageLimit` is a Claude *Settings* key, not a query Option, so
sending it did nothing except earn an immediate 429 result — after which the
Claude path reaped the query and left the chat parked with nothing armed. ADE no
longer sends the key; the session flag is ADE's own opt-out, and the Claude limit
arms the durable row exactly like Codex's. Ordering is explicit: the arm is
awaited (`autoResume.whenArmed`) **before** `resetClaudeQuerySession`, because
reaping first tore the runtime down while the upsert was still in flight.

**Limits are only ever detected from structured provider signals.** Two sources,
both from the provider: an SDK `rate_limit_event` whose status is rejected, and
result-frame errors matching the SDK's `USAGE_LIMIT_ERROR_PREFIXES` or ADE's
session-quota regex. Assistant text is never scanned. It used to be, and an agent
that merely quoted a limit message — reading a log, answering a question about
quota — minted a quota card, marked its own completed turn failed, killed the
process, and parked the chat until the next day.

**Stale state heals itself.** Chats parked by that old prose scan are still on
disk with a park instant, no durable row, and no record that a structured limit
ever happened. On hydration (once the scheduler has read its rows back) and again
at the next turn start, a chat with a park instant or a live quota card but no
armed row, no live quota snapshot, and no stored `usageLimitResume` has the park
cleared, the card dismissed, and its resume state set to null. A false FAILED
turn status from the same bug is superseded by the next completed turn through
the existing `clearLastTurnFailed`.

**One state field, five states.** `AgentChatSessionSummary.usageLimitResume` is
the only thing any client renders the resume pill from — desktop, web, iOS, and
the session-list label in `sessionStatusPresentation` all read it and nothing
else. It is `armed` (a row exists, firing at `fireAt`), `resuming` (the row is
due and waiting for the turn boundary — derived at read time from `fireAt`, not
stored), `paused` (two arms hit the limit again), `opted_out` (Don't continue
while a limit is live), or `no_reset` (a limit is live with no published reset).
Absent or null means no live limit. It rides the session summary, the iOS/web
session projection, and a transient `session_meta_updated` event on every
transition, so a chat open on three devices updates on all three at once.
...
against the known reset, or reports `no_reset`. Resume now is
`chat.resumeUsageLimitNow {sessionId}` (also `ade.agentChat.resumeUsageLimitNow`
over IPC) — it cancels the row, resets the streak, dismisses the quota card, and
sends the same continue prompt as an ordinary user turn tagged
`metadata.usageLimitResume: "manual"`. Because that send spends a real turn, it
refuses rather than firing when the chat has no live limit
(`{ok: false, reason: "no_live_usage_limit"}` — a stale button, a replayed
action, a chat that already recovered) or when the row is already due and
delivering (`reason: "resume_in_flight"`, where a manual send would race the
scheduler into a double prompt). Both refusals send nothing and mutate nothing,
and carry a ready-to-render `message`. The swap itself is ordered so the chat is
never left with nothing: the durable row's cancellation is awaited before the
dispatch, and a dispatch that throws or is refused downstream (disk pressure)
restores the previous `usageLimitResume`, its `usageLimitParkedUntil` mirror and
the `auto-resume:<sessionId>` row before the error reaches the caller — unless
the user opted out or the chat stopped being schedulable while the send was in
flight, in which case the newer state stands and nothing is re-armed. A re-arm
that fails outright reports `no_reset`, never `armed` with no row behind it.
`chat.getSessionSummary` reports the brain's IANA `timeZone` alongside the
...
- **The tag scopes the sweep.** The row carries `source: "auto_resume_limit"`
  and the deterministic id `auto-resume:<sessionId>`, so a repeat failure
  reuses the same row instead of stacking a second resume — and when the reset
  instant has not moved it reuses the row *as it stands*, with no second upsert,
  transition or re-armed timer. A PAUSED row is skipped entirely: the arm writes
  nothing over it, and the chat reports `no_reset` (no fire time, no schedule
  id, the provider detail kept) so the pill offers Retry instead of counting
  down to an instant nothing happens at. Un-pausing the row publishes `armed`
  again from the row's own fire time. ADE's
  cancel-on-user-activity sweep can find its own row without touching a cron,
  wakeup, or `/loop` the user or the agent asked for. Cancellation fires from
  both dispatch commit points — the ordinary send and the accepted branch of
  the steer queue — which together cover send, steer, wake queue, continuity
  capsule, and the headless `runSessionTurn`. A message refused upstream leaves
  the resume armed, and two senders are exempt. A send carrying `scheduledWake`
  is exempt so a fired resume cannot cancel itself. The host's own Resume now
  prompt (`metadata.usageLimitResume: "manual"`) is exempt because it already
  cancelled the row and awaited it: a second sweep at the commit point would
  move the cancel epoch that path captured before dispatching, and a send that
  then failed could no longer restore the streak, the row, or the state. The
  `scheduledWake` exemption is also why the renderer strips
  `scheduledWake` from replayed metadata on a **user-initiated retry**: without
  it the retry would inherit the exemption and leave a schedule armed that the
  transcript notice just promised retrying would cancel.
- **Two arms, then it stops.** The reset instant ADE can see is not always the
  limit that rejected the turn — Claude's snapshot is session-scoped and can
  carry a stale `resetsAtMs` forward, so a weekly-limit rejection would arm
  against a session reset, die at the same limit when it fires, and burn a real
  turn every cycle. After two arms against distinct reset instants the chat
  posts `Paused after 2 tries` and waits for a human. A resume that runs
  *without* hitting the limit again proves the window was right and resets the
  streak, so a healthy headless chat that hits limits days apart never pauses
  itself.
- **It announces itself in the transcript.** All three outcomes are
  `system_notice` rows with `noticeKind: "rate_limit"`, attributed to the failed
  turn, so `ProviderFailureRecoveryCard` renders them beside the failure they
  explain: `Resumes at 7:31 PM ET` when armed, `Resumed after usage limit` when
  it fires, `Paused after 2 tries` when the cap stops it. The host formats those
  times in the host zone *with* a zone label, because one written line is read by
  every client in every zone; the pill and the sheet format `fireAt` in the
  viewer's zone instead. Clients render the notice text as-is. The live schedule
  is represented by the one resume pill above the composer and appears only on
...
- **Claude Agent SDK pipeline.** The Claude adapter is built on the
  stable `query()` API and exactly pins SDK `0.3.280` in both the desktop
  ...
  plugin paths ride `pluginDelivery: "initialize"` when the resolved Claude
  Code binary is the bundled or tools-cache copy — that sends the list over
  stdin instead of one `--plugin-dir` argv flag per plugin, which is what keeps
  a Windows command line under its 32,767-character limit — and a user-supplied
  binary keeps argv delivery because its version is unknown. A query that
  carried plugins logs `agent_chat.claude_plugins_ignored` when the CLI's
  initialize result reports `plugins_applied: false`, independently of the
  hooks report. Every spawned subprocess is
  ...
  resolved runtime package metadata (with `0.3.280` only as the partial-install
  fallback), so packaged telemetry reports the SDK that actually shipped.
- **Claude SDK 0.3.280 event surfaces.** ADE enables SDK hook events,
  ...
  `cancel_queued: true` for **Turn + queue** (`stop_and_clear`, the default).
  Older compatible runtimes
  fall back to the public interrupt plus the capability-probed
  `cancelAsyncMessage` control for attributed queued steers. **Turn only**
  (`stop_only`) interrupts the model step while preserving queued messages
  and background jobs. Modes that also stop background
  (`stop_and_background`, `stop_and_clear_and_background`) call
  `stopActiveClaudeSubagents` and reset the query. Default Stop does not. The SDK's
  level-triggered
...
- **Steer queue.** Follow-up user messages during an active turn can be
  ...
  **Cursor** gets the same split control with the same three modes, because
  `@cursor/sdk` 1.0.31 added `Run.steer()`: **Send during turn** folds the
  message into the live local run, and **Interrupt & continue** cancels the
  run, waits for the turn to settle, and sends the message as the next turn on
  the same agent, which keeps the thread because the SDK's local agent store
  holds it. The transcript shows the previous turn marked interrupted, then the
  new turn streaming. Messages the user had already staged survive the redirect
  (the stop runs in `stop_only` mode with `preserveQueuedSteersOnInterrupt`
  armed until the interrupted turn's own tail consumes it, so a slow settle
  cannot wipe them) and are delivered after the redirect turn finishes. This
  softened stop is Cursor-only: `interrupt-replace` on OpenCode, Pi and Droid
  keeps its previous `stop_and_clear` contract. Cursor's inline channel is
  local-only — a Cursor **Cloud** run refuses every steer, so the renderer
  withholds the inline handler for one and the composer downgrades to *Send
  after turn* (never to interrupt); see
  [Agent Routing](agent-routing.md) for `cursorSessionRunsInCloud` and the
  refusal path. A `dispatchMode: "inline"` Cursor send that the run declines is
  never an error: the message is staged exactly as a queued send would have
  been, the transcript gets one notice saying it will go as a new message, and
  the row keeps its chip. Choosing an item in the split
  ...
  (not at enqueue). **The CTO thread's steer queue cap is zero.** It is
  interrupted constantly — child reports, scheduled wakes, peer notes — and
  holding any of those until the turn boundary would defeat the point of a
  persistent director. So a delivery that named no dispatch mode, which on any
  other chat means "stage it", instead resolves to the provider's first
  non-queue mode; `messageSession` applies the same rule, including for wakes,
  which stay boundary-delivered everywhere else. The guarantee that a non-queue
  mode always exists is `CTO_LIVE_REDIRECT_PROVIDERS` — the CTO can only run on
  Claude, Codex, or Cursor, `ensureIdentitySession` refuses an explicit model
  outside that set with copy naming the three, and its no-preference fallback is
  Claude rather than the general default. Newly submitted immediate modes are one atomic `steer`
  ...
  among **Turn only**, **Turn + queue** (default), **Turn + background**, and
  **Turn + queue + background**. If a queue-clearing mode cancels an ADE-attributed queued
  ...
  original steer ids, text, attachments, and context attachments. Per-task
  `ade.agentChat.stopTask({ sessionId, taskId })` stops one running Claude
  task; siblings keep running.
...
A **scene** is a ```` ```scene ```` fence containing ordinary HTML, CSS, and JavaScript that an agent wrote, rendered live in the transcript. It is the counterpart to a mosaic card and the opposite kind of thing: a mosaic is *data* — a versioned JSON body ADE validates and draws with its own components — while a scene is *code*, which is why it never runs in ADE's renderer.

**A scene shows, a mosaic asks.** That division is a rule, not a style preference. A scene must never draw its own approval control: approvals and destructive confirmations belong to mosaic and to ADE's native surfaces, and a card that approves itself is not a confirmation. The `ade-scene` skill states it to the model in the same words; `parseSceneHostMessage` enforces the other half by allowing only `ready`, `resize`, `settled`, `emit`, and `error` back out of the frame.

### The sandbox is the whole contract

A scene runs in an iframe with **its own origin**, **its own Content-Security-Policy response header**, and `sandbox="allow-scripts"` **without** `allow-same-origin`. Say it plainly: `allow-scripts` plus `allow-same-origin` is the one combination that would hand the frame back its own origin, and nothing in this feature may ever set both.

The origin comes from the transport. In Electron the document is served over the `ade-scene:` scheme — registered in `main.ts` with `{ standard: false, supportFetchAPI: true, stream: false }`, because a non-standard scheme is what gives the frame an opaque origin — and handled by a three-line adapter over `sceneDocumentStore.respond()`. The store holds documents **in memory**: the id in `ade-scene://view/<id>` is a map key and never a path, so the scheme has no filesystem reach by construction. There is no jail here to widen and none of `ade-artifact:`'s allow-root applies. When `window.ade.scene.prepare` is absent or rejects — the browser preview, a build without the main-process half — `SceneFrame` falls back to a blob URL, which is also a separate origin carrying the same policy from the in-document `<meta http-equiv>`.

`rendererCsp.ts` lists `ade-scene:` and `blob:` in **`frame-src` only**. Both are deliberately absent from `img-src`, `media-src`, and `connect-src`: ADE's own document has no legitimate reason to fetch a scene. `blob:` in `frame-src` is a **recorded, deliberate widening** — the comment in `rendererCsp.ts` states it as such rather than leaving the reasoning in the component that needs it. It is safe precisely because every scene frame carries the sandbox attribute without `allow-same-origin`, so a blob frame gets an opaque origin exactly like the custom scheme does; a blob URL there is only ever one this renderer minted from a document it assembled itself; and without it a failed `prepare` would render a blank frame instead of degrading. Removing `allow-same-origin` from that sandbox attribute is what would make the line dangerous, which is why `SceneFrame.test.tsx` asserts the pair can never appear together.

The policy itself (`SCENE_CONTENT_SECURITY_POLICY`) starts `default-src 'none'` and the directive that matters most is **`connect-src 'none'`**: no `fetch`, no XHR, no WebSocket, no beacon, no remote font, no remote image. **A scene cannot FETCH what it is shown out.** That is the honest form of the claim: navigating the frame (`location = "https://elsewhere/?" + secret`) is a navigation, not a fetch, and the scene policy says nothing about it. Two doors outside that policy close it — the renderer's own `frame-src` bounds where a nested context may go, and `main.ts`'s `will-frame-navigate` handler refuses a subframe navigation outside the same allowlist. That second door is not a hand-copied list: it calls `isRendererFrameNavigationAllowed` from `rendererCsp.ts`, built from the same sources `frame-src` is (`'self'`, `file:`, `app:`, local http, `ade-scene:`, `blob:`, `about:`, plus the Vite origin in dev). A narrower second door is its own bug — the handler first shipped without `file:`/`app:`, which in a packaged build is every `file://` spec preview (`SpecPreviewCard`, `PlanMarkdown`) going blank. `script-src 'unsafe-inline'` permits the inline script the model wrote and nothing remote; `img-src data: blob:` allows pixels the host handed over. It travels twice — as the response header from `sceneDocumentStore.respond()` and as the first element in `<head>` from `buildSceneDocument` — and neither half is load bearing alone: the meta survives a blob fallback, the header survives a document whose `<head>` was mangled. `unwrapDocument` strips any `Content-Security-Policy` meta the model itself wrote, so a scene cannot relax its own policy, and drops `<base>` so relative-URL behaviour stays predictable if the policy is ever loosened.

What the sandbox does *not* stop is a scene spinning the CPU or drawing something misleading, which is why `SceneFrame` keeps permanent chrome: a hairline rule down the left, a corner sparkle mark, and a `live` / `drawing` / `frozen` label under the frame. A view a model drew must never be mistakable for ADE's own UI.

### The still: a picture the moment it stops moving

A scene leaves a **still** behind, and it is taken when the scene stops moving rather than when its turn ends. The injected SDK arms a settle watcher at `ready` and posts a `settled` message — the fifth and last thing `parseSceneHostMessage` allows out of a frame — once nothing is animating and the DOM has been quiet for `SCENE_SETTLE_QUIET_MS` (600 ms), or `SCENE_SETTLE_MAX_MS` (4 s) has passed either way. Two signals, because neither is enough alone: `document.getAnimations()` sees `ade.animate` and CSS keyframes but not a `requestAnimationFrame` counter, and the MutationObserver sees `ade.countUp` writing a text node but not a transform that never touches the DOM. The cap is what makes a looping scene produce anything at all, and a frame of a loop is a truthful picture of a view that loops. The host runs the same deadline independently, one quiet window later, so a frame that never reports — an older prepared document, a script that threw before the watcher armed — still gets a picture.

The capture waits for the whole frame to be on screen, the same all-or-nothing rule the freeze uses and for the same reason: main intersects the rect, so a partly visible scene would be kept forever as a picture of a sliver of itself. Unlike the freeze there is no deadline, because there is nothing to lose by waiting — a scene that never comes fully into view simply has no still, which is where we started. The frame keeps running while all this happens; taking the still tears nothing down.

**A still is a picture, not proof.** The bytes go into `.ade/artifacts/computer-use/` through the same jail and the same broker (or the same CTO-only `computer_use_artifacts.ingestSceneSnapshot` runtime action) as the Proof button, and the broker record is what makes the picture findable again — but it is tagged `metadata.kind = "scene_still"` and **excluded from every proof surface**. `buildComputerUseOwnerSnapshot` is that single surface (the drawer's rows, its counts, its activity feed all come from it) and it passes `excludeMetadataKinds: ["scene_still"]`; `listArtifacts` applies the scalar `metadataKind` and `excludeMetadataKind` filters *in the query*, so a page of stills can never push proof out of a limit. Without the tag, every settled scene filed a drawer row nobody asked for.

Two deliberate differences from the Proof route. **`storeStill` never deletes the bytes it could not file** — a record that could not be created makes a proof file pointless, but a still is the picture itself and the renderer is about to show it back; an unlisted still is exactly what was asked for. And **a still is bounded, because nobody chose to keep it**: `fileSceneStill` keeps one still per `sceneScopeKey` (a scene that settles again supersedes its own picture, bytes and all, through the broker's own removal path) and at most `SCENE_STILL_MAX_PER_SESSION` (32) per owning chat, oldest first. Both bounds need an owner, so an unattributed still prunes nothing — the same trade the filing itself makes.

The record carries the still's identity: `sceneScopeKey`, `sceneTitle`, and `voiceCallId` for a scene drawn on a call. That is what makes the **broker the index**, and `sceneStillStore` only a window-lifetime cache in front of it: on a rehydrate, `SceneFrame` (through `useSceneStillRecord`) asks `computerUse.listArtifacts({ ownerKind: "chat_session", ownerId, metadataKind: "scene_still" })` **once per chat** and matches on `metadata.sceneScopeKey`; the voice-call card's tiles come from the same one query, filtered by `metadata.voiceCallId`. The renderer keeps no durable state of its own — it used to hold a `localStorage` index of artifact uris, a second source of truth main could not prune with the bytes.

The scope key is **per fence, not per row**: `sceneScopeKeyFor(rowKey, source)` hashes the fence body into the transcript row key, the way `MosaicCard` keys two answerable cards in one message. Two scene fences in one message used to share the row key, so whichever settled last overwrote the other's still and a reopened chat drew the same picture twice. A scene with no scope key is never stored at all — the voice HUD passes none when there is no call id, because a still nothing can look up is bytes on disk with no reader.

Showing the picture takes the same path the proof drawer's tiles take: the in-memory data URL when this window captured it, `ade-artifact://project/` for a local chat, and `computerUse.readArtifactPreview` pinned to the chat's machine for a remote one. `ade-artifact://` is a local-window protocol, so a chat pinned elsewhere would otherwise draw a permanently broken tile — a remote chat shows a real image or nothing.

On a **rehydrated** mount — scrollback after a remount, a chat reopened in a new window — a frame that begins with `live` false and a stored still shows the picture and **never prepares a document at all**: the code does not run a second time. That latch is taken once rather than derived, because `live` going false at the end of a turn must not yank a frame the user is watching; a scene that was live on its mount plays out and freezes the way it always did. A settled mount holds the latch undecided until the index has answered (`useSessionStillsReady`) and draws a placeholder for that tick: guessing "no still" while the query was in flight would re-run generated code, which is the one thing the still exists to prevent. The Proof button files whichever picture exists — the freeze capture or the settle still — so a scene that was scrolled past no longer files nothing.

### Lifetime: live for one turn, then a picture

`SceneFrame` takes a `live` prop. While it is true the scene executes; when it goes false the host measures the frame's rect, calls `window.ade.scene.snapshot` (clamped by `clampSceneCaptureRect`, because a rect that has scrolled off the page makes `capturePage` resolve an empty image), swaps in the returned PNG, and drops the iframe. **Scrollback shows the picture, never re-executed code.** With no capture route available the frame is left up rather than replaced with nothing.

**A partial capture is never accepted.** The clamp *intersects*, so a scene that is half scrolled off or only partly on screen would freeze to the visible sliver — permanently, and that crop is also what the Proof button files. So the host snapshots only when the shell's rect is entirely inside the viewport; otherwise it waits for the next scroll or intersection and tries once more, and if the scene is still not wholly visible it falls back to the no-capture-route behaviour and simply leaves the live frame up. With a settle-time still already in hand the freeze does not wait at all: it swaps straight to that picture, which is what stops a scrolled-past scene executing in scrollback forever.

The consequence for authors is in the skill: make it readable when it stops moving. A scene whose meaning depends on an animation you had to catch is a scene that is meaningless tomorrow.

`MarkdownBlock` threads a `sceneLive` prop down to the frame. `CtoVoiceHudHost` passes `live` directly for a scene drawn during a voice call, and takes its still the same way every other scene does — see the CTO feature doc for where that picture ends up. In a transcript, liveness is scoped to the scene's **own turn** (`renderEvent`'s `turnActive`, which matches the row's turn id against the active one) — not to the session. Scoping it to the session meant every scene in the transcript came back to life, and re-ran its script, the moment the user scrolled to it during a later turn.

`MarkdownBlock` also passes `streaming` — one flag, not a `live` + `sealed` pair, because only it knows both halves: the turn is live *and* `hasOpenSceneFence` says the closing ``` has not arrived. While that holds, the markdown parser still hands the half-written fence over as a finished code block on every reveal tick, and mounting on one prepared a new document and reloaded the iframe several times a second. A streaming scene shows a quiet `drawing` placeholder and mounts once, when the fence closes.

The **Proof** button under a frozen scene routes the snapshot to the proof drawer through `window.ade.scene.attachProof`. The main-process handler writes the PNG into `.ade/artifacts/computer-use/` and then splits: an in-process build files it through `computerUseArtifactBrokerService` directly, and a **runtime-backed** build — where that broker and `agentChatService` are both null in the desktop process, so the button used to answer `false` every time with its ownership check never reached — hands the path to the daemon through the CTO-only `computer_use_artifacts.ingestSceneSnapshot` action (`services/scenes/sceneSnapshotIngest.ts`), which repeats the same jail and owner-resolution checks against the services it actually owns.

Proof in ADE is chat-scoped, and the claimed session id comes from the renderer, so both routes resolve it against *this* project's own sessions rather than trusting it. A miss drops the owner, never the artifact: an unattributed snapshot is a smaller loss than one filed into another chat's drawer.

The runtime action's jail runs twice. A lexical `isPathInside` check comes first, before anything touches the filesystem, so a path outside the artifact store is refused for being outside it whether or not it exists and the error cannot be used to probe for files elsewhere. Then the same check on the **real** path, with the root resolved too: `resolve` is string arithmetic and knows nothing about symlinks, while the `statSync` and the ingest both follow them, so a link planted under the store passed a string-only jail and filed whatever it pointed at. On macOS the temp root alone is a symlink, which is why the root is realpathed rather than compared raw.

Cleanup of the bytes on disk is a **tri-state**, not "delete unless filed". The two failures are not symmetric: an unreferenced file is invisible and permanent — the store is pruned by drawer record, so nothing collects it — but it is inert, while a drawer record pointing at a file that is gone is a broken artifact the user sees and cannot open. So the handler tracks `filed` / `not-filed` / `unknown` and deletes only on an outcome it is certain of. A lost or failed RPC, where the daemon may well have committed the record before the answer went missing, is `unknown`: the file is kept and logged, and `listBrokenArtifacts` / `pruneBrokenArtifacts` reconcile the genuinely orphaned case.

### The injected SDK is dependency-free on purpose

`window.ade` is installed inside the frame before the model's code runs: `ade.data`, `ade.theme`, `ade.reducedMotion`, `ade.on`, `ade.emit`, `ade.ready`, `ade.resize`, `ade.animate`, `ade.countUp`. It also arms the settle watcher described above, which is the one thing in it a scene never calls itself. It ships no third-party library and never will, for two reasons that both stand alone — the frame has `connect-src 'none'` so it could not fetch one even if a scene referenced it, and inlining one would put that library's licence inside every generated view. The Web Animations API and CSS animations are native to Chromium, cost nothing, and are what models reach for anyway.

`ade.theme` exists because the frame is a different origin and cannot read ADE's stylesheet. `SceneFrame` resolves ADE's palette from computed custom properties and hands the scene **resolved values** — `var(--color-fg)` would mean nothing on the other side — which `buildSceneDocument` re-exposes as `--fg`, `--accent`, `--border`, and the rest on `:root`.

### Limits and failure

- Source over `SCENE_LIMITS.maxSourceBytes` (96,000) parses to a `too-large` failure; an empty body to `empty`. Either way `SceneFrame` renders the fence as highlighted HTML with a one-line reason rather than showing nothing.
- A scene that never calls `ade.ready()` is not necessarily broken, so the 8 s `readyTimeoutMs` stops the spinner, not the scene.
- An in-frame `window.onerror` is posted out as an `error` message and shown as a small warning line beside the title; it does not tear the frame down.
- `sceneDocumentStore` is capped at 64 documents. A long chat can prepare a scene per streamed revision, and the only cost of eviction is that a frame reloading an ancient scene gets a 404 instead of a re-render.
- `clampSceneCaptureRect` **intersects** the measured rect with the window's content box rather than shifting it. Clamping the origin alone while keeping the size froze a scrolled-off scene as a picture of the top of the window.
- `ade code`'s TUI collapses a ```scene fence to one dim line (`[scene: <title>]`, via `summarizeSceneFence`), exactly as it does a ```mosaic fence. The interactive frame is desktop-only, and printing up to 96 KB of HTML into a terminal buries the answer the user asked for.
- The `ade-scene:` scheme is registered and handled in `main.ts`, but the paths exercised by the test suite are the pure store (`respond`, `parseSceneRequestId`, eviction) and the blob fallback in `SceneFrame`; the custom scheme has not been driven end-to-end in a running Electron window.
...
The external-session import flow can turn a CLI session of any of its 10
providers into a full ADE chat. `externalSessionsService.importExternalSession`
checks the request against the shared import policy
(`shared/externalSessionPolicy.ts`), then delegates `target: "chat"` imports to
`agentChatService.importExternalChatSession`. The importer creates a normal
lane-scoped `AgentChatSession`, stamps `importedFrom` provenance (with
`mode: "continue"` or `"fork"`), seeds the ADE transcript from the external
history, and then binds future turns where the provider allows it. A
successful import returns the persisted
...
The importer takes one of four paths:

- **Claude native** (target model of the Claude family, or none). Imports read
  the source JSONL from Claude project storage. When the user copies, or the
  ...
  folder, then points the Claude SDK resume state at that id.
- **Codex native** (target model of the Codex family, or none). Imports read
...
- **Native continue** (Droid, OpenCode, Pi, Copilot, `mode: "resume"`). The
  chat seeds the provider pointer that it already restores from after a restart
  (`droidSdkSessionId`, the OpenCode `providerSessionId`, `piSessionId`,
  Copilot's `acpSessionId`), so the first turn opens the CLI's own session. The
  session folder must be the lane root, and the model must be of the provider's
  family. When the chosen model is the session's recorded model, the chat keeps
  the recorded reasoning effort. The imported history is for display only;
  nothing is replayed into the prompt.
- **Replay copy** (every other case, including a Claude or Codex copy into
  another model family). The history comes from `loadExternalSessionEvents`
  in `externalSessions/events/`, is written into the new chat, and is staged as
  a transcript replay for the chosen model's first turn. The model defaults to
  the session's recorded model when it is of the same family, else that
  provider's default model.

Chat continue for Cursor and the ACP providers Qwen, Kimi, and Grok is not
offered yet: ADE has not proven that its chat runtime can open a session the
CLI created. Copilot chat continue seeds `acpSessionId`, and `copilot --acp`
opens the CLI session over `session/load`. A copy leaves its original untouched, so the importer keeps
listing the original, marked "imported before".
...
happen to begin with `User request:`. `appendImportedChatEvents` runs every
imported history through `withImportedTurnBoundaries`
(`shared/importedTurnBoundaries.ts`), which closes each imported turn that did
tool work with a `done` event. The transcript lists finished tool calls only
in a turn's `done` summary, so without it imported tool calls were invisible.
...
3. `sendMessage({ sessionId, text, attachments? })` via
   ...
   agent-to-agent primitive. `kind: "auto"` steers active sessions using that
   provider's `defaultActiveTurnDispatchMode` and wakes idle sessions. The one
   exception is a Cursor **Cloud** session: its provider default is `inline`,
   which a cloud run refuses on every call, so `auto` routes to `interrupt`
   there — what `auto` meant for cloud before Cursor gained inline. (The
   renderer makes the opposite choice for the same session and downgrades to
   queue; a person must not be promoted into a cancel they did not pick, while a
   programmatic caller has no menu to fall back to.) Explicit `queue` always stages for
   the next turn and never picks the provider default. `wake` starts a normal
   turn, and `interrupt-replace` uses Claude SDK priority `now` on active
   Claude sessions (other providers keep their native interrupt-then-send
   path). The result reports the routed action and whether a
   steer was delivered or queued.    `ade chat send` uses the auto route, while
   `ade chat message --kind ...` exposes the explicit primitive. `ade chat show`
   stays on `chat.getSessionSummary`; `ade chat status` is the dedicated live
   turn snapshot (`chat.getTurnStatus`, RUNNING / BLOCKED / IDLE). Provider
...
`interrupt({ mode: "stop_and_background" })` (`SETTLE_TEARDOWN_STOP_MODE`): it
keeps the user's queued prompts (those are unrecoverable) and still stops
background work (the reason settle exists). That mode does **not** settle Codex
cards: stopping work is
not the same as discarding a decision the user still owns, exactly as
`stop_only` already spares queued follow-ups. Only queue-clearing modes
(`stop_and_clear`, `stop_and_clear_and_background`) — the user pressing a
Stop that clears the queue — settle them, and only the explicit **Dismiss & settle**
transaction discards a
...
On desktop a user bubble carries no status pill. Its delivery state is one
small line under the bubble, right-aligned with it and outside it, so it never
meets the bubble's hover actions (time, undo, copy). The line reads:
...
`queued` rows draw no line: a staged steer lives in the composer's staging
strip, and a queued row without a `steerId` is the optimistic stand-in for a
send that is starting a turn. A `delivered` or `processed` message without a
`steerId` (a subagent's spawn prompt) is an ordinary message and draws no
line. A resolved unprocessed steer's line reads `Started as the next turn` or
`Dismissed`. Lifecycle updates merge onto the same row by `steerId`, so the
line changes in place and the row key stays stable; the row-height estimate
budgets the extra line.
...
renders one compact details line. On desktop that disclosure is one row per
turn: the id-less session-startup snapshot, the turn's own snapshot, and the
turn's recovery receipt merge into it (see
[transcript-and-turns.md](transcript-and-turns.md#render-pipeline)).
...
- `ade new chat --mode chat|cli --type subagent|peer` (alias `--spawn-type`).
  ...
  lineage UI and then lands the spawn chip and subagent card in the parent
  (see [CLI children](#cli-children) below); plain shell launches omit them,
  and CLI lineage never populates `chatSessionId` because that field means
  attached-terminal ownership.
...
- The CTO's `spawnChat` tool sets `orchestrationParentSessionId` to the calling
  ...
  parented chat with no spawn kind and `execute` is reachable without schema
  parsing. The resolved kind is echoed back in the tool result.

A session summary also carries `parentIdentityKey` when its parent is an
*identity* session — `"cto"` today — so a roster can badge a chat's origin
without resolving the parent summary first. It is host-stamped, because identity
...
logos. `ade chat list` stays a flat table; it appends the lane's tracked CLI
children as `kind: "cli"` rows with their terminal status, exit code, and
parent. See
...
### A CTO parent gets one line

When the parent is the CTO, the `subagent_result` card is suppressed and the
completion renders as a single `system_notice` instead. The card restates the
child's whole closing summary, and the CTO runs dozens of chats at once, so a
report that pasted a transcript into the thread would bury everything else it is
holding.

`formatCtoChildReportLine` (`services/chat/ctoTurnContext.ts`) builds it:
`"<child title>" · <provider> · finished | was stopped | failed`, plus
`· PR #<n>` when `readChildPullRequestNumber` finds one — from the completion
report's artifacts first, falling back to the closing summary, because plenty of
agents write the number in prose and never file an artifact. The notice is
`warning` for a failure and `info` otherwise, and carries the same
`spawnCompletion` payload so delivery dedupe still anchors on it.

For a subagent the CTO is then woken with that same line as the wake text, plus
any unread worker discoveries. The wake deliberately carries no
`spawnCompletion` metadata — the notice owns that row, and a second copy would
draw the wake divider over it. Because the CTO thread's steer queue cap is zero,
the wake is delivered into the running turn rather than staged.
...
grandchild's status report cannot revoke its grandparent's. `boardMove` is in
that key list too, so neither a rule nor an agent can forge "the user moved this
card".

The same list decides what counts as **user activity**.
`messageClearsAttentionMarkers(metadata)` is the single predicate every send
...
A `--mode cli` child is a tracked PTY session, not a chat, so it has no turns
to report. It reports once per run instead: `agentChatService` listens on
`ptyService.onExit` and, for a terminal with parent lineage, closes the
parent's `chat:<terminalId>` card through the same delivery path chat children
use (`deliverChildCompletionToParent`) — `subagent` wakes the parent, `peer`
leaves the quiet note, the CTO gets one line. Exit 0 is finished, a non-zero
exit failed, and a closed or orphaned terminal stopped unless the CLI already
wrote its closing message. The report is that closing message (read from the
Codex rollout or Claude Code session file) or the last meaningful terminal
lines. An interactive CLI stays open after it finishes, so its card closes only
when the process exits; use `--mode chat` for a subagent that must report after
every turn. A parent agent polls a CLI child with `ade chat status <id>`, finds
it in `ade chat list`, and reads it with `ade chat read <id>` (last message
plus terminal tail) or `ade terminal read <id>` (full output). After a brain
restart the stale-run sweep hands an ended CLI child's open card to the same
path with quiet routing and never reads the terminal row as a missing chat.
The pure rules live in `apps/desktop/src/main/services/chat/cliChildSessions.ts`.
...
| `ade.agentChat.handoff` | invoke | Create a handoff session. `mode: "brief"` sends a compact summarized hidden first message and may use `targetLaneId` to move the new chat to any active lane in the project; unknown, unavailable, and archived lanes are rejected. `mode: "fork"` requires the same provider on both sides while allowing the target model to change within that provider, and always keeps the new chat in the source lane (a differing `targetLaneId` is rejected). Local forks also seed the source ADE transcript into the new chat with fork provenance. `handoffNote` is an optional user-authored addition: brief mode appends it to the hidden handoff prompt, while fork mode sends it as the first user turn. Claude forks through the SDK session pointer; Codex forks the app-server thread with `thread/fork`; OpenCode calls SDK `session.fork` (`POST /session/{id}/fork`); Droid calls SDK `session.fork()` (`droid.fork_session`) and then re-opens the source branch, because 0.9.x retires the source session handle on fork and the source chat must stay usable. Cursor has no provider fork surface at all and a Cursor thread cannot be resumed twice, so ADE forks it at the ADE layer: the new chat carries no `cursorSdkAgentId`, so its first send opens a brand-new Cursor agent and prefixes that send with the full source transcript replayed verbatim (the same `buildFittedTranscriptReplay` path a cross-provider fork uses, and the same replay an agent rotation stages), trimmed oldest-first if it exceeds the target model's context window or the target provider's input limit — in which case the handoff result carries `replayFork` and the chat shows a truncation notice. Codex replay inputs stay below the app-server's 1,048,576-character text limit. OpenCode and Droid persist the forked provider session as the new chat's resume pointer (`providerSessionId` / `droidSdkSessionId`). Codex targets do not inherit ADE session goals or seed app-server goals during handoff, and forked Codex threads are goal-cleared before any optional note is sent. Cross-machine fork additionally transports and rematerializes provider-native history for Claude, Codex, and OpenCode; cross-machine Cursor and Droid handoffs remain brief-only, Cursor because a replay fork has no provider artifact to send. Forwards `codexFastMode` when the target model supports Fast Mode. |
| `ade.agentChat.send` | invoke | Dispatch a user message + attachments. If the session has ended, sending is the continuation path. |
| `ade.agentChat.steer` | invoke | Send a follow-up message mid-turn. Which `dispatchMode` values a session accepts is `ACTIVE_TURN_DISPATCH_MODES`, not a list here; omitting it stages the message — except on the CTO thread, whose queue cap is zero and which resolves the omission to the provider's first non-queue mode. Claude's `"inline"` / `"interrupt"` are atomic SDK `priority: "next"` / `"now"` deliveries. Codex accepts `"inline"` only — its app-server folds a `turn/steer` request into the turn already running — and is rejected for `"interrupt"`. Cursor's `"interrupt"` is cancel + resend on the same agent thread, and its `"inline"` is `Run.steer()` into the live local run — a refusal there is not an error: the message is staged exactly as a queued send would have been, gets one "will send as a new message" notice, and the queue head is flushed if the turn boundary has already passed, so the returned `queued` flag still reflects what actually happened. OpenCode accepts `"inline"` and `"queue"`: inline admits the message through the v2 session prompt's `delivery: "steer"` into the live agent loop, and a refusal stages the row the same way Cursor's does, with one notice and `queued: true` reporting the truth. Returns `AgentChatSteerResult` (`{ steerId, queued, reason?: "queue_full" }`) — `queued: false, reason: "queue_full"` when the queue is at its cap. |
...
| `ade.agentChat.dispatchSteer` | invoke | Deliver an already staged steer immediately. The accepted `mode` per provider comes from `ACTIVE_TURN_DISPATCH_MODES`, so this is one guard rather than a per-provider ladder. Claude pushes an SDK message with `priority: "next"` (`mode: "inline"`) or `priority: "now"` (`mode: "interrupt"`), both with `shouldQuery: true`, and the staged row is removed only after the input pump accepts it. Cursor accepts both atomic modes. `"interrupt"` splices the staged row out, resolves it with a "Delivering your queued message..." notice, and promotes it to the interrupt-and-continue redirect; the row is put back (and its settlement re-opened) if the redirect throws, so the message is never silently lost. `"inline"` offers the staged row to the live run via `Run.steer()` and removes it **only** on a `complete_delivered` outcome — a refusal (a Cursor Cloud run, attachments, per-message overrides, a turn that declined, a missing steer channel) answers `{ dispatchedAt: null }` **without throwing**, leaves the row staged with its chip visible, emits one "couldn't go into the running turn" notice, and flushes the queue head if the turn boundary has already passed. Callers must therefore read `dispatchedAt` instead of treating a non-throwing reply as delivery — the TUI and the iOS staged strip both do, and keep the queued display when it is null. Codex accepts `mode: "inline"` only: the app-server folds a `turn/steer` request into the turn already running, so the staged row is spliced out, resolved with the same notice, and re-entered through `steerWithOptions({ dispatchMode: "inline" })` rather than a second copy of the `turn/steer` request — which keeps the expected-turn mismatch retry and the accepted-steer bookkeeping in one place. With no `threadId` or no `activeTurnId` there is nothing to fold into, so the row stays staged for the turn boundary instead of firing at a thread that is not running; a throw restores it exactly as Cursor's does. OpenCode accepts `mode: "inline"` only: the staged row is spliced out, resolved with the same notice, and folded into the live agent loop through `dispatchOpenCodeSteerMessage` (the v2 `delivery: "steer"` input); a throw restores the row to the queue and re-opens its settlement, exactly as Cursor's and Codex's paths do. Modes a provider does not carry in the table — every mode on the queue-only providers (Droid, Pi, the ACP providers), plus `"interrupt"` on Codex and OpenCode — are rejected with `unsupportedActiveTurnDispatchModeMessage`. |
...
| `ade.agentChat.interrupt` | invoke | Provider-specific interruption of the in-flight turn. Claude accepts optional `mode: "stop_and_clear" \| "stop_only" \| "stop_and_background" \| "stop_and_clear_and_background"` (default `stop_and_clear`) and returns the chosen mode, cancelled queue count, and an optional eight-second recovery id/expiry. Default `stop_and_clear` stops the turn and clears the queue but leaves Claude background jobs running once `perTaskStopAffordance` is on. Codex reads the queue axis of the same `mode`: a queue-clearing mode settles the session's pending Codex cards through `settleCodexPendingInputs` on every arm — including the two arms that return early because there is nothing to interrupt (no thread id, no active turn), which is the *common* case, since a plan approval is raised after its turn completes and would otherwise stay unanswerable and keep the composer locked. `stop_only` and `stop_and_background` stop the work and leave the cards. Other providers retain their existing stop behavior. |
| `ade.agentChat.stopTask` | invoke | Claude-only: stop one running background task or native subagent by `taskId`. Siblings keep running. Spawned ADE chats (`chat:` prefixes) are not this API; they have their own session interrupt. Desktop Chat Info, spawn cards, iOS Work, `ade chat stop-task`, and `chat.stopTask` over sync all call this. The ADE Code TUI has no per-task control. |
| `ade.agentChat.restoreCancelledQueue` | invoke | Claude-only recovery for a recent queue-clearing Stop. Restores the original ADE-attributed queued steers when the matching recovery id is still live; returns `{ restored, restoredCount }` and never recreates expired or foreign-session entries. |
...
| `ade.agentChat.updateSession` | invoke | Mutate permission modes, `manuallyNamed`, capability mode, the legacy-named `codexFastMode` Fast Mode toggle, Claude SDK session title/tag metadata, `spawnKind` (`subagent` / `peer`), `subagentTakeoverPromptShown`, and `autoContinueAtUsageLimit`. Setting `autoContinueAtUsageLimit: false` persists Don't continue, clears `usageLimitParkedUntil`, cancels any ADE auto-resume schedule, and interrupts a busy Claude query with default Stop (background jobs keep running). An empty tag clears the SDK tag. Demoting to peer posts a quiet takeover note on the parent. |
...
| `ade.agentChat.getContextUsage` | invoke | Claude-only: return the SDK control-channel context usage breakdown (`AgentChatContextUsage`) for the `/context` panel. Categories are classified by `kind` (`used` / `free` / `buffer` / `deferred`), never by the display name `"free"`. The harness also performs this authoritative read after initialization, each settled turn, and compact completion; streamed estimates remain the responsive in-turn signal. |
| `ade.agentChat.rewindFiles` | invoke | Dry-run and apply provider-backed file rewind for a selected user message. Claude uses SDK file checkpoints and leaves conversation history untouched; `skippedLinks` reports linked paths the SDK could not restore and produces a visible notice rather than silently claiming a complete rewind. Codex restores ADE's git-tracked file plan and moves the app-server thread back — `thread/fork` before the selected turn on servers >= 0.145.0, or `thread/rollback` (latest user message only) on pre-0.156 servers when fork is unavailable or the turn id cannot be resolved; 0.156 removed rollback. The renderer dry-run opens the same confirmation dialog with message context, aggregate stats, per-file rows, and lazy diff previews. |
| `ade.agentChat.claudePlugins.list` / `.reload` | invoke | Claude-only: enumerate discovered Claude plugins and force a plugin reload. Backs Claude `/plugin`. |
| `ade.agentChat.codexPlugins.list` | invoke | Codex: `plugin/reconcile` then `plugin/list`. Settings and TUI `/plugin` show installed/enabled plugins only (bundled / local / already-installed remote). Fail-open on missing runtime or `-32601`. No marketplace and no toggle. |
...
| `ade.agentChat.saveTempAttachment` | invoke | Write pasted image bytes to a runtime-owned temp file. This is the base64 leg, so it keeps `LEGACY_MAX_CHAT_ATTACHMENT_BYTES` (10 MB); the image MIME sniff runs on the sync leg only — the local IPC/action handlers check size and extension shape. Clipboard pastes, renderer-converted HEIC, and every file in the hosted web client take it, as does ADE Code clipboard-image paste, so local and remote chats both receive an attachment path the agent can read. For a paired machine that takes the streamed upload, preload does not send this command: it sends the bytes to `ade.remoteRuntime.uploadChatAttachment` as `data` instead, and uses the command only when the upload is not offered or fails for a payload the command can carry. |
| `ade.agentChat.copyTempAttachment` | invoke | Stage a file that already exists on **this** machine's disk by copying it, so the bytes never enter the renderer. Backs the preload `agentChat.stageFileAttachment` whenever the chat's binding is `kind: "local"`, and carries the 50 MB `MAX_CHAT_ATTACHMENT_BYTES` product cap with no type restriction. Reached only after `getAttachmentStagingMode` answered `copy` — which preload answers itself, because "same machine as the main process" is a property of the caller, not of the target. It is also on the ADE action allowlist and therefore reachable over sync RPC from a paired peer; that is not an escalation, because opening that channel requires a runtime-host pairing grant and a peer holding one already reaches `chat.launchCli`. Pairing, not this allowlist, is where that trust is decided. |
| `ade.remoteRuntime.attachmentUploadCapability` / `ade.remoteRuntime.uploadChatAttachment` | invoke | The remote half of the same preload calls. The capability probe answers a `ChatAttachmentStagingMode` directly (`upload` with the host's ceiling, or `base64` with the legacy one); the upload mints a ticket over the authenticated sync socket and streams the file body over HTTP. The upload takes either a `sourcePath` on this machine or base64 `data` with the machine's `maxBytes` (a pasted image); `data` is checked against the smaller of `maxBytes` and the 50 MB product cap, written to a private temp file, uploaded, and removed. The upload channel carries a 6-minute IPC budget in `ipcTimeouts.ts` — above the client's own 5-minute budget, so a slow 50 MB upload reports the legible reason instead of a transport timeout that also drops the pending chip. |
| `ade.agentChat.getImageDataUrl` | invoke | Read a bounded project-local image file through the active runtime and return a data URL for attachment previews. The service validates realpath containment, file type, and size before reading. Every renderer caller goes through `readAttachmentImageDataUrl(path, pin)` (`renderer/lib/attachmentImage.ts`), which reads through the pin's runtime and falls back to the local guarded `app.getImageDataUrl` only when this computer owns the attachment. A remote-owned path is never read locally. |
| `ade.agentChat.listSubagents` | invoke | Provider-neutral subagent snapshot list. Snapshots are re-keyed on `agentId + parentToolUseId` (not just `taskId`) so multiple subagents spawned from the same parent tool call don't collide, and the renderer panel separates them into three tabs: Subagents, Teammates, and Background. Claude `subagent_*` envelopes are enriched with `agentType` (e.g. `code-reviewer`, `Explore`) by stashing the Task tool's `subagent_type` input at the assistant `tool_use` boundary and joining on `parentToolUseId` when the system-message `task_*` envelope arrives; the Task tool's `name` rides separately as `label` (the human-chosen display name for that spawn) and is never substituted for `agentType`. Codex parallel-agent events carry an `Agent #N` label assigned at first announcement (1-based, per-turn) plus the raw threadId as `agentId`. OpenCode snapshots use the child session id, Cursor Task snapshots use the tool call id plus any returned child agent id, and Droid worker snapshots use the worker session id. |
...
| `ade.ai.cursorCloud.fleet` | invoke | Account-wide fleet read for the top-bar Cursor Cloud view: every cloud agent returned by Cursor, with latest-run status, pushed branch/PR, model, ownership (session/lane/Linear id), and `matchedBy`. Returns `relayState` + `lastEventAt` so clients can state honestly whether live updates are configured. Backed by `cursorCloudFleetService.ts`; repository matching remains scoped to safe branch pull/merge actions. |
...
| `ade.ai.devinCloud.openChat` / `.followUp` / `.createSession` / `.watchMirror` | invoke | Mirrored-chat seam: open mirrors the session transcript (`GET messages`) into an ADE chat, followUp sends `POST messages`, createSession launches a lane-bound session with `repos`, `devin_mode`, `bypass_approval`, and `ade`/`ade:lane:<id>` provenance tags, and watchMirror drives the open-chat hydrate tick (also the attention + proof-sync pump). |
...
Codex 0.156.0 can attach questions to an `agentMessage` item: `delivery:
...
Agent SDK 0.3.280 can ask the host to render a dialog
...
- **Cursor local chats Read images from two paths outside a lane worktree.**
  ...
  model to Read that file. Oversized attachments that skip inlining are
  named as `<project>/.ade/attachments/<uuid>.<ext>`, which is also
  outside a worktree lane. The host hook allowlists those two roots for
  **read** only (`cursorSdkPolicy.ts`). The attachments grant needs the
  explicit ADE project root from worker init — a missing root fails
  closed and does not walk up from the lane. After realpath, the
  directory's basename must still be `attachments`, so a junction onto
  `.ade` or `.ade/secrets` cannot inherit the grant. Writes, shell
  `cat`, other projects' Cursor dirs, and `.ade/secrets` stay denied.
  Do not copy attachments into the lane to work around this — that
  dirties git and is the wrong fix.
...
- **Every path that ends a Codex turn must settle that turn's pending cards.**
  ...
  - **Queue-preserving modes do not settle Codex cards.** Settle teardown
    uses `stop_and_background`, which stops work without discarding a
    decision the user still owns, the same way `stop_only` spares queued
...
- **Dispatched steer cancellation.** Every priority message receives a fresh
  ...
  the runtime `cancelAsyncMessage(uuid)` control. SDK `0.3.280` additionally
...
- **Pending input derivation.** The renderer's `derivePendingInputRequests`
  ...
  Rule (f) **marks, it does not delete.** A swept entry stays in the derivation
  carrying `sweptWithoutReceipt: true` — the `tool_result` / `command` /
  `file_change` sweep is not a turn boundary, and the absent receipt is what all
  three have in common. `resolvePendingInputs(derived, summary)` then decides at
  *read* time whether the sweep stands: an entry the summary still names in
  `pendingInputItemId` survives; everything else swept disappears. Only
  `pending_input_resolved` hard-deletes, so an answered card can never be
  resurrected by a summary that is one refresh stale.

  Know how main produces that id before trusting it: `summarizeSessionRow` gates
  on `hasLivePendingInput(liveManaged) || persisted?.awaitingInput === true`,
  and with no resident runtime it falls back to
  `latestPendingInputItemIdFromEvents`, a transcript scan that clears only on
  `pending_input_resolved` / `auto_approval_review` — never on `tool_result` or
  a turn end. It therefore biases toward over-claiming, so after a restart a
  card mooted only by its tool resolving can still be named and drawn. That is
  the intended trade: clicking it takes `respondToInput`'s not-found path, which
  emits a receipt and clears the block, whereas the opposite bias reproduces the
  invisible wedge. Main names at most one card, so two live at once reveal one
  at a time — harmless, since the composer draws only `pendingInputs[0]` and the
  "needs you" badge reads `awaitingInput`, true while any **blocking** card is live.
  Codex `isBlocking: false` steering is the other live card: `summarizeSessionRow`
  sets live-only `steeringInput: true` and must not set `awaitingInput` or
  `pendingInputItemId`. The session list stays Working with a `?` pip.
  `steeringInput` is never persisted.

  This split exists because a card's asker can outlive the turn it was tagged
  with — `requestChatInput` stamps `turnId` from whatever turn is active when
  the card is raised, but a backgrounded `ade actions run`, or an `ask_user`
  whose caller stopped waiting, keeps blocking long after that turn ends. When
  the sweep alone decided, the renderer deleted such a card while main went on
  counting it: no card drawn, `send` refused with "Answer or decline the pending
  request before sending another message", and fork refused because the session
  never read idle — with nothing on screen to answer.

  `resolvePendingInputs` obeys the same placement rule as `resolveTurnActive` —
  outside the derivation and outside the view cache — and is applied at **read**
  time, in `AgentChatPane`'s `resolvedPendingInputsBySession` memo, which every
  reader goes through.
  `appendRetainedChatSessionEvents` writes that cache from a context with no
  summary at all, so a summary-tainted derivation would be cached
  inconsistently and read back stale. Keep the raw derivation raw.
  `derivePendingInputRequests` is also the renderer's *only* pending-input
  derivation; do not add a second copy (one was deleted for having drifted).

  iOS mirrors this whole split — `workPendingInputSweepState` /
  `WorkPendingInputQueue.resolved(hostPendingInputItemId:)` /
  `canonicalPendingInputs` — because the phone had the identical bug. Change one
  side and change the other: they read the same durable transcript, and the one
  place they intentionally differ (iOS sweeps `plan_approval` after a completed
  turn instead of sparing it, relying on the summary to bring it back) is
  recorded in
  [iOS companion](../sync-and-multi-device/ios-companion.md).
- **Answering a card whose asker is gone must still write a receipt.**
  `respondToInput` has one not-found branch per waiter registry — the ADE-local
  card map, Claude's `canUseTool` waiters, OpenCode's `pendingApprovals`,
  Cursor's `permissionWaiters`, and Droid's — plus the terminal
  no-live-runtime fallback, and each is reached when the click lands after the
  thing that raised the card is already finished (a double-click, an interrupted
  turn, a summary that over-claimed after a restart). All six route through
  `settleUnclaimedPendingInput`, which emits `pending_input_resolved` plus a
  `system_notice` reading "That request is no longer active." and persists.
  Returning silently is not an option: the renderer defers to the summary for a
  card swept without a receipt, and the summary's own restart fallback
  (`latestPendingInputItemIdFromEvents`) clears only on
  `pending_input_resolved`, so a silent return leaves the card to be redrawn by
  the next re-derivation — an unanswerable card, the exact failure this path
  exists to prevent. An `accept` / `accept_for_session` is recorded as `cancel`
  because nothing consumed the acceptance, and the transcript is durable and
  synced: calling an approval "accepted" that no runtime ever received would be
  a lie every other surface reads back. These branches log at `warn`, not
  `debug` — they now write a durable receipt and paint a notice, so they need a
  log that explains one.
...
- **OpenCode runtime contracts: prompt-body system prompts, 404-gated
  ...
  of resetting the thread into an empty session. The event pump subscribes,
  then dispatches `session.promptAsync`, then **drains the SSE in the same
  turn without awaiting that prompt ACK first**. The stream is live-only:
  awaiting `promptAsync` before `for await` drops `message.updated` on a fast
  follow-up (the first post-handoff turn is usually slow enough to win the
  race). The role gate then silences every assistant part while `session.idle`
  and `step-finish` still complete the turn, so the chat shows usage and no
  reply. And because
...
- **Never state `external_directory` in an ASKING/DENYING ADE agent's permission
  block — full auto is the one exception, and it states `allow`.**
  ...
  worktree", which is what the edit/plan/helper rulesets want. This binds those
  three, `deny`
  ...
  `ade-full-auto` is the exception because full access that still prompts for a
  path is not full access — reading a pasted screenshot under `.ade/` raised a
  card on a mode whose whole promise is no cards. It states `allow`, which is
  safe where "ask"/"deny" are not: the appended `{pattern:"*"}` rule wins the
  `findLast` for every path, so it grants unrestricted external read/write —
  paths OpenCode would otherwise ask about or deny included. That is the promise
  of full auto; "ask"/"deny" would instead revoke OpenCode's access to its own
...
- [Cross-machine session handoff](../sync-and-multi-device/cross-machine-session-handoff.md) -- clean/published Git, bounded context, destination setup, transport security, and retry semantics for cross-machine handoff.
