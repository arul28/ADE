...
- **Claude Agent SDK pipeline.** The Claude adapter is built on the
  stable `query()` API and exactly pins SDK `0.3.278` in both the desktop
  ...
  resolved runtime package metadata (with `0.3.278` only as the partial-install
  fallback), so packaged telemetry reports the SDK that actually shipped.
- **Claude SDK 0.3.278 event surfaces.** ADE enables SDK hook events,
...
Agent SDK 0.3.278 can ask the host to render a dialog
...
- **Dispatched steer cancellation.** Every priority message receives a fresh
  ...
  the runtime `cancelAsyncMessage(uuid)` control. SDK `0.3.278` additionally
