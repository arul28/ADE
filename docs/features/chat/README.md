...
| `apps/desktop/src/shared/chatScheduledWork.ts` | Cross-target scheduled-work derivation. Folds `scheduled_work_update` envelopes into stable snapshots for Claude wakeups, cron tasks, `/loop`, remote triggers, and background work; partitions completed one-shot wakeups into history; and computes next-fire labels. Consumed by desktop Chat Info, ADE Code Chat Info, and iOS Work chat so scheduled/background activity presents consistently. |
...
| `apps/desktop/src/renderer/components/chat/ChatSubagentsPanel.tsx`, `chatSubagentIdentity.tsx`, `codex/CodexGoalCard.tsx` | Chat Info drawer content: Codex goal card, latest plan, `todo_update` tasks, scheduled work, and subagent/background rosters. The Schedule header exposes the chat-level pause/play control; paused rows dim in place, recurring rows show last-run plus next-fire timing, and fired one-shot wakeups move into a collapsed History group. `chatSubagentIdentity.tsx` centralizes deterministic subagent display names, colours, and identicon glyphs so the drawer and transcript activity bundles render the same agent identity. The goal card sits above the plan/subagent roster, exposes edit/status/clear affordances through typed ADE goal APIs, and shows usage context as tokens/time only; provider token budgets are hidden and cleared so ADE goals stay unlimited. |
...
- **Claude Agent SDK pipeline.** The Claude adapter is built on the
  stable `query()` API (SDK 0.3.202): every chat owns a `ClaudeQuery`,
  ...
  `Query` handle.
- **Claude SDK 0.3.202 event surfaces.** ADE enables SDK hook events,
  ...
  readable badges in the transcript. Claude sessions keep a long-lived idle
  reader attached after a visible turn completes, while ADE's durable
  ...
  Code, and iOS Chat Info. Deliberately not wired yet: SDK `SessionStore` as
  ADE's transcript backend and channels/external message origins.
