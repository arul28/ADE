...
| `apps/desktop/src/main/services/chat/chatLaunchArgs.ts`, `chatLaunchRecords.ts`, `chatLaunchTranscriptCard.ts`, `chatCreateModelResolution.ts`, `apps/desktop/src/shared/uuid.ts` | Launch support modules. `chatLaunchArgs` is the one parser for untrusted launch input (`parseChatLaunchArgs`, `parseChatLaunchIdArgs`, `parseChatLaunchQueueMessageArgs`, `parseChatLaunchCompleteClientArgs`), used by both the action domain and the sync host, and also owns the `chat.create` / `chat.send` field parsers the sync host reuses for its direct chat commands. `chatLaunchRecords` persists launch records under `.ade/cache/chat-launches/` and, on reload, turns a mid-flight launch into an "interrupted" failure and jumps its `sequence` ahead. `chatLaunchTranscriptCard` emits and updates the `lane_setup` `ade_card` (first emit waits for the opening `user_message`, with a 4 s fallback). `chatCreateModelResolution.resolveChatCreateModel` fills an empty `model` with the host's first available one (activating OpenCode, Pi, and ACP provider runtimes first), shared by the sync host's `chat.create` / `chat.launch` and every launch. `shared/uuid.ts` normalizes reserved ids (`requireNormalizedUuid`). |
