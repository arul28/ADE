...
The record carries the still's identity: `sceneScopeKey`, `sceneTitle`, and `voiceCallId` for a scene drawn on a call. That is what makes the **broker the index**, and `sceneStillStore` only a window-lifetime cache in front of it: on a rehydrate, `SceneFrame` (through `useSceneStillRecord`) asks `computerUse.listArtifacts({ ownerKind: "chat_session", ownerId, metadataKinds: ["scene_still"] })` **once per chat** and matches on `metadata.sceneScopeKey`; the voice-call card's tiles come from the same one query, filtered by `metadata.voiceCallId`. The renderer keeps no durable state of its own — it used to hold a `localStorage` index of artifact uris, a second source of truth main could not prune with the bytes.
