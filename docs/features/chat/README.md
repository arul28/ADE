...
| `apps/desktop/src/main/services/opencode/**`, `apps/ade-cli/src/services/opencode/openCodeStoreMaintenance.ts` | OpenCode integration: owned data home + auth seeding + legacy-session continuity, the queue-only steering capability and why, child-session permission routing, the bounded idle probe, and `ade storage opencode` store pruning. See [OpenCode integration](opencode-integration.md). |
