...
- **The model catalog lists connected OpenCode providers only.**
  ...
  and `opencode models` CLI list connected providers only. A provider
  block is still emitted for *every* provider (empty when unconnected),
  so the connect affordance and provider counts survive without the
  bulk. Do not "restore" unconnected models to make browsing work —
