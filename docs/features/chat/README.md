...
`MarkdownBlock` threads a `sceneLive` prop down to the frame. `CtoVoiceHudHost` passes `live` directly for a scene drawn during a voice call; no transcript caller sets `sceneLive` today, so a scene in a chat message freezes as soon as it has drawn.

The **Proof** button under a frozen scene routes the snapshot to the proof drawer through `window.ade.scene.attachProof`.
