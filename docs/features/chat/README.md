...
The still is kept twice, in `sceneStillStore`. The in-memory half holds the PNG data URL the capture produced and belongs to this window. The persisted half holds the **project-relative artifact uri**, written by the `scene.storeStill` route into `.ade/artifacts/computer-use/` through the same jail and the same broker (or the same CTO-only `computer_use_artifacts.ingestSceneSnapshot` runtime action) as the Proof button, and read back through `ade-artifact://project/`. One deliberate difference between the two routes: **`storeStill` never deletes the bytes it could not file.** Proof is a drawer record, so a record that could not be created makes the file pointless; a still is the picture itself and the renderer is about to show it back. An unlisted still is exactly what was asked for.

Both are keyed by the caller's `scopeKey` — the transcript row key, or the call id for a scene drawn on a call — which is what makes two byte-identical scenes at different positions keep their own picture.

On a **rehydrated** mount — scrollback after a remount, a chat reopened in a new window — a frame that begins with `live` false and a stored still shows the picture and **never prepares a document at all**: the code does not run a second time. That latch is taken on the first render rather than derived, because `live` going false at the end of a turn must not yank a frame the user is watching; a scene that was live on its mount plays out and freezes the way it always did. The Proof button files whichever picture exists — the freeze capture or the settle still — so a scene that was scrolled past no longer files nothing.
