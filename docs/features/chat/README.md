...
| `apps/desktop/src/main/services/opencode/**`, `apps/desktop/src/shared/opencodeStoreMaintenance.ts` | OpenCode integration: owned data home + auth seeding + v2 config file, the v2 runner for new chats (real mid-turn steering, `session.wait` + `session.active` completion) with legacy continuity for old session ids, child-session permission routing, the bounded idle probe, and store pruning/automatic retention. See [OpenCode integration](opencode-integration.md). |
...
| `apps/desktop/src/shared/chatErrorPresentation.ts` | Host rewrite for failed turns, shared by every provider path. `presentChatFailure` turns a provider/SDK failure into `{ title, body, nextAction?, technicalDetail? }` for one of `rate_limit` / `auth` / `network` / `busy` / `configuration` / `unknown` (`not_found` deliberately lands on the generic stopped-turn copy, because ADE recycles the agent and resumes). Every sentence is provider-agnostic — `provider` is substituted where the host knows it and the copy stays neutral where it does not — and only a Cursor label may name Cursor's sandbox and ADE's hook fallback. `isSandboxUnsupportedFailureText` is the single sandbox predicate (also imported by `cursorSdkProtocol.ts`), and it **outranks the caller's kind**, so a provider that reports the failure as a plain error still gets the configuration card. `configuration` is the one kind that discards the host message entirely: the raw sentence tells the user to edit `local.sandboxOptions` or `~/.cursor/sandbox.json`, which is never card copy. Raw text survives only in `technicalDetail`, deduplicated across message / detail / error code. `chatErrorKindFromCategory` narrows an event's `errorInfo.category`; `readChatErrorPresentation` is the client-side reader. Clients render `errorInfo.presentation` and never title the row Error or Unknown. `summarizeAcpStderrTail` / `sanitizeAcpStderrTail` turn an ACP agent process's captured 4 KB stderr tail into card-ready copy: ANSI escapes and control characters are stripped, credential-shaped values are redacted (keyword/value pairs, provider prefixes, JWTs, and any opaque 32+-character digit-plus-letter run regardless of case — so a lowercase hex secret cannot survive; a canonical UUID is the one kept id), the last meaningful non-stack-frame line becomes the headline, and the sanitized full tail rides in `technicalDetail` behind Copy. The ACP turn-failure path passes the tail captured on `onProcessExit`; a crash before any exit handler exists (during the `initialize` handshake) rides the pool's rethrown error, which `initializeAcpConnection` augments via `readStderrTail` / `readAcpStderrTailFromError`. So a bad flag or missing config leads the card instead of a generic `connection closed`, and an empty or noise-only tail leaves the existing copy untouched. |
