...
| `apps/desktop/src/shared/chatScheduledWork.ts` | Cross-target scheduled-work derivation. Folds `scheduled_work_update` envelopes into stable snapshots for Claude wakeups, cron tasks, `/loop`, remote triggers, and background work; consumed by desktop Chat Info, ADE Code Chat Info, and iOS Work chat so scheduled/background activity presents consistently. |
...
- **Claude SDK 0.3.202 event surfaces.** ADE enables SDK hook events,
  ...
  reader attached after a visible turn completes, so SDK-origin messages from
  `ScheduleWakeup`, `CronCreate`, `/loop`, remote triggers, and background
  tasks can start synthetic idle turns later; those emit normal transcript rows
  plus `scheduled_work_update` snapshots for desktop, ADE Code, and iOS Chat
  Info. Deliberately not wired yet: SDK `SessionStore` as ADE's transcript
  backend and channels/external message origins.
