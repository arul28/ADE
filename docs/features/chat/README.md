...
- **Native continue** (Droid, OpenCode, Pi, `mode: "resume"`). The chat seeds
  the provider pointer that it already restores from after a restart
  (`droidSdkSessionId`, the OpenCode `providerSessionId`, `piSessionId`), so the
  first turn opens the CLI's own session. The session folder must be the lane
  root, and the model must be of the provider's family. The imported history
  is for display only; nothing is replayed into the prompt.
