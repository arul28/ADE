...
`rendererCsp.ts` lists `ade-scene:` and `blob:` in **`frame-src` only**. Both are deliberately absent from `img-src`, `media-src`, and `connect-src`: ADE's own document has no legitimate reason to fetch a scene. `blob:` is safe in `frame-src` precisely because every scene frame carries the sandbox attribute without `allow-same-origin`, so a blob frame gets an opaque origin exactly like the custom scheme does — and without it a failed `prepare` would render a blank frame instead of degrading.

The policy itself (`SCENE_CONTENT_SECURITY_POLICY`) starts `default-src 'none'` and the directive that matters most is **`connect-src 'none'`**: no `fetch`, no XHR, no WebSocket, no beacon, no remote font, no remote image. **A scene cannot exfiltrate what it is shown.** `script-src 'unsafe-inline'` permits the inline script the model wrote and nothing remote; `img-src data: blob:` allows pixels the host handed over. It travels twice — as the response header from `sceneDocumentStore.respond()` and as the first element in `<head>` from `buildSceneDocument` — and neither half is load bearing alone: the meta survives a blob fallback, the header survives a document whose `<head>` was mangled. `unwrapDocument` strips any `Content-Security-Policy` meta the model itself wrote, so a scene cannot relax its own policy, and drops `<base>` so relative-URL behaviour stays predictable if the policy is ever loosened.
