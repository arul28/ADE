...
- **Work draft launches.** From an empty embedded Work composer, the
  ...
  flag), and `StartedDraftLaunch` (returned session id + kind).
  Foreground auto-create opens the new session in Work. Background
  auto-create records the session without stealing focus and shows a
  dismissible `BackgroundLaunchNotice` that carries `draftKind` so
  the "Open" action can restore the correct Work draft kind.
