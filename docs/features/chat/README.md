...
Continuity for the fresh agent is staged by the same shared seeding used by
Cursor's fork (`stageCursorSdkContinuityContext`): a header, the session's
continuity summary, and a recent-conversation tail, plus a cleared
lane-directive dedupe key so the new agent receives the lane execution
directive. Seeding is skipped entirely when there is nothing to carry, because
a header announcing restored context with no context attached only misleads the
model.
