...
| `apps/desktop/src/shared/claudeGuiSlashCommands.ts`, `claudePermissionDialog.ts`, `claudeAgentSdkFields.ts`, `chatSubagentTree.ts`, `chatTurnStatus.ts` | Claude Agent SDK 0.3.258 GUI adapters: drop terminal-only slash commands (`/exit`, `/quit`, `/statusline` plus init `terminal_slash_commands`) from every AgentChatSurface; `default_to_no` permission options; ambient/`skip_transcript` housekeeping; spawn-depth tree connectors; dedicated `chat.getTurnStatus` snapshot used by `ade chat status`. |
...
| `apps/desktop/src/renderer/components/chat/ProviderFailureRecoveryCard.tsx` | Classifies terminal provider capacity and usage-limit errors into actionable transcript cards. The card explains that the thread remains safe, offers an explicit same-thread **Retry turn**, and opens the composer model picker through a one-shot request for **Choose model**; neither action is enabled while another turn is active. When a usage-limit failure left auto-continue armed, the card shows **Usage limit reached**, the local reset clock, status text **Continue automatically**, and **Don't continue**; Don't continue opts the chat out of Claude SDK `autoContinueAtUsageLimit` and cancels any ADE `auto_resume_limit` schedule. The pending row arrives through `ChatAutoResumeContext`, which `AgentChatPane` fills from the chat's parked-until instant and scheduled-work list. |
| `apps/desktop/src/shared/chatAutoResume.ts` | Auto-resume after a provider usage limit resets. When a turn fails at a usage/rate limit **and** the provider published a reset instant (Claude's plan-quota snapshot, Codex's `account/rateLimits`), the chat service arms one durable scheduled-work row per chat at reset + a 90s buffer, prompting the agent to continue the interrupted task — except Claude chats with `autoContinueAtUsageLimit` left on (the default): those park in the session list (`usageLimitParkedUntil`) and let the SDK continue natively. ADE's own scheduled-work auto-resume is not armed for that Claude path. `sessionAutoContinueAtUsageLimit()` is the one opt-out predicate (explicit `false` only; missing/undefined is on). Don't continue persists that false, clears park, cancels any ADE schedule, and interrupts a busy Claude query with default Stop (`stop_and_clear`, which spares background). A user send while parked clears park without firing the OS "Chat resumed" notification; an ADE auto-resume `user_message` or an SDK `status: started` while still parked does notify. The row uses the deterministic id `auto-resume:<sessionId>` and the `source: "auto_resume_limit"` tag, so a repeat failure replaces it rather than stacking, and ADE's own cancel-on-activity sweep never touches a user- or agent-created schedule. No reset time means no schedule — the manual recovery card stays the only path. The row is cancelled when the user sends a message or retries the turn, when Don't continue runs, when the schedule is cancelled from Chat Info, and when the chat is archived, deleted, or disposed. Delivery is ordinary scheduled work: a turn already active at the due time defers the resume to the next turn boundary instead of double-sending. This module is the shared half: the id/tag grammar, the buffer, the prompt, the local-time label, `autoResumeFireAtMs` (which returns null for an unknown or already-passed reset), `isPendingAutoResumeScheduledWork` (which accepts both status vocabularies — the host record spells the finished state `done`, the client item `completed`), `formatUsageLimitResetLabel`, and `isUsageLimitChatError`, the one predicate the host arms from *and* the renderer's `classifyProviderFailure` delegates its `rate_limit` branch to, so the card and the schedule can never disagree about what counts as a usage limit (Codex spells it `usageLimitReached`, not `usageLimitExceeded`). |
...
| `apps/desktop/src/main/services/chat/chatAutoResumeCoordinator.ts` | The whole auto-resume subsystem on the host: per-chat streak state, the durable row, two system notices, and the races between arming, cancelling and firing. `agentChatService` keeps only thin call sites — it reports a limit error, a dispatch, a dismissal, and a turn finishing, and this decides what each means. Three contracts are load-bearing. **`cancelEpoch`**: arming is asynchronous (it waits for the scheduler, then upserts), so a user message can land while the upsert is in flight and find nothing to cancel; every cancel bumps the epoch and the arm undoes its own row if the epoch moved. **The re-arm cap** (`AUTO_RESUME_MAX_CONSECUTIVE_ARMS = 2`): the reset instant a provider publishes is not always the limit that rejected the turn — Claude's snapshot is session-scoped and can carry a stale `resetsAtMs` forward, so a weekly-limit rejection would arm against a session reset, fail again, and burn a turn every cycle. Two arms against *distinct* reset instants, then the chat emits `Auto-resume paused after two attempts` and waits for a human. A resume that runs without hitting the limit again resets the streak, which is why `noteResumeTurnStarted` / `noteTurnFinished` exist: a fired resume always carries `scheduledWake`, exactly what the cancel sweep skips, so without that watch the cap would be a lifetime budget and a healthy headless chat would pause itself after two limits days apart. **Notice ordering**: both notices are emitted after one microtask, because this runs inside the event commit before the error event has minted its sequence number and an inline emit would render the notice above the failure it explains. Notices are `system_notice` rows with `noticeKind: "rate_limit"`, attributed to the failed turn, de-duped one per armed fire time. |
...
| `apps/desktop/src/renderer/components/settings/providers/bodies/CodexBody.tsx` | Codex Settings card: list-only plugins via `listCodexPlugins`. |
...
snapshot is empty — arms nothing, and the manual recovery card stays the only
path.
...
Three rules are specific to it:
...
- **Two arms, then it stops.** The reset instant ADE can see is not always the
  ...
  posts `Auto-resume paused after two attempts` and waits for a human. A resume
  that runs *without* hitting the limit again proves the window was right and
  resets the streak, so a healthy headless chat that hits limits days apart
  never pauses itself.
- **It announces itself in the transcript.** Both outcomes are
  ...
  explain — and the live schedule with **Continue automatically** / **Don't continue** appears
  only on the newest usage-limit card, never on every one in the thread's
  history.
