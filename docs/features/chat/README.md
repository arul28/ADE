...
| `apps/desktop/src/shared/chatErrorPresentation.ts` | Host rewrite for failed turns. `presentChatFailure` turns a provider/SDK failure into `{ title, body, nextAction?, technicalDetail? }`. Sandbox-unsupported text becomes "Couldn't start this turn" with ADE-hooks copy; raw SDK text stays in `technicalDetail`. Clients render `errorInfo.presentation` and never title the row Error or Unknown. |
