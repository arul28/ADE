...
| `apps/desktop/src/renderer/components/usage/ActivityModule.tsx` | Tabbed activity/tokens/code/clients module. `AgentChatPane` mounts the self-fetching `WorkActivityModule` (compact variant) directly below an empty Work draft composer (desktop and web only); it reads `usage.getAdeStats` through the active `window.ade` adapter and persists the selected tab and day/week/month/year range locally under `ade.activity.module.v1`. |
