...
- Normal Developer ID releases leave Electron's Touch ID WebAuthn platform authenticator disabled because ADE does not ship the restricted `VQ372F39G6.com.ade.desktop.webauthn` keychain access-group entitlement without a provisioning profile. `ADE_ENABLE_TOUCH_ID_WEBAUTHN=1` is only for an explicitly provisioned build that also adds the matching entitlement; `ADE_ENABLE_TOUCH_ID_WEBAUTHN=0` keeps it disabled. The account-selection event always resolves exactly once and uses a native chooser when a site returns multiple discoverable credentials.
- When explicitly enabled in a provisioned build, Electron documents Touch ID credentials as device-bound to that Mac's Secure Enclave and scoped by Electron session metadata. Because ADE uses one global browser partition, the metadata is global across ADE projects in the same release channel, but the credentials are not iCloud Keychain-synced passkeys. See Electron's [`app.configureWebAuthn`](https://www.electronjs.org/docs/latest/api/app#appconfigureweb-authnoptions-macos) and [`select-webauthn-account`](https://www.electronjs.org/docs/latest/api/session#event-select-webauthn-account) contracts.
