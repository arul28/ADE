...
- **Answering a card whose asker is gone must still write a receipt.**
  ...
  card map, Claude's `canUseTool` waiters, Cursor's `permissionWaiters`, and
  Droid's — and each is reached when the click lands after the thing that raised
  the card is already finished (a double-click, an interrupted turn, a summary
  that over-claimed after a restart). All four route through
