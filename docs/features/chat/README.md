...
- **Work draft launches.** From an empty embedded Work composer, the
  ...
  drop loading/error state. The composer is cleared optimistically at
  job creation rather than after the async flow completes, so users can
  ...
  the composer), and Dismiss for terminal jobs. The
