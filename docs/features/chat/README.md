...
`MarkdownBlock` threads a `sceneLive` prop down to the frame. `CtoVoiceHudHost` passes `live` directly for a scene drawn during a voice call. In a transcript, liveness is scoped to the scene's **own turn** (`renderEvent`'s `turnActive`, which matches the row's turn id against the active one) — not to the session. Scoping it to the session meant every scene in the transcript came back to life, and re-ran its script, the moment the user scrolled to it during a later turn.
...
`window.ade` is installed inside the frame before the model's code runs: `ade.data`, `ade.theme`, `ade.reducedMotion`, `ade.on`, `ade.emit`, `ade.ready`, `ade.resize`, `ade.animate`, `ade.countUp`. It ships no third-party library and never will, for two reasons that both stand alone — the frame has `connect-src 'none'` so it could not fetch one even if a scene referenced it, and inlining one would put that library's licence inside every generated view. The Web Animations API and CSS animations are native to Chromium, cost nothing, and are what models reach for anyway.
