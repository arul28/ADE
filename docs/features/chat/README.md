...
| `apps/desktop/src/main/services/builtInBrowser/builtInBrowserService.ts` | Main-process broker for the in-app web browser. Uses the single persistent `persist:ade-browser` authentication profile for every remote-content tab and keys independent visible tab collections by ADE window plus project/personal collection, so cookies and site storage are global while tab strips remain separate. `builtInBrowserProfileMigration.ts` performs one bounded, idempotent migration of unexpired persistent cookies from this channel's legacy project partitions; global cookies win, session cookies are excluded, and legacy directories remain because DOM storage, IndexedDB, service-worker state, and WebAuthn credentials cannot be safely merged. `builtInBrowserStateStore.ts` restores bounded HTTP(S)/blank tab URLs and active-tab selection, but never agent ownership, lightweight sessions, or synthesized session cookies. Each window service manages multiple `WebContentsView` tabs (cap 10), active tab, per-tab lane/chat owner and lease metadata, bounds, visibility, inspect state, targeted status events, screenshot capture, diagnostics, per-tab action traces, and selected-page context. Global-session webRequest events are routed by webContents id to the owning collection. `builtInBrowserAuthentication.ts` adds a sandboxed local HTTP/proxy-auth prompt that never stores or logs the entered credentials and a native client-certificate chooser that accepts only Electron-offered certificates. Permission requests are deny-by-default, limited to managed web contents and secure origins, and resolved through persisted per-origin/embedding-origin decisions plus a native prompt. `builtInBrowserAgentAccess.ts` requires a non-persistent, chat-scoped human grant for every non-local origin and for local origins with allowed privileged permissions; agent-triggered cross-origin navigation and redirects are intercepted, and sensitive popups are blocked until approved. ADE-launched chats receive opaque in-memory browser capabilities bound to their trusted tab collection; unbound/elevated runtime callers are denied, caller-supplied scope is replaced, and the runtime-to-Electron bridge uses a separate verified in-memory credential. Agents cannot read global cookie-domain diagnostics, administer permissions, or see other agents' tab status. Human OAuth popups retain their real opener relationship, and downloads use sanitized unique filenames in Downloads. Scratch observations live under `.ade/cache/browser-observations/`; `ade browser proof` promotes one through the proof broker. Navigation normalization/protocol policy lives in `builtInBrowserNavigation.ts`; permission policy lives in `builtInBrowserPermissions.ts`. Backs the `ade.builtInBrowser.*` IPC surface and is consumed by both `ChatBuiltInBrowserPanel` and `openExternal.ts`. |
