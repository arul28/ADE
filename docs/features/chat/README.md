...
- **Expired access tokens keep the thread.** The Cursor SDK exchanges the user
  ...
  preamble, and no lost conversation. The raw error card is swallowed in the
  bridge. Recovery is silent when nothing had streamed yet (the original prompt
  is re-sent verbatim, logged as
