...
and carry a ready-to-render `message`. `chat.getSessionSummary` reports the
brain's IANA `timeZone` alongside the summary, the same way
`chat.createScheduledWork` does, so a CLI caller can render `fireAt` and
`nextWakeAt` in the zone the brain schedules in. Fork is the existing quota-card
`fork-local` action.
...
- **The tag scopes the sweep.** The row carries `source: "auto_resume_limit"`
  ...
  upserts the same row instead of stacking a second resume, and ADE's
  ...
  the resume armed, and a send carrying `scheduledWake` is exempt so a fired
  resume cannot cancel itself. That exemption is also why the renderer strips
