...
| `ChatPrPane.tsx` | Left floating PR peek for Work chat — not a second PRs tab. Title bar shows `#N` (T3) or a chip switcher when the chat has several linked PRs, plus ↻ refresh and ✕ close: ↻ calls `prs.syncLanePr` and then re-reads the pane's PR, and spins for either a manual sync or a backend reconcile-on-focus (`pr-reconcile`, debounced 300 ms on the hide so a fast reconcile does not flicker). ✕ is wired to the parent's `onClose` (the header PR pill still toggles it). Scopes rows through `selectPrsForChat` (explicit edges first, unedged current-branch fallback, dismissed unlinks stay gone). The body stays a peek: status, review, three churn-ranked files, a stack-offer banner to link unclaimed GitHub stack siblings, and **Open files on PRs tab** / GitHub / copy. Merge and rebase live on the PRs tab inspector, not here. Link another / unlink write the hybrid edge + dismissal tombstone. Shows cached details immediately, then refreshes the linked PR row with the same targeted refresh path so pane toggles surface current merged/closed/check state without a broad PR sync. An unmapped lane PR (projection-derived, `pr.unmapped`) skips the refresh and checks/reviews enrichment — there is no DB row behind its synthetic `gh:` id. With no PR it embeds `ChatPrInlineCreator` and forwards the chat's `sessionTitle` plus `sessionId`; under a `runtimePin` it points at the owning machine instead, since creation is not pinned. Reads, the ↻ sync, and the event subscription all take the pin so a chat on another machine sees its lane's real PR. |
