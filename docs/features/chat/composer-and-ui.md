...
| `ProviderFailureRecoveryCard.tsx` | Friendly recovery surface for terminal provider capacity and usage-limit failures. Shows human-readable error identity and guidance, then offers **Retry turn** and **Choose model** only after the failed turn has released the composer. `classifyProviderFailure` delegates its `rate_limit` branch to the shared `isUsageLimitChatError`, so the card recognises exactly the set of shapes the host arms an auto-resume for. On a usage-limit failure with a known reset time it also reports the armed auto-resume (`Auto-resume scheduled for <local time>`) and a **Cancel auto-resume** button, read from `ChatAutoResumeContext` so only this card re-renders when the schedule appears or is cancelled. The context is **per chat**, so a card renders the schedule only when its own `providerFailureEventId` matches the context's `anchorEventId` — otherwise every usage-limit failure ever written to this transcript, including ones from days ago, would advertise the one armed row. That id is `<envelope timestamp>::<turnId>::<message>`: error events carry no id, `turnId` is optional on this path, and a usage limit produces the same message every time, so the timestamp is what separates two of them. |
...
| `apps/desktop/src/renderer/services/audioCaptureRecorder.ts`, `audioCaptureRequestHandler.ts`, `apps/desktop/src/renderer/components/audio/*`, `apps/desktop/src/main/services/audio/microphoneAccess.ts` | Desktop capture engine, request handler, and permission boundary. The module-level recorder owns mic capture across navigation and writes live state to the root app store; the request handler (installed in `main.tsx`, outside React) serves capture requests main routes to this window and always settles them. `GlobalAudioCaptureIndicator` in the TopBar is the ONLY capture UI — a plugin can ask from anywhere, so the controls live in the always-mounted chrome — and the pill always names the requesting plugin. Main asks Electron for the platform microphone status; Windows denied/restricted states return explicit Privacy & security > Microphone guidance, while unknown/not-determined states are left to Chromium's origin permission flow. |
| `apps/desktop/src/main/services/audio/*` | Generic audio capture. ADE records clips on a plugin's behalf and never interprets them — there is no speech model, transcriber, or glossary in core. `audioCaptureService.ts` writes captured mono PCM to a WAV under `<ADE home>/tmp/audio-captures` and reports `{audioPath, durationMs}`; it refuses to delete any path it did not mint. `audioCaptureBroker.ts` turns the one-way main→renderer request and renderer→main reply into one promise, refusing with `audio_capture_busy` / `audio_capture_mic_unavailable` and failing an in-flight capture if its window closes. `legacyAudioArtifacts.ts` wipes the pre-plugin `<userData>/whisper` model on every start and sweeps stale clips by age. |
...
Browser, Electron Control, and iOS panels also fail closed when their local-only
...
- **Model selection.** `ProviderModelSelector` is embedded and filters
  ...
  **Everything a prompt box offers is scoped to the machine set in that
  ...
  that in-process registry. Tools / git / iOS / Electron Control still
...
- **Audio capture.** ADE has no dictation of its own; a plugin that wants the
  microphone calls `ade.audio.captureClip` (see the plugins doc) and ADE
  records on its behalf. There is no mic button in the composer — a plugin
  contributes one as an ordinary `composer-action`, and the capture UI itself
  is the always-mounted pill in the TopBar, because a plugin can ask from
  anywhere. Capture is owned by the app-global `audioCaptureRecorder`, so
  recording survives composer unmounts and tab/pane navigation; it down-samples
  mic input to 16 kHz mono PCM and hands it to main, which writes a WAV and
  returns its path. The pill always names the requesting plugin. macOS
  permission requests use `systemPreferences.askForMediaAccess`; Windows reads
  Electron's media access status and blocks only explicit `denied` /
  `restricted` results, with guidance pointing at **Settings > Privacy &
  security > Microphone** rather than macOS System Settings copy.
...
- **Overflow control.** Issue context, orchestrator mode, parallel models,
  and the iOS Simulator / Electron Control drawer toggles are folded behind one
...
A turn reports its file changes exactly once, at its done divider. Which
