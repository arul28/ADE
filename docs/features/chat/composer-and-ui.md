...
For a locally opened Work project, the **Handoff** tab starts with
**Continue on another machine**. **Send to machine** opens a staged modal that
checks source publication, lets the user choose an eligible connected runtime
and optional continuation note, explains clone/storage/model/route failures,
and shows exactly what the bounded capsule includes and excludes before final
confirmation. Remote-bound source tabs do not offer this action: the source
project must be opened on the machine that owns its lane and chat. The complete
Git, capsule, idempotency, and security rules live in
[Cross-machine session handoff](../sync-and-multi-device/cross-machine-session-handoff.md).
