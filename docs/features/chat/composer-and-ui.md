...
Choosing **Continue on another machine** opens the **Send to machine** staged
modal that checks source publication, lets the user choose an eligible connected
runtime, brief or fork mode, and an optional continuation note, explains
clone/storage/model/route failures, and shows exactly what the capsule includes
and excludes before final confirmation.
