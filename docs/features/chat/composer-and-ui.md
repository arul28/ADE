...
| `apps/desktop/src/shared/chatScheduledWork.ts` | Pure scheduled-work derivation. Folds `scheduled_work_update` envelopes into Chat Info schedule rows for Claude wakeups, cron tasks, `/loop`, remote triggers, and background work; partitions active rows from fired one-shot history; and formats next-fire labels. Shared by desktop, ADE Code, and iOS. |
...
| `ChatSubagentsPanel.tsx`, `ChatSubagentStrip.tsx` | Chat Info panels. They render the Codex goal card, latest plan, tasks, schedule, and subagent/background rosters. The Schedule header owns the per-chat pause/play action; active rows dim and read `paused` while paused, cron rows combine last-run and next-fire timing, and fired one-shot wakeups live in a collapsed History group. For Codex sessions the goal card stays above plan/subagent progress so the current objective stays visible without crowding the chat header. |
...
Completed one-shot wakeups leave the active list and appear in a collapsed
`History (N)` group with their fired time and optional `late` marker.
