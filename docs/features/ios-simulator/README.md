...
| `apps/desktop/src/renderer/components/chat/H264VideoCanvas.tsx` | Decodes the `idb-h264` stream with WebCodecs and draws it to a canvas. The canvas holds the device screen and no window chrome, so a click maps straight to a device point. |
