...
- [**ADE SDK**](./features/sdk/README.md) — Embeddable chat sidecar (`@ade-dev/sdk`, `@ade-dev/chat-ui`) that spawns an isolated ADE runtime as a guest of a third-party app. Strict MCP isolation is Claude-only as a guarantee; `mcpCapability` is the honesty report.
...
- [**iOS Simulator**](./features/ios-simulator/README.md) — macOS-only control of the system iOS Simulator from `ade ios-sim` and the Work tools pane. Agents launch in the background from the lane worktree; the drawer is an optional live view. ADEInspector publishes per-frame SwiftUI element metadata so taps become source-anchored chat context.
