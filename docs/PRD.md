...
- [**Apple device**](./features/apple-device/README.md) — macOS-only control of a per-lane iOS simulator from `ade apple` and the Work tab Apple column. A vendored helper streams the device framebuffer (no Screen Recording grant, no Simulator window). Agents launch in the background from the lane worktree. Beyond launching: clone-or-attach devices, `simctl`-backed device tools, recordings with an auto-record contract, element queries, proof bundles, and ADEInspector source-anchored chat context.
