...
- [**Mac Desktop**](./features/mac-desktop/README.md) — A private macOS screen per lane. ADE creates a virtual display, parks the lane's windows on it, and drives them through the Accessibility API, so the user's real display and pointer stay untouched. macOS-only on the runtime host; any client can watch a Mac-hosted lane read-only. Driven by `ade mac-desktop` (`ade desktop` remains the app launcher) and a bundled native driver; global pointer/keyboard input sits behind a per-chat lease, and screenshots/recordings reach the proof drawer through the existing computer-use broker as the `ade-mac-desktop` backend.
- [**iOS Simulator**](./features/ios-simulator/README.md) — macOS-only control of the system iOS Simulator from `ade ios-sim` and the Work tools pane. Agents launch in the background from the lane worktree; the drawer is an optional live view, captured from the Simulator window locally or encoded on the machine that owns the device so a remote Mac can be watched. Beyond launching: app-free device sessions, `simctl`-backed device tools (appearance, Dynamic Type, accessibility, location, permissions, push, status bar, app state), an interleaved device/ADE event log, element queries that name a control instead of a coordinate, and proof bundles. ADEInspector publishes per-frame SwiftUI element metadata so taps become source-anchored chat context.
