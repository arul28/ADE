...
`ade session` is the sibling family for *filing* a session rather than talking to it: `ade session show <id> --text` prints settle/snooze state and the wake reason, plus what the session is actually doing: its canonical status word and elapsed (**Working 4m**, **Background work ×2 2h**, **Needs you**) from the same shared presentation module the Work rows and `ade code` use, how much background work is live, and the agent SDK processes the session currently owns with their ages. `runtime state` alone reads `idle` for a chat holding a warm agent process and two background jobs open, which is the state people were dropping to `ps` to diagnose, `ade session snooze <id> --for 1h` (or `--until <iso>`) parks a row, `ade session wake <id> [--reason timer|needs_you|error|turn_complete|manual]` brings it back, and `ade session clear-woke <id>` drops the "woke early" marker after visiting the row. There is no `ade session settle` / `unsettle` (or `ade chat settle` / `unsettle`): those were removed in 2026-07 and now fail with an explanation pointing at the two remaining paths — the user settling the row in ADE, and the `autoSettleLaneSessionsOnPrMerge` policy. Nothing derives a settle either; a clean process exit leaves a row `ended`, never `settled`. Every subcommand takes the session id as a positional, accepts `--session <id>`, and falls back to `$ADE_CHAT_SESSION_ID`, so a bound agent can file itself. Duration grammar and the 30-day cap live in `apps/ade-cli/src/sessionSnoozeDuration.ts` and are shared verbatim with `ade code`'s `/session …` commands.
