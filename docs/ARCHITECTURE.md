...
Application-wide shell state also crosses this boundary. The renderer derives
the loud session-attention count from the canonical session state and calls
`window.ade.app.setDockBadgeCount`; the main process validates and normalizes
that count before forwarding it to Electron's `app.setBadgeCount`. Quiet ready,
idle, stale, ended, and settled rows never contribute to the Dock badge.
