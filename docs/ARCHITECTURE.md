...
- Off-LAN transport: a default-on **cloud tunnel relay** (`apps/tunnel-relay`, §2.7) advertised as the lowest-priority `relay` address candidate. Direct LAN/Tailscale routes remain preferred. Settings > Sync and `ade sync relay disable` are the explicit kill-switches; a deliberate off is persisted with `enabledSetByUser` and survives upgrades.
