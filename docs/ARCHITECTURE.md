...
**Service managers.** `apps/ade-cli/src/serviceManager/installLaunchd.ts` (macOS), `installSystemd.ts` (Linux), `installWindows.ts` (Windows) register the brain as a login-time service. `index.ts` is the platform router; `common.ts` carries shared types (`ServiceManagerResult`, `ServiceManagerStatusResult`). Windows uses a per-user, per-channel `ONLOGON` Scheduled Task whose name is a stable hash of service identity and account name. Installation writes a UTF-8-with-BOM PowerShell launcher under the channel's ADE home; the launcher restores the complete resolved brain environment (including `ELECTRON_RUN_AS_NODE`, `NODE_PATH`, ADE paths/channel, and role) and starts the executable with CRT-compatible argv quoting. Status uses `Get-ScheduledTask` objects instead of localized `schtasks /FO LIST` text. Install/uninstall also removes the exact legacy `ADE Runtime` task and fails closed when the old task cannot be queried or removed.
...
**Bundled runtime artifacts.** Per-platform `ade-<platform-arch>` binaries plus their native dep tarballs live under `apps/desktop/resources/runtime/`, with packaged ADE CLI resources providing the `ptyHostWorker.cjs` used by remote terminals. `release-core.yml` builds the cross-platform set, validates that every darwin/linux arm64/x64 runtime binary and native archive is present, and publishes those runtime assets plus `install.sh` and `SHA256SUMS` on the GitHub release. Windows desktop packages include all four Darwin/Linux sidecar pairs, not just the host's Windows CLI resources, so a Windows client can bootstrap the supported macOS/Linux SSH runtimes. `bootstrapRemoteRuntime` uploads missing or hash-mismatched artifacts on first SSH connect from the desktop client; Windows as an SSH-bootstrap runtime target is still out of scope.
...
routing hides iOS Simulator and the native Attention Notch outside macOS;
...
3. **Windows package proof** (`package-win`) — a `windows-latest` runner downloads every Darwin/Linux runtime sidecar, runs the release-contract test, builds an unsigned x64 NSIS preview, performs packaged CLI/TUI/PTY/provider/CR-SQLite/updater-authority smoke validation, and uploads the installer, blockmap, and `latest.yml` for 14 days.
4. **Gate** (`ci-pass`) — all required jobs, including `package-win`, must pass (`if: always()` with failure/cancelled detection).
...
- `npm run dist:win:signed` — production path. The wrapper requires signing credentials and enables electron-builder's fail-closed code-signing mode; post-build validation requires a trusted RFC3161 timestamp, the protected expected subject or thumbprint, and the same signer on the NSIS installer and packaged `ADE.exe`.
...
- `release-core.yml` runs the signed Windows job only when repository variable `ADE_WINDOWS_SIGNED_BUILD_ENABLED=1`. If enabled, missing signing credentials or pinned publisher identity fail the Windows job and block publication; if disabled, the skipped Windows job does not block the macOS release. Windows assets are added to the draft release only when `ADE_WINDOWS_PUBLIC_RELEASE_ENABLED=1` as well. Keep that second gate off until the signed installer passes the clean-host release checks.
