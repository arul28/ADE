...
| `ios/` | `iosSimulatorService.ts`, `simulatorWindowCapture.ts` | macOS-only iOS Simulator backend: tool readiness probes, simctl device + app discovery, build/install/launch with progress events (hardened with `simctl bootstatus` and `simctl install` timeouts), screenshot + ADEInspector + accessibility hit-test, Simulator.app window live-view status, idb-backed input, and single-owner chat session locking. Window placement, parking, Screen Recording / Automation probes, and System Settings openers live in `simulatorWindowCapture.ts`. `registerIpc.ts` keeps the handlers because they need the active `BrowserWindow`. See [features/ios-simulator/README.md](./features/ios-simulator/README.md). |
