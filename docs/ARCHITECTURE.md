...
`AppShell`, presents it first through the global-header Activity popover, and
uses `/activity` for the larger two-column Activity pane. The popover and pane
share two buckets: Sessions (Needs you, Working, and Done) and Inbox (PR/CI and
other outcomes); the pane adds filters and item detail, while ADE Notch shows
the same stream as a compact native toast/surface.
...
`AttentionItem` wire contract in the Activity right pane, and opens an item's exact ADE
