...
| `macosVm/` | `macosVmService.ts` | Apple silicon macOS VM lifecycle: Swift helper (`VZMacOSRestoreImage` / `VZMacOSInstaller` / VirtioFS / ScreenCaptureKit) drives install, boot, focus, screenshot, click, type; service manages bases (snapshots) and per-lane VMs cloned via APFS clonefile copy-on-write. Wizard-driven onboarding, cockpit + drawers for steady state, cross-lane + global concurrency overlays. See [features/terminals-and-sessions/macos-vm.md](./features/terminals-and-sessions/macos-vm.md). |
