...
- **No config arrives from a repository**: `.ade/ade.yaml` is not read. A legacy committed file is carried over into `local.yaml` once — non-executable keys only, dropping `testSuites`, `laneOverlayPolicies`, `automations`, `laneEnvInit`, `laneTemplates`, `defaultLaneTemplate` — and then deleted. A clone therefore cannot supply a `command`, a `setupScript`, or an agent prompt, which is why the old SHA-256 trust gate could be retired rather than replaced.
