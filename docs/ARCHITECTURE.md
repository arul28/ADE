...
- Push: APNs alert pushes (deep-linked), Android FCM data notifications, and
  Live Activity updates via `apps/push-relay` (§2.7). Signed-in iOS registers account APNs and
  push-to-start tokens directly with the authenticated Attention relay;
  signed-in Android registers FCM directly with the same ownership-epoch fence;
...
Related sync docs: [Sync and multi-device](./features/sync-and-multi-device/README.md), [iOS companion](./features/sync-and-multi-device/ios-companion.md), [Android companion](./features/android-companion/README.md), and [Remote commands](./features/sync-and-multi-device/remote-commands.md).
...
│   ├── android/        # Native Compose thin client (invalidation-only WebSocket sync)
│   ├── web/            # Marketing + download landing (Vite + React)
│   ├── push-relay/     # Cloudflare Worker + D1: APNs/FCM push + Live Activity relay
...
| `apps/android` | Gradle project; `:sync:test`, `:app:lintDebug`, `:app:assembleDebug`, `:app:bundleRelease`. |
...
2. **Parallel checks**:
   ...
   - `test-android` — JVM sync tests plus Android lint and debug assembly on SDK 36.
