...
| `git/` | `git.ts`, `gitOperationsService.ts`, `gitConflictState.ts` | Low-level git runner, high-level lane-scoped ops, conflict state queries. `gitOperationsService.getSyncStatuses` batches lane-id keyed sync reads for the workspace graph while preserving the single-lane `getSyncStatus` response shape. |
