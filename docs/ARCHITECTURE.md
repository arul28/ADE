...
| `builtInBrowser/` | `builtInBrowserService.ts`, `builtInBrowserNavigation.ts`, `builtInBrowserPermissions.ts` | In-app web browser owned by the main process. Every remote-content `WebContentsView` uses the single persistent `persist:ade-browser` storage partition, while service keys combine the ADE window id with a project/window/personal tab-collection key so visible tabs stay independent. Project roots route commands and scratch observations; they never partition cookies or site storage. The service caps each collection at 10 tabs, positions views over renderer bounds, drives navigation, uses CDP for inspection/background input, captures screenshots, saves sanitized unique downloads, and emits targeted events. Tabs carry owner/lease metadata, and the runtime injects the bound agent's lane/chat identity into browser reads and actions; bound agents cannot force or impersonate a takeover. Browser sessions bind one workflow to one tab and observations live under `.ade/cache/browser-observations/`. Desktop bridge calls carry the runtime project root only to reach the matching window/project collection. Navigation/protocol policy lives in `builtInBrowserNavigation.ts`; permission policy lives in `builtInBrowserPermissions.ts`; WebAuthn account selection lives in `builtInBrowserWebAuthn.ts`. |
