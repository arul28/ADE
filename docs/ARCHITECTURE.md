...
| `account/` | Required ADE account authentication. The machine brain owns loopback OAuth, the account-directory device-code bridge for SSH/display-less hosts, `account.session.v1` refresh storage, and in-memory `ADE_ACCOUNT_TOKEN` credentials for agents and CI. Interactive and token-provisioning actions are CTO-only, including `repairMachinePairing`, which re-pairs this machine after it was removed from the account and lifts the durable push gate only once the directory accepts. The device-code flow also carries the machine key so the directory can mint the single-use, 10-minute pairing grant that re-pair requires, and the machine's display name so the browser page can name the computer that asked. `deleteMachine` is stricter than CTO-only: it refuses any caller whose identity names a chat, run, step or attempt, and it requires `confirmation: "REMOVE"`. `thisMachineRefusalText.ts` gives the CLI's words for a directory refusal of this computer (removal date, give-up, `ade machines reconnect`), which `ade auth status`, `ade sync status --text` and the `ade doctor` Publish health row show. |
...
| `runtime/adeCliShim.ts` | Body of the `ADE_CLI_PATH` shim a brain writes for the agents it launches. The shim names its brain: when the caller's env sets neither `ADE_HOME` nor `ADE_RUNTIME_SOCKET_PATH`, it sets both to that brain's values. See [features/agents/tool-registration.md](./features/agents/tool-registration.md). |
| `proof/adeCaptureRegistry.ts` | The files that the RPC server's capture actions wrote, by path and hash. An `ingest_computer_use_artifacts` call is filed as ADE's capture only when each input matches an entry. Each match is one-shot. See [features/proof.md](./features/proof.md#already-filed-bytes-and-older-videos). |
| `workTools/workToolShowRequests.ts` | `ade ui show <surface>` (and `ade apple show`) on the brain side. The brain publishes a `work_tool_show_request` event on the runtime event stream. The desktop renderer that shows that chat opens the surface (`apple`, `floating-apple`, `browser`, `proof`) and answers `shown` or `held`. No answer in the wait window gives `no_desktop`. |

Outside `services/`, `src/lib/cliDelegation.ts` (loaded first by `cliDelegationEntry.ts`) hands an `ade` run to `ADE_CLI_PATH` when that path runs a different CLI entry. A shell rc that rebuilds `PATH` can hide ADE's shim and leave an older `ade` first on `PATH`; delegation re-runs the same argv through the CLI that ADE meant. It never delegates from a source-checkout build, and `ADE_CLI_DELEGATED=1` (set on the child) and `ADE_CLI_NO_DELEGATE=1` turn it off. `src/lib/cliGlobalArgs.ts` lists the global flags that take a value, so the delegation check and the CLI skip the same tokens. `src/help/` holds subcommand help text (`appleHelp.ts` for `ade apple`) and the shared `banner.ts`.
...
(`ADE_LOOP_WATCHDOG_MS`, default 30 s) without a matching sleep/suspend signal,
the worker atomically writes an `event-loop-wedge.json` breadcrumb (the wedged
command, blocked duration, threshold, and the latest event-loop/memory/resource
...
`readProcessStartTimeMs` (`main/services/processes/processStartTime.ts`, one
...
already owns the restart.

Staleness alone was still not enough, because sleep suspends the watcher and the
brain together and leaves a beat that is legitimately 40 minutes old. Two more
facts are required, and `evaluateBrainHeartbeat` names each shortfall as its own
verdict. **The watcher must have been awake to watch the beat go stale**: it
records its own last run in `<ADE home>/runtime/watchdog-check.json` and
compares that gap against `brainWatcherSuspendFloorMs` (`max(staleAfterMs, 3 ×
the check interval)`) — a gap that large with a beat age that fits inside it is
`machine_slept`, and nothing dies. **And a previous check must already have seen
this exact beat**, matched on the beat's timestamp rather than its age; until
then the verdict is `stale_unconfirmed`. A brain the OS merely descheduled beats
again before the next check; a wedged one is still on the same timestamp a
cadence later. The Windows supervisor applies the identical three rules inline.
The check never
...
**Session identity.** The runtime resolves caller role from ADE context env vars and command flags. Role vocabulary: `cto`, `agent`, `external`, `evaluator`. `ADE_DEFAULT_ROLE` is an authority ceiling, not an identity grant: `resolveSessionBoundRole` clamps a chat-bound caller that would otherwise inherit a daemon-wide `cto` role to `agent`, preserves an explicitly declared lower role, and never accepts a requested role above the runtime ceiling. SDK-backed chats receive `ADE_CHAT_SESSION_ID` plus `ADE_DEFAULT_ROLE=agent`, and tracked provider CLI launch/resume does the same. Persistent SDK guidance names the concrete `--session <id>` for lifecycle commands so shared provider servers do not depend on process-global env inheritance. Browser automation adds a separate bearer capability: ADE-launched chat and owned-terminal environments receive an opaque `ADE_BROWSER_ACTOR_TOKEN` bound in Electron memory to that chat's trusted lane/project or personal tab collection. The runtime requires the token, strips caller-supplied routing, and carries it only over the authenticated desktop bridge. Electron both mints and validates it — the daemon requests the token over the authenticated desktop bridge when it builds the agent environment, because the registry lives only in Electron memory — before restoring the bound scope; role alone never grants access to a human-authenticated browser profile.

**Trusted agent provenance on chat messages.** The three chat actions whose caller metadata is persisted onto a `user_message` — `chat.messageSession`, `chat.sendMessage`, `chat.steer` — pass through `withTrustedAgentProvenance` in `adeRpcServer.ts` before any role- or scope-specific branch. It deletes every host-authored provenance key (the list is exported from `apps/desktop/src/main/services/chat/spawnMissionOwnership.ts`), then re-derives `spawnDispatch` when the caller is the target's persisted direct parent, or `agentRelay` when any other bound agent is the caller. Positional invocation of those three actions is rejected outright — they take object arguments (`--input-json`) — so metadata cannot route around the re-derivation, and the automation service strips the same keys before dispatching a `chat` action from rule config. This provenance decides whether a spawned child's turn completion wakes its parent and for how long the parent keeps that claim, so no caller, elevated or not, may supply it. See [features/chat/README.md](./features/chat/README.md#mission-ownership-decides-the-wake).
...
`ade session` is the sibling family for *filing* a session rather than talking to it: `ade session show <id> --text` prints settle/snooze state and the wake reason, plus what the session is actually doing — a `status` line carrying the canonical status word, its live count, and its elapsed (**Working 4m**, **Background work ×2 2h**, **Needs you**) from the same `sessionStatusPresentation` / `sessionElapsedLabel` helpers the Work rows and `ade code` use, and an `agent processes` line listing the agent SDK processes the session currently owns with their ages (in-memory and host-local, so empty after a restart). Those two exist because `runtime state` alone reads `idle` for a chat holding a warm agent process and two background jobs open, which is the state people were dropping to `ps` to diagnose. `ade session move <id> --to needs-you|working|done` performs the same Work-board move the desktop drag does (both the on-screen `needs-you` and the wire `needs_you` are accepted; `waiting` is refused with the reason it is derived), `ade session snooze <id> --for 1h` (or `--until <iso>`) parks a row, `ade session wake <id> [--reason timer|needs_you|error|turn_complete|manual]` brings it back, and `ade session clear-woke <id>` drops the "woke early" marker after visiting the row. There is no `ade session settle` / `unsettle` (or `ade chat settle` / `unsettle`): those were removed in 2026-07 and now fail with an explanation pointing at the two remaining paths — the user settling the row in ADE, and the `autoSettleLaneSessionsOnPrMerge` policy. Nothing derives a settle either; a clean process exit leaves a row `ended`, never `settled`. Every subcommand takes the session id as a positional, accepts `--session <id>`, and falls back to `$ADE_CHAT_SESSION_ID`, so a bound agent can file itself with the subcommands it is authorized for — which does not include the board move. `session.moveOnBoard` / `undoBoardMove` sit in `actionPolicy`'s CTO-only `session.only` list next to the settle family, for the same reason: a move writes the same lifecycle columns a settle does and then tells the agent the user moved it, so a session-bound agent authenticating as `agent` is refused whether it names its own session or another. The id fallback is what resolves the target; the role is what decides. Snooze, wake, clear-woke, and show stay open to a bound agent filing itself. Duration grammar and the 30-day cap live in `apps/ade-cli/src/sessionSnoozeDuration.ts` and are shared verbatim with `ade code`'s `/session …` commands.
...
**Bundled runtime artifacts.** Per-platform `ade-<platform-arch>` binaries plus their native dep tarballs live under `apps/desktop/resources/runtime/`, with packaged ADE CLI resources providing the `ptyHostWorker.cjs` used by remote terminals. `release-core.yml` builds and validates the darwin/linux arm64/x64 pairs plus `ade-win32-x64.exe` and `ade-win32-x64.native.tar.gz`. A desktop package bundles only its own build target's sidecar pair — `apps/desktop/scripts/runtime-resource-targets.cjs` is the single source of truth, `ADE_RUNTIME_TARGET` pins each packaging job to one target, and a pre-build gate, the pre-package validator, and the electron-builder `afterPack` hook all fail on any other target's payload. Bundling all five is what pushed the macOS update ZIP past the 1 GiB Squirrel.Mac limit. Cross-platform bootstrap still works: `remoteSidecarCache.ts` fetches the missing `ade-<target>` pair from the desktop's *exact* release tag, verifies it against that release's `SHA256SUMS`, and caches it under `<ADE home>/runtime-sidecars/<tag>/<target>/`, so macOS and Windows clients can bootstrap macOS, Linux, and Windows 10 22H2/Windows 11 x64 machines over SSH. Windows bootstrap requires Windows OpenSSH Server, PowerShell 5.1 or newer, and the built-in `tar.exe`; WSL, ARM64, and Windows Server are excluded from Windows v1. The standalone Windows runtime, `install.ps1`, and its checksums are published: v1.2.52 shipped the first signed Windows installer and the website download gate (`VITE_ADE_WINDOWS_DOWNLOAD_ENABLED`) is retired. `ADE_WINDOWS_PUBLIC_RELEASE_ENABLED` remains the live workflow-side publication gate. `bootstrapRemoteRuntime` uploads missing or hash-mismatched artifacts over SFTP, verifies their SHA-256 digests remotely, and starts the channel-separated runtime on first SSH connect.
...
the bare app version (`1.2.74`, from the desktop package version or the update
feed), so `releaseAssetUrl` in `apps/ade-cli/src/lib/releaseAssets.ts` maps a
bare semver onto its tag through `releaseTagForVersion` before building the
download URL; an already-tagged value and the `latest` alias pass through
unchanged. Without that mapping every asset URL built from a bare version points
at a release that does not exist, and the remote **Update & restart** fails on a
404 rather than on anything the user could act on. The bare version reaches it
from the desktop's Connections pane, which sends the newest version it knows
through `machine.updateAndRestart` to the target machine's `ade brain update`.
The desktop's remote sidecar cache also builds its URLs through
`releaseAssetUrl`, but tags the version itself first via
`releaseTagForAppVersion`, so the mapping is a no-op on that path.
...
**Diagnostic report (`ade report-issue [--open] [--send]`).** `apps/ade-cli/src/commands/reportIssue.ts` prints the same redacted Markdown report the desktop's **Report issue** button produces, and `--open` copies the report to the system clipboard and then opens the prefilled GitHub issue in the browser (the template asks the user to paste the report, which is far too big for a query string; `--json` reports whether the copy succeeded as `copied`) (`lib/externalLinks.ts`, which allows `http(s)`/`mailto:` for general URLs and a separate editor-SSH allowlist for `vscode://vscode-remote/ssh-remote+…` / `zed://ssh/…` via `openEditorExternalUrl`, and falls back to Electron's `shell.openExternal` when the OS opener is unavailable). It reads local files only — it never starts or contacts the brain — so it still works on the machine where ADE will not come up, and on a headless or Windows host with no desktop error screen to press. The builder, the redactor, and the machine source collector (`services/diagnostics/diagnosticReport.ts`, `diagnosticSources.ts`) are shared with the desktop, so a log added for one appears in both. `--send` — and the desktop's **Send to ADE** action next to the same button — posts that identical redacted document to `POST /diagnostics/upload` on the account directory Worker and shows a short reference id, so a user who cannot be walked through a terminal still gets the report to us in one action; both go through the single client in `apps/desktop/src/shared/diagnosticsUpload.ts`, with the CLI attaching the machine's account token when it has one and the desktop renderer (which cannot reach the credential store) uploading anonymously. The upload is capped at 512 KB, limited to five a day per user-or-IP, and stored in R2 without ever being echoed back. See [features/storage-and-recovery/README.md](./features/storage-and-recovery/README.md#diagnostic-reports-report-issue).

**GitHub App authorization (`ade github app-auth login | status | clear`).** The device flow, the stored credential, and its status read, from a terminal. `status` answers exactly one question — re-authorize now, or wait — and only `credentialState` answers it, so that is what the `--text` rendering leads with. An access token whose 8-hour life has lapsed while `credentialState` is `authorized` is healthy: it renews on use. `blocked` means ADE paused its own retries until the reported retry time and re-authorizing cannot help; `needs_reauth` and `missing` are the only states that call for `login`. The same rule binds every surface, which is why the desktop panel and the banners read the field rather than the expiry — see [automations → How ADE stops a GitHub App refresh storm](./features/automations/README.md#how-ade-stops-a-github-app-refresh-storm).
...
The endpoint path is resolved by `apps/ade-cli/src/services/projects/machineLayout.ts`: `<adeHome>/sock/desktop-bridge.sock` on macOS / Linux (e.g. `~/.ade/sock/desktop-bridge.sock` stable, `~/.ade-beta/sock/desktop-bridge.sock` beta), and `\\.\pipe\ade-desktop-bridge-<channel>-<hash>` on Windows. The hash uses the same canonical ADE-home/channel/current-user identity as the runtime pipe. `ADE_DESKTOP_BRIDGE_SOCKET_PATH` overrides it for dev launches against a non-default ADE home. The server lives in `apps/desktop/src/main/services/builtInBrowser/desktopBridgeServer.ts`, wired up from `main.ts` right after `builtInBrowserService` is constructed and torn down with it on app shutdown. The runtime-side proxy is `apps/ade-cli/src/services/builtInBrowser/desktopBridgeClient.ts`; `createAdeRuntime` in `bootstrap.ts` assigns it to `runtime.builtInBrowserService` so the existing action registry slot resolves transparently (skipped when `runtimeProfile === "chat"`). Both sides share the same method allowlist (`BUILT_IN_BROWSER_DESKTOP_BRIDGE_METHODS` in `apps/ade-cli/src/services/builtInBrowser/desktopBridgeMethods.ts` — regenerate this line from there): `getStatus, requestOriginAccess, claim, startHandoff, waitForHandoff, startSession, listSessions, endSession, showPanel, setBounds, navigate, createTab, switchTab, closeTab, reload, goBack, goForward, stop, observe, getTrace, click, typeText, dispatchKey, scroll, fill, clear, wait, startInspect, stopInspect, captureScreenshot, selectPoint, selectCurrent, clearSelection, setEmulation, setZoom, findInPage, stopFindInPage, setDevTools, setNetworkLogging, getNetworkLog, exportHar, hover, drag, selectOption, uploadFile, startRecording, stopRecording`. Profile diagnostics and permission administration are deliberately absent, and so is `endHandoff` — hand-back is the human pressing a button in their own renderer, which reaches the service through local IPC and never through the bridge. Two methods are served by the bridge itself rather than dispatched onto `BuiltInBrowserService`: `issueActorCapability` / `revokeActorCapability` (capability lifecycle) and `getStatusForRuntime`, a read-only tab-list projection for the Work-tools pane that carries no cookies, no observation bytes and no way to act on a tab. All three are gated on bridge authentication alone, because the runtime daemon that calls them has no chat and so can never hold an actor capability. Only the two capability-lifecycle methods are exempt from the client's runtime-scope rewrite (they carry their own scope in the request body); `getStatusForRuntime` carries the calling daemon's `projectRoot` and is answered out of that project's window collection, not the frontmost window's — the socket is machine-wide but the daemon asking is project-scoped, so an unscoped answer would show one project's tabs on another project's phone. No window open for that project yields `browserUnavailable: "desktop_not_attached_for_project"`; a window that has it open but has never used its Browser pane yields `"browser_pane_not_opened"` instead, because the read never materializes a collection and the two states carry opposite instructions.

The socket is not an authority by itself. Electron mints a rotating 256-bit bridge token in memory for each desktop launch, sends it only through the trusted local desktop client's `ade/initialize` request, and requires it on every bridge request using a timing-safe comparison. The token is held by the runtime proxy and is not placed in agent environments. Independently, `builtInBrowserActorCapabilities.ts` issues each opaque per-chat actor capability in Electron; the runtime daemon, a separate process, obtains and revokes them through `built_in_browser.issueActorCapability` / `built_in_browser.revokeActorCapability` on the bridge rather than minting tokens nothing could validate. `adeRpcServer.ts` requires the connecting CLI's token, strips caller-supplied lane/project/personal routing, and carries it through the authenticated bridge. `desktopBridgeServer.ts` validates the token against Electron's issuer-owned in-memory registry, replaces routing with the capability's bound chat/lane/project or personal scope, and forces `force: false`; revoked or cross-process-fabricated tokens fail closed. This two-token boundary prevents a raw local socket caller, an unbound CLI, or an agent that forges action arguments from inheriting the human browser session.

Today only the `built_in_browser` domain rides this bridge; the pattern is generic and other Electron-only domains can be added the same way. The client lazy-connects on first call and reconnects on the next call after any failure. When no desktop is running, each call surfaces a clear `No ADE Desktop browser is attached to this machine (bridge socket <path> is not listening)…` error and every other runtime domain stays functional. `navigate` / `createTab` / `showPanel` are the exception: on a headless machine they emit a `built_in_browser_remote_request` runtime event and return `{ status: "forwarded_to_desktop", requestId }`, so a desktop that has this machine's lane pinned can open the URL in its own browser over a port-forward and acknowledge through `built_in_browser.acknowledgeRemoteRequest`. This is distinct from the retired renderer-hosted RPC mode used before the multi-project runtime: the ADE runtime still owns the full action surface, and the bridge is narrowly scoped to services that physically require an Electron renderer host.
...
**Platform window shell.** `src/main/windowAppearance.ts` is the source of
truth for BrowserWindow chrome. macOS keeps `hiddenInset` and traffic lights;
Windows uses a hidden title bar with a native window-controls overlay so
caption buttons, dragging, maximize, and Snap Layouts remain native. `main.ts`
sets the packaged Windows AppUserModelID to the same `com.ade.desktop` identity
used by electron-builder. Renderer startup stamps `data-platform` on the root
element for the minimal platform-specific drag/caption spacing. Capability
routing hides iOS Simulator and the native Activity Notch outside macOS;
persisted unavailable Work-sidebar state falls back to Git without disabling
the platform-neutral App Control and Browser tabs.
...
brain. A relay failure may degrade only to this computer's local
`getMachineSnapshot`; signed-out mode uses that same explicitly labeled local
scope. Later mutations are fenced on the loaded account owner and, per item, on
the alert fingerprint the caller had on screen; bulk acknowledgments are chunked
at 64 (the relay's own bound) and report a three-way `acknowledged` / `stale` /
`unreached` outcome. The renderer keeps this source warm in `AppShell`, presents
it first through the global-header Activity popover, and uses `/activity` for the
larger two-column Activity pane. Activity is an agent feed: the session column
carries agent items grouped by the six shared state groups (needs-you, failed,
planning, working, idle, done), while pull requests, checks, and review outcomes
render in a separate Notifications column. The header popover hides the two
resting bands; the pane adds filters (including a single-select state-group
glyph strip) and item detail, and ADE Notch mirrors the same ordering as a
compact native strip of clickable state-glyph badges plus an Agents/Events
panel.

The product and UI name is **Activity**, but the cross-process, network,
persistence, telemetry, and native-helper contracts intentionally retain their
existing `attention` naming. That includes `AttentionItem` wire fields,
`attention.call`, `ade.attention.*` IPC channels, `/attention/account/*` relay
routes, `attention_*` SQLite columns, analytics/log identifiers, and the
`ade-attention-notch` product name. Do not rename those compatibility
boundaries. `/attention` remains a legacy navigation alias for `/activity`.

**Runtime binding pools.**

- `apps/desktop/src/main/services/localRuntime/localRuntimeConnectionPool.ts` — desktop-side client for the local brain endpoint. Spawns or attaches to the machine endpoint, registers local projects with `projects.add`, dispatches local runtime actions, applies short per-call timeouts for project registration / file actions / event polling, emits a `local_runtime.action_slow` warning log when an action call exceeds 500 ms or throws (the log breaks the total into `ensureProjectMs` / `connectMs` / `daemonCallMs` so a stalled renderer call is debuggable before the IPC timeout fires) and records that call into a bounded (age + 5,000-sample) rolling 24 h window surfaced as `getRuntimeHealth()` (count + nearest-rank p95) behind the `ade.app.getRuntimeHealth` IPC, and best-effort installs the background service in packaged builds. Local project windows use this binding consistently outside unit tests. `probeMachineRuntimeIdentity()` answers what is on the endpoint precisely — version, build hash, pid, the expected build hash, and whether it is a compatible newer brain — replacing the older `probeMachineRuntimeHealth()`, whose `ok` could not distinguish the newly installed brain from the pre-update one that survived a service reinstall. Its opportunistic service repair is throttled (`serviceRepairBackoffMs`: 0 / 5 s / 15 s / 30 s / 60 s, then once a minute; it previously spawned one installer per connect attempt) and suppressible: `beginUpdateWindow` / `endUpdateWindow` bracket an applying update for at most `LOCAL_RUNTIME_UPDATE_WINDOW_MAX_MS` (2 min, self-expiring), during which a missing or incompatible endpoint is the updater's own doing rather than a fault to repair, and refused connects report `LOCAL_RUNTIME_UPDATE_IN_PROGRESS_MESSAGE`.
...
pane. `/attention` remains accepted as an unadvertised compatibility alias.

Model setup shares the desktop registry: GPT-6 Astra leads the Codex
list, GPT-5.6 Sol/Terra/Luna follow, Astra is the default, and host-advertised reasoning defaults and effort
...
not reach composer code as `null`. Account Activity is a separate direct push-relay read:
the global header/pane snapshot does not follow the selected sync
...
- **`apps/push-relay/`** — merges the bounded ADE Activity snapshots published by every signed-in brain, exposes an incremental account snapshot/ACK/presence/preferences/device API to desktop, hosted web, ADE Code, and iOS, and fans policy-selected events out as APNs alerts plus one prioritized account-wide Live Activity (Worker + one D1 database; free-plan compatible, no Durable Objects). Its routes, schema, payloads, and stored fields intentionally retain `attention` compatibility names. A machine publish requires both its existing HMAC signature and a verified Clerk account token; account routes require a verified Clerk bearer token. Primary and secondary identity domains are complete, distinct issuer/JWKS/OAuth-client triples selected by exact `iss`; OAuth audience metadata must match through `aud` or `azp`, and the D1 user key is namespaced by verified issuer. `npm run deploy` separates schema/trigger health from account-auth health: it requires both binding triples and short-lived issuer-specific smoke tokens, then checks `/health` and calls a real account snapshot with each token after deployment. The relay stores bounded attention previews/destinations/acknowledgments in addition to device tokens, revoked machines, and delivery receipts; it does not store chat transcripts or diff contents. Removing a machine is a real, terminal operation: `DELETE /attention/account/machines/:machineKey` tombstones and deletes that machine's items in one revision, records the revocation, and drops its push and Live-Activity delivery targets, after verifying the account actually knows the key. `POST /attention/account/machines/:machineKey/pairing` lifts it again and is gated on directory provenance (a constant-time `x-ade-directory-auth` comparison against the shared `DIRECTORY_AUTH_SECRET`) before it reads anything. The Live Activity projection lives in `liveActivity.ts` with the shared vocabulary in `attentionShared.ts`, and the expensive expiry and orphaned-machine sweeps are cron-only rather than hung off the request path. APNs auth is an ES256 provider JWT from the `.p8` (wrangler secrets `APNS_KEY` / `APNS_KEY_ID` / `APNS_TEAM_ID`). Brain-side publisher lives at `apps/ade-cli/src/services/push/`; desktop also launches a native AppKit/SwiftUI ADE Notch helper which consumes the renderer's account snapshot through typed IPC instead of polling the relay independently. Physical-notch Macs merge the surface with the hardware cutout; other Macs keep the real ADE icon in the menu bar and open an anchored transient panel instead of a permanent imitation notch. The helper has two reveal modes (always / hover) that render an identical state-glyph strip, and a click — in either mode — opens the Agents/Events panel. See [features/sync-and-multi-device/push-notifications.md](./features/sync-and-multi-device/push-notifications.md).
...
- **Maintenance hooks**: `openKvDb` attaches an optional `maintenance`
  ...
  prunes for `automation_ingress_events` / `pull_request_snapshots`, a
  zero-peers-only cr-sqlite tombstone compaction, and
...
| `lane_usage_tombstones` | One row per removed lane, written by `cleanupLaneDatabaseRows` before the delete cascade so lifetime activity stats are not survivor stats. Integer counters, the created/deleted calendar days, and a hex active-day bitmap only — no transcript text, path, branch, lane name, or per-day breakdown. `absorbed` duplicates are recorded without counting a second lane created; a lane that failed to finish being created is not tombstoned at all. Read back by `usageStatsStore`. |
...
| `usage_machine_rollups` / `usage_machine_rollup_meta` | Per-machine day × provider × model usage aggregates plus one identity row per machine (label, platform, capture time, transcript-source marker). Aggregates only — no transcript record, session id, prompt, or path is ever written here, and roots are stored as digests. They replicate desktop-to-desktop through CRR so an offline machine still counts toward account totals, and are in `MOBILE_CHANGESET_EXCLUDED_TABLES` because the phone reads usage from its host over `usage.getAdeStats` instead. Uniqueness is the composite primary key `(machine_key, day, provider, model)`, since `crsql_as_crr` rejects any UNIQUE index other than the primary key; writers skip no-op upserts so republishing unchanged history does not churn the CRR clock. |
| `test_suites` / `test_runs` | Declared test suites and their execution history. |
| `pull_requests` / `pr_review_threads` / `pr_checks` | GitHub PR projections with queue and stack metadata. `pull_requests` also stores the checks rollup's explanation (`checks_reason`) and the required contexts that never reported (`checks_missing_required`, JSON), so every device renders the same verdict without re-deriving it. Both columns were added through `crrAwareDb`'s `crsql_begin_alter` / `crsql_commit_alter` wrapper and replicate like the rest of the table. |
...
│   ├── project-icons/           # Imported project icon overrides (tracked when project.iconPath points at one)
...
└── ~/.ade/                      # Stable-channel ADE home (beta/alpha use their own homes)
    ...
    └── runtime/
        ...
        ├── watchdog-check.json  # The watchdog's own last-run time and last stale beat seen
...
1. **Git-tracked shared scaffold** — `.ade/.gitignore`, human-authored `templates/**`, `skills/**`, `workflows/linear/**`, `project-icons/**`. This is the only `.ade/` subset that flows through normal clone/pull. The shared `.ade/.gitignore` is now `*` with explicit allowlist entries for those scaffold files (so the next time someone touches `.ade/` from a fresh tool the runtime state stays out of git automatically).
...
- Targeted per-domain migrations live in `kvDb.migrations.test.ts`, which covers the consolidated upgrade path plus later CRR-safe schema cleanups.
...
- **Detection pipeline**:
  ...
  - `providerConnectionStatus.ts` — builds the `AiProviderConnections` snapshot surfaced to the renderer, including the signed-in account and plan for Claude and Codex, read through `usage/providerAccountIdentity.ts` so Settings > Providers and the live quota cards name one account.
...
**Machine provider identity and harness routing.** The runtime-owned
`apps/ade-cli/src/services/providerInstances/providerInstanceStore.ts` keeps
Claude and Codex account identities, default selection, and smart-balance /
auto-start settings machine-scoped; it stores config-home references, not
provider tokens. The shared contracts in
`apps/desktop/src/shared/types/providerInstances.ts`, `apiCredentials.ts`,
`subscriptionProxy.ts`, and `machineInventory.ts` keep account rows, stored-key
summaries, subscription-proxy status, and paired-machine inventory secret-free
across the action, preload, renderer, and sync boundaries. Harness presets then
pair a body with one of those account/key/subscription sources; their launch
resolver owns isolated config homes and only supplies a provider-shaped proxy
environment. See [onboarding and settings](./features/onboarding-and-settings/README.md)
and [harness presets](./features/onboarding-and-settings/harness-presets.md).
...
- `MODEL_REGISTRY` — static CLI-wrapped entries + dynamically populated API-key/local entries. The OpenAI/Codex block is ordered GPT-6 Astra, GPT-5.6 Sol, Terra, Luna, then the retained GPT-5.5 and older rows; `pickDefaultCodexModel` chooses Astra when present, otherwise the newest Sol row, so new Codex sessions default to GPT-6 Astra. Astra has a 1,050,000 context window and advertises Fast. All three GPT-5.6 descriptors have a 372k context window and advertise Fast. Astra and Luna expose `low | medium | high | xhigh | max`; Sol and Terra expose `low | medium | high | xhigh | max | ultra`. The product labels those tiers Light, Medium, High, Extra High, Max, and (for Sol/Terra) Ultra. Runtime app-server ladders pass through in their advertised order, including Max. `defaultReasoningEffort` is `low` for Astra/Sol and `medium` for Terra/Luna and is honored by desktop, TUI, iOS, CTO, and handoff pickers. The Claude block is ordered for every picker as Fable 5.1, Opus 5.5, Sonnet 5, Haiku 4.5, then Opus 5. Fable 5.1 uses provider model `claude-fable-5-1` (1,000,000 context / 128,000 max output), defaults to `high` effort, and advertises Fast plus `ultracode`. Opus 5.5 uses provider model `claude-opus-5-5` (1,000,000 context / 128,000 max output), defaults to `medium` effort, and advertises Fast at $4 / $20 per million input / output tokens. Opus 5 uses provider model `claude-opus-5` (1,000,000 context / 128,000 max output), defaults to `high` effort, and advertises Fast. Sonnet 5 uses provider model `claude-sonnet-5` (1,000,000 context / 128,000 max output); removed Sonnet 4.6, Fable 5, Opus 4.8, basic Opus 4.7, and Opus 4.7 1M ids resolve forward as compatibility aliases but do not appear as selectable rows. The generic `opus` alias resolves to Opus 5.5. Opus 5 remains selectable after Haiku; retired `opus[1m]` / `claude-opus-4-8` / `claude-opus-4-7[1m]` aliases resolve to it. `ModelDescriptor.serviceTiers?: string[]` advertises optional service tiers (today: `"fast"`, set on Fable/Opus, GPT-6 Astra, GPT-5.6 and older fast-capable Codex entries, and dynamic Cursor SDK/CLI rows) that the UI's Fast Mode toggle keys off. Codex maps it to the JSON-RPC `serviceTier` argument; Cursor SDK maps it through discovered model parameters, and Cursor CLI launches use the matching fast model alias when present.
- `ModelProviderGroup` = `"claude" | "codex" | "opencode" | "cursor" | "droid" | "pi"`. Cursor, Droid, and Pi each have their own top-level provider group used by the model picker, identity routing, and tracked CLI provider catalog.
...
- Reasoning tier passthrough (`providerOptions.ts`) maps tier strings directly to each provider's native config (`thinking.type`, `reasoningEffort`, `thinkingConfig.thinkingLevel`, etc.) — no arbitrary token budgets. Claude Opus 5.5 and Opus 5 advertise `low | medium | high | xhigh | max`; Fable 5.1 advertises `low | medium | high | xhigh | max | ultracode`; Sonnet 5 advertises `low | medium | high | max`. Opus 5.5's default effort is `medium`.
...
- **Runtime-only namespaces.** `window.ade.chatLaunch` (`start` / `get` / `list` / `cancel` / `retry` / `startNow` / `queueMessage` / `completeClient` / `onEvent`, each with an optional trailing `OpenProjectBinding` pin) has no Electron IPC channel and no in-process implementation: every call goes to the bound or pinned runtime's `chat` action domain (`startLaunch` … `completeLaunchClient`) through `callChatLaunchAction`, and an unbound window rejects with "New-lane launches need a connected ADE runtime." The mutating launch actions are in `MUTATING_CHAT_ACTIONS`. `onEvent` subscribes to the runtime event type `chat_launch_event` (`{ type: "chat_launch_event", event: ChatLaunchEvent }`) through a pinned pump when a pin is given, otherwise through the `remoteChatLaunchEventFanout` entry in `REMOTE_RUNTIME_FANOUTS`. The hosted web adapter implements the same namespace over the `chat.startLaunch` … sync commands and the pushed `chat_launch_event` envelope (`renderer/webclient/adapter/chatLaunch.ts`, `webclient/sync/chatLaunchEvents.ts`). See [features/chat/README.md](./features/chat/README.md#new-lane-launches).
- **Pinned event pumps.** A window runs several independent event pumps at once: the active binding's pump (in `preload.ts`) plus, for every binding the window has open but is *not* bound to, the pumps in `apps/desktop/src/preload/pinnedRuntimeEvents.ts`. That module owns one shared PTY pump per pinned binding (fed by both polling and `ade.runtime.event` push notifications, with its own cursor, epoch generation, dedupe ring, and failure backoff) and a per-listener generic pump used by pinned chat/project subscriptions; the active pump shares its epoch-normalization, stale-event, and dedupe helpers. Each pinned binding's PTY pump cannot share the active pump's single mutable cursor — switching the window binding would reset it, and polling two machines through it would cross-contaminate epoch and dedup state. Main-side subscriptions are reference-counted per `(binding, category)` so several pumps on the same binding share one, and preload sends `ade.runtime.events.release` when the last of them goes away (see [§5.4](#54-event-subscriptions-push-not-poll)). A pushed event announcing a *new* epoch is not dispatched: it rewinds the cursor so a local pin replays the restarted buffer and a remote pin re-anchors to the live head with `{ cursor: 0, replay: false }`.
- **One registration per remote event domain.** Every domain the renderer can receive over a remote (or pinned-local) binding used to be four hand-copied edits: a `Set` of callbacks, a clause in `hasRemoteRuntimeEventSubscribers`, an `if (toWrappedEvent(...))` block in the dispatcher, and a `subscribeRemote*` function. Thirty-four domains meant two independently hand-maintained lists of thirty-four, and a domain missing from the `has...` list silently stopped the event pump for that domain with nothing failing anywhere. `apps/desktop/src/preload/remoteRuntimeFanout.ts` is the single shape that knows all four, so the two lists collapse into one array and a new domain is one entry in it. A fanout owns its `eventType`, a live `hasSubscribers` flag, `subscribe` (which runs the shared pump's `onSubscribe`), and `dispatch`, which returns whether the payload belonged to it. Defaults cover the ordinary `{ type, event }` envelope; the few domains carrying a different shape (auth status, sync/usage snapshots, automations) pass their own `extract`, and optional `invalidate` (renderer caches this event stales, run once per matched event before delivery) and `shouldDeliver` (the per-id `pty_data` subscription filter) hooks cover the rest.
- Pinned APIs are opt-in trailing arguments, never a mode switch: `pty.create` / `resumeSession` / `sendToSession` / `write` / `resize` / `dispose` / `setDataSubscriptions` / `onData` / `onExit`, `terminal.preview`, `sessions.get` / `list` / `readTranscriptTail`, the chat/session APIs, the iOS Simulator and App Control domains with their `onEvent` subscriptions, the computer-use artifact mutations, and the cross-machine handoff trio (`agentChat.prepareCrossMachineHandoff` / `validateCrossMachineSource` / `markCrossMachineHandoff`) all accept an optional `OpenProjectBinding`. Omitting it takes the byte-for-byte unchanged bound path plus its local IPC fallback (`callPinnedOrBoundRuntimeActionOr`). The hosted web adapter cannot route a pin to a second host, so `renderer/webclient/adapter/runtimePinGuard.ts` guards every leaf of the federated adapter — not a couple of shims — with a typed `RuntimePinArg = OpenProjectBinding | null | undefined` that each pinned member takes. `assertWebRuntimePinRoutable` lets a pin naming the host and project the adapter is already bound to proceed unpinned (it restates the call's existing destination) and throws loudly on anything else instead of silently answering from the single web host. A `kind: "local"` pin gets its own message — "This chat runs on a machine ADE Web can't reach directly" — because the browser has no path to a desktop-local runtime at all and a local binding carries no `targetId` the remote comparison could use. The handoff trio is genuinely implemented in the web adapter over the registered sync remote commands `chat.prepareCrossMachineHandoff` / `chat.validateCrossMachineSource` / `chat.markCrossMachineHandoff`, rather than falling through to the proxy that answered `undefined` and made the handoff modal look like it had succeeded.
- The `pr` read surface is pinned the same way, because a lane's PR row lives in the `.ade` database of the machine that owns the lane. `callPrReadRuntimeActionOr(pin, action, request, local)` delegates to `callPinnedOrBoundRuntimeActionOr`, and `prs.getForLane` / `syncLanePr` / `listAll` / `refresh` / `getStatus` / `getChecks` / `getComments` / `getReviews` plus the `prs.onEvent` subscription all take the trailing pin. The Graph's bound-machine `prs.getDetailBundle` read groups status, checks, reviews, and comments into one response while retaining the existing sidecar shapes and successful values on partial failure. Here `pin: null` is a stated choice rather than an absence — it means "the machine the project tab is bound to", which is exactly what the bound-machine-scoped PRs tab wants — so every tab-scoped call site passes it explicitly rather than relying on the default. A pinned subscription must follow its reads: the bound runtime's `prs-updated` feed describes a different database and would leave a pinned PR pill permanently stale. See [features/pull-requests/README.md](./features/pull-requests/README.md#which-machine-answers-a-pr-read).
- The `files` surface is pinned the same way, because a file lives on the disk of the machine that owns the lane. Every `window.ade.files.*` method takes an optional **second positional** `pin?: OpenProjectBinding | null` routed by `callPinnedFileActionOr`: absent or `null` keeps the bound path byte for byte (remote route, strict local route, local IPC fallback, project-transition guard); a binding targets that machine and, being explicitly targeted work, bypasses the transition guard and the fallback. Reads *and* writes honour it — `writeText` / `writeTextAtomic` / `createFile` / `createDirectory` / `rename` / `delete` / `watchChanges` / `stopWatching` included — which is what lets a file from a chat on another machine open fully editable without rebinding the window. Two exceptions: `openExternalPath` takes no pin (the path comes from this machine's Finder/drag-drop and no runtime exposes an action for it), and `external-local:*` workspaces deliberately drop a supplied pin so a caller that pins uniformly still gets local behaviour. Coverage has to be total: a Files workspace id *is* a lane id and lane rows sync across machines, so an unpinned write while pinned elsewhere does not fail — it silently writes this machine's worktree for that lane. See [features/files-and-editor/README.md](./features/files-and-editor/README.md#which-machine-answers-a-file-call).
...
- `window.ade.sessions.moveOnBoard(sessionId, to, pin?)` and
  `.undoBoardMove(sessionId, moveId, pin?)` are the Work-board drag. A move is
  two writes that land together — the lifecycle columns for the target column,
  plus a host-authored message telling the agent what the user did, staged for
  five seconds so the drag stays reversible. They are registered as
  `ade.sessions.moveOnBoard` / `ade.sessions.undoBoardMove`, as the CTO-only
  `session.moveOnBoard` / `session.undoBoardMove` ADE actions, and as
  non-queueable sync remote commands — non-queueable because a move's whole
  contract is a five-second reversible window, and a command replayed minutes
  later after a reconnect would move a card the user stopped looking at long
  ago. The web adapter deliberately applies no optimistic patch: a move writes
  several lifecycle columns at once and one of them raises the attention hand.
  The staged message is drained on graceful shutdown before the services it
  needs are torn down. See
  [Terminals and sessions › The Work board](./features/terminals-and-sessions/README.md#the-work-board).
...
ade.recovery.*               # brain-independent project-open recovery: diagnose / repair
                             ...
                             # plus the main → renderer push ade.recovery.repairStep, one
                             # RepairStepResult as each step finishes so a long restart wait
                             # reads as progress rather than a hang. diagnose/repair validate
                             # the renderer-supplied root against services/ipc/knownProjectRoots.ts.
                             # See features/storage-and-recovery/README.md
ade.diagnostics.*            # openIssue: assemble the redacted diagnostic report behind every
                             # error surface's "Report issue" button, save it under
                             # <userData>/diagnostic-reports/, copy it to the clipboard, and open
                             # a prefilled GitHub issue. Same builder as `ade report-issue`.
                             # See features/storage-and-recovery/README.md#diagnostic-reports-report-issue
...
ade.lanes.*                  # lane list/create/delete/stack/template/env/port/proxy/rebase
                             ...
                             # (new-lane chat launches are not IPC: window.ade.chatLaunch.*
                             # is runtime-only, runtime event type chat_launch_event)
...
ade.runtime.*                # runtime event delivery: the ade.runtime.event push channel
                             # (main -> renderer, carrying bindingKey + eventEpoch) and
                             # ade.runtime.events.release, the renderer -> main teardown a
                             # preload pump sends when it stops reading a binding. The
                             # release argument mirrors the subscribe descriptor — remote
                             # `{ id, projectId }` or local `{ rootPath }`, plus an optional
                             # category — so main re-derives the request key instead of
                             # trusting one the renderer guessed
ade.git.*                    # stage/commit/push/sync/revert/cherry-pick/stash;
                             # getSyncStatus/getSyncStatuses read lane sync state
...
ade.prs.*                    # stacked PR queue, integration, rebase/issue
                             # resolver sessions, merge readiness, timeline,
                             # Graph detail bundle (ade.prs.getDetailBundle),
                             # comment edit (ade.prs.updateComment), and
                             # reactions (ade.prs.reactToComment)
...
ade.files.*                  # runtime-routed file workspace/tree/read/write/watch/search actions,
                             ...
                             # run the same fileService code. The optional machine pin is a
                             # preload-level routing argument (§5.1), not a separate channel.
                             # searchText is served by `git grep` in a git work tree and a
                             # streaming JS scan otherwise — the name index holds no contents.
ade.agentChat.*              # agent chat sessions, model inventory, parallel launch state.
                             ...
                             # optional for older mobile clients. Attachment staging is three
                             # channels, not one: saveTempAttachment is the base64 leg (10 MB,
                             # image-only; the MIME sniff applies on the sync leg only),
                             # copyTempAttachment stages a file that already
                             # exists on this machine's disk by copying it (50 MB, any type,
                             # bytes never enter the renderer), and the sync-only chunked
                             # begin/append/finish/abort family carries a file-shaped
                             # attachment at the same 50 MB ceiling in 512 KiB base64 slices
                             # for clients that have only a WebSocket (a phone on the relay).
                             # The preload picks between the first two — and the remote HTTP
                             # upload route — from getAttachmentStagingMode.
...
ade.ai.*                     # AI integration status + provider auth (storeApiKey/deleteApiKey/getStatus/updateConfig/...).
                             ...
                             # Pi in-app sign-in: piLoginProviders, piLoginStart / piLoginSubmit /
                             # piLoginCancel, piAuthStatus (push). piLoginStart blocks on a human
                             # finishing an OAuth or device-code grant, so it carries its own
...
ade.computerUse.*
ade.iosSimulator.*           # macOS-only iOS Simulator drawer + Preview Lab: , plus the device hub: , plus the ade.iosSimulator.event push channel, plus the Apple device hub: deviceCreate/deviceAttach/deviceList/deviceDelete/recordStart/recordStop/recordList/recordDelete/frame. No channel here touches a Simulator.app window: the vendored Swift helper reads the simulator's framebuffer directly, so there is no window placement, no parking follow, and no Screen Recording grant.
ade.appControl.*             # Electron app control bridge over Chrome DevTools Protocol. Mirrors ADE_ACTION_ALLOWLIST.app_control in apps/desktop/src/main/services/adeActions/registry.ts: , plus the ade.appControl.event push channel (session-started/updated/stopped, selection, screencast frame, diagnostics)
ade.builtInBrowser.*         # in-app web browser owned by `builtInBrowserService`. Mirrors the IPC.builtInBrowser* channels in apps/desktop/src/shared/ipc.ts: getStatus/requestOriginAccess/getProfileDiagnostics/listPermissions/clearPermissions/agentAccess.get/agentAccess.setMode/agentAccess.answer/agentAccess.revoke (+ the agentAccess.event push)/loginImport.capabilities/loginImport.listSources/loginImport.listDomains/loginImport., plus the ade.builtInBrowser.event push channel (status / open-request / selection / selection-cleared / handoff-started / handoff-ended / recording / login-import-completed / remote-request / error). Backs the Work sidebar's Browser tab, personal chat's independent tab collection on the global profile, and the renderer-wide `openUrlInAdeBrowser()` link router. Profile diagnostics and permission administration are trusted-renderer-only and rejected on CLI/runtime bridge paths.
...
ade.keepAwake.*              # get / setLevel / fixSystemSleep for the opt-in
                             # keep-awake setting. Backed by
                             # main/services/power/keepAwakeService.ts; hosted web and
                             # the browser mock answer with the frozen inert snapshot.
ade.remoteRuntime.*          # remote target registry, connect/projects/project-open,
                             ...
                             # host's sync port. It takes a sourcePath, or base64 data
                             # (a pasted image) that main writes to a private temp file
                             # and uploads the same way. The upload channel is one of the few with
...
- **Multi-window shell** — the app can host multiple `BrowserWindow` instances (for example when opening another project in a dedicated window). Handler tracing already carries **window ID** so logs and diagnostics distinguish which renderer surface invoked a channel; `main.ts` ties each window to its **set** of open project roots before routing into services. Three maps in `main.ts` drive this: `windowProjectRoots` tracks the active foreground project per window, `windowProjectTabRoots` tracks every project root that window currently has open as a tab, and `windowKnownLocalProjectRoots` accumulates every local root the window has actually opened this session. Project-scoped event broadcasts (`emitToProjectWindows`) deliver to any window whose active **or** open-tab set contains the project, so background tabs keep receiving live updates. `ade.app.getWindowSession` returns `{ project, binding, openProjectTabs, pendingLocalProjectRoots, knownLocalProjectRoots }` for the requesting window; the renderer mirrors its open-tab list back to main with `ade.app.setWindowProjectTabs({ rootPaths })` so the main process can keep those project contexts warm and clean up on window close.

  That mirrored list is renderer-supplied, so `services/projects/windowTabRootAuthorization.ts` (`resolveWindowTabRoots`) splits it into two different things before anything acts on it: `tabRoots`, what the window is displaying, only ever used to look projects back up, where an unknown path is harmless; and `authorizedLocalRoots`, the window's local runtime scope, which `runtimeBridge` reads as "this window opened this checkout". A path that never opened must not earn local-runtime access merely by appearing in a tab list, so only roots resolving to a project this process actually opened pass through. `windowKnownLocalProjectRoots` exists because neither of the other two maps can answer "was this window ever working in this local checkout?" — `windowProjectTabRoots` is replaced wholesale by the renderer on every tab change, and `windowProjectRoots` is nulled the moment the tab binds to a remote machine. A chat pinned to "This computer" asks exactly that question: its lane lives on this machine no matter which machine the tab is currently bound to, and rejecting it leaves a cross-machine Work view unable to act on its own local sessions. The set surfaces on the window session as `knownLocalProjectRoots` and `runtimeBridge.collectAuthorizedLocalRuntimeRoots` folds it into the authorized set. Membership is granted only by the window opening the project itself, never by a renderer-supplied path. Renderer tab switches use cached project/lane snapshots for warm activation, retain caches for every open tab root even if a project is absent from recents, keep Work and Lanes mounted after first visit, and cover cold switches with a project-transition veil.
...
| `ade.ai.opencodeOAuthStatus` / `ade.ai.piAuthStatus` | openCodeAuthService / piAuthService | Settings › Providers connect + sign-in flows |
| `ade.app.command` | native menu, via `main/appCommandDispatch.ts` | `TopBar`, and whichever surface claimed the command |

**Native-menu commands (`IPC.appCommand`).** Electron consumes accelerators — ⌘+/−/0, ⌘F, ⌘W — in the browser process *before* any renderer keydown fires, and the built-in browser's page has focus in a different `WebContents` entirely, so a pane that binds them with `onKeyDown` is shadowed on the packaged app while passing every jsdom test. The menu therefore sends one command down. `createAppCommandSender` in `apps/desktop/src/main/appCommandDispatch.ts` is the single sender for both kinds (`{ kind: "zoom" | "menu", command }`), replacing two channels, two preload bridges, and two near-identical helpers in `main.ts`.

Three cases a naive `webContents.send` gets wrong, and this owns. A window ADE did not open (a DevTools window, a native panel) has no ADE renderer, so the command would vanish — those run the caller's `fallback`. A window whose renderer has crashed or is wedged in a long task also swallows it, and for ⌘W that matters because ⌘W is the escape hatch for exactly that window: liveness is probed with a round-trip scheduled on the renderer's own event loop (a crashed one rejects, a wedged one never resolves, a healthy one is back within a frame), bounded by `APP_COMMAND_LIVENESS_TIMEOUT_MS` (1,200 ms) — long enough that a busy-but-healthy renderer always wins, short enough that a wedged window still feels like a keystroke. And a window whose renderer has not yet *mounted* its subscriber swallows it invisibly, because `executeJavaScript` resolves as soon as the JS context exists, a second or more before React mounts `TopBar`; readiness is therefore **announced** (`hasCommandSubscriber`) rather than inferred, and until that ack arrives a command with a fallback runs the fallback. `role: "close"` used to close from the browser process unconditionally, and `fallback` is what restores that guarantee.

On the renderer side, `lib/appCommandClaims.ts` is the claim registry both command kinds instantiate (`appMenuCommands.ts`, `appZoomCommands.ts`). A claim is a *chance* to handle, not a takeover: handlers stack newest-first and return `false` when it is not the right moment (the browser pane is mounted but the composer has focus), and the app-wide default in `TopBar` then runs as before.

Runtime-backed events reach the renderer through `ade.runtime.event` instead, and their subscriptions are held by `apps/desktop/src/main/services/ipc/runtimeEventSubscriptionRegistry.ts`. Subscriptions are keyed by **(sender, requestKey)** — the request key being `<bindingKey>:<category|*>:<replay|live>` — because one renderer legitimately runs several pumps at once (the active binding, one pinned PTY pump per foreign lane, one pinned chat pump), and keying by sender alone would let each new pump tear down its siblings. Since key-per-sender no longer doubles as garbage collection, a stale entry is reclaimed by idle expiry: every live pump refreshes its subscription on each poll (750 ms–5 s normally, 30 s at the slowest failure backoff), a 60 s sweep drops anything unrefreshed for 180 s or whose sender is destroyed, and the renderer's explicit `ade.runtime.events.release` is the fast path so a switched-away binding stops streaming immediately rather than at expiry. A release covers the whole `(binding, category)` prefix, because a pump's replay flag flips from `live` to `replay` once it is caught up and it can therefore own both key variants. Removal has exactly one implementation (`removeRuntimeEventSubscription`), used by release, the ended callback, remote disconnect, sender death, and the sweep, so disposal and registry pruning cannot drift apart; disconnecting a remote target drops **every** subscription that window holds against it, not just the newest.
...
| `attention/` | `attentionAccountCoordinator.ts`, `attentionNotchHelper.ts`, `attentionNotchRouter.ts`, `remoteProjectIdentity.ts`, `attentionOpenErrors.ts` | Account-first desktop Activity boundary plus the native macOS helper lifecycle. The internal directory and service names remain part of the established compatibility vocabulary. The coordinator reads the relay independently of the selected project/remote binding, permits only an explicit local-machine fallback, fences mutations by loaded account owner plus a per-item alert fingerprint, and chunks bulk acknowledgments at the relay's 64-id bound with a three-way acknowledged/stale/unreached outcome. `remoteProjectIdentity.ts` reconciles the publisher's per-machine project uuid against a runtime's `project_<hash>` registry id by falling back to the project root both sides carry, and `attentionOpenErrors.ts` names which of pair/connect/open failed in the user's own terms. The helper consumes the renderer snapshot, reports physical-notch vs menu-bar surface state, preserves empty/error availability, and routes native open/refresh/settings/acknowledgment requests back through typed IPC. `dismiss_item` carries a `mode`, narrowed once by `normalizeAttentionNotchOutput` before it leaves the module: `"seen"` acknowledges the row so it stops interrupting but leaves it in Activity (what closing a takeover card means), `"dismiss"` files it away (the panel's dismiss, and what a helper too old to send the field meant). An absent mode is `"dismiss"`; an unrecognised one from a *newer* helper degrades to `"seen"`, the less destructive action. The guard never rejects on `mode`, so an unknown word cannot cost the whole acknowledgement. Its refresh cadence is keyed off what is actually on screen: 15 s while it has a reported surface and the display is awake, 60 s while it has none or the screen is locked/suspended. `main.ts` feeds that through `setScreenAwake`, tracking `powerMonitor` lock and suspend as independent facts so a resume after a sleep that did not lock cannot declare a locked screen awake. Changing the interval rebuilds the timer, and a respawned child starts with no surface rather than inheriting the previous one's. |
...
| `config/` | `projectConfigService.ts`, `laneOverlayMatcher.ts` | Load/save `.ade/local.yaml` (the committed `ade.yaml` is carried over once, then deleted); lane overlays. |
| `conflicts/` | `conflictService.ts` | Pairwise dry-merge simulation, risk matrix, proposal generation. |
| `cto/` | `ctoStateService.ts`, `ctoMemoryService.ts`, `ctoPromptContent.ts`, `ctoVoiceCallService.ts`, `ctoVoiceConfirmation.ts`, `ctoVoiceRuntimeService.ts`, `ctoVoiceWiring.ts`, `linearClient.ts`, `linearIssueTracker.ts`, `linearCredentialService.ts`, `linearOAuthService.ts`, `linearTokenRefresh.ts`, `linearLaneCardService.ts`, `linearLiveStatusService.ts` | CTO identity, the smart-memory file store, session logs, the OpenAI Realtime voice call, and the Linear read/credential/OAuth surface. The voice call brain is **runtime-hosted**: `ctoVoiceRuntimeService.ts` is constructed in `apps/ade-cli/src/bootstrap.ts` and exposed as `AdeRuntime.ctoVoiceCallService`, because the call needs the chat service and CTO identity, which are null on the desktop `AppContext` in every runtime-backed build. `ctoVoiceWiring.ts` is the desktop router only — it owns the microphone-holding window, the audio pump and the HUD broadcast, and reaches the brain through the `cto_voice` ADE action domain (fail-closed to `cto` role via `ADE_ACTION_CTO_ONLY`'s `{ allExcept: [] }`) plus the `cto_voice` runtime event category, which carries call state and never audio. The category rule itself is one module — `shared/runtimeEventPolicy.ts` (`hidesVoiceEvent`, `refusesVoiceCategory`, `withoutVoiceEvents`, `voiceCategoryRefusalMessage`), each helper taking the caller's role flag — so the two RPC servers cannot express half of it: a stream asked for BY NAME is refused, and an uncategorised drain is *filtered* so the cursor still advances past what was withheld. That category is gated on the `cto` role at the runtime RPC surface (`runtimeEvents.subscribe` and the `stream_events` tool) and excluded from renderers in four places that each stand alone — the subscription-name guard and `shouldForwardRuntimeEvent` in `runtimeBridge.ts`, the polled-batch filter `withRendererVisibleEvents`, and the preload's own predicate on both inbound paths. `linearLaneCardService` posts the Linear attachment card and builds the cross-machine ADE deeplink that backs the card's URL; `linearLiveStatusService` is the optional launch/PR/merge status round-trip. |
| `deeplinks/` | `protocolHandler.ts`, `ownerAwareNavigation.ts`, `localProjectResolution.ts`, `projectNavigationWindowSelection.ts` | Registers the `ade://` OS protocol handler for the packaged Stable desktop build, owns the single-instance lock, buffers cold-start URLs until `app.whenReady()`, and dispatches parsed URLs through `IPC.appNavigate` to the focused window. Beta, Alpha, and source builds can receive explicitly delivered links but do not claim the OS-default handler. Re-used by the iOS Send-to-Mac sync command (`syncRemoteCommandService.deeplinks.open`). Account-owned targets route first through `ownerAwareNavigation.ts`: `localProjectResolution.ts` resolves the link's project against this machine (exact id, then the carried root path, then a recomputed canonical id), and `projectNavigationWindowSelection.ts` picks a window for a remote project without ever rebinding the one the user is working in. Shared parser + builder live in `apps/desktop/src/shared/deeplinks.ts`; the PR "Open in ADE" footer is in `apps/desktop/src/shared/adeDeeplinkFooter.ts`. See [features/deeplinks/README.md](./features/deeplinks/README.md). |
| `devServers/` | `devServerRegistry.ts` | Passive dev-server discovery. Rides `ptyService`'s existing output pass with one bounded regex per chunk (plus a carry for lines split across chunks) over the ready lines frameworks already print, and publishes `{ port, url, source: { sessionId, laneId, projectRoot }, detectedAt }` into an in-memory registry. Nothing here opens a socket or makes a request, and nothing is persisted — a port that was live last week says nothing about this app session. Feeds the built-in browser's launchpad list and, when `browser.autoOpenDevServer` is on, one background tab that never steals focus. |
| `devTools/` | `devToolsService.ts` | Probe for git + `gh` CLI availability. |
| `diffs/` | `diffService.ts` | Diff computation for file panes. |
| `diagnostics/` | `diagnosticReportService.ts` | Assembles the redacted diagnostic report behind every error surface's **Report issue** button: the shared machine sources (`ade-cli/src/services/diagnostics/`) plus the desktop-only extras — its own jsonl logs, the local runtime status snapshot, the recovery diagnosis for the open project, the typed last-failure store, and an Electron-aware volume reader. Saves it `0600` under `<userData>/diagnostic-reports/`, copies it, and opens a prefilled GitHub issue carrying only a short (also redacted) stub. Never runs `ade doctor` and never starts the brain: the report has to be collectable on a machine whose brain will not start. |
| `editors/` | `editorDetection.ts`, `openPathInEditor.ts`, `editorProcessEnv.ts` | Detects installed editors from `EDITOR_TARGETS` in `shared/editorTargets.ts` (`which` / `where.exe`, plus macOS `open -Ra` with duplicate `macAppName` collapsed) and opens a local workspace by spawning the editor CLI / `open -a`, or a remote SSH workspace by minting an OS URL (`vscode://vscode-remote/ssh-remote+host/path`, `zed://ssh/host/path`) that `openEditorExternalUrl` allowlists. Paired remotes (`transport: "paired"`) cannot mint those URLs and are excluded by `isRemoteEditorOpenRequest`. IPC: `ade.app.getInstalledEditors`, `ade.app.openPathInEditor`. Renderer UI: `OpenInSubmenu`. See [files-and-editor](./features/files-and-editor/README.md#opening-in-an-external-editor). |
...
| `git/` | `git.ts`, `gitOperationsService.ts`, `gitConflictState.ts` | Low-level git runner, high-level lane-scoped ops, conflict state queries. `gitOperationsService.getSyncStatuses` batches lane-id keyed sync reads while preserving the single-lane `getSyncStatus` response shape. |
...
| `localRuntime/` | `localRuntimeConnectionPool.ts` | Desktop-side client for the local brain endpoint. Spawns or attaches to the machine endpoint, registers local projects with `projects.add`, dispatches local runtime actions with per-call timeouts where needed, emits `local_runtime.action_slow` warn logs (with `ensureProjectMs` / `connectMs` / `daemonCallMs` breakdown) whenever a call exceeds 500 ms or throws and aggregates those calls into a bounded rolling 24 h window exposed as `getRuntimeHealth()` (count + p95) for the `ade.app.getRuntimeHealth` IPC / Storage diagnostics tile, polls/subscribes to runtime events while preserving `eventEpoch`/gap metadata, and installs the background service best-effort in packaged builds. `callSync` takes a per-call `timeoutMs` so a caller standing in for a live stream can opt out of the long default; `callAttention` uses it to run every Attention poll under the 30 s sync-domain timeout, because a snapshot poll inheriting the ten-minute action budget pins the renderer on syncing long after the account stream has wedged. Also owns the runtime-identity probe the update transaction verifies restarts against (`probeMachineRuntimeIdentity`, `getStaleMismatchedRuntime`) and the update window that suppresses its own service repair while an update applies (`beginUpdateWindow` / `endUpdateWindow`). |
| `macDesktop/` | `macDesktopService.ts`, `macDesktopDriverLifecycle.ts`, `macDesktopDriverClient.ts`, `macDesktopStreaming.ts` | Per-lane macOS virtual display. The service runs in the brain; `macDesktopDriverLifecycle.ts` owns the `ade-desktop-driver` process, and the client's stderr lines are logged as `mac_desktop.driver_stderr` into `brain.jsonl`. Hidden on a non-Mac runtime host; every client of a Mac-hosted lane can still see it. See [features/mac-desktop/README.md](./features/mac-desktop/README.md). |
| `onboarding/` | `onboardingService.ts`, `onboardingSuggestedConfig.ts` | First-run flow, defaults detection, existing lane discovery. `onboardingSuggestedConfig.ts` contains pure workflow parsing and suggested project config generation. |
...
| `power/` | `powerStateService.ts`, `keepAwakeService.ts`, `systemSleepConfig.ts`, `machinePowerBrainBridge.ts` | This machine's power and sleep truth, plus the opt-in keep-awake setting. `powerStateService` is a process singleton wrapping the shared `MachinePowerSource` from `apps/ade-cli/src/services/power/` with Electron's `powerMonitor`; `machinePowerBrainBridge` forwards suspend/resume to the brain as `machine.reportPowerTransition` on a 2-second budget so the directory learns about a closed lid in the beat before the machine goes dark. `keepAwakeService` holds a `powerSaveBlocker` only while a turn is running and owns the macOS lid-sleep switch; `systemSleepConfig` reads and fixes the OS's own idle-sleep timer. See [onboarding and settings](./features/onboarding-and-settings/README.md#keeping-the-machine-awake). |
| `projects/` | `adeProjectService.ts`, `configReloadService.ts`, `projectService.ts`, `logIntegrityService.ts`, `recentProjectSummary.ts`, `projectBrowserService.ts`, `projectDetailService.ts`, `windowTabRootAuthorization.ts` | Project detection + `.ade` repair/bootstrap, reload on config change, recent-project metadata. `recentProjectSummary.ts` emits local and remote recent summaries without disk-inspecting remote paths, and attaches each local checkout's `origin` URL — read straight from git config rather than by spawning `git` per recent, undecorated the way git parses config values, resolved through a linked worktree's metadata dir back to the main repo's config, and cached per root on the config file's mtime. That URL is the key the renderer's top bar joins local and remote checkouts of one repository on. `projectBrowserService` is the in-app directory autocomplete used by the Command Palette project browser (typed-path completion, `.git` detection, home expansion, system-picker fallback); `projectDetailService` returns repo metadata (branch, dirty count plus staged/unstaged/untracked breakdown, ahead/behind, last commit, README excerpt inputs, language mix, lane count, last-opened) for the palette's preview pane. `windowTabRootAuthorization.ts` is the pure split of a renderer-named tab list into displayed roots versus the window's authorized local runtime scope. |
| `prs/` | `prService.ts`, `prCommentMutations.ts`, `prPollingService.ts`, `prSummaryService.ts`, `requiredChecks.ts`, `githubStackStore.ts`, `integrationPlanning.ts`, `integrationValidation.ts` | PR CRUD, polling (with per-PR `last_polled_at` cursor), timeline comment PATCH + reaction decoding (`prCommentMutations.ts`; IPC `ade.prs.updateComment` / `ade.prs.reactToComment`), AI summary cache keyed by `(prId, head_sha)`, required-status-check resolution (rulesets → classic protection → `mergeStateStatus` corroboration, cached per repo+base branch), native GitHub stack reconciliation, integration planning, and the PR reads/mutations behind the agent-facing `pr_*` RPC tools (`pr_get_checks`, `pr_get_review_comments`, `pr_rerun_failed_checks`, `pr_reply_to_review_thread`, `pr_resolve_review_thread`) — those tools are dispatched from `apps/ade-cli/src/adeRpcServer.ts`, not from this directory, and `pr_get_checks` deliberately returns the persisted verdict over a row-level rollup so an agent cannot read "passing" while every human surface says "not run", and merge-into-existing-lane proposal adoption. The checks verdict itself is derived by the shared `prChecksRollup.ts`, not here. See [features/pull-requests/README.md](./features/pull-requests/README.md#checks-rollup-what-counts-as-a-pass). |
| `pty/` | `ptyService.ts` | `node-pty` spawn, PTY I/O bridging, transcript writing. Its single output pass is also where passive dev-server discovery rides (`devServers/devServerRegistry.ts`), so no second observer is attached to terminal output. |
...
| `scenes/` | `sceneDocumentStore.ts`, `sceneSnapshot.ts`, `sceneSnapshotIngest.ts` | Agent-authored generated UI. `sceneDocumentStore.ts` holds prepared scene documents **in memory** (capped at 64) and serves them over the `ade-scene:` scheme — the id in `ade-scene://view/<id>` is a map key, never a path, so the scheme has no filesystem reach by construction. `sceneSnapshot.ts` owns `clampSceneCaptureRect`, the freeze-to-a-picture geometry; `sceneSnapshotIngest.ts` is the CTO-only `computer_use_artifacts.ingestSceneSnapshot` runtime action that files a snapshot as proof on a runtime-backed build, repeating the artifact-store jail against the services it actually owns. See [features/chat/README.md › Scenes](./features/chat/README.md#scenes). |
...
| `shared/` | `utils.ts`, `imageDimensions.ts`, `remoteTrackingBranch.ts`, `packLegacyUtils.ts`, `transcriptInsights.ts`, `pathCompare.ts`, `providerConfigHomes.ts`, `agentObservationCache.ts`, `processExecution.ts`, `processOrphans.ts` | Cross-domain utilities, including shared record guards, remote tracking-branch refresh, case/separator-correct path comparison, and PNG/JPEG dimension parsing used by App Control and iOS Simulator capture paths. `agentObservationCache.ts` is the one copy of the two disk sweeps the built-in browser (per tab) and App Control (per session) both need over their `<id>.json` / `<id>.png` / `<id>.map.png` observation triples — one copy, because a fork of it deletes files. `providerConfigHomes.ts` is the one place that knows where each provider CLI keeps its user-level config — `claudeConfigHome` (`CLAUDE_CONFIG_DIR`), `codexConfigHome` (`CODEX_HOME`), `factoryConfigHome` (`FACTORY_HOME_OVERRIDE`, which replaces HOME rather than naming the directory) — and it carries the config-ownership rule every provider adapter follows: name a key only when ADE owns it, because ADE's settings land above the user's own config files. Consumed by the chat adapters, `pty/ptyService.ts`, `externalSessions/`, and `chat/droidModelsDiscovery.ts`. See [Provider config ownership](./features/chat/agent-routing.md#provider-config-ownership). `processExecution.ts` owns the spawn invocation helpers, including the Windows `cmd` wrapper and the trusted System32 PowerShell and `taskkill` invocations (a sync tree kill, and an async one for startup paths), and `processOrphans.ts` is the one copy of the pieces the OpenCode and Cursor worker orphan sweeps share: the process-listing parser, the wait for exit, and the kill ladder. |
...
**Cross-surface request channels.** State a surface owns cannot be written by a
surface that does not know its scope, so the caller files a request instead.
`renderer/lib/pendingRequestChannel.ts` is the one implementation of that
pattern, in two shapes that are deliberately not one primitive:
`createPendingRequestChannel` (broadcast to live listeners **and** hold one
...
**Zoom accelerators.** ⌘/Ctrl `+`, `-`, and `0` are native View-menu
accelerators, and Electron consumes an accelerator in the browser process
*before* the renderer sees the keydown — so a panel binding them with
`onKeyDown` is shadowed on the packaged app while passing every jsdom test. The
menu sends one `zoom` command into the renderer and
`renderer/lib/appZoomCommands.ts` decides who answers it. A claim is a *chance*
to handle, not a takeover: a handler returns false when it is not the right
moment (the browser pane is mounted but the composer has focus) and app zoom
happens as before. Claims stack newest-first, so two mounted browser panes
cannot fight over one keystroke.
...
cto/            # single-thread CTO page, CtoSettingsPage, memory/prompt panels, Talk button, voice HUD + host, shared/designTokens.ts
onboarding/     # first-run flows
settings/       # keybindings, agents, data, context, sync
chat/           # AgentChatPane + composer + subpanels, ChatRuntimeScope.tsx
...
**Every chat-scoped surface routes to the chat's machine, not the tab's.** A chat's runtime actions belong to the machine the chat's lane lives on, which is frequently not the machine the globally selected project tab is bound to; `chat/ChatRuntimeScope.tsx` is the one derivation of that scope (`useChatRuntimeScope()` yields binding, `isRemote`, machine name, lane, lane worktree path, and root path, with `useChatRuntimeScopeForPin` for an explicitly supplied pin), and the resulting binding is what components pass as the trailing preload pin. `apps/desktop/eslint.config.mjs` enforces the rule inside `src/renderer/components/chat/**`: `no-restricted-syntax` selectors flag `useAppStore` / `useRootAppStore` reads of `projectBinding` or `lanes` — subscribed or through `getState()`, since an imperative read of the tab's binding is the same wrong answer as a subscribed one — and reads of `project.rootPath`, each message pointing at `useChatRuntimeScope()`; `no-restricted-imports` bans importing `selectActiveProjectRoot` from `state/appStore` as a path pattern, because files in `chat/codex`, `chat/hooks`, and `chat/usage` reach that module at different depths. `AgentChatPane.tsx` is exempt because it owns the tab-versus-chat distinction, `ChatRuntimeScope.tsx` because it *is* the derivation, and tests are exempt. Pin-taking components outside `components/chat` — `lanes/LaneGitActionsPane.tsx`, `lanes/LaneDiffPane.tsx`, `lanes/CommitTimeline.tsx`, `terminals/WorkSidebar.tsx` — carry the same invariant but are deliberately not lint-enforced, because each still makes intentional tab-scoped reads the selectors would flag; treat their pin handling as unenforced until those reads are audited and marked.

Design tokens have been intentionally trimmed. The CTO design tokens at `apps/desktop/src/renderer/components/cto/shared/designTokens.ts` are the example style: a small set of Tailwind class constants (`cardCls`, `surfaceCardCls`, `shellBodyCls`, `inputCls`, `labelCls`, etc.) and a constrained accent palette (`ACCENT.purple/blue/green/pink/amber`). Lane design tokens live at `lanes/laneDesignTokens.ts` and are imported across lanes/PRs/settings. Notices and overlays have one primitive each. `ui/notice/` provides `Banner` (docked, floating, or inline layout), the `useAppBanner` registry rendered by the single `AppBannerHost` in `AppShell`, `ToastCard` for `showToast` toasts in `app/toast/ToastViewport`, and the shared `noticeTone` palette. `ui/dialog/` provides the `Dialog` modal shell and the imperative `confirmDialog` / `promptDialog`, rendered by `DialogHost`. `app/HeaderSheet.tsx` is the one top-bar dropdown. `ui/zLayers.ts` (`Z_LAYERS`) is the one stacking scale. The local ESLint plugin `apps/desktop/eslint-rules/ade-ui.mjs` warns on code that bypasses these primitives. `npm run lint:ci` fails CI when any file's warning count grows past `apps/desktop/lint-baseline.json`. The guide is `docs/design/notices.md`.
...
- Layout state persists to SQLite (`layout` and `tilingTree` domains via the `kv` table).
...
10. Persistence callbacks dedupe against the last-saved value so a load that writes the same preference back does not persist a no-op.
...
- `renderer/components/app/toast/` - shared renderer-only toast primitive. `toastStore.ts` owns stack order, timers, hover pause/resume, sticky toasts, and in-place replacement; `ToastStack.tsx` renders toast cards inside the single `ToastViewport` mounted by `AppShell`. Lane lifecycle and automated rebase terminal events subscribe through `useLaneEventToasts.ts`.
- `renderer/components/ui/notice/` — shared banner and notice primitives. `Banner` provides the docked, floating, and inline layouts; `AppBannerHost` is mounted once in `AppShell` and controls app-wide placement, order, and overflow. `IntegrationBanners` registers project integration states with that host. Dismissal is durable and fingerprint-aware via `renderer/lib/bannerDismiss.ts` (localStorage-backed, so a dismissal survives restart and project reopen; a dismissed banner auto-resurfaces after ~2 weeks or the moment its underlying state changes/regresses to a different fingerprint). GitHub App health is derived in `renderer/lib/githubIntegrationStatus.ts` (see [Onboarding and settings](./features/onboarding-and-settings/README.md)), so the banner and Settings never disagree. A `blocked` account credential — ADE renewing its own authorization, or GitHub pausing the renewal — raises **no** banner at all: ADE retries on its own, there is nothing the user can do, and every repo reading taken while it is paused is unproven. Only `missing` and `needs_reauth` reach the banner. When `describeGithubOutage` reports a githubstatus.com-corroborated incident, the three GitHub banners are suppressed and replaced by a single neutral `info` notice linking the live incident — three separate accusations against the user's setup would all be symptoms of one cause they cannot fix, and re-authorizing during a GitHub outage is how a working credential gets destroyed. The suppression is gated on the same condition that renders the replacement, and the notice is pinned ahead of severity ordering so the slice into overflow can never hide the explanation while it is still silencing the banners; only the GitHub family is affected. See [Pull requests](./features/pull-requests/README.md#telling-a-github-outage-apart-from-a-broken-credential). The relay banner reads `routeHealth.relay` from `AppShell`'s `sync-status` subscription seeded by `sync.getLocalStatus` (relay belongs to the physical machine, not to whichever runtime a remote-bound project routes to): a deliberate suppression is reported immediately, while a plain outage waits out `RELAY_OUTAGE_GRACE_MS` (2 minutes) of uninterrupted failure on a single armed timer rather than a poll. See [Sync and multi-device](./features/sync-and-multi-device/README.md).
- `renderer/onboarding/docsLinks.ts` — typed registry of internal/public doc URLs (`docs.lanes`, `docs.cto`, …) used by `SmartTooltip`, the welcome surface, account help, and the `HelpMenu`.
- `renderer/components/onboarding/LaunchGate.tsx` — fresh-process account gate. New installs see the welcome card first, then sign-in. ADE requires an account, so a first run offers no way past; `accountGateMode` reports `required` for a machine that has never held a session. A machine whose session later expired or became unreadable reports `recoverable` and always offers **Continue to your work**, because local work must never be blocked — `AccountSignedOutBanner` then nags on every surface until the user signs in. Resolving the gate is process-local so extra windows and renderer reloads do not repeat it.
- `renderer/components/onboarding/WelcomeVideoGate.tsx` — app-level one-time welcome card using the website's canonical desktop/mobile/terminal hero assets, a YouTube thumbnail button that hands off to the system browser (the packaged app's `file://` origin makes YouTube's embed player reject an actual iframe with error 153), and the ADE Mobile TestFlight QR/download/copy panel. Seen/dismissed state is stored in the global app state file, separate from per-project setup onboarding.

### 7.7 Shared renderer hooks

Cross-cutting interaction behaviour lives in `apps/desktop/src/renderer/hooks/` rather than being re-derived per component. Two hooks (both adapted from the MIT-licensed interior.dev component set) own interactions that hand-rolled copies kept getting subtly wrong:

- `useCopyToClipboard.ts` — the single owner of "copy, then flash **Copied**". It holds a monotonic run ticket so a slow write resolving after a newer one cannot clobber the newer flash, a mounted guard so a late resolve is a no-op, and one timer that is always cleared. It serves both shapes in the renderer: an unkeyed button (`copied`) and a keyed list where exactly one row may flash (`copiedKey` / `isCopied`). The default transport is `navigator.clipboard` with a hidden-textarea fallback for the Electron case where the document is not focused; panes that must write through the main process pass a `write` override, which then owns the whole write with no fallback behind it. Two contracts matter at call sites: copying empty text is a no-op that returns `false` rather than clearing the clipboard, and a failed write returns `false` instead of throwing — anything that shows a success message must gate on the returned boolean. All copy affordances across chat, PRs, settings, automations, CTO, onboarding, and remote-target surfaces route through it.
...
| API provider keys (project) | `.ade/secrets/api-keys.v1.bin` plus account vault | Credential store, then the macOS Keychain, then the environment. Account/device provenance; legacy plaintext `api-keys.json` is imported and removed |
| API provider keys (machine) | `~/.ade/secrets/` via `resolveMachineAdeLayout` | Same store, machine scope, device-only. Used by the CTO voice key, which follows the user rather than the project |
| Account settings cache | `~/.ade/account-settings.json` | Non-secret settings only; owner-tagged `0600` cache that survives sign-out and is discarded on account switch |
| Account vault cache | `~/.ade/account-vault.json.enc` | Atomic `0600` machine-key-encrypted cache; legacy plaintext is accepted only for migration and removed after encrypted write succeeds |
...
store is shared across processes; a `safeStorage` file is Electron-only, so
every credential that more than one process co-owns is pinned to the file store
by `FILE_BACKED_CREDENTIAL_KEYS`: the account session (`account.session.v1`),
its crash-safe rotation journal (`account.session.rotation.v1`, only meaningful
beside the session it describes), the sync bootstrap token
(`sync.bootstrapToken.v1`), and the GitHub App user credential
(`github.appUserToken.v1`). The App credential is there because it carries a
**rotating** refresh token and, beside it, the refresh ledger every ADE process
coordinates through — two copies of that record means two processes refreshing
the same rotating token, which GitHub answers by revoking the grant. See
[automations → How ADE stops a GitHub App refresh storm](./features/automations/README.md#how-ade-stops-a-github-app-refresh-storm).
The safeStorage migration retains those keys in the file store — copying
everything else across, pruning the migrated duplicates out of the file store,
and deleting the legacy files only when nothing is retained. Writing a pinned
key through the safeStorage store throws rather than silently signing the brain
out of a machine whose app is signed in, and a legacy store that cannot be
decrypted aborts the migration instead of being replaced with an empty one.

Pinning the write side is only half of it: the desktop app also has to **read**
those keys from the shared file. `createDesktopCredentialStore` therefore wraps
the Electron store in `createRoutedCredentialStore`, which sends file-backed
keys to the shared file and everything else to `safeStorage`. Without the
router, a desktop read of `account.session.v1` or the App credential went to
`credentials.safe.enc`, found nothing, and answered "not connected" about a
credential sitting in the file next to it. The router deliberately implements no
whole-map `updateSync`: a routed store spans two files, so any single-file
answer to a whole-map updater is wrong in one direction or the other. It routes
`updateKeySync` per key instead, and `getWithReadState` per call rather than
through the shared "last read" state, so an async caller learns the state of
*its* read. When `safeStorage` is unavailable the Electron half is replaced by
`createUnavailableCredentialStore`, which throws the reason on every call rather
than answering "no value" — an empty answer reads as "never connected" and
invites the user to reconnect over credentials that are still on disk — while
the file-backed keys stay reachable through the same router.

A build that already wrote a file-backed key into the Electron-only store left
it stranded there. `adoptFileBackedCredentials` runs once per secrets directory
per process and moves those records back. When both stores hold the key the
fresher record wins by its `updatedAt` (or `obtainedAt`), which is usually the
shared file's, because the brain writes there — but a desktop build that wrote
the App token into the Electron store left the *only* current copy there, and
keeping the older shared copy would hand GitHub a refresh token it has already
rotated away. The stranded copy is deleted only once the shared file holds one,
and when neither record says when it was written the stranded one is left where
it is: an undated record is not evidence that the shared copy is newer, and
deleting a secret on that guess is unrecoverable. A pass that could not read the
Electron store does not count as complete, so the next store built in the
process retries it.

Stores over one file must also agree on a **name** for it.
`credentialStoreIdentity()` reports the store's path through
`credentialPathKey`, which folds away redundant separators and `.`/`..`
segments, then folds the separator itself on Windows and the case on Windows and
macOS (Linux filesystems are case-sensitive, so that step is conditional). Two
spellings of one secrets directory reading as two different stores is not a
cosmetic problem: the GitHub App refresh coordinates through this identity, and
two identities for one file means two processes each believing they hold the
only refresh lease.

Every one of those writes goes through `updateCredentialKeySync`, one ladder for
changing a single key: `updateKeySync` (atomic for one key, and the only rung a
routed store can take), then `updateSync` wrapped over the whole map, then plain
get/set. Six call sites used to spell those three rungs by hand. A caller that
must not silently degrade to the third rung asks `supportsAtomicCredentialUpdate`
first — a compare-and-swap degraded to check-then-set is not a weaker version of
the same write, it is a different write that can clobber a peer.

The shared file store is sealed with the bare machine key, never with
...
**CSP** (`rendererCsp.ts`): `default-src 'self'`; `script-src 'self'` (no eval, no inline scripts); `style-src 'self' 'unsafe-inline'` (required for Tailwind); `connect-src 'self'`; `img-src 'self' data:` plus a host-scoped allowlist (no blanket `https:`) for the image origins PR/README surfaces actually load — the GitHub avatar/asset hosts (`*.githubusercontent.com`, `github.githubassets.com`, …) and `www.gravatar.com` / `secure.gravatar.com` (commit-author identicon fallback). `frame-src` is `'self'` plus local sources plus `FRAME_SRC_EXTRA_SCHEMES` (`ade-scene:`, `blob:`, `about:`); there is no external frame host, because the welcome video is now a thumbnail that hands off to the system browser. `ade-scene:` and `blob:` are there for agent-authored scenes and are deliberately absent from `img-src`, `media-src`, and `connect-src` — ADE's own document has no reason to fetch one (see [features/chat/README.md › Scenes](./features/chat/README.md#scenes)). The Electron header hook applies this policy only to ADE renderer main-frame documents; external subframes keep their own response CSP so embedded players can execute their host-provided scripts.

**Subframe navigation is a second door on the same allowlist.** `frame-src` bounds where a nested context may be *loaded from*; it says nothing about a frame navigating itself, which is how a sandboxed scene could exfiltrate by `location =` rather than by `fetch`. So `main.ts` registers a `will-frame-navigate` handler that ignores main frames and answers every subframe with `isRendererFrameNavigationAllowed(url, { rendererUrl, devServerUrl })` from `rendererCsp.ts` — the same `FRAME_SRC_EXTRA_SCHEMES` / `PACKAGED_FRAME_SCHEMES` literals `frame-src` is built from, so there is no hand-copied list to drift, and the refusal is logged as `window.frame_navigation_blocked`. It is tighter than `frame-src` in exactly one place on purpose: other local http ports are allowed only while a dev server is configured, because a packaged renderer frames nothing over http. A door narrower than the CSP is its own bug — the handler first shipped without `file:`/`app:`, which blanked every packaged spec preview.

Every IPC handler **validates** its arguments; invalid args return structured errors, never crash. Every handler has a **30s timeout** by default; `ipcTimeouts.ts` carries per-channel overrides for long-running operations (`ade.iosSimulator.launch` → 10 min) and reads the `{ domain, action }` payload of `localRuntime.callAction` / `remoteRuntime.callAction` so action-specific timeouts (e.g. `lane.delete` → 4 min) apply even when the channel itself is generic. Every handler emits structured tracing.
...
It is the local source the Connections panel can use for the This computer identity,
pairing code, and local Phone/Web device lists.
Renderers do not call it directly: `renderer/lib/localSyncStatusReader.ts` is the
single shared reader every subscriber goes through, so the several consumers that
all re-read on the same `sync-status` broadcast cost one invoke rather than N, and
a degraded local runtime is backed off instead of hammered. When the local runtime
cannot answer, main returns a synthesized machine-only snapshot carrying
`degradedReason` (`SyncRoleSnapshot`), which is what the renderer keys its backoff
on — route health is legitimately all-down on healthy machines that host nothing.
Main also rate-limits the matching `sync.local_status_degraded` warning to once a
minute per distinct reason, since a degraded runtime stays degraded for as long as
it takes a human to fix it.
...
paired-host fallback. Both shells chunk bulk acknowledgments through the shared
`runAcknowledgmentChunks` in `shared/types/attention.ts`, so neither can drift on
the 64-id batch bound, the abort-on-first-failing-chunk policy, or the
`acknowledged` / `stale` / `unreached` outcome.

`window.ade.account.*` gained the device-authorization login trio
(`ade.account.startDeviceLogin` / `pollDeviceLogin` / `cancelDeviceLogin`) and
`ade.account.repairMachinePairing`. The device flow exists alongside the loopback
one because only it passes through ADE's account directory, which is what lets
the directory mint the pairing grant a removed machine needs; the repair channel
is what backs "Reconnect this computer" on the Account page, the shell bar, the
Connections pane and the Machines list.

Account settings are exposed through the typed
`window.ade.accountSettings.*` channels (`list`, `get`, `set`, and `sync`).
They return an unavailable result when the brain is offline rather than
pretending that a local read is an account write. The renderer never receives a
vault value through this bridge: account credentials use the brain-backed
`account_vault` action domain, whose `list`, `get`, `set`, `remove`, and `sync`
operations are policy-gated separately from the non-secret
`account_settings` domain. Desktop main services reach the vault through
`accountVaultBridge`, and the account migration runner hydrates provider keys,
Linear refresh credentials, and repository-scoped project secrets after sign-in.
...
- Role validation: only `cto`, `agent`, `external`, `evaluator` accepted.
...
- **No config arrives from a repository**: `.ade/ade.yaml` is not read. A legacy committed file is carried over into `local.yaml` once — only `project`, `environments`, `github`, `ai`, `linearSync`, `ui`, and `browser`; executable `testSuites`, `laneOverlayPolicies`, `automations`, `laneEnvInit`, `laneTemplates`, `defaultLaneTemplate`, `git`, and `laneCleanup` are dropped — and then deleted. A clone therefore cannot supply a `command`, a `setupScript`, or an agent prompt, and there is no trust gate to approve.
- **Provider sign-in relays, never stores**: Pi sign-in runs Pi's own `ModelRuntime.login` in a worker; Pi's `AuthStorage` writes `auth.json` under its cross-process lock. ADE receives prompt text only, passes the user's answer straight back, and keeps nothing on the status event. `ade.ai.piLoginSubmit`'s `value` argument is in the per-channel redaction map because for an API-key prompt that argument *is* the credential. That map lives in `services/ipc/ipcChannelRedaction.ts` (`ipcChannelRedactionMap` + `redactIpcArgsForChannel`) rather than inside `registerIpc.ts` so the contract is reachable from a test — a channel silently dropping out of it leaks into verbose IPC traces and nothing else fails.
- **IPC trace redaction is two rules, kept visibly apart.** `ipcChannelRedactionMap` is the specific statement, and it names the three key-store channels (`ai.storeApiKey`, `ai.storeMachineApiKey`, `ai.setOpencodeProviderKey`, all on `key`) and the three project-secret channels (`projectSecrets.set` on `value`, `previewEnvImport` on `content`, `importEnv` on `secrets`) alongside the terminal, browser, and app-control channels. `shouldRedactIpcKey` is the backstop for a channel nobody remembered to list: a SUBSTRING match for a family of names (`token`, `secret`, `password`, `authorization`) and an EXACT match for one that would over-match (`apikey`, `api_key`, `pairingpin`, `pairing_pin`). A bare `key` is deliberately **not** in the backstop — it is a credential on the key-store channels, which the map names outright, and the ordinary word for a lookup key everywhere else, where blanking it generically would empty the traces that exist to explain those calls.
- **Untrusted project code stays unexecuted**: the Pi worker constructs Pi's `SettingsManager` with `projectTrusted: false` and passes it to both the resource loader and the session, so only the user's own `~/.pi/agent/extensions` can load — never a checked-out repository's `.pi/extensions`. If a Pi build will not accept that flag, extensions are disabled rather than loaded unconstrained.
...
- `GitRunOptions` also takes `onStderrLine` and `signal`. `onStderrLine` receives each stderr line as it arrives, splitting on both `\r` and `\n` because git redraws progress in place; a throwing observer never breaks the git call, and an unterminated line is capped so it cannot grow without bound. `parseGitCheckoutProgressLine` turns `Updating files:  78% (2104/2700)` into `{ percent, completed, total }` — lane creation uses it with `GIT_PROGRESS_DELAY=0` so checkout progress streams through a pipe. Aborting `signal` kills the git process tree (`SIGKILL`) and resolves with exit code 130 only after the process has exited (or a 2 s safety window), so a caller cleaning up after a cancelled `worktree add` never races a git still writing files; an already-aborted signal (e.g. cancelled while queued for a git slot) never spawns git. `resolveGitCommit(ref, cwd)` returns the sha `ref^{commit}` resolves to, or null.
...
- High-level ops in `gitOperationsService.ts` — wrap every mutation in `runLaneOperation()`: resolve lane, capture pre-HEAD, record operation, execute, capture post-HEAD, finalize record, fire `onHeadChanged` if needed. Its `getSyncStatuses` method accepts lane IDs once and returns the existing `getSyncStatus` shape keyed by lane ID.
...
- **GitHub stacked PRs** (`githubStackStore.ts`) — repository-scoped stack reconciliation and membership snapshots.
...
Channels (under `ade.computerUse.*`; preload namespace `window.ade.computerUse`):

- `ade.computerUse.listArtifacts`, `ade.computerUse.getOwnerSnapshot`, `ade.computerUse.deleteArtifacts`, `ade.computerUse.listBrokenArtifacts`, `ade.computerUse.pruneBrokenArtifacts`, `ade.computerUse.recoverArtifact`, `ade.computerUse.updateArtifactReview`, `ade.computerUse.readArtifactPreview`, plus a `ade.computerUse.event` push channel.
- `ade.computerUse.mediaBaseUrl` returns the base URL of main's loopback media server (`artifactMediaServer.ts`). Proof videos play from it, because Electron's `protocol.handle` cannot serve the Range reads a seekable `<video>` makes. The server listens on `127.0.0.1` and every path starts with a per-launch token.
...
- **Machine power and sleep**: a host publishes its own battery/wall-power reading and an explicit `awake`/`asleep` state into the account directory (`power`, `sleep_state`, `sleep_state_at`, migration `0006`), so clients never have to infer a closed lid from silence. Detection is deliberately two-tier: the OS pre-suspend event where one exists (Electron `powerMonitor` on the desktop, forwarded to the brain as `machine.reportPowerTransition`), plus a heartbeat-gap detector as the universal fallback — a 15-second tick that fires more than 60 seconds late means the machine was out. The gap detector needs no platform-specific code, which is what gives a headless brain and Linux sleep detection at all. `resolveMachinePresence` (`apps/desktop/src/shared/types/power.ts`) is the single presence decision every client renders from, and a fresh `asleep` announcement outranks `connected`, because a channel to a sleeping machine does not report itself closed. See [sync and multi-device](./features/sync-and-multi-device/README.md#account-directory-and-connection-leases).
- **Independent desktops**: a second computer can run its own ADE runtime and work independently through git without joining an ADE sync session. The tracked `.ade/` scaffold/config layer makes a clone look like an ADE project immediately.
...
- Every fresh signed-out app launch starts at sign-in, which is the primary,
  PIN-less path through the account directory and Relay. A phone already paired
  to a computer also offers **Continue to your work**, which keeps QR + PIN,
  Nearby + PIN, and the advanced SSH bootstrap available; a phone with no paired
  host has nothing to pass through to and is offered nothing. An unreadable
  pairing credential offers **Pair again** instead. There is no pairing-link
  paste or manual address + PIN surface. Signed-in launches enter directly.
...
- Sub-protocols: changeset sync, project catalog/switch, file access,
  ...
  all-projects chat roster feed backing the Hub, pushed new-lane launch
  progress (`chat_launch_event`, sent to every peer of the project because a
  launch is visible before its chat exists), command routing, and
...
- Widgets: `ADELockScreenWidget` is one definition serving the accessory families and the Home Screen families (`ADEActivityHomeWidget` draws the small/medium/large views); it prefers the account Activity snapshot and falls back to the shared `WorkspaceSnapshot` in the App Group container through `LockScreenPriorityStatus`. Every surface grades snapshot age as fresh / aging (10 min) / untrusted (2 h) and shows a staleness tag, fed by a dedicated `com.ade.ios.activity.refresh` background task plus silent pushes. `ADEAgentActivityWidget` registers an ActivityKit Live Activity + Dynamic Island for active agent runs, leading with the state-group tally. Control Center surfaces are not registered.
...
- Account-owned provider keys, Linear OAuth refresh credentials, and account
  project secrets follow the signed-in account through the encrypted vault;
  local stores retain provenance and purge account-origin values on sign-out or
  account switch. GitHub tokens, vendor CLI refresh credentials, and other
  device-only credentials remain on the host.
...
2. **Parallel checks**:
   ...
   - `typecheck-ade-cli` — `cd apps/ade-cli && npm run typecheck`, then a `node --test` release-guards step covering the runtime archive/packaging validators, the docs validator, and the POSIX standalone installer's staging and rollback paths (`install-runtime-rollback.test.mjs`).
...
- `validate-mac-artifacts.mjs` / `validate-win-artifacts.mjs` — per-platform artifact validators; confirm expected binaries, release signing state, bundled ADE CLI help, isolated ADE Code TUI help, PTY/provider resolution, CR-SQLite mutation, updater authority, and every required bundled ADE Agent Skill `SKILL.md`. They also fail if the bundled TUI references `__dirname` / `__filename` without ESM shims. Windows signing verification is mandatory in `dist:win:signed` and opt-in for lower-level validation with `--require-signed` or `ADE_REQUIRE_WIN_SIGNING=1`.
- `validate-packaged-tree.mjs` — the assertions both per-platform validators share, extracted so they cannot drift: artifact size budgets, the bundled agent-skill roster, the node-pty addon search, and the rule that no runtime-fetched agent-tool package may ship (checked in `app.asar.unpacked` *and* inside `app.asar`). Each validator injects its own `fail` so its CI output keeps its own prefix.
- `runtime-resource-targets.cjs` / `assert-runtime-resource-target.mjs` — the runtime sidecar target set and its pre-build gate (see 2.1 "Bundled runtime artifacts").
- `runtime-fetched-tool-packages.mjs` — the exact-name set of agent-tool platform packages that are fetched at runtime rather than bundled; `after-pack-runtime-fixes.cjs` prunes them and then throws if any survives. See `docs/features/onboarding-and-settings/agent-tools-cache.md`.
- `artifact-size-budget.cjs` / `assert-artifact-size-budget.mjs` — post-packaging size ceilings (800 MiB per macOS ZIP, 900 MiB per Windows installer), run by `npm run assert:artifact-size` in both release jobs. The macOS number is a crash guard, not hygiene: Squirrel.Mac buffers the whole ZIP and dies past ~1 GiB. See `docs/features/onboarding-and-settings/desktop-auto-update.md`.
...
- Per-lane macOS screen · [Mac Desktop](./features/mac-desktop/README.md)
