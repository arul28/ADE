...
**Optional ADE account auth.** `ade login` preserves the local-browser loopback OAuth path, but selects the account-directory device authorization bridge for explicit `--headless`, SSH, display-less hosts, or a failed browser launch. The brain generates and retains the device redemption secret, polls the bridge, and persists the resulting refresh-capable session under `account.session.v1`. `ADE_ACCOUNT_TOKEN` takes precedence without starting a login flow: JWT access credentials are used through their declared expiry, while opaque refresh credentials are exchanged and rotated only in memory. `ade account token create` prints the current interactive refresh credential once for secret-manager provisioning. Account status exposes `loopback`, `device`, or `env-token`; signed-out state never gates local projects, `ade code`, or PIN workflows.
...
Scheduled-work recovery is part of that typed chat family: `ade chat scheduled-work list [session] [--all]` and `ade chat scheduled-work cancel <session> <id>` call the explicitly adapted `chat.listScheduledWork` / `chat.cancelScheduledWork` actions. The list reads the KV-backed management snapshot rather than reconstructing ownership from transcript events; provider-owned cancellation is reported as requested vs confirmed instead of being hidden optimistically.
...
ade.agentChat.*              # agent chat sessions, model inventory, parallel launch state.
                             ...
                             # listScheduledWork, cancelScheduledWork, and
