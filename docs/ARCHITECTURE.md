...
ade.builtInBrowser.*         # in-app web browser owned by `builtInBrowserService`: , plus the ade.builtInBrowser.event push channel (status / open-request / selection / selection-cleared / error). Backs the Work sidebar's Browser tab, personal chat's explicitly global profile, and the renderer-wide `openUrlInAdeBrowser()` link router. Profile diagnostics and permission administration are rejected for chat-bound agents.
