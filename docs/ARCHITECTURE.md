...
| `ios/` | `iosSimulatorService.ts`, `simulatorWindowCapture.ts`, `iosDeviceHub.ts`, `iosDeviceTools.ts`, `iosEventLog.ts`, `iosSemanticActions.ts`, `iosVideoStreamServer.ts`, `h264AnnexB.ts` | macOS-only iOS Simulator backend: tool readiness probes, simctl device + app discovery, build/install/launch with progress events (hardened with `simctl bootstatus` and `simctl install` timeouts), screenshot + ADEInspector + accessibility hit-test, Simulator.app window live-view status, idb-backed input, and single-owner chat session locking. Window placement, parking, Screen Recording / Automation probes, and System Settings openers live in `simulatorWindowCapture.ts`. `iosDeviceHub.ts` owns the device half — app-free device sessions, device tools, the event log, semantic element actions, and proof bundles — over `iosDeviceTools.ts` (one `simctl` call per tool, plus the `notifyutil` announcement the accessibility preferences need), `iosEventLog.ts` (the `log stream` reader and the ring that interleaves device rows with ADE rows), and `iosSemanticActions.ts` (pure element matching and the `id:`/`component:`/`label:`/`pos:` ref tiers). `iosVideoStreamServer.ts` serves the `idb-h264` live view over a token-guarded loopback endpoint, framed by the pure `h264AnnexB.ts`. Every device-hub method sits behind the same darwin gate as the rest of the surface and rejects elsewhere, while `getStatus` and the chat-close release run on every platform. `registerIpc.ts` keeps the handlers because they need the active `BrowserWindow`. See [features/ios-simulator/README.md](./features/ios-simulator/README.md). |
