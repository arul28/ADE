...
- Opens WebSocket candidates after direct TCP ranking, then races the authenticated hellos (250 ms stagger, at most three candidates, 10-second budget) across route diversity — a dead LAN IP no longer delays the live Tailscale/Relay route, and only `hello_ok` can win. Sends local `db_version` plus the per-host-DB cursor map (`remoteDbVersionBySite`); host replies with its `serverDbSiteId` and sends catch-up changesets.
