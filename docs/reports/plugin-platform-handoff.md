...
Today's inventory (`plugins/`): ade-linear (real, supersedes, Unit A landed), ade-cursor-cloud (real, supersedes; remaining optional gap: PX-11 tab badge), ade-review (real, supersedes; engine stays in core), ade-history (real, supersedes; git/operation engines stay in core), ade-graph (real, supersedes; React Flow engine stays in core), ade-ios-sim / ade-app-control (`enables` gating shells with 0 lines of code — they only unlock compiled surfaces), ade-log-viewer (489 lines), ade-voice (1.7k lines), themes.
Required end state for each of ios-sim and app-control: a real plugin that re-implements the compiled feature on desktop AND mobile (where the compiled feature exists on mobile), flipped to `supersedes` exactly like Linear: compiled stays in the install; plugin install hides every compiled surface and the plugin's version stands in its place; disable brings the compiled version back.
First step per shell: a parity map like `docs/reports/linear-plugin-parity-map.md`. Canvas language for History and Graph is landed: vocabulary `canvas` with host engines `git-dag`, `swimlane`, `graph`, and `workspace` (data-only; desktop paints, phone/TUI list). History uses `git-dag`. Graph uses `workspace` to mount the compiled React Flow page. Remaining: ios-sim and app-control still need a process/stream seam, or a declared capability the host brokers. "Expand language or logic as needed" is the owner's explicit permission to grow the platform.
