...
## Handoff commit
`54df53935` at ticket creation. LANDED: logging.md Plugins section (d23d957fc); plugins README waves 1-3 (3e6e3be99); TriggerCard builtin gating (e8654e229). IN FLIGHT: the rest of the scoped /quality fixes (plugin view/model seam blockers B1-B3, H1-H4, M3-M5, M8 in plugins/ade-linear; H6, M1, M6, M7 desktop/CLI) and a scoped `/quality` pass over `c35bc0b88^..HEAD`. If they landed, later commits on the branch and a comment on this ticket say so. If no later commit exists, the docs are NOT updated and quality did NOT run — do them first. `/test` was NOT run at the handoff (owner.s call).
