...
Canonical durable memory store for project-, mission-, and agent-scoped memories. Retrieval uses hybrid FTS4 BM25 (30%) + cosine similarity (70%) + MMR re-ranking when embeddings are available, with graceful fallback to lexical/composite scoring. Memory lifecycle sweeps (temporal decay, tier demotion, hard limits, orphan cleanup) and batch consolidation (Jaccard trigram clustering + LLM merge) keep the store healthy.
...
Stores 384-dim vectors generated by `@huggingface/transformers` (all-MiniLM-L6-v2) running locally. Background `embeddingWorkerService.ts` processes new entries asynchronously and backfills existing ones on startup. Used by the hybrid retrieval pipeline for cosine similarity scoring and MMR re-ranking.
