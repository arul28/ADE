...
<AccordionGroup>
  ...
  <Accordion title="Reusable templates" icon="copy">
    ...
    The setup script runs last, after every other init step, as shell commands in the lane's worktree — so pipes, `&&`, and variables work. Your commands see the lane's runtime variables (`PORT`, `PORT_RANGE_START`, `PORT_RANGE_END`, `HOSTNAME`, `PROXY_HOSTNAME`, `LANE_NAME`, `LANE_SLUG`, `LANE_BRANCH`, `LANE_WORKTREE`), plus `$PRIMARY_WORKTREE_PATH` when **Expose primary lane path** is on. Commands run in order and stop at the first one that fails; a script file that is configured but missing counts as a failure.
  </Accordion>
