...
<Tip>
Stop a lane's dev server when you're done with it, or delete the lane — both return its leased ports to the pool. Archiving a lane also brings down the Docker Compose services its env init started, and unarchiving starts them again — the whole env-init sequence when ADE had to recreate the worktree, and just the Docker step when it did not, so your env files and installed dependencies are left alone. Use **Init env** only when you want the full sequence re-run. If a lane ever flaps unhealthy on a slow cold start, the health bar settles once the server finishes binding.
</Tip>
