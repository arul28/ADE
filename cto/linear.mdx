...
The CTO is the owner of ADE's bidirectional Linear integration. Once connected, the CTO monitors your Linear workspace for issue changes, routes incoming issues to the right execution target (worker, mission, or PR convergence), and pushes updates back to Linear when work completes.

This is not a simple webhook mirror — the CTO makes intelligent decisions about how to handle each issue based on configurable workflow definitions. It tracks issue state transitions, matches triggers against assignees/labels/priorities, and orchestrates the full lifecycle from intake to closeout.
