...
The CTO owns ADE's bidirectional Linear integration. It monitors your workspace for issue changes, routes them to the right execution target (worker, worker run, or PR issue resolution), and pushes updates back when work completes — tracking the full lifecycle from intake to closeout based on configurable workflow definitions.
...
<Steps>
  ...
  <Step title="Configure workflow definitions">
    ...
    - **Execution target** — what ADE does (launch a worker run, start a worker session, or route to a review gate)
...
| Target | When to use |
|--------|-------------|
| **Worker run** | Complex issues that need a structured plan, multiple steps, and validation |
...
| **PR resolution** | Issues that map directly to an existing PR that needs issue resolution |
...
<Steps>
  ...
  <Step title="Dispatch">
    The dispatcher creates the execution target — starts a worker run, opens a chat session, or launches a worker run. The issue's description, comments, and attachments are injected as context.
  </Step>
  <Step title="Tracking">
...
- **Proof artifacts** — attach worker run artifacts, test reports, or screenshots as Linear attachments or comment links
...
| **Best for** | Ongoing issue management, complex routing, multi-step workflows | Simple notifications, one-shot tasks, scheduled checks |
