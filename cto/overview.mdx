...
<CardGroup cols={2}>
  <Card title="Persistent identity" icon="id-badge">
    One CTO per project, with continuity that survives long conversations — not a fresh chat each time. Its voice is fixed by ADE, so it reads the same on day one and day ninety.
  </Card>
  ...
  <Card title="Knows ADE itself" icon="circle-question">
    It knows the application, not only your repository. Ask where a setting lives or why a control is disabled and it answers from ADE's own documentation, with a deeplink to the place.
  </Card>
</CardGroup>
...
You can change the CTO's model at any point from **Settings → Model**. The thread, its transcript, and its memory carry over to the new model — a switch is not a reset. Favor a capable model for project-wide reasoning; drop to a faster one for quick lookups.

The list is narrower than elsewhere in ADE on purpose: the CTO only runs on models that can take a message into a turn already underway, because it is interrupted constantly by the chats it starts and by its own wake-ups.

<CardGroup cols={2}>
  <Card title="Set up the CTO" icon="wand-magic-sparkles" href="/cto/setup">
    Pick a model, then optionally tune reasoning effort and connect Linear.
  </Card>
