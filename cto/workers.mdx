...
3. The CTO reads the PR diff (via the GitHub integration ADE CLI tool), cross-references it against its project memory, checks for conflicts with active lanes, and returns a structured review with: summary, concerns, risk level, and a recommendation.
...
If you have an OpenClaw account or a compatible external agent platform, you can connect it to ADE. The CTO acts as the designated router for inbound requests from OpenClaw:

1. In **Settings > CTO > External Connections**, enable **OpenClaw Bridge**
2. ADE generates a local RPC endpoint URL for OpenClaw to connect to
3. OpenClaw sends development requests to ADE by addressing them to the CTO via the `cto.request` RPC action
