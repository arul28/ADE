...
<Frame caption="CTO chat — delegation plan, worker created, active status with lane link">
  <img
    src="/images/cto-chat-delegation-worker-feature-lane.png"
    alt={`A CTO chat thread showing a delegation workflow — the user's message at top, the CTO's response explaining its plan (including "I've created a worker 'feature-impl-001' in lane/user-preferences"), then a status update message showing the worker is active with a link to the lane.`}
  />
</Frame>
...
<Frame caption="CTO Settings — External Connections, OpenClaw, masked MCP URL">
  <img
    src="/images/cto-settings-external-connections-openclaw.png"
    alt={`The CTO > Settings sub-tab showing the "External Connections" section with OpenClaw listed, a toggle switch, a "Connected" badge, and a masked MCP endpoint URL with a copy button.`}
  />
</Frame>
