...
<Info>**Screenshot: A CTO chat thread showing a delegation workflow — the user's message at top, the CTO's response explaining its plan (including "I've created a worker 'feature-impl-001' in lane/user-preferences"), then a status update message showing the worker is active with a link to the lane.</Info>
...
<Info>**Screenshot: The CTO > Settings sub-tab showing the "External Connections" section with OpenClaw listed, a toggle switch, a "Connected" badge, and a masked MCP endpoint URL with a copy button.</Info>
