...
**Feature Documentation** (in `newDocs/features/`):
...
**Architecture Documentation** (in `newDocs/architecture/`):
...
| 3 | Files Tab + UI Polish | NOT STARTED | File explorer (Zed-inspired), Monaco editor, diff modes, Run tab rename, lane selector, guest mode, untracked sessions |
...
**Task References**:
- FILES-001 through FILES-021: All TODO
  - Phase 3a (File Tree + Basic Editor): FILES-001, FILES-002, FILES-003, FILES-004, FILES-005, FILES-006, FILES-007, FILES-008, FILES-009, FILES-013, FILES-014, FILES-015, FILES-018
  - Phase 3b (Diff + Conflict Modes): FILES-010, FILES-011, FILES-012, FILES-017
  - Phase 3c (Advanced Editor): FILES-016, FILES-019, FILES-020, FILES-021
- TERM-024: Terminal theme sync
- TERM-032: Untracked session mode
- LANES-034: Keyboard shortcuts for lane navigation
- LANES-035: Lane search/filter
- PROJ-033: Tab rename to "Run" with play/pause nav icon
- PROJ-034: Lane selector for command execution context
- ONBOARD-025: Guest mode (no-account usage)
- ONBOARD-026: Guest mode banner
- PACK-030: Guest mode template narratives

**New Services Required**:
- File tree listing service (recursive directory listing, gitignore filtering, lazy loading)
- File watching service (chokidar-based, debounced, gitignore-aware)

**IPC Channels** (new):
- `ade.files.listTree`, `ade.files.readFile`, `ade.files.watchChanges`, `ade.files.stopWatching`
...
*This document is the authoritative implementation plan for ADE. It is maintained alongside the feature and architecture documentation and updated as phases are completed. All task IDs referenced here are defined in their respective feature documents under `newDocs/features/`.*
