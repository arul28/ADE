...
<Info>**📸 Screenshot needed:** The `.ade/` directory expanded in a file tree, showing `ade.yaml`, `local.yaml`, `local.secret.yaml`, and the `worktrees/` and `artifacts/` subdirectories. Overlay a badge reading "commit this" on `ade.yaml` and "gitignore" on the two `local.*` files.</Info>
...
<Info>**📸 Screenshot needed:** The config approval dialog — show a diff view with old and new `ade.yaml` side-by-side, the new lines highlighted in green, and "Approve" / "Reject" buttons at the bottom. A warning banner at the top reads "Config changed — commands paused until you approve."</Info>
...
nightShift:
  idleThresholdMinutes: 15              # Minutes idle before Night Shift activates
  runWindowStart: "23:00"               # Earliest Night Shift can start
  runWindowEnd: "06:00"                 # Latest Night Shift runs until
...
<Info>**📸 Screenshot needed:** The Settings → Config panel showing the three config files listed with their edit buttons, the "Approved SHA" display showing the current hash and a green checkmark, and a "Re-approve current config" button.</Info>
