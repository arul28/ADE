...
The `worktrees/` and `artifacts/` subdirectories are fully managed by ADE. Do not edit files inside them directly — use ADE's UI or the `ade` CLI instead.
...
This file holds your API keys and service credentials. It is always gitignored. Only the Electron main process reads it — the renderer (React UI) never has access to its raw contents.
