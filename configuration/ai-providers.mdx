---
title: "AI Providers"
description: "Configure Anthropic, OpenAI, OpenRouter, OpenCode, and local models in ADE. Manage API keys, set budget caps, and control which model each agent uses."
icon: "microchip"
---
## Supported Providers
...
Chat and Missions share the same permission picker. Each provider exposes a small set of presets that map directly to provider flags.
...
There is no separate "Guarded edit" preset anymore. The previous Guarded edit preset is replaced by **Default permissions**, which is the same workspace-write sandbox but with the `on-request` approval policy rather than `on-failure`. The Codex sandbox in Missions is derived from the chosen mode — the separate sandbox dropdown has been removed from the Worker Permissions editor.
...
<Tip>
For solo developers running many automations, routing automation runs to `claude-haiku-4-5` and reserving `claude-opus-4-7` for mission planning can reduce monthly spend by 60–80% with minimal quality impact on routine tasks.
</Tip>
