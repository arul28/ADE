...
<CardGroup cols={3}>
  ...
  <Card title="Integrations" icon="plug" href="#integrations">GitHub and Linear.</Card>
  ...
  <Card title="Notifications" icon="bell" href="#notifications">Native, in-app, and mobile push delivery.</Card>
  <Card title="Mobile Push" icon="mobile" href="#mobile-push">iOS push notifications for agents.</Card>
  <Card title="Diagnostics" icon="stethoscope" href="#diagnostics">Runtime health and diagnostics dashboard.</Card>
...
| Event | macOS notification | In-app banner | Mobile push |
|-------|-------------------|---------------|-------------|
...
<Steps>
  <Step title="Open Settings → Mobile Push">
    Navigate to **Settings → Notifications → Mobile Push**.
  </Step>
...
The diagnostics dashboard (**Settings → Diagnostics**) gives a live view of ADE's internal runtime health.
...
Use the diagnostics panel before filing a bug report — the summary output can be copied directly from the panel.
...
<Warning>
`verbose` log level logs every ADE RPC tool call including parameters. Do not use verbose logging in production environments — it may produce large log files and can expose non-secret parameters in plaintext. API keys are always redacted regardless of log level.
</Warning>
...
<Tabs>
  ...
  <Tab title="Power User">
    ...
    - Enable verbose RPC debug logging only when diagnosing specific issues
  </Tab>
</Tabs>
