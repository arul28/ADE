...
<Info>**Screenshot: The Budget & Usage settings panel showing: a "Current Month" spending summary card ($34.20 of $100.00 monthly cap), a stacked bar chart breaking down spend by provider (Anthropic, OpenAI), and below it a table showing spend by lane for the last 7 days.</Info>
...
<Info>**Screenshot: The Developer settings panel showing: a "Log Level" dropdown set to "info", a "Log file location" path field with a folder-open button, an "Open DevTools" button, a "MCP Debug Mode" toggle (off), and a "Database Inspector" button that opens a read-only SQLite browser.</Info>
