...
<Frame caption="Settings — full left nav with General active and content on the right">
  <img
    src="/images/settings-page-sidebar-all-sections.png"
    alt={`The Settings page with the left navigation sidebar visible, showing all section categories listed (General, AI Routing, Context, Budget & Usage, AI Providers, Integrations, Computer Use, CTO, Automations, Permissions, Notifications, Developer). The "General" section is active and its content is shown on the right.`}
  />
</Frame>
...
<Frame caption="Budget & Usage — monthly cap card, provider chart, spend by lane table">
  <img
    src="/images/settings-budget-usage-spend-summary-chart.png"
    alt={`The Budget & Usage settings panel showing: a "Current Month" spending summary card ($34.20 of $100.00 monthly cap), a stacked bar chart breaking down spend by provider (Anthropic, OpenAI), and below it a table showing spend by lane for the last 7 days.`}
  />
</Frame>
...
<Frame caption="Developer settings — log level, paths, DevTools, MCP debug, DB inspector">
  <img
    src="/images/settings-developer-panel-log-devtools-mcp-db.png"
    alt={`The Developer settings panel showing: a "Log Level" dropdown set to "info", a "Log file location" path field with a folder-open button, an "Open DevTools" button, a "MCP Debug Mode" toggle (off), and a "Database Inspector" button that opens a read-only SQLite browser.`}
  />
</Frame>
