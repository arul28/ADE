---
title: "Settings"
description: "Complete reference for every section in ADE Settings — General, Appearance, Workspace, AI, Sync, Mobile Push, Integrations, Memory, Lane Templates, and Usage."
icon: "sliders"
---
...
Settings are organized into ten sections in the left navigation panel. Changes take effect immediately in most sections — no restart required. Exceptions are noted in the relevant section below.
...
<CardGroup cols={3}>
  ...
  <Card title="Sync" icon="desktop" href="#sync">Multi-device sync, device registry, pairing.</Card>
  <Card title="Mobile Push" icon="mobile" href="#mobile-push">iOS push notifications for agent events.</Card>
...
<Note>
Mobile Push is a local-only section that only appears in the desktop app (it manages device registration on this machine). All other sections sync across paired devices when multi-device sync is enabled.
</Note>
...
Control ADE's multi-device sync. Pair additional machines or an iPhone so state replicates between them.
...
| Pair new device | Generate a QR code or a short pairing code for another machine | — |
| Sync transport | How changes are exchanged (local WebSocket, relay) | Local |
| Sync history | Per-domain last-synced timestamps | — |

<Note>
Sync uses cr-sqlite for CRDT-based replication. Missions, lanes, memory, and session history all replicate. `.ade/artifacts/` blobs are fetched on demand by the requesting device.
</Note>
...
ADE can send push notifications to your iPhone when agents need input or complete tasks, so you do not have to keep the desktop app in the foreground to stay aware of progress.

<Note>
This section is local to the desktop install (it manages device registration on this machine). It is hidden on builds that do not ship the iOS companion.
</Note>
...
<Steps>
  ...
  <Step title="Choose which events trigger a push">
    Toggle individual event types. The most useful defaults: mission complete, mission failed, and intervention required (agent needs your input).
  </Step>
</Steps>

You can register multiple iOS devices. Each device receives the same notifications. Devices are listed in the Mobile Push panel with their registration date and last-received timestamp. Tap **Remove** next to any device to deregister it.
...
<Tabs>
  ...
  <Tab title="Power User">
    ...
    - Pair multiple devices and an iPhone for always-on status awareness
  </Tab>
</Tabs>
