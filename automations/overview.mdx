...
<Frame caption="Automations tab — list with trigger, executor mode, and status">
  <img
    src="/images/automations-tab-active-rules-list.png"
    alt="The Automations tab showing a list of active automations with name, trigger type, executor mode, and status columns."
  />
</Frame>

## Overview

Automations is ADE's background execution layer. Each automation is a simple rule:

> **Trigger** → **Execution**

When a trigger fires (time-based or action-based), ADE executes it through one of three surfaces:
...
These execution types are configured per rule so you can choose the right level of cost, visibility, and reliability for each scenario.
