...
Measured against Agent SDK 0.3.258, and not re-measured against a later pin: `allowedTools` and `disallowedTools` are enforced, because the CLI removes a denied tool from the model's catalog. The `canUseTool` callback, however, did not fire on any permission mode tried, so the prompt path cannot be relied on to enforce anything.
