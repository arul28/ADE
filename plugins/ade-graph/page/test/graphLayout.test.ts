/**
 * The compiled `graphLayout.test.ts`, over the ported copy.
 *
 * Every case is the app's own, unchanged: the module is pure and moved verbatim,
 * so a difference here is a porting error rather than a design decision.
 */

import { describe, expect, it } from "vitest";

import type { LaneSummary } from "../src/lib/types";
import { computeAutoLayout, laneHierarchyFromPrimary, normalizeGraphPreferences } from "../src/lib/graphLayout";

function lane(
  partial: Partial<LaneSummary> & Pick<LaneSummary, "id" | "name" | "laneType" | "parentLaneId">,
): LaneSummary {
  return {
    description: null,
...
describe("normalizeGraphPreferences", () => {
  ...
  it("migrates legacy preset state to the last view mode", () => {
    const legacyState = {
      ...
      presets: [{ name: "Risk preset", byViewMode: { risk: { viewMode: "risk" } } }],
    };
...
describe("laneHierarchyFromPrimary", () => {
  ...
  it("marks lanes not under primary with a large sentinel depth", () => {
    ...
    expect(laneHierarchyFromPrimary(lanes).depthByLaneId.get("o")).toBe(10_000);
  });
...
describe("computeAutoLayout aligned modes", () => {
  it("matches stack and risk positions to overview for the same lanes", () => {
    ...
    expect(computeAutoLayout(lanes, "stack", {}, {})).toEqual(allPos);
    expect(computeAutoLayout(lanes, "risk", {}, {})).toEqual(allPos);
  });
  ...
  it("activity mode sorts siblings by activity score within the same depth", () => {
    ...
    const pos = computeAutoLayout(lanes, "activity", { low: 1, high: 99 }, {});
