 ...
 *
 * Ported from `components/graph/graphNodes/PluginNode.tsx`, unchanged except
 * that `pluginIcon` and the tone table are the page's own narrowed copies.
 *
...
import { COLORS, cn } from "@ade-dev/ui";

import { pluginIcon } from "../pluginIcon";
import type { GraphNodeData } from "../../lib/graphTypes";
import type { PluginBadgeTone } from "../../lib/pluginGraphNodes";

/** The five tones, over the kit's `COLORS` — the same table `socketUi.tsx` holds. */
...
export function GraphPluginNode({ data, selected }: NodeProps<Node<GraphNodeData>>) {
  ...
  return (
    ...
    >
      ...
      {payload.detail ? <div className="mt-0.5 truncate text-[10px] text-muted-fg">{payload.detail}</div> : null}
      <div
