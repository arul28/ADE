/**
 * Every line on the canvas, in one edge renderer.
 *
 * Ported from `components/graph/graphEdges/RiskEdge.tsx`, unchanged except that
 * `APP_FONT_STACK` comes from `@ade-dev/ui` rather than the app's
 * `laneDesignTokens` — the same stack, single-sourced in the kit.
 */

import { Position, getBezierPath } from "@xyflow/react";
import type { Edge, EdgeProps } from "@xyflow/react";
import { APP_FONT_STACK } from "@ade-dev/ui";

import { riskStrokeColor, prOverlayColor, prCiDotColor } from "../../lib/graphHelpers";
import type { GraphEdgeData } from "../../lib/graphTypes";
...
export function RiskEdge(props: EdgeProps<Edge<GraphEdgeData>>) {
  ...
  if (edgeType === "plugin") {
    ...
    // rather than red, the same ceiling every other plugin surface puts on a
    // tone: a plugin does not get to paint a lane as failed.
  ...
  const effectiveOpacity = data?.dimmed
    ? 0.16
    : data?.highlight
      ? 1
      : pr?.activityState === "stale" || data?.stale
        ? 0.38
        : 0.92;
    ...
    <g>
      ...
      {pr ? (
        ...
        >
          ...
          />
          <text x={-badgeWidth / 2 + 18} y={3} fontSize={10} fill="var(--color-fg)" fontFamily={APP_FONT_STACK}>
            {badgeText}
          </text>
          <text
            x={badgeWidth / 2 - 8}
