/**
 * Pairwise conflict risk, as a grid.
 *
 * Ported from `components/graph/shared/RiskMatrix.tsx`, unchanged except for the
 * `cn` and type imports. Every animation — the enter, the increase/decrease
 * pulse, the skeleton and its stagger — is the compiled component's, and the
 * classes it names are implemented in `styles/page.css`.
 */
...
import type { BatchOverlapEntry, LaneSummary, RiskMatrixEntry } from "../../lib/types";
...
function cellClasses(risk: RiskMatrixEntry["riskLevel"], selected: boolean, stale: boolean): string {
  ...
  if (risk === "high") {
    return cn(
      "bg-red-500/25 backdrop-blur-sm text-red-100 shadow-[0_0_10px_-3px_rgba(239,68,68,0.25)]",
      base,
      staleClass,
    );
  }
  if (risk === "medium") {
    return cn(
      "bg-amber-500/20 backdrop-blur-sm text-amber-100 shadow-[0_0_8px_-3px_rgba(245,158,11,0.2)]",
      base,
      staleClass,
    );
  }
  if (risk === "low") {
    return cn(
      "bg-emerald-500/15 backdrop-blur-sm text-emerald-100 shadow-[0_0_6px_-3px_rgba(16,185,129,0.15)]",
      base,
      staleClass,
    );
  }
...
function hasSamePair(
  a: { laneAId: string; laneBId: string },
  b: { laneAId: string; laneBId: string },
): boolean {
...
export function RiskMatrix({
  ...
  const matrix = React.useMemo(() => {
    ...
    for (const entry of entries) map.set(pairKey(entry.laneAId, entry.laneBId), entry);
    return map;
  }, [entries]);

  const overlapsByPair = React.useMemo(() => {
    const map = new Map<string, string[]>();
    for (const overlap of overlaps) map.set(pairKey(overlap.laneAId, overlap.laneBId), overlap.files);
    return map;
  }, [overlaps]);
  ...
  if (lanes.length === 0) {
    return (
      <div className="rounded-lg border border-border/10 bg-card/40 backdrop-blur-sm p-3 text-xs text-muted-fg shadow-card">
        No lanes to compare.
      </div>
    );
  }
  ...
  return (
    <div className="relative overflow-auto rounded-lg border border-border/10 shadow-card bg-card/30 backdrop-blur-sm">
      ...
      <table className="w-full min-w-[580px] border-collapse text-xs">
        <thead>
          <tr>
            <th className="sticky left-0 top-0 z-20 bg-bg px-2 py-2 text-left text-muted-fg">Lane</th>
            {lanes.map((lane) => (
              <th
                key={lane.id}
                className={cn(
            ...
            <tr key={rowLane.id}>
              ...
              {lanes.map((colLane, colIndex) => {
                ...
                const isSelected = selectedPair != null
                  && hasSamePair(selectedPair, { laneAId: rowLane.id, laneBId: colLane.id });
                ...
                const staleTitle = stale
                  ? formatStaleLabel(entry?.computedAt ?? null)
                  : `${rowLane.name} vs ${colLane.name}`;
