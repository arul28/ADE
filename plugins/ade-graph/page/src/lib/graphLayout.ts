/**
 * Layout and persisted preferences, moved verbatim.
 *
 * Ported from `components/graph/graphLayout.ts` with one change: the import of
 * the app's `shared/types` became `./types`. Every function here is pure, which
 * is why `test/graphLayout.test.ts` is the compiled suite's own cases, unchanged.
 */
...
/**
 * Session snapshots can end up with a partial `filters` object (e.g. only
 * `{ search }` if the previous `snapshot.filters` was undefined when spreading).
 * Coalesce with defaults so filter UI and `*.length` checks never see undefined
 * array fields.
 */
...
function readLegacyLastViewMode(state: Record<string, unknown>): GraphViewMode | null {
  ...
  const presets = state.presets.filter(
    (preset): preset is Record<string, unknown> => Boolean(preset) && typeof preset === "object",
  );
...
export function normalizeGraphPreferences(state: unknown): {
  ...
  if (state && typeof state === "object") {
    ...
    if (isGraphViewMode(record.lastViewMode)) {
      return { preferences: createGraphPreferences(record.lastViewMode), migrated: false };
    }

    const legacyViewMode = readLegacyLastViewMode(record);
    if (legacyViewMode) {
      return { preferences: createGraphPreferences(legacyViewMode), migrated: true };
    }
  }

  return { preferences: createGraphPreferences(), migrated: Boolean(state) };
}

/** Workspace primary lane and hierarchy relative to it. */
...
/** Primary centered on the top row, descendants on deeper rows. */
