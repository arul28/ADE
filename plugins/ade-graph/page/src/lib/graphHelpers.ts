/**
 * The canvas's small decisions, moved.
 *
 * Ported from `components/graph/graphHelpers.ts`. Two imports changed and
 * nothing else: `shared/types` became `./types`, and the PR colour functions
 * come from `./prVisuals` — the same three functions, over the same `COLORS`.
 */
...
import { getPrCiDotColor, getPrEdgeColor } from "./prVisuals";
...
export function proposalPairwiseResults(
  proposal: IntegrationProposal,
): IntegrationProposal["pairwiseResults"] {
...
export function laneSummaryConflictsWith(
  laneSummary: IntegrationProposal["laneSummaries"][number],
): string[] {
 ...
 * reaches a user as the raw enum, and so it reads as absence ("not run") rather
 * than as a state that could be mistaken for a result.
 */
