/**
 * Which lane is an integration lane, and what feeds it.
 *
 * Ported from `renderer/lib/integrationLanes.ts`, unchanged except for the type
 * import and the dropped `console.warn` on a missing source lane — a guest's
 * console goes nowhere a reader can reach, and the row is skipped either way.
 */

import type { IntegrationProposal, LaneSummary } from "./types";

export type IntegrationLaneSource = { laneId: string; laneName: string };
...
export function buildIntegrationSourcesByLaneId(
  ...
  for (const proposal of proposals) {
    ...
    for (const laneId of proposal.sourceLaneIds ?? []) {
      ...
      next.push({ laneId: normalizedLaneId, laneName: lane.name });
    }

    if (next.length > 0) map.set(integrationLaneId, next);
  }
