...
import { LINEAR_BRAND } from "@ade-dev/ui";
 ...
 *
 * PLACEMENT. The compiled component portalled into `document.body` and pinned
 * itself to the viewport's bottom-right, because it was drawn by the app shell
 * and the whole window was its stage. The full-page TAB keeps exactly that
 * (`portal`), since the tab's document is the reader's whole workspace.
 *
 * The POPOVER does not. There, `document.body` is the guest's own document —
 * the popover's box — so a portal buys nothing, and the panel is short enough
 * that a viewport-fixed toast would sit over the list it is reporting on. It
 * renders inline instead, absolutely positioned inside the panel (whose root is
 * `relative`), which is where the compiled toast appeared to the reader anyway:
 * the bottom-right corner of the Linear surface. Everything else, including
 * every class name and the 3.2s auto-dismiss, is the compiled component's.
 */
export function BatchLaunchStatusToast({
  ...
  /** True in the tab placement: portal to `document.body` and pin to the viewport. */
  portal?: boolean;
}) {
  ...
  const body = (
    <div
      className={
        portal
          ? "fixed bottom-4 right-4 z-[10001] w-[min(340px,calc(100vw-32px))]"
          : "absolute bottom-4 right-4 z-[10001] w-[min(340px,calc(100vw-32px))]"
      }
    >
      <div
