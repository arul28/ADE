/**
 * The compiled `LinearIssueSelectModal`, moved.
 *
 * Source: `apps/desktop/src/renderer/components/app/LinearIssueSelectModal.tsx`.
 * Every class name, pixel value and string is the compiled one; the three things
 * that changed are the imports (`@ade-dev/ui` and the page's own copies of the
 * Linear shapes), and the one host call in `LinearIssueDetails` (see below).
 */
      ...
      <div className="flex shrink-0 items-center justify-end gap-2 border-t border-white/10 px-3 py-2">
        {issue.url ? (
          <button
            ...
            data-variant="ghost"
            onClick={() => {
              // The compiled button branched on `canOpenInAdeBrowser(issue.url)`
              // between `openUrlInAdeBrowser` and `openExternalUrl`. `openLink`
              // takes both `http(s)` and `ade://` and the HOST decides which of
              // the two browsers a URL deserves, so the branch collapses to one
              // call — the page no longer has to know whether ADE's own browser
              // exists in this window.
              void openLink(issue.url!);
            }}
          >
            Open
          </button>
