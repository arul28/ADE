...
function createLinearApi(options = {}) {
  ...
  /**
   * Every webhook this workspace has, so a re-register can find its own.
   *
   * A port of `linearClient.ts:1282`. `first: 100` is Linear's own page and the
   * compiled client took the same one: a workspace with more than a hundred
   * webhooks is not a workspace this plugin's one hook can be lost in.
   */
  async function listWebhooks() {
    const data = await request(
      `query Webhooks {
        webhooks(first: 100) {
          nodes { id url enabled label resourceTypes allPublicTeams }
        }
      }`,
      null,
      { maxRetries: 1, operationName: "Webhooks" },
    );
    const nodes = Array.isArray(data?.webhooks?.nodes) ? data.webhooks.nodes : [];
    return nodes
      .filter((node) => node && typeof node.id === "string" && typeof node.url === "string")
      .map((node) => ({
        id: node.id,
        url: node.url,
        enabled: node.enabled !== false,
        label: typeof node.label === "string" ? node.label : null,
        resourceTypes: Array.isArray(node.resourceTypes) ? node.resourceTypes.map(String).filter(Boolean) : [],
        allPublicTeams: node.allPublicTeams === true,
      }));
  }

  /**
   * Create the workspace webhook that points at ADE's relay.
   *
   * A port of `linearClient.ts:1323`, including the two defaults: the resource
   * types and `allPublicTeams`, because a hook scoped to one team delivers
   * nothing for the rest of the workspace and a reader who registered from ADE
   * asked about their workspace.
   *
   * The SECRET is generated by the caller and handed to Linear here. Linear
   * never shows it again, which is why the caller stores it in the same act.
   */
    ...
    const data = await request(
      `mutation CreateWebhook(
        ...
        webhookCreate(input: {
          ...
          success
          webhook { id url enabled label resourceTypes allPublicTeams }
        }
      }`,
      {
        url: params.url,
        secret: params.secret,
        label: String(params.label ?? "").trim() || "ADE automations",
        resourceTypes: Array.isArray(params.resourceTypes) && params.resourceTypes.length
          ? params.resourceTypes
          : ["Issue", "Comment", "IssueLabel"],
        allPublicTeams: params.allPublicTeams !== false,
      },
      { maxRetries: 1, operationName: "CreateWebhook" },
    );
    const webhook = data?.webhookCreate?.webhook;
    if (!webhook || typeof webhook.id !== "string" || typeof webhook.url !== "string") {
      throw new LinearApiError("invalid_response", "Linear accepted the webhook but named none.");
    }
    return {
      id: webhook.id,
      url: webhook.url,
      enabled: webhook.enabled !== false,
      label: typeof webhook.label === "string" ? webhook.label : null,
      resourceTypes: Array.isArray(webhook.resourceTypes) ? webhook.resourceTypes.map(String).filter(Boolean) : [],
      allPublicTeams: webhook.allPublicTeams === true,
    };
  }

  /** Remove a webhook, by the id Linear gave it. A port of `linearClient.ts:1392`. */
  async function deleteWebhook(webhookId) {
    const id = String(webhookId ?? "").trim();
    if (!id) throw new LinearApiError("invalid_args", "Name the webhook to delete.");
    const data = await request(
      `mutation DeleteWebhook($id: String!) {
        webhookDelete(id: $id) { success }
      }`,
      { id },
      { maxRetries: 1, operationName: "DeleteWebhook" },
    );
    if (data?.webhookDelete?.success !== true) {
      throw new LinearApiError("invalid_response", "Linear did not report the webhook as deleted.");
    }
  }
