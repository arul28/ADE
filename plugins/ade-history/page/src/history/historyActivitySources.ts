import { getActivitySupplement } from "../host/actions";
import type {
...
export async function fetchSupplementalTimelineRecords(
  ...
  const safeLimit = Number.isFinite(roundedLimit)
    ...
    : 500;
  const supplement = await settle(getActivitySupplement(safeLimit));
  return buildSupplementalTimelineRecords({
    chats: supplement?.chats ?? [],
    ctoSnapshot: supplement?.ctoSnapshot ?? null,
  });
}
