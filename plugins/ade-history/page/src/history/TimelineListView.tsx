...
export function TimelineListView({
  ...
  const show = useCallback(
    (columnId: TimelineColumn) => visibleColumns.has(columnId),
    [visibleColumns],
  );
          ...
          {group.events.map((ev) => {
            ...
            return (
              ...
              >
                ...
                {show("author") ? (
                  <span className="hidden w-[88px] shrink-0 truncate font-mono text-[9px] text-muted-fg/60 md:inline">
                    {eventActor(ev)}
                  </span>
                ) : null}

                {show("lane") && ev.laneName ? (
                ...
                {show("sha") ? (
                  <span className="hidden w-[58px] shrink-0 text-right font-mono text-[9px] text-muted-fg/60 sm:inline">
                    {eventSha(ev)}
                  </span>
                ) : null}
              </button>
