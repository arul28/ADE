...
export function CommitDetailPanel({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void getCommitGraph(laneId, 500)
      .then(async (graph) => {
        if (cancelled) return;
        const found = graph.commits.find((r) => r.sha === commit.sha);
        if (found) {
          setResolvedCommit(found);
          return;
        }
        try {
          const targeted = await lookupCommit(laneId, commit.sha);
          if (!cancelled && targeted.commit) setResolvedCommit(targeted.commit);
        } catch {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void getCommitGraph(laneId, 1)
      .then((graph) => {
        if (!cancelled) setHeadSha(graph.commits[0]?.sha ?? null);
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    void getCommitDetail(laneId, activeCommit.sha)
      .then((detail) => {
        if (cancelled) return;
        setFullMessage((detail.message ?? "").trim() || activeCommit.subject);
        setFiles(detail.files);
        if (detail.commit) setResolvedCommit(detail.commit);
      })
      .catch(() => {
        if (!cancelled) {
          setFullMessage(activeCommit.subject);
          setFiles([]);
        }
      })
      .finally(() => {
