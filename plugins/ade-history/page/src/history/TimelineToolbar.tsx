...
import type { ExportHistoryResult, GitConflictState, HistoryLane } from "../lib/types";
import type { HistorySurface } from "./timelineTypes";
import { cn } from "@ade-dev/ui";
import { Button } from "@ade-dev/ui";
import { Chip } from "@ade-dev/ui";
import * as hostActions from "../host/actions";
import { prompt as hostPrompt } from "../host/ui";
  ...
  const handleExport = useCallback(async () => {
    ...
    const trimmed = typeof raw === "string"
      ...
      : raw && typeof raw === "object"
        ? String((raw as Record<string, unknown>).value ?? (raw as Record<string, unknown>).text ?? "").trim()
        : "";
    if (trimmed.length) {
    ...
    try {
      const result = await hostActions.exportOperations({
        format: "json",
        limit: chosenLimit,
        ...
        <div className="flex items-center gap-0.5">
          {SURFACE_OPTIONS.map(({ value, label, Icon }) => (
            <button
              ...
              data-ade-history-surface={value}
            >
...
function LaneGitActionsMenu({
  ...
  const refreshConflictState = useCallback(async () => {
    ...
    try {
      setConflictState(await hostActions.getConflictState(laneId));
    } catch {
      setConflictState(null);
    }
  }, [laneId]);
