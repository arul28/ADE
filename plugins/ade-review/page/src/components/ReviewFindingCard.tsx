/**
 * The finding card, moved from
 * `apps/desktop/src/renderer/components/review/ReviewFindingCard.tsx` (759).
 *
 * Markup for markup: the severity pill, the finding-class chip, the feedback
 * badge, the suppression band, the Action block, the primary-evidence block, the
 * details disclosure with its diff excerpt, tool signals, evidence trail and
 * review note, and the four feedback verbs with their modal. Three things
 * changed and nothing else:
 *
 * 1. `Button`, `Chip` and `cn` come from `@ade-dev/ui` rather than from the
 *    renderer's own `components/ui`. They are the same modules — the desktop app
 *    consumes them through `file:../../packages/ui`.
 * 2. `onOpenInEditor` is optional at the CALL site as well as the prop: the host
 *    verb behind it (`ui.openPathInEditor`) is new in this wave, and a host
 *    without it makes the press a no-op rather than an error. That is exactly
 *    what the compiled card did — it called
 *    `window.ade.app.openPathInEditor?.(…) ?? Promise.resolve()` and swallowed
 *    every rejection.
 * 3. `data-review-*` attributes, so the seam test can find a card and a verb
...
function pickPrimaryEvidence(evidence: ReviewEvidence[]): ReviewEvidence | null {
  return (
    evidence.find((entry) => entry.kind !== "tool_signal" && (entry.summary || entry.quote || entry.filePath))
...
function describeFeedback(
  record: ReviewFeedbackRecord | null | undefined,
          ...
          return (
            <span
              ...
              className={cn(
                base,
                lineColor,
                line.highlighted ? "border-l-2 border-amber-400/70" : "border-l-2 border-transparent",
              )}
            >
  ...
  return (
    <div className="space-y-1.5">
      {toolSignals.map((entry, idx) => {
        ...
        return (
          <div key={`${sig.source}-${idx}`} className={cn("rounded-lg border p-2 text-[11px]", tone)}>
...
function DismissModal({ open, initialKind, finding, onClose, onSubmit }: DismissModalProps) {
  ...
  const kindOptions: Array<{
    value: Exclude<ReviewFeedbackKind, "acknowledge">;
    label: string;
    ...
    >
      ...
      >
        ...
        <div className="space-y-4 p-5">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            {kindOptions.map((opt) => (
              <button
                ...
                data-review-feedback-kind={opt.value}
                className={cn(
          ...
          {kind === "snooze" ? (
            <div>
              <label className="text-[10px] uppercase tracking-[0.18em] text-[#6E7F92]" htmlFor="review-snooze-days">
                Hide for
              </label>
              <div className="mt-1 flex items-center gap-2">
                <input
                  id="review-snooze-days"
                  type="number"
                  min={1}
                  max={180}
                  value={snoozeDays}
          ...
          {kind === "suppress" ? (
            <div>
              <span className="text-[10px] uppercase tracking-[0.18em] text-[#6E7F92]">Suppression scope</span>
              <div className="mt-1 grid grid-cols-3 gap-2">
                {(["repo", "path", "global"] as const).map((scope) => (
                  <button
                    ...
                    data-review-suppression-scope={scope}
                    className={cn(
                  ...
                  >
                    {scope === "repo"
                      ...
                      : scope === "path"
                        ? finding.filePath
                          ? `Path (${finding.filePath.split("/").slice(-1)[0]})`
                          : "Path"
                        : "Global"}
                  </button>
                ))}
              ...
              <p className="mt-2 text-[10px] text-[#6E7F92]">
                Future runs skip similar findings within the chosen scope. You can remove suppressions later from the
                Learnings panel.
              </p>
            </div>
          ) : null}
          ...
          <div>
            <span className="text-[10px] uppercase tracking-[0.18em] text-[#6E7F92]">Reason</span>
          ...
          <div>
            <label className="text-[10px] uppercase tracking-[0.18em] text-[#6E7F92]" htmlFor="review-feedback-note">
        ...
        <div className="flex items-center justify-end gap-2 border-t border-white/[0.06] bg-white/[0.02] px-5 py-3">
          ...
          <Button
            ...
            data-review-action="submit-feedback"
          >
            {submitLabel}
          </Button>
        </div>
...
export function ReviewFindingCard({
  ...
  const fileLabel = formatPathLabel(
    finding.filePath ?? primaryEvidence?.filePath,
    finding.line ?? primaryEvidence?.line,
  );
    ...
    >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {findingClassLabel ? (
            <span
              ...
              className={cn(
                "cursor-help rounded-full border px-1.5 py-0.5 text-[9px] uppercase tracking-[0.08em]",
                toFindingClassTone(findingClass),
              )}
            >
              {findingClassLabel.label}
            </span>
          ) : null}
          {feedbackBadge ? <Chip className={cn("text-[9px]", feedbackBadge.tone)}>{feedbackBadge.label}</Chip> : null}
          {isSuppressed ? (
            ...
            {fileLabel ? (
              <div className="mt-1 flex min-w-0 items-center gap-1.5 text-[11px] text-[#8FA1B8]">
                ...
                <span
                  ...
                  title={finding.filePath ?? primaryEvidence?.filePath ?? undefined}
                >
                  {fileLabel}
                </span>
              </div>
          ...
          <div className={cn("rounded-lg border px-2.5 py-2", toEvidenceTone(primaryEvidence.kind))}>
            <div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.12em]">
              ...
              <span className="rounded-full border border-current px-1.5 py-0.5">
                {reviewEvidenceKindLabel(primaryEvidence.kind)}
              </span>
                  ...
                  <div key={`${finding.id}-${idx}`} className="rounded-lg border border-white/[0.06] bg-black/20 p-2">
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-[#8FA1B8]">
                      <span className="font-mono uppercase tracking-[0.08em]">
                        {reviewEvidenceKindLabel(entry.kind)}
                      </span>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        {finding.filePath && onOpenInFiles ? (
          <Button size="sm" variant="outline" onClick={() => onOpenInFiles(finding)} data-review-action="open-files">
            <FileText size={12} /> Open in files
          </Button>
        ) : null}
        {finding.filePath && onOpenInEditor ? (
          <Button size="sm" variant="outline" onClick={() => onOpenInEditor(finding)} data-review-action="open-editor">
        ...
        {onCopyFinding ? (
          <Button
            ...
            data-review-action="copy-finding"
          >
