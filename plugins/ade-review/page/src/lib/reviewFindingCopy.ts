/**
 * Moved from `apps/desktop/src/renderer/components/review/reviewFindingCopy.ts` (70),
 * word for word.
 *
 * This is what a reader pastes into a chat or a PR comment, so the format is the
 * product rather than an implementation detail — a page that shortened it would
 * change what colleagues receive.
 */

import type { ReviewFinding } from "../types";
import { reviewEvidenceKindLabel, reviewFindingClassLabel } from "./reviewFindingLabels";
...
export function formatReviewFindingsForClipboard(args: {
  ...
  blocks.push(
    ...args.findings.map((finding, index) => formatReviewFindingForClipboard(finding, index + 1)),
  );
  return blocks.join("\n\n");
}
