/**
 * Moved from `apps/desktop/src/renderer/components/review/reviewFindingLabels.ts` (77),
 * word for word. These are the sentences the reader sees on a finding, and a
 * page that reworded them would be a second Review with different vocabulary.
 */
...
export function reviewFindingClassLabel(
  value: ReviewFindingClass | null | undefined,
...
export function reviewPublicationLabel(args: {
  ...
  if (args.adjudication?.publicationEligible) {
    return {
      ...
      tooltip:
        "This finding has enough evidence to post if publishing is enabled; this run kept it local.",
    };
  }
  return {
    label: "Saved locally",
