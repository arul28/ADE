...
- Creates a small, reviewable change (one route handler + one test)
- Exercises file creation, code writing, and test generation
- Gives you a clear way to verify the result (`curl localhost:PORT/health`)
- Completes in under a minute with most models
...
**Context injection**: Before the agent sees your message, ADE prepends the Lane Pack — a structured context block with your project's architecture, the lane's intent, and recent file state. This means the agent already knows what project it is in and what branch it is working on.

**Tool calls**: The agent reads files, writes code, and runs commands using ADE's ADE CLI tools. Each tool call appears as an expandable block in the chat. You do not need to grant per-call permissions in the default configuration.

**Session recording**: Every message, tool call, and response is captured in the session transcript. You can review it later in the History tab.

**Interrupting**: Press **Escape** or click the **Stop** button to interrupt a running agent at any time. The agent reports partial results and you can continue the conversation or start a new session.
