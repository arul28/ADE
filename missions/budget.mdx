...
Workers reach ADE through the `ade` CLI. The mission creation form exposes a tool palette (repo, git, github, linear, mission, tests) that maps to the ADE RPC domains each worker may call. Tools outside the palette are rejected at the CLI boundary for that worker.
