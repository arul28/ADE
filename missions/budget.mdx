...
description: "Configure budget caps, tool permissions, proof capture, mission persistence, templates, and troubleshoot common mission issues."
icon: "coins"
---
...
<AccordionGroup>
  ...
  <Accordion title="Tool palette">
    Workers access ADE through the `ade` CLI. The mission creation form exposes a tool palette (repo, git, github, linear, memory, mission, tests) that maps to the ADE RPC action domains workers may invoke. Tool groups not in the palette are rejected at the CLI boundary for that worker.
  </Accordion>
  <Accordion title="Proof capture">
    Workers can capture intentional evidence during a mission by calling the `ade proof` CLI — screenshots, short recordings, or ingested browser traces. There is no mission-wide policy chip anymore: each worker decides when a step deserves a proof and runs the CLI directly.

    Captured proofs are linked to the mission automatically and appear in the mission's **Artifacts tab**. See [Proof Overview](/computer-use/overview) for the full workflow.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Mission fails to start (preflight errors)">
    ...
    - **Proof capture unavailable** — the host is missing a prerequisite for `ade proof` (for example, macOS Screen Recording permission has not been granted). Run `ade proof status --text` in a terminal to see the specific gap.
