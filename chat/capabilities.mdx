...
description: "What the chat agent can do — file operations, terminal commands, git, PRs, Linear integration, worker run spawning, repo inspection, and ADE action tools."
icon: "toolbox"
---
...
<CardGroup cols={2}>
  ...
  <Card title="Worker run spawning" icon="target">
    Escalate the current task into a new Worker run, carrying over conversation context as the planner's seed.
  </Card>
  <Card title="Repo inspection" icon="database">
  ...
  <Card title="ADE actions" icon="plug">
    Agents call ADE's typed actions — lane, git, PR, process, chat, test, and proof tools — through the ADE RPC surface. Toggle tool groups for the session from the chat header.
  </Card>
  <Card title="PR issue resolution" icon="wrench">
    Specialized workflow tools for fixing PR issues: read failing check output, read review comments, apply fixes, and verify the fix resolves the issue. Available when a lane has an open PR.
  </Card>
</CardGroup>
...
<Tip>
  If your project has a PR description template, keep it in the repository. The agent can inspect it when generating PR descriptions.
</Tip>

---

## Spawning a Worker run from Chat

When a task is too large for a single chat session, escalate to a Worker run without losing context.
...
<Steps>
  ...
  <Step title="Say 'start a worker run'">
    Type "start a worker run for this" or "this should be a worker run." ADE opens the Worker run creation dialog, pre-filled with:
  ...
  <Step title="Review and launch">
    Adjust the worker run config as needed (add lanes, set budget cap, configure permissions), then click **Start Worker run**. The worker run opens in the Worker run detail view. The chat session remains in History for reference.
  </Step>
</Steps>
