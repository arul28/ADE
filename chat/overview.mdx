...
<Frame caption="Agent Chat — conversation with tool call blocks, context meter, and MCP indicator">
  <img src="/images/chat-overview.png" alt="Agent Chat pane showing a conversation with code tool calls, the context usage meter, and model selector" />
</Frame>
...
|----------|---------|------|
| **Claude** | Anthropic SDK v2 sessions | API key |
| **Codex** | OpenAI app-server JSON-RPC | API key |
| **Cursor** | ACP (Agent Control Protocol) | Cursor CLI auth |
| **Unified** | Vercel AI SDK (in-process) | API key |
...
<Frame caption="Chat header — computer-use policy chip, MCP badge, model selector, and context meter">
  <img src="/images/chat-header.png" alt="Close-up of chat header showing CU policy toggle, tool count, model dropdown, and context usage bar" />
</Frame>
...
ADE automatically generates descriptive session titles using AI. Controlled by:
- `ai.chat.autoTitleEnabled` — toggle auto-titling on or off
- `autoTitleReasoningEffort` — depth of analysis for title generation
- `autoTitleRefreshOnComplete` — regenerate the title when the session ends
...
- **Codex** — session terminates; partial output is preserved in the transcript
